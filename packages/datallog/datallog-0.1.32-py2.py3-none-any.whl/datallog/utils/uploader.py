...
def upload(filename: str, file: bytes, max_size: int = 1, auth_token: str = None, x_api_key: str = None):
    ...
    try:
        ...
        if code == "EntityTooLarge":
            raise APIError(f"File upload failed: file is too large ({len(file)} bytes). Allowed size: ({max_size * 1024} bytes). Please update your max_size parameter in your uploader.upload function call")
        else:
