...
def opacity(values : list) -> dict[css.PropertyName, css.Value]:
    '''Parse the `opacity` value into a dict containing the needed property'''
    succesful, number = expect(values, 'opacity', (
    ...
    '''Parse the `font` shorthand value into a dict containing the needed properties'''
    # Basic implementation - parse font-family and font-size
    if not values:
        return {}

    result = {}

    # Look for size and family in the values
    for token in values:
        if token.type in ('dimension', 'number'):
            # Assume this is font-size
            if token.type == 'dimension':
                result['font_size'] = (token.value, css.Unit.px)
            else:
                result['font_size'] = (token.value, css.Unit.px)
        elif token.type == 'string':
            # Font family
            result['font_family'] = token.value.strip('"\'')
...
def justify_content(values : list) -> dict[css.PropertyName, css.Value]:
    '''Parse the `justify_content` value into a dict containing the needed property'''
    succesful, literals = expect(values, 'justify_content', (
        (VALUE.literal, 1, False),
    ))
    if not succesful:return {}
    xs = css.JustifyContent.str_to_enum(functions.snake_case(literals[0]))
    if xs is None:return {}
    return {
        'justify_content' : xs
    }
...
def align_items(values : list) -> dict[css.PropertyName, css.Value]:
    '''Parse the `align_items` value into a dict containing the needed property'''
    succesful, literals = expect(values, 'align_items', (
        (VALUE.literal, 1, False),
    ))
    if not succesful:return {}
    xs = css.AlignItems.str_to_enum(functions.snake_case(literals[0]))
    if xs is None:return {}
    return {
        'align_items' : xs
    }
...
def flex_direction(values : list) -> dict[css.PropertyName, css.Value]:
    '''Parse the `flex_direction` value into a dict containing the needed property'''
    succesful, literals = expect(values, 'flex_direction', (
        (VALUE.literal, 1, False),
    ))
    if not succesful:return {}
    xs = css.FlexDirection.str_to_enum(functions.snake_case(literals[0]))
    if xs is None:return {}
    return {
        'flex_direction' : xs
    }
...
def validate(rule_name : css.PropertyName, value_tokens : list) -> dict[css.PropertyName, css.Value]:
    ...
    elif rule_name == 'justify_content':return justify_content(value_tokens)
    elif rule_name == 'align_items':return align_items(value_tokens)
    elif rule_name == 'flex_direction':return flex_direction(value_tokens)
