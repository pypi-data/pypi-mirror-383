...
class Composite:
    ...
    def create_txt_element(self, render_information : RenderInformationContainer, text : str) -> Context:
        ...
        self.canvas.itemconfig(ctx.txt_token,fill=render_information.foreground_color)
    ...
    def create_composite(self, node : RenderNode) -> (Composite, Context):
        ...
        ctx = Context()
        canvas = tk.Canvas(scrollregion=(0, 0, max(node.layout_information.content_size_width, node.layout_information.width if node.layout_information.width is not None else 0, 1), max(1, node.layout_information.content_size_height,  node.layout_information.height if node.layout_information.height is not None else 0)),closeenough=0,highlightthickness=0, borderwidth=0, yscrollincrement=1, xscrollincrement=1, background=node.render_information.background_color if node.render_information.background_color else 'black')
    ...
    def update_composite_element(self, node : RenderNode):
        ...
        node.own_composite.canvas.config(scrollregion=(0, 0, max(node.layout_information.content_size_width, node.layout_information.width if node.layout_information.width is not None else 0, 1), max(1, node.layout_information.content_size_height, node.layout_information.height if node.layout_information.height is not None else 0)), background=node.render_information.background_color if node.render_information.background_color else 'black')
