...
import argparse_from_file as argparse  # type: ignore
...
def getparams_cmd(tpl: dict, host: str) -> None:
    ...
    try:
        res = subprocess.run(
            f'docker-machine env {host}'.split(),
            capture_output=True,
...
def main() -> None:
    ...
    opt.add_argument(
        '-B', '--nobackup', action='store_true', help='do not create a backup file'
    )
    opt.add_argument(
        '-S', '--nostrict', action='store_true', help='disable strict host key check'
    )
    opt.add_argument('name', help='docker machine name')
            ...
            for line in fp:
                if re.match(r'^Host\s+' + host + r'\s*$', line):
                    ...
                    if not args.replace and not args.delete:
                        sys.exit(
                            f'{host} already exists in {SSHFILE}. '
