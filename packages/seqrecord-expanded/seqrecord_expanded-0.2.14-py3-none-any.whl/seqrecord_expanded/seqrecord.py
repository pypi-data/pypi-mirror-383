...
class SeqRecordExpanded:
    ...
    def _clean_taxonomy(self, taxonomy):
        ...
        if taxonomy:
            for key, value in taxonomy.items():
                ...
                self.taxonomy[key] = re.sub(r"\W", "_", value)
    ...
    def first_codon_position(self):
        ...
        if self.reading_frame == 1:
            ...
            raise MissingParameterError(
                f"reading_frame attribute for gene {self.gene_code} " "should be either 1, 2 or 3."
            )
        return seq[::3]
    ...
    def translate(self, table=None):
        ...
        new_seq = Seq(str(self.seq).replace("?", "N"))
        try:
    ...
    def _check_translation_table(self, table):
        if self.table is None and table is None:
            raise MissingParameterError(
                "It is necessary to specify the translation"
