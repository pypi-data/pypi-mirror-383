...
def check_ocs_availability(
    *, timeout: int = 15, retries: int = 2, verbose: bool = False
    ...
    if users_resp.status_code >= 400:
        ...
        return False, f"Users endpoint returned HTTP {users_resp.status_code}: {users_resp.text[:200]}"

    # HTTP 2xx considered OK
    if verbose:
        logger.debug("OCS health check passed: capabilities and users endpoints reachable")
    return True, None
