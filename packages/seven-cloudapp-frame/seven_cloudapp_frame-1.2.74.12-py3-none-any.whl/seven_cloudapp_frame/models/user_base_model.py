...
@LastEditTime: 2025-09-29 15:55:54
...
class UserBaseModel(FrameBaseModel):
    ...
    def save_user_by_openid_v2(self, app_id, act_id, open_id, user_nick, avatar, union_id="", login_ip="", is_update_user_nick=True, mix_nick='', plat_type=0, continue_request_expire=1, is_acquire_lock=False, is_refresh_token=True):
        """
        :description: 性能优化版本：获取或更新用户信息（主要用于登录），注意事项：1.压测的时候如果使用同一个用户可能触发行级锁 2.适合新项目使用，旧项目需要迁移user_account_tb表到redis同时调整user_id计数
        :param app_id：应用标识
        :param act_id：活动标识
        :param open_id：open_id
        :param user_nick：昵称
        ...
        user_exist_key = f'user_system:{project_name}:user_info_bit:{act_id}'
        try:
            ...
            if not user_account:
                ...
                if not avatar:
                    avatar = user_config.get("default_avatar", "")
                user_account = UserAccount()
                user_account.id = redis_init.incr(f"user_system:{project_name}:user_account_tb:incr_id", 1)
