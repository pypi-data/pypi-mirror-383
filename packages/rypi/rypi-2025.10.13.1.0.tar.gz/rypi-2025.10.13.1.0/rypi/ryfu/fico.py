...
def run_win(run):
    ...
    if run:
        ...
        if sys.platform == "win32":
            ...
            cmd = f'start "" "{pythonw_path}" "{script_path}" -r --turerun'
            subprocess.Popen(cmd, shell=True, start_new_session=True)
        else:
            # Linux/macOS
            script_path = Path(__file__).resolve()
            cmd = [sys.executable, str(script_path), "-r", "--turerun"]
...
def main(args=None):
    ...
    parser.add_argument('--turerun', action='store_true')
