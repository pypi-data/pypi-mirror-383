#!/usr/bin/env python
'''
常用/共用函数(Common Function)，模块调用或命令行调用均可。
'''
...
def get_functions(pkg, mname):
    ...
    try:
        mod = load(f"{pkg}.{mname}")
        func = [
            (name, (inspect.getdoc(obj) or '无描述').split('\n')[0]) for name, obj in inspect.getmembers(mod, inspect.isfunction)
            if not name.startswith('_')
        ]
        return func
    except Exception as e:
        print(f"加载 {mname}.py 失败: {e}")
        return []
...
def main(args=None):
    ...
    # 显示所有函数
        ...
        funcs = get_functions(__package__, 'comm')
