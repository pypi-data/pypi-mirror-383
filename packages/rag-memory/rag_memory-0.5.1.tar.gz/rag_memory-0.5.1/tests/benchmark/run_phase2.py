"""
Phase 2 Benchmark Runner - Multi-Query Retrieval

Runs all test queries using multi-query retrieval and captures metrics.
"""
...
def run_multi_query_search(query: str, collection: str = "claude-agent-sdk", limit: int = 5) -> Dict[str, Any]:
    """
    Run a single multi-query search and capture results.
    ...
    searcher = get_multi_query_search(db, embedder, collection_mgr)
    ...
    try:
        ...
        for r in results:
            processed_results.append({
                ...
                'rrf_score': float(r.similarity),  # This is RRF score from multi-query fusion
...
def run_phase2_benchmark(output_dir: str = "test-results") -> str:
    """
    Run Phase 2 (Multi-Query Retrieval) benchmark suite.
    ...
    """
    print("=" * 80)
    print("PHASE 2 BENCHMARK - Multi-Query Retrieval (Query Expansion + RRF)")
    print("=" * 80)
    ...
    for i, query_spec in enumerate(test_queries, 1):
        ...
        result = run_multi_query_search(query_text)
        ...
        if result['success']:
            ...
            if result['num_results'] > 0:
                print(f"   Top RRF score: {result['results'][0]['rrf_score']:.4f}")
    ...
    summary = {
        ...
        'avg_top_rrf_score': round(sum(r['results'][0]['rrf_score'] for r in successful_queries if r['num_results'] > 0) / len([r for r in successful_queries if r['num_results'] > 0]), 4) if any(r['num_results'] > 0 for r in successful_queries) else 0,
        'by_category': {}
    }
    ...
    for category in ['well_formed', 'abbreviation', 'poorly_worded', 'technical']:
        ...
        if cat_results:
            summary['by_category'][category] = {
                ...
                'avg_top_rrf_score': round(sum(r['results'][0]['rrf_score'] for r in cat_results if r['num_results'] > 0) / len([r for r in cat_results if r['num_results'] > 0]), 4) if any(r['num_results'] > 0 for r in cat_results) else 0
            }

    # Create output structure
    output = {
        'benchmark_type': 'phase2_multiquery',
    ...
    output_file = Path(output_dir) / f"phase2-multiquery-{timestamp_str}.json"
    ...
    print(f"  Avg top RRF score: {summary['avg_top_rrf_score']:.4f}")
    ...
    for cat, stats in summary['by_category'].items():
        ...
        print(f"    Avg RRF score: {stats['avg_top_rrf_score']:.4f}")
    print()
