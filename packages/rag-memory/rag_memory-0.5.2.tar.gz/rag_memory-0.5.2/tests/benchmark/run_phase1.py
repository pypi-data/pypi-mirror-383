"""
Phase 1 Benchmark Runner - Hybrid Search

Runs all test queries using hybrid search and captures metrics.
"""
...
def run_hybrid_search(query: str, collection: str = "claude-agent-sdk", limit: int = 5) -> Dict[str, Any]:
    """
    Run a single hybrid search query and capture results.
    ...
    searcher = get_hybrid_search(db, embedder, collection_mgr)
    ...
    try:
        ...
        for r in results:
            processed_results.append({
                ...
                'rrf_score': float(r.similarity),  # This is RRF score, not cosine similarity
...
def run_phase1_benchmark(output_dir: str = "test-results") -> str:
    """
    Run Phase 1 (Hybrid Search) benchmark suite.
    ...
    """
    print("=" * 80)
    print("PHASE 1 BENCHMARK - Hybrid Search (Vector + Keyword + RRF)")
    print("=" * 80)
    ...
    for i, query_spec in enumerate(test_queries, 1):
        ...
        result = run_hybrid_search(query_text)
    ...
    output_file = Path(output_dir) / f"phase1-hybrid-{timestamp_str}.json"
