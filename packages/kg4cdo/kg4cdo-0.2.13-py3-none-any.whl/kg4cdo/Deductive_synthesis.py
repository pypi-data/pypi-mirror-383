...
def deductive_synthesis(model_req_path, facts_path, max_level, test_mode=False):
    ...
    for j in range(max_level):
        ...
        for item_1 in level_model:
            item_1_list.append(str(item_1.id) + '|' + str(item_1.model_type) + '|' + str(item_1.node_type) + '|' + str(
                item_1.name) + '|' + str(item_1.parent_id) + '|' + str(item_1.level_num))
        item_2_list = []
        for item_2 in level_facts:
            item_2_list.append(str(item_2.id) + '|' + str(item_2.model_type) + '|' + str(item_2.node_type) + '|' + str(
                item_2.name) + '|' + str(item_2.parent_id) + '|' + str(item_2.level_num))
        item_1_set = set(item_1_list)
        item_2_set = set(item_2_list)
        delta_set = item_1_set - item_2_set

        if (delta_set == {} and test_mode != True):
            t3 = float(time.time() * 1000)
            resolution = str("The model is proved. Level number: " + str(j + 1) + " - Exec. time for Level: " + str(j + 1) + " - " + str(t3 - t2) + "ms.")
            ...
            continue
    t3 = float(time.time() * 1000)
    resolution = str("The model is not proved. Exec. time for Level: " + str(j + 1) + " - " + str(t3 - t2) + "ms.")
    print(datetime.now(), ' -', resolution)
