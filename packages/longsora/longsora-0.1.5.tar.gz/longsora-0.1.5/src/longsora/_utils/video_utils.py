    ...
    try:
        ...
        with open(concat_file, "w") as f:
            for seg_path in segment_paths:
                # Use absolute path and convert to POSIX format for cross-platform compatibility
                # FFmpeg supports forward slashes on all platforms including Windows
                abs_path = seg_path.absolute()
                posix_path = abs_path.as_posix()
                # Escape single quotes and wrap path in single quotes
                safe_path = posix_path.replace("'", "'\\''")
