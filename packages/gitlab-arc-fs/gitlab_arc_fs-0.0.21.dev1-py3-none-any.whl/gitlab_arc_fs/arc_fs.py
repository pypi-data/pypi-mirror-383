    ...
    def upload(self,
        ...
        with self._lock:
            ...
            try:
                ...
                header_up.setdefault("Content-Length", str(total_size))

                file.seek(0, 0)

                # optional early-reject optimization for large uploads
                header_up.setdefault("Expect", "100-continue")

                put_res = requests.put(
                    url_upload,
                    headers=header_up,
                    data=file,            # CHANGED: file object (streams) instead of generator
                    timeout=(30, 600),    # ADDED: (connect, read) timeouts; adjust as needed
                )
            ...
            path_sanitized = utils.clean_file_ext(rel_path)

            LFSFile._modify_gitattributes(repo_id, self.token, branch, Path(rel_path))
            LFSFile._commit_pointer_file(path_sanitized, sha_hex, repo_id, self.token, str(total_size), branch)
