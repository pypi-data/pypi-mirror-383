...
def normalized_any(
    ...
    input = normalize_minmax(input, 1e-5, min_floor, max_ceil)
    target = normalize_minmax(target, 1e-5, min_floor, max_ceil).view_as(input)
    return loss_fn(input, target)


def cosine_loss(inp: Tensor, tgt: Tensor) -> Tensor:
