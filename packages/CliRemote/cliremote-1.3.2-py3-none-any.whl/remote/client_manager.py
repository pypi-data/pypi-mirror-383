...
async def get_or_start_client(phone_number: str) -> Optional[Client]:
    ...
    - در صورت وجود 2FA، هنگام start با password اجرا می‌شود.
    """
    ...
    try:
        ...
        # اگر در JSON پسورد 2FA داشتیم، در _make_client_from_json روی اتریبیوت خصوصی ذخیره شده
        twofa = getattr(cli, "_twofa_password", None)
        if twofa:
            await cli.start(password=twofa)
        else:
            await cli.start()
