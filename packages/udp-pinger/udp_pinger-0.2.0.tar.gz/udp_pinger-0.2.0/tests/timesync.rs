...
fn main() {
    ...
    let should_be_10 = ts.remote_time_during_instant(now);
    assert!(should_be_10 > Duration::from_secs(9));
    assert!(should_be_10 < Duration::from_secs(11));

    let ts = TimeSync::new_with_latency(Duration::from_secs(15), Duration::from_secs(12));
    let now = std::time::Instant::now();
    let should_be_27 = ts.remote_time_during_instant(now);
    assert!(should_be_27 > Duration::from_secs(26));
    assert!(should_be_27 < Duration::from_secs(28));
