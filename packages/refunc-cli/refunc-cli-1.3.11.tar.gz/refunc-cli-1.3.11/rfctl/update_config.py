...
from rfctl.common import reduce_event_source, event_arns, encode_base64
...
def update_config_command(ctx: click.Context):
    ...
    if funcdef["spec"].get("description"):
        lambda_args.extend(["--description", encode_base64(funcdef["spec"].get("description"))])
    if environment:
