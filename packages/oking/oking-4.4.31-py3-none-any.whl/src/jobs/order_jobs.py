        ...
        with conn.cursor() as cursor:
            ...
            for item in order.itens:
                ...
                item_params = queries.get_command_parameter(db_config.db_type, [
                   order_id,  # pedido_id,
                   item.sku_principal if item.sku_principal else '',  # sku
                   item.sku_variacao if item.sku_variacao else '',  # codigo_erp
                   item.quantidade if item.quantidade else 0,  # quantidade
                   item.ean if item.ean else '',  # ean
                   item.value if item.value is not None else 0.0,  # valor
                   item.valor_desconto if item.valor_desconto is not None else 0.0,  # valor_desconto
                   0,  # valor_frete
                   # '',  # codigo_filial_erp
                   item.filial_expedicao if item.filial_expedicao else '',  # codigo_filial_expedicao_erp
                   item.filial_faturamento if item.filial_faturamento else '',  # CODIGO_FILIAL_FATURAMENT_ERP
                   item.cnpj_filial_venda if item.cnpj_filial_venda else '',  # cnpj_filial_venda - AQUI ESTAVA O PROBLEMA!
                   item.valor_taxa_servico if item.valor_taxa_servico is not None else 0.0,  # valor_taxa_servico
                   item.impostos.valor_substituicao_tributaria if item.impostos.valor_substituicao_tributaria is not None else 0.0,
                   item.impostos.iva if item.impostos.iva is not None else 0.0,
                   item.impostos.icms_intraestadual if item.impostos.icms_intraestadual is not None else 0.0,
                   item.impostos.icms_interestadual if item.impostos.icms_interestadual is not None else 0.0,
                   item.impostos.valor_icms_interestadual if item.impostos.valor_icms_interestadual is not None else 0.0,
                   item.impostos.percentual_ipi if item.impostos.percentual_ipi is not None else 0.0,
                   item.impostos.valor_ipi if item.impostos.valor_ipi is not None else 0.0
                ])
