        ...
        with conn.cursor() as cursor:
            ...
            for item in order.itens:
                ...
                item_params = queries.get_command_parameter(db_config.db_type, [
                   order_id,  # pedido_id,
                   str(item.sku_principal) if item.sku_principal else '',  # sku
                   str(item.sku_variacao) if item.sku_variacao else '',  # codigo_erp
                   int(item.quantidade) if item.quantidade else 0,  # quantidade
                   str(item.ean) if item.ean else '',  # ean
                   ...
                   0.0,  # valor_frete
                   # '',  # codigo_filial_erp
                   str(item.filial_expedicao) if item.filial_expedicao else '',  # codigo_filial_expedicao_erp
                   str(item.filial_faturamento) if item.filial_faturamento else '',  # CODIGO_FILIAL_FATURAMENT_ERP
                   str(item.cnpj_filial_venda) if item.cnpj_filial_venda else '',  # cnpj_filial_venda
