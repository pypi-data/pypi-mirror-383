...
class ABV:
    ...
    def __init__(
        ...
        *,
        ...
        self.gateway = None
        if gateway and gateway.get("enabled", True):
            if api_key:
                from abvdev._client.gateway import GatewayClient

                self.gateway = GatewayClient(
                    api_key=api_key,
                    region=gateway.get("region", "us"),
                    abv_client=self,
                )

                abv_logger.debug(
                    f"Initialized ABV Gateway with region={gateway.get('region', 'us')}"
                )
            else:
                abv_logger.warning(
                    "Gateway configured but no API key provided. Gateway will not be initialized."
                )
