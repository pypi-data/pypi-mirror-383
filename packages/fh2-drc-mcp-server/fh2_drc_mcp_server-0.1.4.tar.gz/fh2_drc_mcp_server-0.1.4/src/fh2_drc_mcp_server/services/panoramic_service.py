...
async def panoramic_shooting(
    ...
    try:
        ...
        result = await gimbal_reset_horizontal(proj_uuid, filled_device_sn, filled_uuid, payload_index, token)
        ...
        for i in range(3):
            result = await camera_aim(proj_uuid, 0.5, 1.0, filled_device_sn, filled_uuid, payload_index, "wide", False, token)
        ...
        for i in range(10):
            # 瞄准位置
            result = await camera_aim(proj_uuid, 0, 0.5, filled_device_sn, filled_uuid, payload_index, "wide", False, token)
            ...
            # 拍照
            result = await camera_photo_take(proj_uuid, filled_device_sn, filled_uuid, payload_index, token)
        ...
        result = await gimbal_reset_downward(proj_uuid, filled_device_sn, filled_uuid, payload_index, token)
