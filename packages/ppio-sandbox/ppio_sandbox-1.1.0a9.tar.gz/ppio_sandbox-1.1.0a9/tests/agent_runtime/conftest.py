...
# Import Client模块的models（用于client测试）
            ...
            async with AgentRuntimeClient(api_key=real_api_key) as client:
                if real_template_id:
                    ...
                    try:
                        ...
                        pytest.skip(f"指定的模板 {real_template_id} 不可用: {e}")
                else:
                    # 使用第一个可用模板
                    templates = await client.list_templates()
                    if not templates:
                        pytest.skip("没有可用的模板进行测试")
                    return templates[0]
        except Exception as e:
            # 如果真实 API 调用失败，跳过测试
            pytest.skip(f"无法连接到真实 API 进行集成测试: {e}")
