...
if __name__ == '__main__':
    ...
    th = int(0.6*h)
    calarea = (-w, -h, w, h)
    calTargetPos = [[-tw,  th],[   0,  th],[ tw,  th],
                    [-tw,   0],[   0,   0],[ tw,   0],
                    [-tw, -th],[   0, -th],[ tw, -th]]

    valTargetPos = [[-tw/2,  th],[ tw/2,  th],
                    [-tw,  th/2],[   0,   th/2],[ tw,   th/2],
                    [-tw/2,   0],[ tw/2,   0],
                    [-tw, -th/2],[   0,  -th/2],[ tw,  -th/2],
                    [-tw/2, -th],[ tw/2, -th]]

    random.shuffle(calTargetPos)
    calTargetPos.insert(0, [0,0])
    random.shuffle(valTargetPos)
    valTargetPos.insert(0, [0,0])
