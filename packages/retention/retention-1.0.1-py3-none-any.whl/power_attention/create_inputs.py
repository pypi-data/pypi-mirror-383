...
def create_inference_inputs(b=2, tq=1, tk=1024, h=8, d=32, qhead_ratio=1, dtype=torch.bfloat16, device='cuda', gating=False,
    ...
    Q = torch.randn(size=(b, tq, h * qhead_ratio, d), dtype=dtype, device=device) / math.sqrt(d)
    K = torch.randn(size=(b, tk, h, d), dtype=dtype, device=device) / math.sqrt(d) if tk > 0 else None
    V = torch.randn(size=(b, tk, h, d), dtype=dtype, device=device) / math.sqrt(d) if tk > 0 else None
    if gating:
    ...
    s = None if fused_normalizer else torch.randn(size=(b, h, D), dtype=torch.float32, device=device)
    if requires_grad:
