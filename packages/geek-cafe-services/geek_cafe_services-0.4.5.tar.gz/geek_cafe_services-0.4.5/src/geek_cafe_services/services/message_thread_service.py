...
class MessageThreadService(DatabaseService[MessageThread]):
    ...
    def create(self, tenant_id: str, user_id: str, payload: Dict[str, Any]) -> ServiceResult[MessageThread]:
        """Create a new message thread from a payload."""
        try:
            ...
            # Ensure the creator (user_id) is listed as a participant
            ...
            thread = MessageThread().map(payload)
    ...
    def get_unread_count(self, thread_id: str, tenant_id: str, user_id: str) -> ServiceResult[int]:
        """Get the number of unread messages for a user in a thread."""
        try:
            # First, ensure the user has access to this thread.
            thread_result = self.get_by_id(thread_id, tenant_id, user_id)
            if not thread_result.success:
                return ServiceResult(success=False, error=thread_result.error, error_code=thread_result.error_code)

            # TODO: Implement actual unread count logic.
            # This would involve checking the 'read_by' status of each message.
            # For now, we return a placeholder value.
            return ServiceResult.success_result(0)

        except Exception as e:
            return self._handle_service_exception(e, 'get_unread_count',
                                                thread_id=thread_id, tenant_id=tenant_id)
