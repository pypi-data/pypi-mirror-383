# src/geek_cafe_services/lambda_handlers/groups/get/app.py
...
def handler(event: Dict[str, Any], context: object, injected_service=None) -> Dict[str, Any]:
    """
    Lambda handler for retrieving a single group by its ID.
    
    Args:
        event: API Gateway event
        context: Lambda context
        injected_service: Optional GroupService for testing
    """
    try:
        group_service = injected_service if injected_service else group_service_pool.get()
        ...
        result = group_service.get_by_id(
