...
def _pg_normalize_result(resp: dict, acc_gen_list: list = None) -> dict:
    ...
    crit = resp.get("critical_samples") or resp.get("dataset_preview")
...
class AMLSDK:
    BASE_URL = os.environ.get(
        "DSF_AML_BASE_URL",
        "https://dsf-h8039q9un-api-dsfuptech.vercel.app/"
    )
    ...
    def pipeline_generate_critical_safe(
        ...
        # Acumulador principal para las métricas de todas las llamadas parciales
        ...
        for start in range(0, len(original_dataset), max_chunk):
            ...
            for retry in range(max_413_retries):
                try:
                    ...
                    # --- INICIO DEL FIX ---
                    # Acumuladores para el bucle de reintentos de una tarea parcial
