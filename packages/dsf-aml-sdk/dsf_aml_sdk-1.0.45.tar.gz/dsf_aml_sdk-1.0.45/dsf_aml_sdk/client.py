...
class AMLSDK:
    ...
    def pipeline_generate_critical(self, config, seeds=None, advanced=None, **kwargs):
        ...
        # Auto-seeds si no llegan pero mandan original_dataset
        original_ds = kwargs.get("original_dataset")
        if seeds is None and original_ds:
            tkp = float(kwargs.get("top_k_percent", 0.1))
            # Evita enviar dataset completo: usa versión segura local
            local = self.pipeline_identify_seeds_safe(original_ds, cfg, top_k_percent=tkp)
            seeds = [s.get("data", s) for s in (local.get("seeds") or [])]

        if seeds:
            req["seeds"] = _sanitize_records(seeds)

        # Auto-cap del original_dataset (p.ej. 2K) para evitar 413
        if original_ds:
            capped = _cap_list(_sanitize_records(original_ds), cap=int(os.getenv("DSF_ORIGINAL_CAP", "2000")))
            if len(capped) < len(original_ds):
                logger.info(f"[SDK] original_dataset capped {len(original_ds)}→{len(capped)} to avoid 413")
            req["original_dataset"] = capped

        # Advanced defaults prudentes
    ...
    # ---- NUEVO: wrapper seguro con manejo de partial y 413 ----
    def pipeline_generate_critical_safe(
        self,
        config,
        original_dataset: Optional[List[Dict]] = None,
        seeds: Optional[List[Dict]] = None,
        top_k_percent: float = 0.3,
        k_variants: int = 6,
        advanced: Optional[Dict[str, Any]] = None
    ) -> Dict:
        ds = _sanitize_records(original_dataset) if original_dataset else None
        # Primer intento
        req_kwargs = {
            "original_dataset": ds,
        ...
        result = self.pipeline_generate_critical(config, seeds=seeds, advanced=advanced, **req_kwargs)

        # Reintentos si viene 'partial'
        partial_acc = []
        while isinstance(result, dict) and result.get("status") == "partial":
            wait = int(result.get("retry_after", 2))
            cursor = result.get("cursor")
            partial = result.get("partial_results", [])
            partial_acc.extend(partial)
            time.sleep(wait)
            req_kwargs.update({"cursor": cursor, "partial_results": partial_acc})
            result = self.pipeline_generate_critical(config, seeds=seeds, advanced=advanced, **req_kwargs)

        # Si el backend devuelve 413, reduce aún más y reintenta una vez
        return result
