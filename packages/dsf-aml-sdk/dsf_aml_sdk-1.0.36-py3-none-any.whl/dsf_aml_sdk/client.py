...
class AMLSDK:
    BASE_URL = os.environ.get(
        "DSF_AML_BASE_URL",
        "https://dsf-fd6gge8gy-api-dsfuptech.vercel.app/"
    )
    ...
    def _make_request(self, endpoint: str, data: Dict) -> Dict:
        ...
        try:
            ...
            if response.status_code == 429:
                ra = response.headers.get("Retry-After", "60")
                raise APIError(f"Rate limited. Retry after {ra}s", status_code=429)
            ...
            if 400 <= response.status_code < 500:
                try:
                    ...
                    context = {k: j[k] for k in ("status","verified_tier","has_license_key","stage","cursor") if k in j}
                    if context:
                    ...
                    raise APIError(f"Client error {response.status_code}: {response.text[:200]}", status_code=response.status_code)

            # 5xx — dejan que el decorador reintente
            try:
                j = response.json()
                raise APIError(j.get("error","Server error"), status_code=response.status_code)
            except Exception:
                raise APIError(f"Server error {response.status_code}: {response.text[:200]}", status_code=response.status_code)
