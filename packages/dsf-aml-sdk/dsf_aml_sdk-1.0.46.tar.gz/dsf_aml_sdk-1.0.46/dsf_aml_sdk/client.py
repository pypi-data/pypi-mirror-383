...
def _to_serializable(v: Any) -> Any:
    # Convierte tipos numpy/None a JSON‐friendly
    try:
        import numpy as np  # opcional: no falla si no está
    ...
    if isinstance(v, (float, int, str, bool)) or v is None:
        return v
    # objetos raros: hacer str para no romper POST
    return float(v) if isinstance(v, (complex,)) else v
...
class AMLSDK:
    ...
    def pipeline_generate_critical(self, config, seeds=None, advanced=None, **kwargs):
        ...
        # Dataset original (opcional), usado tanto por el backend como para calcular el cap
        original_ds = kwargs.get('original_dataset')
        if original_ds:
            req['original_dataset'] = original_ds

        # Autoseeds si no nos pasan seeds y hay dataset original
        if seeds is None and original_ds:
            tkp = float(kwargs.get('top_k_percent', 0.1))
        ...
        # >>> CAP Y SANITIZACIÓN DE SEEDS (AQUÍ) <<<
        if seeds:
            base_len = len(original_ds) if isinstance(original_ds, list) else 3000  # base prudente si no hay dataset
            max_seeds = min(300, max(100, int(0.10 * base_len)))  # 10% del dataset o [100..300]
            # usar la función ya existente en el SDK
            seeds = [_sanitize_record(s) for s in seeds if isinstance(s, dict)]
            if len(seeds) > max_seeds:
                seeds = seeds[:max_seeds]
                logger.info(f"[SDK] seeds capped to {max_seeds}")
            req['seeds'] = seeds

        # Hyperparámetros por defecto prudentes
        adv = dict(advanced or {})
        adv.setdefault('epsilon', 0.08)
        adv.setdefault('diversity_threshold', 0.90)
        adv.setdefault('non_critical_ratio', 0.20)
        adv.setdefault('max_seeds_to_process', min(64, len(seeds) if seeds else 64))
        adv.setdefault('max_retries', 5)
        adv.setdefault('require_middle', False)
        req['advanced'] = adv

        # Otros kwargs soportados
        if 'k_variants' in kwargs:
            req['k_variants'] = int(kwargs['k_variants'])
        if 'vectors_for_dedup' in kwargs:
            req['vectors_for_dedup'] = kwargs['vectors_for_dedup']
        if 'top_k_percent' in kwargs:
            req['top_k_percent'] = float(kwargs['top_k_percent'])
    ...
    def pipeline_generate_critical_safe(
        ...
        """
        Versión 'safe' con reintentos ante 413. No asume que el caller haga chunking manual.
        """
        # Normaliza entrada base
        ds = original_dataset or []
        if not isinstance(ds, list):
            raise ValidationError("original_dataset must be a list of dicts")

        # Cap inicial prudente de dataset (si el caller no lo acotó)
        if len(ds) > 2000:
            ds = ds[:2000]

        # Prepara seeds: si no vienen, se calculan en pipeline_generate_critical; si vienen, se sanitizan allí.
        # Aquí sólo pasamos lo recibido.
        req_kwargs = {
            "original_dataset": ds,
        ...
        adv.setdefault("diversity_threshold", 0.90)
        adv.setdefault("non_critical_ratio", 0.20)
        adv.setdefault("max_seeds_to_process", 64)
        adv.setdefault("require_middle", False)
        adv.setdefault("max_retries", 5)
        ...
        for retry in range(max_413_retries):
            try:
                result = self.pipeline_generate_critical(
                    config,
                    seeds=seeds,
                    advanced=adv,
                    **req_kwargs
                ...
                # Sólo hacemos backoff si es un 413
                if getattr(e, "status_code", None) == 413 and retry < max_413_retries - 1:
                    # Backoff agresivo: reduce dataset, reduce seeds, baja k_variants
                    if len(ds) > 1000:
                        ds = ds[: int(len(ds) * 0.75)]
                    else:
                        ds = ds[: max(600, int(len(ds) * 0.80))]

                    # reducir seeds a la mitad pero nunca < 50
                    if seeds and isinstance(seeds, list) and len(seeds) > 50:
                        seeds = seeds[: max(50, len(seeds) // 2)]

                    # baja variantes
                    req_kwargs["k_variants"] = 3

                    # actualiza original_dataset para el siguiente intento
                    req_kwargs["original_dataset"] = ds

                    logger.info(f"[SDK] 413 backoff: ds={len(ds)}, seeds={len(seeds) if seeds else 0}, k_variants={req_kwargs['k_variants']}")
                    continue

                # otro error o sin más reintentos
                raise

        if result is None:
            raise APIError("Failed to generate critical samples after backoff attempts")

        # Manejo de respuestas parciales (si el backend lo soporta)
        partial_acc = []
        while isinstance(result, dict) and result.get("status") == "partial":
            partial_acc.extend(result.get("partial_results", []))
            retry_after = float(result.get("retry_after", 1.5))
            time.sleep(retry_after)

            # Re-lanzar con cursor
            ...
            result = self.pipeline_generate_critical(
                ...
                advanced=adv,
                original_dataset=ds,
                k_variants=req_kwargs.get("k_variants", 3),
                top_k_percent=req_kwargs.get("top_k_percent", top_k_percent),
                cursor=cursor,
        ...
        for cap in sizes_try:
            ...
            try:
                ...
                if getattr(e, "status_code", None) == 413 and cap > 2000:
                    logger.warning(f"[SDK] 413 on full_cycle with {len(sub)} rows. Retrying with cap {cap-2000}")
                    continue
                raise e
