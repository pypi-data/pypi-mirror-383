"""
Main API class for frappeye providing a unified interface.
...
class FrappEye:
    """
    Main API class for frappeye - the unified interface for hook analysis.
    ...
    def __init__(
        ...
        """
        Initialize FrappEye with configuration options.
    ...
    def export_for_ci(
        ...
        **kwargs
        ...
        if self.reporter:
            return self.reporter.export_for_ci(result, output_file)
        else:
            ...
            "conflicts": [
                {
                    ...
                    "hook_type": conflict.hook_type.value,
    ...
    def quick_scan(self, path: Union[str, Path]) -> dict:
        ...
        from .models import ConflictSeverity
        critical_count = len([c for c in result.conflicts if c.severity == ConflictSeverity.CRITICAL])
