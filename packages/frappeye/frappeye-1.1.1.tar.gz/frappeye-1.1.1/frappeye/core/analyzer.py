"""
Advanced conflict analyzer for frappeye with intelligent conflict detection.
...
class ConflictAnalyzer:
    ...
    def analyze_hook_coverage(self, hooks: List[HookDefinition]) -> Dict[str, Dict[str, int]]:
        ...
        for hook in hooks:
            coverage[hook.app_name][hook.hook_type.value] += 1
            
            # Track DocType coverage for doc_events
            if hook.hook_type == HookType.DOC_EVENTS and hook.target:
    ...
    def get_conflict_summary(self, conflicts: List[ConflictReport]) -> Dict[str, any]:
        ...
        for conflict in conflicts:
            by_hook_type[conflict.hook_type.value] += 1
