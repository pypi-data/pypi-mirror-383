"""
Main CLI entry point for frappeye with comprehensive command structure.
...
def cli(ctx, version, verbose):
    ...
    Examples:
        frappeye scan /path/to/bench
        frappeye scan /path/to/site --format json
        frappeye check /path/to/bench --strict
        frappeye validate /path/to/app
    """
    if version:
        console.print(f"FrappEye version {__version__}", style="bold green")
        sys.exit(0)
...
def scan(ctx, path, site, output_format, output, min_severity, strict, no_cache, max_workers):
    ...
    Examples:
        frappeye scan /home/frappe/frappe-bench
        frappeye scan /home/frappe/frappe-bench/sites/mysite
        frappeye scan /home/frappe/frappe-bench/apps/erpnext --format json
    """
...
def check(ctx, path, exit_code, critical_only, strict, quiet):
    ...
    Examples:
        ...
        frappeye check /path/to/site --critical-only --quiet
    """
...
def validate(ctx, path):
    ...
    Examples:
        frappeye validate /path/to/app/hooks.py
        frappeye validate /path/to/app
        frappeye validate /path/to/bench --fix
    """
...
def export(ctx, path, output_file, export_format, site, fail_on_conflicts, fail_on_critical):
    ...
    Examples:
        frappeye export /path/to/bench results.json
        frappeye export /path/to/site report.csv --format csv
        frappeye export /path/to/bench ci-report.json --fail-on-critical
    """
...
def info(clear_cache, show_stats):
    """
    ℹ️  Show frappeye information and manage caches.
    
    Display version information, performance statistics,
    and manage internal caches for optimal performance.
    """
    console.print(f"🔍 FrappEye v{__version__}", style="bold blue")
