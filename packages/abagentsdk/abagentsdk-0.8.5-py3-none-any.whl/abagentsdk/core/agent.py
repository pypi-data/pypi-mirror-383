...
# Optional imports guarded to avoid circular errors if modules are absent during partial installs
try:
    from .handoffs import Handoff, handoff as handoff_factory, RunContextWrapper
except Exception:
    # Minimal fallback for type hints; real class comes from .handoffs in normal installs
...
class Agent:
    ...
    def __init__(
        ...
        *,
        ...
        # ----- Tools: normalize list (accept Tool objects, decorated functions, or plain callables) -----
    ...
    def _normalize_and_index_tools(self, tools_in: List[Union[Tool, Callable[..., Any]]]) -> Dict[str, Tool]:
        ...
        def _is_bad_kind(x: Any) -> Optional[str]:
            if isinstance(x, str):
                return "str"
        ...
        for idx, item in enumerate(tools_in):
            # Already a Tool?
            if isinstance(item, Tool):
                normalized.append(item)
                continue

            # Obvious user mistakes
            bad = _is_bad_kind(item)
            if bad:
                raise TypeError(
                    f"tools[{idx}] is a {bad}. Pass a Tool instance or a function, not {bad}."
                )

            # Plain callable → try to wrap via function_tool()
            if callable(item):
                try:
                    wrapped = function_tool()(item)  # decorator factory → Tool
                    normalized.append(wrapped)
                    continue
                except Exception:
                    # Last resort: minimal wrapper so we NEVER crash here
                    fn_name = getattr(item, "__name__", f"tool_{idx}")
                    ...
                    normalized.append(_FallbackTool())
                    continue

            # Not supported
            raise TypeError(
                f"tools[{idx}] has unsupported type {type(item).__name__}. "
                "Pass a Tool instance or a callable."
            )

        # Final safety: ensure each tool has a usable name
        tools_dict: Dict[str, Tool] = {}
        for t in normalized:
            t_name = getattr(t, "name", None) or getattr(t, "__name__", None)
            if not t_name:
                raise TypeError(f"Tool {t} has no name attribute.")
            tools_dict[t.name] = t
        return tools_dict
    ...
    def _execute_tool(self, call: ToolCall) -> str:
        ...
        try:
            ...
            # If tool supports ctx (internal), call through hidden hook; else plain run
