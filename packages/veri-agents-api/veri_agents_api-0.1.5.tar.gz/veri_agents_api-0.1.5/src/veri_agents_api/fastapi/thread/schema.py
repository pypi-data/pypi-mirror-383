...
class ToolCall(BaseModel):
    ...
    def from_lc(cls, tool_call: langchain_messages_tool.ToolCall) -> Self:
        return cls(name=tool_call["name"], args=tool_call["args"], id=tool_call["id"])
...
class AbstractStreamData(AbstractStreamPayload):
    ...
    def __str__(self) -> str:
        return f"data: {self.model_dump_json(serialize_as_any=True)}\n\n"


class StreamWarning(AbstractStreamData):
    type: Literal["warn"] = "warn"  # pyright: ignore[reportIncompatibleVariableOverride]
    content: str
...
class StreamMessageChunk(AbstractStreamData):
    type: Literal["message_chunk"] = "message_chunk"  # pyright: ignore[reportIncompatibleVariableOverride]
