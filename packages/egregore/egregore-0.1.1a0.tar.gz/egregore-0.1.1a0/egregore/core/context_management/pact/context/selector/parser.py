...
# Optional depth scope before the chain (e.g., d1..d5 or d1..d*)
chain: scope_opt? simple (combinator simple)*

scope_opt: "d" SIGNED_INT (".." "d" (SIGNED_INT | "*"))?
...
# Position can contain multiple coordinates or a depth range
...
coord_item: coordinate | coord_range

# Coordinate vector: depth + optional index-or-range dims (only last dim may be a range)
coordinate: "d" SIGNED_INT ("," idx_or_range)*

# Depth range (existing form)
coord_range: "d" SIGNED_INT ".." "d" (SIGNED_INT | "*")
...
class _ToAST(Transformer):
    ...
    def scope_opt(self, *items) -> DepthScope:
        # items: SIGNED_INT [SIGNED_INT|"*"] (the literals 'd', '..', 'd' are not included)
        if not items:
            return DepthScope(start=0)
        start_tok = items[0]
        start = int(str(start_tok))
        end = None
        if len(items) > 1:
            val = str(items[1])
            end = val if val == "*" else int(val)
        return DepthScope(start=start, end=end)
    ...
    def group_item(self, *items):
        ...
        for it in items:
            if isinstance(it, list):
                ...
                # DepthScope not allowed inside group; ignore safely
                pass
        return coords
    ...
    def coordinate(self, first: Token, *rest) -> List[object]:
        # depth + optional index/range dims (only last dim may be a range)
        vals: List[object] = [int(str(first))]
        for it in rest:
    ...
    def coord_range(self, start: Token, end: Token) -> DepthScope:
        end_val = str(end)
        if end_val == "*":
            return DepthScope(start=int(str(start)), end="*")
        return DepthScope(start=int(str(start)), end=int(end_val))
