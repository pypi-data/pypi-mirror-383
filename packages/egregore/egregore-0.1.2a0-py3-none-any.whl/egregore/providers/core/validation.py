...
def validate_outlines_availability() -> bool:
    ...
    Validates that the outlines library can be imported and is ready
    for use as a structured output fallback.
    
    Returns:
        True if outlines is available and functional, False otherwise
...
def validate_structured_output_request(
    ...
    # For non-streaming, check if outlines is available as fallback
    if not is_streaming:
        if not validate_outlines_availability():
            ...
            if model_capabilities and hasattr(model_capabilities, 'structured_output_native'):
                if not model_capabilities.structured_output_native:
                    return "Neither native structured output nor outlines fallback available"
            else:
                return "Outlines library not available for structured output fallback"
