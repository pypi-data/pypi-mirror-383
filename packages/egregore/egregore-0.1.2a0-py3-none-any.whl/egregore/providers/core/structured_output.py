...
class StructuredResponse:
    ...
    Attributes:
        ...
        fallback_used: Strategy used ("native", "outlines", None if no fallback)
...
def _apply_outlines_fallback(raw_response: Any, result_type: Type[Any]) -> Union[Any, None]:
    """Apply outlines library fallback for structured output generation.
    
    Uses the outlines library to extract structured data from raw text response.
    ...
    Returns:
        Structured result if successful, None if outlines fails
    """
    import outlines
    import json
    import re
    ...
    try:
        ...
        logger.debug(f"Applying outlines fallback for type: {result_type}")
        ...
        logger.warning(f"Outlines fallback could not handle type: {result_type}")
        return None
        
    except Exception as e:
        logger.error(f"Outlines fallback failed: {e}")
        return None
...
def _validate_outlines_availability() -> bool:
    """Check if outlines library is available and properly configured.
