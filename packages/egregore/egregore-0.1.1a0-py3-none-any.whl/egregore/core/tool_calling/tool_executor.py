...
class ToolExecutor(BaseModel):
    ...
    def execute_tool(self, tool_call: Union[ProviderToolCall, "ToolCall"]) -> ContextComponent:
        ...
        # Phase 3: Register tool pair if we have component ID (Task 3.3)
        if call_component_id and hasattr(self, 'context') and self.context:
            try:
                result_component_id = result.id
                self.context._registry.register_tool_pair(
                    tool_call_id=tool_call_id,
                    call_id=call_component_id,
                    result_id=result_component_id
                )
                logger.debug(f"Registered tool pair: {tool_call_id} → ({call_component_id}, {result_component_id})")
            except Exception as e:
                logger.warning(f"Failed to register tool pair: {e}")
