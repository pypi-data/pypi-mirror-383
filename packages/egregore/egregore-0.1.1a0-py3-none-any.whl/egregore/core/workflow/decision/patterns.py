...
import inspect
from .exceptions import AttributeMatchingError, PredicateEvaluationError, InvalidPatternError
...
class Pattern(ABC):
    ...
    def __init__(self, target_node):
        ...
        Args:
            target_node: The node to execute if this pattern matches
        """
        from ..base_node import BaseNode
        if not isinstance(target_node, BaseNode):
            raise InvalidPatternError(target_node, "target_node must be a BaseNode instance")
...
class PredicatePattern(Pattern):
    ...
    def __init__(self, predicate: Callable[[Any], bool], target_node):
        ...
        if not callable(predicate):
            raise InvalidPatternError(predicate, "predicate must be callable")
    ...
    def matches(self, input_value: Any, state: Any = None) -> bool:
        ...
        try:
            if self._accepts_state and state is not None:
                return bool(self.predicate(input_value, state))
            else:
...
class DefaultPattern(Pattern):
    ...
    @property
    def priority(self) -> int:
        return float('inf')  # Lowest priority - always last
