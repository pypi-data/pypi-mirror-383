...
class Agent:
    ...
    def __init__(self, provider: str = None, scaffolds: Optional[list] = None, tools: Optional[list] = None,
        ...
        Args:
            ...
            operation_retention: Retention capacity policy for scaffold operations (default: 10 operations)
                - int: Same retention for all operations
                - Dict[str, int]: Per-operation retention (e.g., {"append": 2, "update": 5})
    ...
    def _initialize_tool_and_context_systems(self) -> None:
        ...
        # Message scheduler - always needed (bound to context)
        self._message_scheduler = MessageScheduler(self.context)
    ...
    def _initialize_scaffold_system(self) -> None:
        ...
        if self._agent_config.scaffolds:
            for scaffold in self._agent_config.scaffolds:
                ...
                # Scaffolds are positioned via MessageScheduler, not added directly to system_header
                # (system_header is a PACTCore component without add_child method)
    ...
    def _get_scaffold_by_id(self, scaffold_id: str) -> Optional['BaseContextScaffold']:
        ...
        Args:
            scaffold_id: The metadata.id of the scaffold component
        ...
        for scaffold in self._scaffold_list:
            if hasattr(scaffold, 'metadata') and scaffold.metadata.id == scaffold_id:
                return scaffold
        return None
