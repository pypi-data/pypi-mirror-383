...
def handle_last_to_play(state):
    ...
    elif kernel.last_to_play == kernel.history[-1].legal_action.player_idx:
...
def handle_draw_discard_action(kernel, action):
    ...
    kernel.players[p_idx] = player # TODO: make it this: players=kernel.players.set(p_idx, player)
    return kernel.set(
        players=kernel.players,
        decks=kernel.decks,
    )
...
def handle_keep_action(kernel, action):
    ...
    kernel.players[player.idx] = player.set(discard_tray=v())
...
def deal_cards_to_each_player_discard_tray(kernel, deck_idx, num_cards):
    ...
    for player in kernel.players:
        ...
        for _ in range(num_cards):
            if deck.facedown_stack.cards:
                card = deck.facedown_stack.cards.pop()
                player = player.set(discard_tray=player.discard_tray.append(card))
        updated_players.append(player)

    return kernel.set(players=updated_players, decks=kernel.decks)
...
def getpublicplayer(s, p):
    ...
    return PublicPlayer(
        ...
        discard_deck_counts=discard_deck_counts,
