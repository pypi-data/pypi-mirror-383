...
class PathRecord(PClass):
    idx = field(type=int)
    edge_uuid = field(type=str)
    segment_uuids = field(type=PVector)  # List[str]
    def __todict__(self):
        return {
            "idx": self.idx,
            "edge_uuid": self.edge_uuid,
            "segment_uuids": list(self.segment_uuids),
        }
    @staticmethod
    def __fromdict__(d):
        return PathRecord(
            idx=d["idx"],
            edge_uuid=d["edge_uuid"],
            segment_uuids=pvector(d["segment_uuids"]),
        )
...
def init_path_record(path: Path2) -> PathRecord:
    return PathRecord(
        ...
        segment_uuids=pvector([segment.uuid for segment in path.segments]),
    )
...
class StateKernel(PClass):
    ...
    idx2pathrecord = field(type=PVector)  # List[PathRecord]

    edges = field(type=PVector)  # List[BiEdge]
    nodes = field(type=PVector)  # List[Node]
    regions = field(type=PVector)  # List[Region]
    ...
    def __fromdict__(d):
        return StateKernel(
            ...
            idx2pathrecord=pvector([Path2.__fromdict(v) for v in d["idx2pathrecord"]]),
...
def init_state_kernel(game_config, **kwargs):
    ...
    regions = kwargs.get('regions', v())

    idx2pathrecord = v()
    edgeuuid2idx = {edge.uuid: idx for idx, edge in enumerate(edges)}
    for edge in edges:
        for path in edge.paths:
            idx2pathrecord = idx2pathrecord.append(init_path_record(path))
    pieceuuid2piece = {}
    ...
    player_graphs_3 = calc_player_graphs3(list(nodes), uuid2edgerecord, list(idx2pathrecord), uuid2segmentrecord, pieceuuid2piece, game_config.num_players)
    ...
    return StateKernel(
        ...
        player_idxs=kwargs.get('player_idxs', pvector(list(range(game_config.num_players)))),
...
class PublicState(PClass):
    ...
    def __fromdict__(d):
        return PublicState(
            ...
            goals=pvector([FrozenGoal.__fromdict__(goal) for goal in d["goals"]]),
            nodes=pvector([Node.__fromdict__(n) for n in d["nodes"]]),
            edges=pvector([BiEdge.__fromdict__(e) for e in d["edges"]]),
            regions=pvector([Region.__fromdict__(r) for r in d["regions"]]),
...
def get_edges(board_config, nodeuuid2idx):
    ...
    for link_idx, link in enumerate(board_config.links):
        ...
        for matching_board_path in matching_board_paths:
            ...
            segments = pvector([
                Segment2(uuid=s.uuid, unit_uuid=s.unit_uuid, pieces=v()) for s in matching_board_path.path.segments
            ])
            path = Path2(
                idx=path_idx,
                edge_uuid=link.uuid,
...
def handle_move_pieces_to_path_action(kernel, action):
    ...
    for piece_uuid, segment_uuid in zip(default.piece_uuids, path_record.segment_uuids):
        ...
        if piece_idx is not None:
            ...
            uuid2segmentrecord[segment_uuid] = segment_record
    player = player.set(pieces=player_pieces)
    kernel = kernel.set(uuid2segmentrecord=uuid2segmentrecord)
    ...
    kernel = kernel.set(
        player_graphs_3=calc_player_graphs3(
            kernel.nodes, 
            kernel.uuid2edgerecord, 
            kernel.idx2pathrecord, 
            kernel.uuid2segmentrecord, 
...
@dispatch(type(v()), dict, type(v()), dict, dict, int)
def calc_player_graph3(nodes, uuid2edgerecord, idx2pathrecord, uuid2segmentrecord, pieceuuid2piece, player_idx):
    node2neighbors = {node.idx: set() for node in nodes}

    for edge_record in uuid2edgerecord.values():
        for path_idx in edge_record.path_idxs:
            path_record = idx2pathrecord[path_idx]
...
@dispatch(PVector, dict, PVector, dict, dict, int)
def calc_player_graph3(nodes, uuid2edgerecord, idx2pathrecord, uuid2segmentrecord, pieceuuid2piece, player_idx):
    node2neighbors = {node.idx: set() for node in nodes}
...
@dispatch(PVector, dict, PVector, dict, dict, int)
def calc_player_graphs3(nodes, uuid2edgerecord, idx2pathrecord, uuid2segmentrecord, pieceuuid2piece, num_players):
    return pvector([
        calc_player_graph3(nodes, uuid2edgerecord, idx2pathrecord, uuid2segmentrecord, pieceuuid2piece, player_idx)
...
def is_move_pieces_to_path_action_legal(state, action):
    ...
    remaining_segment_records = pvector([state.kernel.uuid2segmentrecord[segment_uuid] for segment_uuid in path_record.segment_uuids])
...
def get_sample_path_fulfillment(kernel, player_idx, path_idx):
    ...
    remaining_segment_records = pvector([kernel.uuid2segmentrecord[segment_uuid] for segment_uuid in path_record.segment_uuids])
        ...
        for path_idx in edge_record.path_idxs:
            ...
            for segment_uuid in path_record.segment_uuids:
                ...
                for piece_uuid in segment_record.piece_uuids:
                    pieces = pieces.append(piece_uuid)
                segments = segments.append(
                    Segment2(
