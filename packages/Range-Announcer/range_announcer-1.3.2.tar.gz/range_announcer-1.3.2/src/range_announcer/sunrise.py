...
from range_announcer.timezone import LocalTimezone
...
class Sun:
    ...
    def __init__(self, settings: dict):
        ...
        :param settings: dict: dictionary containing the longitude and latitude values.
        """
        try:
            self.lat = settings['latitude']
        except KeyError as ex:
            if ex.args[0] == 'latitude':
                self.lat = 44.0121221
        try:
            self.long = settings['longitude']
        except KeyError as ex:
            if ex.args[0] == 'longitude':
                self.long = -92.4801989
    ...
    def __timefromdecimalday(day):
        ...
        """
        # this is creating a day value greater than one so lets fix that
        day = day % 1
        hours = 24.0 * day
        h = int(hours)
        minutes = (hours - h) * 60
