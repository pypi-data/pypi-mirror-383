...
class APIKeyViewSet(PaymentBaseViewSet):
    ...
    permission_classes = [permissions.IsAuthenticated]  # Authenticated users
    ...
    def get_queryset(self):
        """
        Optimized queryset with user-specific filtering.
        - Regular users see only their own API keys
        - Staff/Admin see all API keys
        """
        queryset = super().get_queryset().select_related('user')

        # Non-staff users can only see their own API keys
    ...
    def perform_create(self, serializer):
        """Ensure API key is created for the current user."""
        # Non-staff users can only create API keys for themselves
        if not self.request.user.is_staff:
    ...
    def perform_update(self, serializer):
        ...
        # Non-staff users can only update their own API keys
        if not self.request.user.is_staff and api_key.user != self.request.user:
    ...
    def perform_destroy(self, instance):
        ...
        # Non-staff users can only delete their own API keys
        if not self.request.user.is_staff and instance.user != self.request.user:
            from rest_framework.exceptions import PermissionDenied
            raise PermissionDenied("You can only delete your own API keys.")
