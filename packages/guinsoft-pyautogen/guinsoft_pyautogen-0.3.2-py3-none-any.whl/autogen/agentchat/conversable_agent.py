...
class ConversableAgent(Agent):
    ...
    def __init__(
        ...
        self.register_reply([Agent, None],
                            ConversableAgent.a_generate_code_execution_reply,
                            is_async=True)
        self.register_reply([Agent, None],
    ...
    def _append_oai_message(self, message: Union[Dict, str], role, conversation_id: Agent) -> bool:
        ...
        oai_message = {
            k: message[k]
            for k in ("content", "function_call", "name", "context") if k in message
        }
    ...
    async def a_send(
        ...
        valid = self._append_oai_message(message, "assistant", recipient)
        if valid:
            await recipient.a_receive(message,
                                      self,
                                      request_reply,
                                      silent,
                                      global_chat_messages=global_chat_messages,
    ...
    def receive(
        ...
        callback_manager.on_chat_model_start(dumpd(self),
                                             messages=[[message]],
                                             sender=sender.name,
                                             receiver=self.name)
        if not silent:
    ...
    async def a_receive(
        ...
        run_manager = await callback_manager.on_chat_model_start(dumpd(self), messages=[[]])
        ...
        if not isinstance(self, GroupChatManager):
            ...
            await run_manager[0].on_text(text=reply,
                                         sender=self.name,
                                         receiver=receiver,
                                         category="autogen")
    ...
    def initiate_chat(
        ...
        **context,
        ...
        self.send(self.generate_init_message(**context),
                  recipient,
                  silent=silent,
                  global_chat_messages=global_chat_messages,
    ...
    async def a_initiate_chat(
        ...
        **context,
        ...
        """
        self._prepare_chat(recipient, clear_history)
        await self.a_send(self.generate_init_message(**context),
                          recipient,
                          silent=silent,
                          global_chat_messages=global_chat_messages,
                          callbacks=run_manager.get_child())
            ...
            if self._consecutive_auto_reply_counter[
                ...
                if self.human_input_mode == "NEVER":
                    ...
                    reply = self.get_human_input(
                        ...
                        if terminate else
                        f"Please give feedback to {sender.name}. Press enter to skip and use auto-reply, or type 'exit' to stop the conversation: "
                    )
                    no_human_input_msg = "NO HUMAN INPUT RECEIVED." if not reply else ""
                    # if the human input is empty, and the message is a termination message, then we will terminate the conversation
            ...
            if self._consecutive_auto_reply_counter[
                ...
                if self.human_input_mode == "NEVER":
                    ...
                    try:
                        ...
                        self.input = 'exit'
                    reply = list(self.input.values())[0] if isinstance(self.input,
                                                                       dict) else self.input
                    no_human_input_msg = "NO HUMAN INPUT RECEIVED." if not reply else ""
                    # if the human input is empty, and the message is a termination message, then we will terminate the conversation
                    reply = reply if reply or not terminate else "exit"
            elif self._is_termination_msg(message):
    ...
    async def a_generate_reply(
        ...
        **kwargs,
        ...
        for reply_func_tuple in self._a_reply_func_list:
            ...
            if self._match_trigger(reply_func_tuple["trigger"], sender):
                if asyncio.coroutines.iscoroutinefunction(reply_func):
                    final, reply = await reply_func(self,
                                                    messages=messages,
                                                    sender=sender,
                                                    config=reply_func_tuple["config"],
                                                    callbacks=callbacks,
    ...
    def execute_code_blocks(self, code_blocks):
        ...
        for i, code_block in enumerate(code_blocks):
            ...
            if lang in ["bash", "shell", "sh"]:
                exitcode, logs, image = self.run_code(code,
                                                      lang=lang,
    ...
    async def a_execute_code_blocks(self, code_blocks, run_manager):
        ...
        for i, code_block in enumerate(code_blocks):
            ...
            if image is not None:
                self._code_execution_config["use_docker"] = image
            # 过程日志
            await run_manager.on_text(text=logs, category="autogen")
            logs_all += "\n" + logs
            if exitcode != 0:
                return exitcode, logs_all
        return exitcode, logs_all
    ...
    async def a_execute_function(self, func_call, run_manager):
        ...
        if func is not None:
            ...
            if arguments is not None:
                ...
                await run_manager.on_text(text=f"EXECUTING FUNCTION {func_name}...\n\n",
                                          category="autogen")
                try:
                    content = func(**arguments)
                    is_exec_success = True
