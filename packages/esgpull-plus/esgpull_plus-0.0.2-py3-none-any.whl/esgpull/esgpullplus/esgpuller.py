...
from esgpull.config import Config, Credentials
...
class Esgpull:
    ...
    def legacy_query(self) -> Query:
        legacy = LegacyQuery
        if (legacy_db := self.db.get(Query, "LEGACY")) and legacy_db is not None:
            legacy = legacy_db
        # else:
        # self.db.add(legacy)
    ...
    async def download(
        ...
        if subset_criteria:
            ...
            queue = [
                ...
                for file in queue
                if all(
                    getattr(file, key, None) == value
                    for key, value in subset_criteria.items()
                )
            ]
            ...
            with self.ui.live(
                ...
                async for result in self.iter_results(
                    ...
                    match result:
                        ...
                        case Err():
                            queue_size -= 1
                            main_progress.update(main_task_id, total=queue_size)
    ...
    def replace_queries(
        ...
        to_replace = [q for q in graph.queries.values() if q.require == mapping[0]]
