...
def write_file(
    ...
    if file_path.startswith(".cursor"):
        ...
        service_name = Path(service_url).stem if os.path.exists(service_url) else service_url
        frontmatter = f"""---
...
async def get_instructions(
    ...
    if not instructions_url.startswith(("http://", "https://")):
        print(f"Error: '{instructions_url}' is neither a valid file path nor a valid URL")
        return None
...
async def get_text_chunks() -> List[str]:
    ...
    chainlit = await get_instructions(services["Chainlit"])
    wfs_chunks: List[str] = []
    chainlit_chunks: List[str] = []
    llindex_chunks: List[str] = []
    if wfs:
        wfs_chunks = wfs.split(CHUNKS_SEPARATOR)
    if chainlit:
        chainlit_chunks = chainlit.split(CHUNKS_SEPARATOR)
    if llindex:
        llindex_chunks = llindex.split(CHUNKS_SEPARATOR)
    # filter out empty chunks
    return (
        [chunk for chunk in wfs_chunks if chunk]
        + [chunk for chunk in chainlit_chunks if chunk]
