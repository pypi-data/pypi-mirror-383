...
class TestDataApi(unittest.TestCase):
    ...
    def test_query_success(self, mock_post):
        ...
        mock_response.json.return_value = {
            ...
            'data': {
                ...
                'items': [
                    ['2022-10-12', '700.HK', '腾讯控股', 1000000],
                    ['2022-10-12', '9988.HK', '阿里巴巴', 800000]
                ]
            }
        }
        ...
        # 执行查询
        result = self.api.query('hsgt_page_list', trade_date='2022-10-12')
        ...
        self.assertIn('hsgt_page_list', call_args[0][0])
    ...
    def test_query_with_fields(self, mock_post):
        ...
        mock_response.json.return_value = {
            ...
            'data': {
                ...
                'items': [['2022-10-12', '700.HK']]
            }
        }
        mock_post.return_value = mock_response

        result = self.api.query('hsgt_by_stock', fields='trade_date,stock_code', stock_code='700.HK')
    ...
    def test_query_api_error(self, mock_post):
        ...
        with self.assertRaises(Exception) as context:
            self.api.query('hsgt_page_list', trade_date='invalid-date')
    ...
    def test_query_empty_data(self, mock_post):
        ...
        result = self.api.query('hsgt_page_list', trade_date='2022-01-01')
    ...
    def test_getattr_dynamic_method(self):
        ...
        with patch.object(self.api, 'query') as mock_query:
            ...
            # 调用动态方法
            self.api.hsgt_page_list(trade_date='2022-10-12')

            # 验证query方法被正确调用
            mock_query.assert_called_once_with('hsgt_page_list', trade_date='2022-10-12')
    ...
    def test_pagination_response(self, mock_post):
        ...
        result = self.api.query('hsgt_page_list', page=1, page_size=20)
...
class TestIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.base_url = "http://localhost:10000"
        self.token = "4e2e929a1ba14b06b8e8ca7e7e560efe"  # 使用有效的token
        self.client = api(token=self.token, base_url=self.base_url)

    @unittest.skip("需要实际API服务运行")
    def test_real_hsgt_page_list_query(self):
        """测试真实的HSGT分页查询"""
        result = self.client.hsgt_page_list(page=1, page_size=5)
    ...
    @unittest.skip("需要实际API服务运行")
    def test_real_hsgt_by_stock_query(self):
        """测试真实的按股票查询"""
        result = self.client.hsgt_by_stock(stock_code='00700.HK', limit=3)
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("1. 查询分页数据")
        df = client.hsgt_page_list(page=1, page_size=3)
        print(f"   查询结果: {len(df)} 条记录")

        print("2. 查询指定字段的分页数据")
        df = client.hsgt_page_list(
            page=1,
            page_size=3,
            fields='trade_date,stock_code,stock_name,hold_market_cap'
        )
        print(f"   查询结果: {len(df)} 条记录，字段: {list(df.columns)}")

        print("3. 按股票代码查询")
        df = client.hsgt_by_stock(stock_code='00700.HK', limit=5)
        print(f"   查询结果: {len(df)} 条记录")

        print("4. 按股票代码查询并指定字段")
        df = client.hsgt_by_stock(
            stock_code='00700.HK',
            limit=3,
            fields='trade_date,stock_code,stock_name,hold_market_cap'
        )
