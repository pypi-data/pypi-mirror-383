...
def _validate_default_value(default: str, validation_func: Callable[[str], bool] | None, min_length: int, max_length: int | None, logger: Any) -> str:
    ...
    if validation_func is not None and not validation_func(default):
        logger.warning(f"Default value '{default}' does not pass validation function.")
        is_valid = False

    if min_length > 0 and len(default) < min_length:
        logger.warning(f"Default value length ({len(default)}) is less than min_length ({min_length}).")
        is_valid = False

    if max_length is not None and len(default) > max_length:
        logger.warning(f"Default value length ({len(default)}) is greater than max_length ({max_length}).")
        is_valid = False

    if not is_valid:
        logger.error("Invalid default value provided. Using empty string as fallback.")
        return ""
    return default
...
def _try_set_future_exception(input_future: Any, exception: Exception, logger: Any) -> None:
    """Try to set future exception."""
    try:
        if not input_future.done():
            input_future.set_exception(exception)
    except asyncio.InvalidStateError:
        logger.debug(f"Input future already done when exception occurred: {exception!s}")


def _check_input_validation(
    user_input: str, min_length: int, max_length: int | None, validation_func: Callable[[str], bool] | None, validation_message: str
) -> bool:
...
async def _get_input_with_validation(
    ...
    while True:
        try:
            ...
            is_valid = _check_input_validation(user_input, min_length, max_length, validation_func, validation_message)
            ...
            _try_set_future_exception(input_future, InputError(f"Error getting user input: {e!s}"), logger)
            return


async def _handle_input_timeout(input_future: Any, timeout: int, default: str, logger: Any) -> None:
    """Handle input timeout by setting default value."""
    await asyncio.sleep(timeout)

    if _try_set_future_result(input_future, default, logger):
        logger.warning(f"Input timed out after {timeout} seconds. Using default value.")
        print(f"\nInput timed out. Using default: '{default}'")


def _check_non_interactive_environment(prompt: str, default: str, logger: Any) -> str | None:
    """Check if environment is non-interactive and return default if so."""
    if not sys.stdin.isatty():
        logger.warning("Non-interactive environment detected (stdin is not a TTY). Using default input value.")
...
async def async_input(
    ...
    non_interactive_result = _check_non_interactive_environment(prompt, default, logger)
    if non_interactive_result is not None:
        return non_interactive_result

    # Validate default value for consistency
    default = _validate_default_value(default, validation_func, min_length, max_length, logger)
    ...
    asyncio.create_task(  # noqa: RUF006
        _get_input_with_validation(input_future, prompt, min_length, max_length, validation_func, validation_message, logger)
    )

    # Start timeout task if needed
    if timeout > 0:
        asyncio.create_task(_handle_input_timeout(input_future, timeout, default, logger))  # noqa: RUF006
