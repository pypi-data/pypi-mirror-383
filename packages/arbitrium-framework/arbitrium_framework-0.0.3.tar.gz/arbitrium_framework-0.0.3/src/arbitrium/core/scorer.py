...
class ScoreExtractor:
    ...
    def extract_scores(self, evaluation_text: str, model_names: list[str]) -> dict[str, float]:
        ...
        if len(scores) >= len(model_names):
            self.logger.debug(f"Successfully extracted all {len(scores)} scores")
        else:
            self.logger.warning(f"Found only {len(scores)}/{len(model_names)} scores")
    ...
    def extract_scores_from_evaluation(
        ...
        if detect_apology_or_refusal(evaluation_text):
            self.logger.error(f"[{evaluator_name}] Model returned apology/refusal instead of evaluation")
            return {}
    ...
    def normalize_score(self, score: float, evaluator: str) -> float | None:
        ...
        if score < 0.5 or score > 10.5:
            self.logger.error(f"Rejecting invalid score from {evaluator}: {score} (must be 1.0-10.0)")
            return None

        # Normalize scores that are slightly out of bounds
        if score > 10:
            normalized = min(score / 10.0, 10.0)
            self.logger.warning(f"Normalizing score from {evaluator}: {score} → {normalized:.2f}")
            return normalized
                ...
                try:
                    if len(match.groups()) >= 2 and match.group(2):
                        try:
                            ...
                            self.logger.debug(f"Extracted score {score_value} for {model_name}")
                            return score_value
        ...
        for alt_name, score in extracted.items():
            if alt_name in numbered_mapping:
                ...
                if original_name not in scores:
                    ...
                    self.logger.debug(f"Mapped '{alt_name}' → '{original_name}' with score {score}")
