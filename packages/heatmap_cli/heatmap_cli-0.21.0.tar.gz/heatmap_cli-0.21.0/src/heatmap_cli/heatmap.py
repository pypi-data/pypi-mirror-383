...
def _generate_animated_heatmap_by_week(config: argparse.Namespace) -> None:
    ...
    for seq, day in enumerate(days_in_current_week, 1):
        frame = _generate_single_heatmap_for_day(
...
def _generate_single_heatmap_for_day(
    ...
    Returns:
        Image.Image: The generated PIL Image object.
    """
    ...
    return _generate_heatmap(
        seq, cmap, config_for_heatmap, pivoted_dataframe, return_image=True
    )
...
def _generate_heatmaps(config: argparse.Namespace) -> None:
    ...
    args = [
        (*seq_cmap, config, dataframe, False)
