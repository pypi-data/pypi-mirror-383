...
class Task:
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Convert task to dictionary format (only essential fields for persistence)."""
        return {
            ...
            # Runtime fields excluded: id, type, dependencies, priority, created_at, updated_at, metadata
        }
    ...
    async def plan_tasks(self, context: PlanningContext) -> List[Task]:
        ...
        try:
            ...
            if context.conversation_history:
                logger.debug(f"[PLANNER] HTTP Request Body context: {context.project_context}")
...
"""
    ...
    def _summarize_conversation_history(self, history: List[Message]) -> str:
        """Create conversation summary using full context strategy."""
        if not history:
    ...
    def _get_safe_context_config(self):
        ...
        try:
            ...
            # 记录使用的策略
            logger.debug(f"Using full conversation context strategy")

            return context_config
        except Exception as e:
            # 创建最小可用配置
            logger.warning(f"Failed to load conversation context config: {e}, using default full strategy")
