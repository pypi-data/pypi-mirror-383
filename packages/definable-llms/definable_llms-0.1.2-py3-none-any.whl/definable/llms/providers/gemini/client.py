...
if GEMINI_AVAILABLE:
  ...
  def _convert_message_to_gemini(self, message: Message) -> dict:
      ...
      for content in message.content:
        if content.type == ContentType.TEXT:
          ...
          if content.image_url:
            ...
            parts.append({"inline_data": {"mime_type": "image/jpeg", "data": content.image_url}})
  ...
  def _convert_gemini_message(self, content: dict, finish_reason: Optional[str] = None) -> Message:
    """Convert Gemini content format to internal message format.

    Args:
        content: Gemini content dict
  ...
  def _handle_gemini_error(self, error: Exception) -> Exception:
    ...
    if "google.generativeai" in str(type(error)) and "ClientError" in str(type(error)):
    ...
    elif "google.generativeai" in str(type(error)) and "ServerError" in str(type(error)):
  ...
  async def generate_image(self, request: ImageRequest, **kwargs) -> ImageResponse:
    ...
    try:
      ...
      image_data = []
      for generated_image in getattr(response, "generated_images", []):
        # Gemini returns GeneratedImage objects
        # Access the actual image bytes from the image.image_bytes property
        import base64
        ...
        if hasattr(generated_image, "image") and hasattr(generated_image.image, "image_bytes"):
          ...
          # Fallback: try to get bytes from image attribute directly
          image_bytes = getattr(generated_image, "image", None)

        if image_bytes and isinstance(image_bytes, (bytes, bytearray)):
          b64_image = base64.b64encode(bytes(image_bytes)).decode("utf-8")
          image_data.append(
            ImageData(
              b64_json=b64_image,
            )
      ...
      async with self._timed_request(
        ...
        if hasattr(operation, "response") and hasattr(operation.response, "generated_videos"):
          for generated_video in operation.response.generated_videos:
            # Return the Video object WITHOUT downloading
            # Backend will handle downloading and uploading to their storage
            video_obj = generated_video.video
            video_info = {
              "video": video_obj,  # Video object - backend can download this
              "uri": video_obj.uri if hasattr(video_obj, "uri") else None,
        ...
        self.logger.info(
          ...
          has_generated_videos=hasattr(operation.response, "generated_videos") if hasattr(operation, "response") else False,
        )
