...
def spark_factory(cls: Type[T]) -> Type[T]:
    ...
    setattr(cls, 'build_dataframe', build_dataframe)
    setattr(cls, 'build_dicts', build_dicts)
    setattr(cls, 'create_dataframe_from_dicts', create_dataframe_from_dicts)
    
    # Store reference to the factory class for advanced use cases
    setattr(cls, '_polyspark_factory', factory_class)
