...
def solve(
    ...
    while change > change_threshold and loop < iterations:
        ...
        sK = ((KE.flatten()[np.newaxis]).T * (density_min + xPhys ** penal_in_iteration * (density_max - density_min))).flatten(order='F')
        ...
        # Solve linear system
        deformation[free, :] = np.reshape(spsolve(K, f[free]), [len(free), -1])
        ...
        for e in deformation.T:
            deformation_element = e[element_dof_matrix].reshape(element_num, dof_per_element)
            energy_element = (np.dot(deformation_element, KE) * deformation_element).sum(1)  # strain energy of an element
