...
class CustomIngestionPipeline:
    def __init__(
        ...
        embed_model: BaseEmbedding | None
            the embedding model to use for the ingestion pipeline
            default is `CohereEmbedding`
        embed_dim: int | None
            the embedding dimension to use for the ingestion pipeline
            default is `None`
        """
        ...
        if embed_model is not None:
            ...
            if testing:
                self.embed_model = MockEmbedding(embed_dim=self.embedding_dim)
            else:
    ...
    def run_pipeline(
        ...
        transformations : list[TransformComponent]
            list of transformations to apply to the documents
            if `None`, it would use the default transformations which are:
        ...
        if transformations is None:
            ...
            transformations_pipeline = transformations
