    ...
    def hhfilter(
        ...
        if shutil.which(binary) is None:
            raise FileNotFoundError(
                f"未找到 hhfilter 可执行文件: '{binary}'。请安装 HH-suite 并将其加入 PATH，或使用 data.sample_method=diversity-max / diversity-min / sample-pretrained。"
            )
        with tempfile.TemporaryDirectory(dir=current_directory) as tempdirname:    # dir="/userhome/zhouyq/gaozhq/zhyk/projects/emb-transformer/data"
            ...
            if result.returncode != 0:
                ...
                raise RuntimeError(
                    f"运行 hhfilter 失败(returncode={result.returncode})，stderr: {stderr[:300]}...\n建议改用 data.sample_method=diversity-max"
                )
    ...
    def select_diverse(self, num_seqs: int, method: str = "hhfilter") -> "MSA":
        ...
        if method == "hhfilter":
            try:
                ...
                print(f"[Warn] hhfilter 不可用或运行失败: {e}\n[Warn] 回退到 diversity-max 策略。可通过覆盖 data.sample_method=diversity-max 避免此警告。")
