...
class ProjectManagementApi:
    ...
    async def post_api_projects_projectid_reset_settings(
        ...
        post_api_projects_projectid_reset_settings
         ...
         Reset the settings of an existing **Project** to their default values.  Default values are:   Setting | Default | -----------|---------|  `temperature` | 0.0 |  `rasterizationDPI` | 115 |  `maxOutputTokens` | 0 (no limit) |  `degradedMode` | Reject |  `maxTokensSmartExample` | 16000 |  #### Error Responses: `404 Not Found` - If a **Project** with the specified `projectId` does not exist.  `403 Forbidden` - If the user does not have permission to update this **Project**.  `403 Locked` - If the **Project** is locked.
