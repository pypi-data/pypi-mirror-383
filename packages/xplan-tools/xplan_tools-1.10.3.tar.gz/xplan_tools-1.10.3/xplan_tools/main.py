...
from xplan_tools.util import serialize_style_rules
...
def convert(
    ...
    xtrasse: Annotated[
        bool,
        typer.Option(help="Specify input as XTrasse file"),
    ] = False,
):
    ...
    def dump_features(input: str, output: str):
        ...
        if xtrasse:
            repo = repo_factory(input, data_type="xtrasse")
            data_type = "xtrasse"
        else:
        ...
        if to_version:
            path = migration_paths.get((version_prefix, to_version))
            if not path:
                error_console.log(
                    f"Migration from version {data_type} {repo.version} to {data_type} {to_version} not yet implemented"
                )
                return
            for step in path:
                console.log(f"Migrating from version {version} to {step}")
                collection = transformer_factory(collection, step).transform()
                version = step
            if to_version == "plu":
                data_type = "plu"
            else:
        ...
        if xplan_validator:
            success = asyncio.run(
                xplan_validate(
                    ...
                    format=validator_report_format,
        ...
        if output == "stdout":
            ...
            if single_plans and output[:4] not in ["post", "gpkg", "sqli"]:
                ...
                repo_factory(output, version, data_type).save_all(
                    collection,
                    write_geometry=write_geometry,
                    write_bbox=write_bbox,
