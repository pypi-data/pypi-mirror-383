...
class EnumetypedPydantic(Enumetyped[Content], metaclass=EnumetypedPydanticMeta):
    ...
    def __variant_constructor__(
        ...
        # TODO: inspect and test
        if issubclass(cls.content_type(), pydantic.BaseModel) and not isinstance(value, cls.content_type()):
            pm: pydantic.BaseModel = cls.content_type()  # type: ignore  #noqa
            value = pm.model_validate(value)
