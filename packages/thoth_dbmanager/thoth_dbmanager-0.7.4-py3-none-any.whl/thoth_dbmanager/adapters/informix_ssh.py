...
class InformixSSHAdapter(DbAdapter):
    ...
    def get_foreign_keys_as_documents(self) -> List[ForeignKeyDocument]:
        ...
        Uses sysconstraints, sysreferences, sysindexes, and syscolumns to retrieve 
        FK relationships with actual column names. This implementation uses 
        sysindexes.part1 to get column numbers instead of array access (ref.foreign[1]),
        which is compatible with older Informix versions that don't support array syntax.
        
        Note: This implementation handles single-column foreign keys. Multi-column FKs
        will only show the first column pair (part1).
        ...
        try:
            ...
            for row in results:
                # Access row values directly (Row object doesn't have .get() method)
                source_table = row['source_table'].strip() if row['source_table'] else ''
                target_table = row['target_table'].strip() if row['target_table'] else ''
                constraint_name = row['constraint_name'].strip() if row['constraint_name'] else ''
            ...
            logger.info(f"Retrieved {len(documents)} foreign key relationships using compatible query")
            return documents
            
        except Exception as e:
            logger.error(f"Error getting foreign keys: {e}")
            logger.error("Returning empty FK list due to error")
            return []
