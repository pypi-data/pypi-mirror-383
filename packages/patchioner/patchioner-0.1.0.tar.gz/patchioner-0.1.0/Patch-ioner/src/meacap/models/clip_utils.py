...
class CLIP(nn.Module):
    ...
    def compute_image_text_similarity_via_embeddings(self, image_embeds, text_embeds) -> Tuple[torch.Tensor, torch.Tensor]:
        '''
    ...
    def compute_image_text_similarity_via_embeddings_new(self, image_embeds, text_embeds) -> Tuple[torch.Tensor, torch.Tensor]:
        '''
        image_embeds: [B, D]
        text_embeds: [N, D]
        '''
        ...
        logits_per_image = image_embeds @ text_embeds.T  # [B, N]

        logit_scale = self.model.logit_scale.exp()
        logits_per_image = logits_per_image * logit_scale

        return logits_per_image.softmax(dim=1), logits_per_image / logit_scale
