...
class AttentionPool2d(nn.Module):
    ...
    def forward(self, x, return_local_features=False):
        ...
        if return_local_features: return x[0], x[1:]
...
class ModifiedResNet(Backbone):
    ...
    def __init__(self, layers, output_dim, heads, input_resolution=224, width=64, 
        ...
        if self.pool_vec or create_att_pool:  # pool a vector representation for an image
            ...
            #last_feat = 'res5' if 'res5' in self._out_features else 'res4'
            #embed_dim = self._out_feature_channels[last_feat]
            #stride = self._out_feature_strides[last_feat]
            stride = 32
            embed_dim = width * stride


            self.attnpool = AttentionPool2d(input_resolution // stride, embed_dim, heads, output_dim)
    ...
    def stem(self, x):
        """
        Stem of the ResNet.
        Computes the first 3 convolutions and the average pooling.
        Do not call this method directly, use forward() instead.
        """
    ...
    def forward(self, x):
        ...
        x = self.stem(x) # det2 resnet50: [64, 200, 304]; CLIP resnet50: [64, 56, 56]
        ...
        x = x_5 if "res5" in self._out_features else x # det2 resnet50: [2048, 25, 38]; CLIP resnet50: [2048, 7, 7]
        ...
        if self.pool_vec:  # pool a vector representation for an image, for global image classification
            x = self.attnpool(x_5) # CLIP resnet50: [1024]
            return x
        else:  # for FPN
            return outputs
    
    def forward_return_spatial_feats(self, x, use_layer3=False, use_attnpool_for_spatial_feats=True):
        """
        Forward pass that returns spatial features for each layer.
        outputs is a dict with keys:
         - x_norm_clstoken
         - x_norm_patchtokens
        The values are the normalized features for the class token and patch tokens.
        Since this is a ResNet, the class token is the 
        
        Args:
            x: input tensor
            use_layer3: if True, use features from layer3 instead of layer4 for spatial features
        """
        outputs = {}
        ...
        # Choose which layer to use for spatial features
        if use_layer3:
            spatial_x = x_3  # Use layer3 features (1024-dim, higher spatial resolution)
        else:
            spatial_x = x_5 if "res5" in self._out_features else x_3
        
        # Always use layer4 for global CLS token (via attention pooling)
        if self.pool_vec:
            if "res5" in self._out_features:
                if use_attnpool_for_spatial_feats:
                    x_norm_cls_token, spatial_features = self.attnpool(x_5, return_local_features=True)
                else:
                    x_norm_cls_token = self.attnpool(x_5, return_local_features=False)
                    x_5 = x_5.reshape(x_5.shape[0], x_5.shape[1], x_5.shape[2] * x_5.shape[3]).permute(2, 0, 1)  # NCHW -> (HW)NC
                    spatial_features = self.attnpool.c_proj(x_5)  # Use the projection layer directly
                    
                # spatial_features has shape # (HW)NC
                # we want it to be (B, H*W, C)
                spatial_features = spatial_features.permute(1, 0, 2)  # (B, H*W, C)
                
            else:
                x_norm_cls_token = self.attnpool(x_5)
        else:
            x_norm_cls_token = None
            spatial_features = None
   
        if spatial_features is None:  # when res5 not in out_features, use layer3 features  
            B, C, H, W = spatial_x.shape
            spatial_features = spatial_x.reshape(B, C, H * W).transpose(1, 2)  # [B, H*W, C]

        # apply normalization
        spatial_features = F.normalize(spatial_features, dim=-1)  # Normalize along the feature dimension
        x_norm_cls_token = F.normalize(x_norm_cls_token, dim=-1) if x_norm_cls_token is not None else None
        outputs["x_norm_clstoken"] = x_norm_cls_token
        outputs["x_norm_patchtokens"] = spatial_features
