...
from talk2dino import ProjectionLayer
...
if __name__ == "__main__":
    ...
    if args.use_t2d:
        # loading Talk2DINO
        print(f"Loading Talk2DINO weights from {args.talk2dino_weights_path}")
        talk2dino = ProjectionLayer.from_config(args.talk2dino_config_path)
        talk2dino.load_state_dict(torch.load(args.talk2dino_weights_path, device))
        talk2dino.to(device)
        talk2dino.eval()
    else:
        talk2dino = None
    ...
    for idx in tqdm(range(0,len(textual_data),batch_size), dynamic_ncols=True):
        ...
        clip_embeds = vl_model.compute_text_representation(text_list)
        if args.use_t2d:
            with torch.no_grad():
                clip_embeds = talk2dino.project_clip_txt(clip_embeds)
        clip_embeds = clip_embeds.detach().cpu()
