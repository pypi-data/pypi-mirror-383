...
async def test_apply_filters(settings, sample_filtered_response):
    ...
    client.get_json = AsyncMock(return_value=sample_filtered_response.model_dump(by_alias=True))
    ...
    client.get_json.assert_called_once_with("GET", "/raindrops/search", params=expected_params)
...
async def test_get_filtered_bookmarks_by_collection(settings, sample_filtered_response):
    ...
    expected_params = filter_params.model_dump(exclude_none=True)
    client.get_json.assert_called_once_with("GET", "/raindrops/5", params=expected_params)