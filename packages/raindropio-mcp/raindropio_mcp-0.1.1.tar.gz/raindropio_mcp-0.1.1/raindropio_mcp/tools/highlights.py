...
def register_highlight_tools(
    ...
    async def list_highlights(bookmark_id: int) -> list[dict[str, Any]]:
        highlights = await client.list_highlights(bookmark_id)
        return [highlight.model_dump(by_alias=True, exclude_none=True) for highlight in highlights]
    ...
    async def get_highlight(highlight_id: int) -> dict[str, Any]:
        highlight = await client.get_highlight(highlight_id)
        return highlight.model_dump(by_alias=True, exclude_none=True)
    ...
    async def create_highlight(bookmark_id: int, payload: dict[str, Any]) -> dict[str, Any]:
        model = HighlightCreate.model_validate(payload)
        created = await client.create_highlight(bookmark_id, model)
    ...
    async def update_highlight(highlight_id: int, payload: dict[str, Any]) -> dict[str, Any]:
        model = HighlightUpdate.model_validate(payload)
        updated = await client.update_highlight(highlight_id, model)
