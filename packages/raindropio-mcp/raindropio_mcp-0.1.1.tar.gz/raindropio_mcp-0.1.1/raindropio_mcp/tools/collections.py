...
def register_collection_tools(
    ...
    @registry.register(
        ToolMetadata(
            ...
            description="List all Raindrop.io collections visible to the authenticated account.",
            category=ToolCategory.COLLECTIONS,
            examples=[
                {
                    "args": [],
                    "description": "Retrieve metadata for folders to let the user choose where to store bookmarks.",
                }
            ],
        )
    )
    async def list_collections() -> list[dict[str, Any]]:
        ...
        return [collection.model_dump(by_alias=True, exclude_none=True) for collection in collections]
    ...
    @registry.register(
        ToolMetadata(
            ...
            examples=[
                {
                    "args": [12345],
                    "description": "Load collection metadata before presenting it to the user.",
                }
            ],
        )
    )
    async def get_collection(collection_id: int) -> dict[str, Any]:
        ...
        return collection.model_dump(by_alias=True, exclude_none=True)
            ...
            examples=[
                {
                    "args": [
                        {
                            "title": "AI Research",
                            "description": "All papers related to retrieval-augmented generation.",
                        }
                    ],
    ...
    async def create_collection(payload: dict[str, Any]) -> dict[str, Any]:
        ...
        created = await client.create_collection(model)
        return created.model_dump(by_alias=True, exclude_none=True)

    @registry.register(
        ToolMetadata(
            name="update_collection",
            description="Update collection metadata such as title, description, or color.",
            category=ToolCategory.COLLECTIONS,
        )
    )
    async def update_collection(collection_id: int, payload: dict[str, Any]) -> dict[str, Any]:
        model = CollectionUpdate.model_validate(payload)
        updated = await client.update_collection(collection_id, model)
        return updated.model_dump(by_alias=True, exclude_none=True)

    @registry.register(
        ToolMetadata(
            name="delete_collection",
            description="Delete a collection by id. Bookmarks inside move to Uncategorized (id=0).",
            category=ToolCategory.COLLECTIONS,
        )
    )
