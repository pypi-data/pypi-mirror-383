...
def register_filter_tools(
    ...
    @registry.register(
        ToolMetadata(
            name="apply_filters",
            description="Apply various filters to search and organize bookmarks across all collections.",
            ...
            examples=[
                {
                    ...
                    "description": "Find important AI research bookmarks created this year."
                }
            ],
        )
    )
    async def apply_filters(payload: dict[str, Any]) -> dict[str, Any]:
        ...
        result = await client.apply_filters(model)
    ...
    async def get_filtered_bookmarks_by_collection(collection_id: int, payload: dict[str, Any]) -> dict[str, Any]:
        model = BookmarkFilter.model_validate(payload)
        result = await client.get_filtered_bookmarks_by_collection(collection_id, model)
        return result.model_dump(by_alias=True, exclude_none=True)
