...
def register_bookmark_tools(
    ...
    async def get_bookmark(bookmark_id: int) -> dict[str, Any]:
        ...
        return bookmark.model_dump(by_alias=True, exclude_none=True)
    ...
    async def create_bookmark(collection_id: int, payload: dict[str, Any]) -> dict[str, Any]:
        model = BookmarkCreate.model_validate(payload)
        bookmark = await client.create_bookmark(collection_id, model)
        return bookmark.model_dump(by_alias=True, exclude_none=True)
    ...
    async def update_bookmark(bookmark_id: int, payload: dict[str, Any]) -> dict[str, Any]:
        model = BookmarkUpdate.model_validate(payload)
        bookmark = await client.update_bookmark(bookmark_id, model)
        return bookmark.model_dump(by_alias=True, exclude_none=True)
