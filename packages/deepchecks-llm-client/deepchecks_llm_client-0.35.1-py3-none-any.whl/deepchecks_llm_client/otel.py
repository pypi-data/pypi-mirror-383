...
class JsonExporter(BaseExporter):
    ...
    def export(
        ...
        try:
            ...
            # write spans in json format to spans.json:
            spans_json_file = self.output_folder / f'spans{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
            with open(spans_json_file, 'w', encoding='utf-8') as f:
                json.dump([json.loads(span.to_json()) for span in spans], f, indent=2)
            return SpanExportResult.SUCCESS
        except Exception as e: # pylint: disable=broad-except
            # Handle export failure
            logger.error(f"Export failed: {e}")
