...
class KitechClient:
    ...
    def __init__(self, config: Config | None = None, token: str | None = None):
        ...
        Args:
            ...
            token: API token. If not provided, uses token from config (if available)
        """
        self.config = config or Config.load()
        self.auth_manager = AuthManager(self.config)

        # Use provided token, or fall back to token from config
        token_to_use = token or self.config.api_token
        if token_to_use:
            self.auth_manager.login(token=token_to_use)
    ...
    def list_files(
        ...
        try:
            # Try new API endpoint first: /repositories/{id}/files
            response = self.client.get(
                f"/repositories/{repository_id}/files", params=params
            )
            response.raise_for_status()
            data = response.json()
    ...
    def get_download_url(self, repository_id: int, path: str | None = None) -> str:
        ...
        try:
            response = self.client.get(
                f"/repositories/{repository_id}/download", params=params
            )
            response.raise_for_status()
            data = response.json()
    ...
    def get_batch_download_info(
        ...
        try:
            response = self.client.post(
                f"/repositories/{repository_id}/download-list", json={"paths": paths}
            )
            response.raise_for_status()
            data = response.json()
    ...
    def upload_file(
        ...
        try:
            ...
            with open(file_path, "rb") as f:
                ...
                response = self.client.post(
                    f"/repositories/{repository_id}/upload", files=files, data=data
                )
