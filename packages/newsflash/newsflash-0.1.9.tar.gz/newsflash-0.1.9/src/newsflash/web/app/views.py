...
def parse_request_inputs(request: HttpRequest) -> tuple[str, dict[str, str], ChartDimensions]:
...
def resolve_forward_ref(widget_type: Type[Widget], callback_fn: Callable) -> Type[Widget]:
...
def process_callback_arg(callback_fn: Callable, parameter: str) -> tuple[Type[Widget], str, WidgetIO]:
...
def render_callback_outputs(callback_outputs: dict[str, Widget], request: HttpRequest) -> bytes:
    result: str = ""
    ...
    for callback_output in callback_outputs.values():
        ...
        if isinstance(callback_output, Chart):
            result += ("\n" + callback_output.render_chart(request, authenticated=request.user.is_authenticated))
        else:
...
def build_button_view(app: Type[App]) -> Callable:
    ...
    def click(request: HttpRequest) -> HttpResponse:
        trigger, additional_inputs, chart_dimensions_dict = parse_request_inputs(request)
