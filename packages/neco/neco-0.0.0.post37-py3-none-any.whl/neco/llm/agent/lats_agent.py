...
class LatsAgentNode(ToolsNodes):
    ...
    def get_reflection_chain(self, state: LatsAgentState, config: RunnableConfig):
        ...
        async def reflection_chain_async(inputs):
            ...
            candidate_messages = inputs.get("candidate", [])
            
            # 将候选消息转换为字符串
            candidate_str = ""
            if isinstance(candidate_messages, list):
                candidate_str = "\n".join([
                    f"消息{i+1}: {msg.content if hasattr(msg, 'content') else str(msg)}"
                    for i, msg in enumerate(candidate_messages)
                ])
            else:
                candidate_str = str(candidate_messages)
            
            # 组合系统消息和用户输入
            full_message = f"{system_message}\n\n用户问题: {user_input}\n\n候选回答:\n{candidate_str}"

            result = await self.call_with_structured_output(
                llm=llm, user_message=full_message, pydantic_model=Reflection
            )
            return result
