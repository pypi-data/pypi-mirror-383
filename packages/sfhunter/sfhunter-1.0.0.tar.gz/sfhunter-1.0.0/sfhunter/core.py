...
class SFHunter:
    ...
    def check_salesforce_headers(self, response: requests.Response) -> Tuple[bool, List[str]]:
        ...
        salesforce_headers = [
            ...
            ('x-request-id', 'Salesforce Request ID (Alt)')
        ]
    ...
    def check_salesforce_content(self, content: str) -> Tuple[bool, List[str]]:
        ...
        # Aura/Lightning detection (primary method)
        if "aura" in content_lower or "lightning" in content_lower:
            signals.append("Aura/Lightning")
        
        # Salesforce branding
        if "salesforce" in content_lower or "visualforce" in content_lower:
    ...
    def follow_redirects(self, url: str) -> Tuple[str, List[str]]:
        ...
        for _ in range(max_redirects):
            try:
                ...
                logger.warning(f"Error following redirect for {current_url}: {e}")
                break
        ...
        try:
            ...
            for chain_url in redirect_chain:
                if self.is_salesforce_url(chain_url):
                    logger.info(f"Salesforce detected in redirect chain: {chain_url}")
            ...
            if self.is_salesforce_url(response.url):
                logger.info(f"Salesforce detected in final URL: {response.url}")
            ...
            if is_salesforce:
                logger.info(f"Salesforce detected in headers for: {response.url} - Signals: {', '.join(signals)}")
            ...
            if is_salesforce:
                logger.info(f"Salesforce detected in content for: {response.url} - Signals: {', '.join(signals)}")
                return self.create_detection_result(url, response.url, redirect_chain, "content", signals)
            
            logger.info(f"No Salesforce indicators found for: {url}")
            return None
...
"""
        ...
        try:
            ...
            logger.error(f"Telegram embed error: {e}")
    ...
    def print_progress(self):
        ...
        with self.stats_lock:
            ...
            if total > 0:
                ...
                print(f"\r[PROGRESS] {processed}/{total} ({percentage:.1f}%) | Rate: {rate:.1f}/s | ETA: {eta_minutes:.1f}m | Success: {success} | Failed: {failed} | Verified: {verified} | Unverified: {unverified}", end='', flush=True)
