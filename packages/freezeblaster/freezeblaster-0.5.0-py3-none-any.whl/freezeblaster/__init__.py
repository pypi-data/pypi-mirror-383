...
_Freezable = (
    ...
    | datetime.timedelta
    | types.FunctionType
    | Callable[[], str | datetime.datetime | datetime.date | datetime.timedelta]
...
_is_cpython = hasattr(platform, "python_implementation") and platform.python_implementation().lower() == "cpython"
...
def fake_localtime(t: float | None = None) -> time.struct_time:
    if t is not None:
...
def fake_gmtime(t: float | None = None) -> time.struct_time:
    if t is not None:
...
def _get_fake_monotonic_ns() -> int:
    # For monotonic timers like .monotonic(), .perf_counter(), etc
    current_time = get_current_time()
    return (calendar.timegm(current_time.timetuple()) * 1000000 + current_time.microsecond) * 1000
...
class FakeDatetime(real_datetime, FakeDate, metaclass=FakeDatetimeMeta):
    ...
    @classmethod
    def fromtimestamp(cls, t: float, tz: datetime.tzinfo | None = None) -> "FakeDatetime":
        if tz is None:
    ...
    @staticmethod
    def _time_to_freeze() -> datetime.datetime | None:
        if freeze_factories:
            return get_current_time()
        return None
...
) -> Tuple[Type[FakeDatetime], Tuple[int, int, int, int, int, int, int, datetime.tzinfo | None]]:
...
def _parse_time_to_freeze(time_to_freeze_str: _Freezable | None) -> datetime.datetime:
...
def _parse_tz_offset(tz_offset: datetime.timedelta | float) -> datetime.timedelta:
...
class TickingDateTimeFactory:
    ...
    def tick(self, delta: datetime.timedelta | float = datetime.timedelta(seconds=1)) -> datetime.datetime:
...
class StepTickTimeFactory:
    ...
    def tick(self, delta: datetime.timedelta | float | None = None) -> datetime.datetime:
        if not delta:
...
class _freeze_time:
    def __init__(
        ...
        tz_offset: int | datetime.timedelta,
        ignore: list[str],
        tick: bool,
        as_arg: bool,
        as_kwarg: str,
    ...
    def __call__(
        self,
        func: Type[T2] | "Callable[P, Awaitable[Any]]" | "Callable[P, T]",
    ) -> Type[T2] | "Callable[P, Awaitable[Any]]" | "Callable[P, T]":  # type: ignore
    ...
    def __enter__(self) -> StepTickTimeFactory | TickingDateTimeFactory | FrozenDateTimeFactory:
        return self.start()

    def __exit__(self, *args: Any) -> None:
        self.stop()

    def start(self) -> StepTickTimeFactory | TickingDateTimeFactory | FrozenDateTimeFactory:
        if self.auto_tick_seconds:
            freeze_factory: StepTickTimeFactory | TickingDateTimeFactory | FrozenDateTimeFactory = StepTickTimeFactory(
...
def freeze_time(
    time_to_freeze: _Freezable | None = None,
    tz_offset: int | datetime.timedelta = 0,
    ...
    if isinstance(time_to_freeze, types.FunctionType):
        return freeze_time(
            time_to_freeze(), tz_offset, ignore, tick, as_arg, as_kwarg, auto_tick_seconds, real_asyncio=real_asyncio
        )
