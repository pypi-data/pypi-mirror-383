...
def main(image_paths, jsons, datasets):
    ...
    #-----------------加载权重-----------------------
    model_path = "/data/cbl/IQA/zhicheng/AGRM/checkpoint_gen/clipiqa2_ori_3kmos_ep200_tsp15_sche/1/quality_best_ckpt.pt"
    checkpoint = torch.load(model_path, map_location='cuda')
    iqamodel.load_state_dict(checkpoint['model_state_dict'])
    iqamodel.to('cuda')
    print('Successfully load ' + model_path)
    #----------------------------------------------
...
def run_base_constant_ablation(dataset_config, image_paths_all):
    ...
    # base_norm_values = [0.4, 0.6, 1.0, 1.4]
    # alpha_values = [1.0]
    ...
    for dataset_name, json_paths in dataset_config.items():
        ...
        for llddata in tqdm(all_data):
            ...
            if dataset_name == 'AGIQA-1k':
                raw_image = agiqa1k_nrm_process(raw_image_ini).unsqueeze(0)
            elif dataset_name == 'SPAQ':
                raw_image = spqa_nrm_process(raw_image_ini).unsqueeze(0)
            else:
                raw_image = nrm_process(raw_image_ini).unsqueeze(0)
            ...
            I_resized = F.interpolate(raw_image, size=(224, 224), mode='bilinear', align_corners=False).to('cuda')
            X_sub = torch.cat([X_sub, I_resized], dim=0)              
            clipiqa, image_features_org = iqamodel(X_sub) 
...
checkpoint_dir = 'rebuttal/alpha_beta1.0_0.6'
score_type = 'mos'
