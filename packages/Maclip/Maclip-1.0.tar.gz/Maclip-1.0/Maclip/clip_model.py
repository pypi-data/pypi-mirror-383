...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_token=False, pos_embedding=False):
        ...
        token2 = self.ln_post(x)
        # ipdb.set_trace()
        x = self.ln_post(x[:, 0, :])

        if self.proj is not None:
            x = x @ self.proj
            token2 = token2 @ self.proj
...
class CLIP(nn.Module):
    ...
    def encode_image2(self, image, pos_embedding):
        return self.visual(image.type(self.dtype), pos_embedding=pos_embedding, return_token=True)
    ...
    def forward(self, image, text, pos_embedding=False, text_features=None):
        ...
        # normalized features
        # image_features_nrm = image_features.norm(dim=-1, keepdim=True)
        
        # image_features_abs = torch.abs(image_features)
        # image_features_nrm = image_features_abs.std(dim=-1, keepdim=True)
