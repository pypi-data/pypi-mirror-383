...
def query_sls_logs(logstore_name: str = '',
    ...
    to_print_file(f"日志库中查询, 共 {response.get_count()} 条日志")
    logs = response.get_logs()
    log_set = set()
    for log in logs:
        log_set.add(clean_up_msg(get_log_msg(log.contents, default_tz=default_tz), clean_up_type=clean_up_msg_type))
    log_list = list(log_set)
    log_list.sort()
    to_print_file(f"去除重复以后, 共 {len(log_list)} 条日志")
    for log in log_list:
        to_print_file(log)
...
def search_lts_logs(keywords: str | None = '',
    ...
    if logs and logs.logs:
        to_print_file(f"日志库中查询, 共 {len(logs.logs)} 条日志")
        log_set = set()
        for log in logs.logs[::-1]:
            log_set.add(clean_up_msg(get_log_msg(json.loads(str(log)), default_tz=default_tz), clean_up_type=clean_up_msg_type))
        log_list = list(log_set)
        log_list.sort()
        to_print_file(f"去除重复以后, 共 {len(log_list)} 条日志")
        for log in log_list:
