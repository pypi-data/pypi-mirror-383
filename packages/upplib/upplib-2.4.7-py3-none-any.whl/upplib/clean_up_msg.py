...
def clean_up_msg_1(msg: str = None) -> str:
    try:
        """
            ...
            ->
            2025-10-09T11:29:30.561+08:00 -97- com.leo.rcs.biz.aspect.RcsReportAspect - datafeaturecore-0a5a030c-488883-287895 - RequestBody:{"bizData":{"user_id":1073850014231710218,"gaid":"364de12f-2fc9-4769-b2b6-e0b47bdf841d"},"extContext":{"mainDecisionId":"20251009112924085WITHDRAW02124","standardScene":"WITHDRAW"},"ignoreCache":false,"ignoreStatus":false,"interfaceId":"mobilewalla","methodId":"multiagents"}
        """
        SEP_S = '- traceId: -'
        time1 = msg.strip().split(' ')[0]
        msg1 = msg[len(time1):].split(SEP_S)
        msg10 = msg1[0].strip()
        thread_id = ' '
...
def clean_up_msg_2(msg: str = None) -> str:
    try:
        """
            ...
            ->
            2025-10-09T13:45:49.687123+08:00 -04- c.l.r.b.s.device.impl.DeviceServiceImpl - customer-product-0a5a0329-488885-107496 - checkDeviceId lock key: 1073852969169211259
            ...
            ->
            2025-10-10T20:16:30.071887+08:00 -05- .l.r.b.s.d.s.i.DelayTaskCoreServiceImpl - - DelayTaskCoreServiceImpl queryTaskResult response: {"sign":null,"hitResultList":[],"branchRejectInfo":[]}
            
        """
        time1 = msg.strip().split(' ')[0]
        msg0 = msg[len(time1):].strip()
        try:
            thread_id = get_thread_id_for_log(msg0.split('] ')[0].split('-')[-1])
        except Exception:
            thread_id = ' '
        method = msg0.split(' : ')[0].strip()[-31:]
        if len(method) < 31:
            method = ' ' * (31 - len(method)) + method
        other = msg0[msg0.find(method) + len(method):].strip()
        SEP_S = ': (catTraceId:'
        if SEP_S in other:
            trace_id = other[other.find(SEP_S) + len(SEP_S):other.find(') - ')].strip()
            other = other[other.find(trace_id) + len(trace_id) + 4:].strip()
        else:
            trace_id = ''
        if other.startswith(':'):
            other = other[1:].strip()
...
def clean_up_msg_4(msg: str = None) -> str:
    try:
        ...
        trace_id_mark = ']:(catTraceId:'
        if trace_id_mark in msg1[5]:
            trace_id = msg1[5].strip().split(trace_id_mark)[1].strip().split(') - ')[0].strip()
            other = msg0[msg0.find(trace_id) + len(trace_id) + 3:].strip()
        else:
            trace_id = msg1[4].strip()[msg1[4].strip().find('[') + 1:].strip()
            if trace_id.startswith('-'):
                trace_id = trace_id[1:].strip()
            if len(trace_id) == 0:
                other = msg0[msg0.find(method) + len(method) + 1:].strip()
            else:
                other = msg0[msg0.find(trace_id) + len(trace_id) + 1:].strip()
        return f'{time1}{thread_id}{method} - {trace_id} - {other}'
    except Exception:
        return msg
...
def clean_up_msg_5(msg: str = None) -> str:
    try:
        """
            ...
            ->
            2025-10-10T13:45:56+08:00 -57- infei.xdecision.redis.RedisLock - - Release distributed lock，clientId=10.90.0.132,lockKey=LOCK_PENDING_REENTRY_20251010135455914APPLY03025
