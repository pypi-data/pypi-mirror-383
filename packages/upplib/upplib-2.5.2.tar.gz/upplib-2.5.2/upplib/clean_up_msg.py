...
def clean_up_msg_1(msg: str = None) -> str:
    try:
        ...
        method, _, other = msg0.strip().partition(' - traceId: - ') if ' - traceId: - ' in msg0.strip() else ('', '', msg0.strip())
...
def clean_up_msg_5(msg: str = None) -> str:
    try:
        ...
        trace_id = f' - {trace_id} - ' if len(trace_id) > 0 else ' - '
        return f'{time1}{thread_id}{method}{trace_id}{other}' if len(other) > 0 else f'{msg}'
    except:
        return msg
