...
def search_lts_logs(keywords: str | None = '',
    ...
    if logs and logs.logs:
        ...
        for log in logs.logs[::-1]:
            log_list.append(clean_up_msg(get_log_msg(json.loads(str(log)), default_tz=default_tz), clean_up_type=clean_up_msg_type))
        log_list.sort()
        front_log = None
        log_count = 0
        for log in log_list:
            if log != front_log:
                ...
                to_print_file(log)
            front_log = log
        to_print_file(f"去除重复以后，共 {log_count} 条日志:")
    else:
