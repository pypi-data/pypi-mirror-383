...
rule coercive_injection {
    ...
    strings:
        ...
        $mandatory_execution = /(Always\s+(consult|use|run|execute|invoke|call|trigger|employ)\s+this\s+(tool|function|command|operation)\s+(immediately|first|before|prior|initially)|This\s+(tool|function|command|operation)\s+(is|must be)\s+(mandatory|critical|required|essential|necessary|crucial|vital|imperative))/i
