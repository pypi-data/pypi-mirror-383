...
def curl_to_request(curl_command: str) -> requests.Request:
    ...
    post_data: str | None = parsed_args.data or parsed_args.data_binary
    if post_data:
        method = "post"
    ...
    # Firefox wrongly prepends query params with a caret (^)
    parsed_args.url = parsed_args.url.replace("^", "")
