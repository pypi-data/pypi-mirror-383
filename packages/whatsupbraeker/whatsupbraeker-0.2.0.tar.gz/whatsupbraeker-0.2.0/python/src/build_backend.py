...
class build_go(Command):
    ...
    def run(self) -> None:
        ...
        gocache = Path(env.setdefault("GOCACHE", str(project_root / ".gocache")))
        gomodcache = Path(env.setdefault("GOMODCACHE", str(project_root / ".gomodcache")))
        ...
        go_version = subprocess.check_output(["go", "version"], cwd=project_root, env=env).decode().strip()
        self.announce(f"using go binary: {shutil.which('go')} ({go_version})", level=2)
