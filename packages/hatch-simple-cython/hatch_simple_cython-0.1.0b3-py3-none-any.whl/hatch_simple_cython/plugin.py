...
class SimpleCythonBuildHook(BuildHookInterface):
    ...
    def options(self) -> dict[str, Any]:
        ...
        if self._options is None:
            # Support both direct config and nested under "options" key
            # This allows flexibility in configuration style
    ...
    def package_dir(self) -> str:
        ...
        # Try to read packages from pyproject.toml
        pyproject_path = os.path.join(self.root, "pyproject.toml")
        packages = []
        
        self.app.display_debug(f"Looking for pyproject.toml at: {pyproject_path}")
        
        if os.path.exists(pyproject_path):
            try:
                with open(pyproject_path, "rb") as f:
                    pyproject_data = tomllib.load(f)
                    self.app.display_debug("Loaded pyproject.toml")
                    # Check for packages in tool.hatch.build.targets.wheel
                    packages = (
                    ...
                    self.app.display_debug(f"Found packages in config: {packages}")
            except Exception as e:
                self.app.display_warning(f"Failed to read pyproject.toml: {e}")
        
        if packages:
            # Use the first package specified
            pkg_name = packages[0]
            self.app.display_debug(f"Using package from config: {pkg_name}")
        else:
            # Fallback: Try to auto-detect from filesystem
            ...
            if os.path.exists(os.path.join(self.root, derived_name)):
                ...
                if self.is_src_layout:
                    search_dirs.append(os.path.join(self.root, "src"))
                ...
                if candidates:
                    ...
                    self.app.display_debug(f"Auto-detected package: {pkg_name}")
                else:
                    pkg_name = derived_name
                    self.app.display_debug(f"Using derived name: {pkg_name}")
    ...
    def find_cython_files(self) -> list[str]:
        ...
        self.app.display_debug(f"Searching in: {base_dir}")
        self.app.display_debug(f"Patterns: {patterns}")
        ...
        self.app.display_debug(f"Found files: {files}")
...
"""
    ...
    def _clean_intermediate_files(self) -> None:
        """Clean up intermediate files (.c, .cpp) generated by Cython.
        
        For wheel builds, we remove the generated C/C++ files to keep the
        source tree clean. The actual exclusion of files from the wheel is
        handled by hatchling's exclude configuration in pyproject.toml.
        
        We do NOT delete .py or .pyx source files here, as they may be needed
        for subsequent builds or debugging.
        """
        files_to_clean = self.find_cython_files()
        
        self.app.display_info("Cleaning intermediate build files...")
        
        for file_path in files_to_clean:
            full_path = os.path.join(self.root, file_path)
            
            # Remove generated C/C++ files only
            base = os.path.splitext(full_path)[0]
            for ext in [".c", ".cpp"]:
                c_file = base + ext
                if os.path.exists(c_file):
                    self.app.display_debug(f"Removing intermediate file: {c_file}")
                    try:
                        os.remove(c_file)
                    except OSError as e:
                        self.app.display_warning(f"Failed to remove {c_file}: {e}")

    def clean(self, versions: list[str]) -> None:
        """Clean up build artifacts.
        
        This is called by hatch's clean command to remove all build artifacts.
        """
        ...
        for root, dirs, files in os.walk(self.root):
            ...
            for filename in files:
                ...
                # Remove generated C files
                    ...
                    py_file = os.path.join(root, f"{base}.py")
                    # Only remove if there's a corresponding .pyx or .py source
                    if os.path.exists(pyx_file) or os.path.exists(py_file):
                        filepath = os.path.join(root, filename)
                        self.app.display_debug(f"Removing {filepath}")
                        try:
                            os.remove(filepath)
