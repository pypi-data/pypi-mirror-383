...
class Cloudflare:
    ...
    def get_domains(self: str) -> dict:
        ...
        # Need either X-Auth-Key or Authorization header
        if "X-Auth-Key" not in self._headers and "Authorization" not in self._headers:
    ...
    def get_custom_ruleset(self, domain_name: str) -> RulesetObject:
        ...
        .. note::
            If no custom ruleset is found, a default will be created automatically.

        >>> cf.get_custom_ruleset("example.com")
        >>> {"id": "a1b2c3", "name": "default", "source": "firewall_custom", "phase": "http_request_firewall_custom", ...}
        """

        rulesets = self.get_rulesets(domain_name)

        custom_rulesets = [x for x in rulesets["result"] if x.get("source") == "firewall_custom"]

        if not custom_rulesets:
            self.create_custom_ruleset(domain_name)
            return self.get_custom_ruleset(domain_name)

        custom_ruleset = custom_rulesets[0]
    ...
    def create_custom_ruleset(self, domain_name: str) -> bool:
        """Create a custom ruleset for a specific domain

        :exception Error: If the custom ruleset already exists

        >>> cf.create_custom_ruleset("example.com")
        # Will create a new default custom ruleset if none exists
        """

        rulesets = self.get_rulesets(domain_name)
        zone_id = rulesets["zone_id"]

        if any(x for x in rulesets["result"] if x.get("source") == "firewall_custom"):
            raise Error("Custom ruleset already exists")
    ...
    def get_rules(self, domain_name: str) -> dict:
        ...
        r = requests.get(
            f"https://api.cloudflare.com/client/v4/zones/{zone_id}/rulesets/{custom_ruleset_id}",
            headers=self._headers,
            timeout=5,
        )
    ...
    def create_rule(
        ...
        * position -> Rule position, with 1 being the first rule in the list
        ...
        if self.active_rules < self.max_rules:
            r = requests.post(
                f"https://api.cloudflare.com/client/v4/zones/{zone_id}/rulesets/{custom_ruleset_id}/rules",
                headers=self._headers,
                json=new_rule,
                timeout=5,
            )
            ...
            raise Error(
                f"Cannot create more rules ({self.active_rules} used / {self.max_rules} available)\n"
    ...
    def update_rule(
        ...
        * position -> Rule position, starting from 1
        ...
        r = requests.patch(
            f"https://api.cloudflare.com/client/v4/zones/{zone_id}/rulesets/{custom_ruleset_id}/rules/{rule_id}",
            headers=self._headers,
            json=updated_rule,
            timeout=5,
        )
        ...
        * actions_all -> Set the same action for all imported rules, \
        ...
        for file in files:
            if file.endswith(".txt"):
                ...
                if file.removesuffix(".txt") not in rules:
                    if self.active_rules < self.max_rules:
                        if actions_all:
