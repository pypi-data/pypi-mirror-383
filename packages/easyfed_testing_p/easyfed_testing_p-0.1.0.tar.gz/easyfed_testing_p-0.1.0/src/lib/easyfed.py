...
from src.lib.backends import Backend
from src.lib.backends.numpy import __execute as execute_numpy
from src.lib.backends.torch import __execute as execute_torch
from src.lib.config import JobConfiguration, Server, Client
from src.lib.job import execute_job_config
from inspect import getfile

def Federated(server: Server, clients: List[Client], easyfed_config: str, num_rounds=1, backend=Backend.PYTORCH):
     """
    ...
    Example:
        >>> server = Server()
        >>> clients = [Client(id=i) for i in range(3)]
        >>> Federated(server, clients, "configs/easyfed.yaml", num_rounds=10)
    """
    # TODO: get backend from cls
    def decorator(cls):
