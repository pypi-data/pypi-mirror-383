...
def build_model_from_schema(
    ...
    def _build(name: str, schema: Dict[str, Any]) -> Type[BaseModel]:
        ...
        for fname, fs in props.items():
            ...
            if ftype == "object":
                sub = _build(f"{name}_{fname.capitalize()}", fs)
                ann = sub if fname in required else Optional[sub]  # type: ignore
            elif ftype == "array":
                items = fs.get("items", {}) or {}
                if items.get("type") == "object":
                    sub = _build(f"{name}_{fname.capitalize()}Item", items)
    ...
    return create_model(name, __base__=StrictBase, **fields)
    ...
    for name, f in payload.model_fields.items():
        out_fields[name] = (f.annotation, Field(default=(... if f.is_required() else None)))
