...
class QwenProvider(LLMProvider):
    ...
    def __init__(self, api_key: str | None = None, model: str = "qwen-turbo", base_url: str | None = None):
        """
        ...
        if api_key:
            ...
            self.api_key = config_manager.get_provider_api_key("qwen")

        if not self.api_key:
            config = config_manager.get_provider_config("qwen")
            api_key_env = config.get("api_key_env") if config else "QWEN_API_KEY"
            raise ValueError(f"Qwen API key not found. Set {api_key_env} environment variable.")

        # Get base URL from parameter, env var, or config
        if base_url:
            self.base_url = base_url
        else:
            self.base_url = config_manager.get_provider_base_url("qwen")

        # Use OpenAI client with Qwen endpoint
