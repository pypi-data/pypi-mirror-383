...
class OpenRouterProvider(LLMProvider):
    """OpenRouter API provider for multiple model access."""

    def __init__(self, api_key: str | None = None, model: str = "openai/gpt-3.5-turbo", base_url: str | None = None):
        """
        ...
        if api_key:
            ...
            self.api_key = config_manager.get_provider_api_key("openrouter")

        if not self.api_key:
            config = config_manager.get_provider_config("openrouter")
            api_key_env = config.get("api_key_env") if config else "OPENROUTER_API_KEY"
            raise ValueError(f"OpenRouter API key not found. Set {api_key_env} environment variable.")

        # Get base URL from parameter, env var, or config
        if base_url:
            self.base_url = base_url
        else:
            self.base_url = config_manager.get_provider_base_url("openrouter")
