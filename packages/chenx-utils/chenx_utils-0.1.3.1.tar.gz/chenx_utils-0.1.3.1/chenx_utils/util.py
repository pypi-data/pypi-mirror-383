...
def is_base_model_class(type_hint: Type) -> bool:
    """
    检查类型是否是BaseModel的子类
    """
    try:
...
def generate_nested_metadata(model_class: BaseModel, prefix: str = "") -> dict:
    """
    递归生成嵌套BaseModel的元数据，返回嵌套结构
    """
    metadata = {}
    ...
    for field_name, field in annotations.items():
        ...
        # 检查是否是嵌套的BaseModel
        if is_base_model_class(field_info.annotation):
            # 递归解析嵌套的BaseModel
            nested_metadata = generate_nested_metadata(
            ...
            # 处理列表类型的特殊属性
            if "list" in field_metadata["type"] and field_info.json_schema_extra:
                field_metadata["max_length"] = field_info.json_schema_extra.get("max_length", 1)
...
def generate_metadata(model_class: BaseModel) -> dict:
    """
    生成模型元数据，支持嵌套BaseModel解析，返回嵌套结构
    """
    return generate_nested_metadata(model_class)


def generate_metadata_with_groups(model_class: BaseModel) -> dict:
    """
    生成带分组的元数据，将嵌套字段按组分类，返回嵌套结构
    """
    return generate_nested_metadata(model_class)
