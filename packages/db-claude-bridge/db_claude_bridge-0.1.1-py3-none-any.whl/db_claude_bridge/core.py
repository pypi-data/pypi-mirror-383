"""Core functionality for DB Claude Bridge - enables Claude Code CLI to authenticate and run through Databricks Foundation Models using Databricks OAuth."""
...
from .exceptions import AuthenticationError, ConfigurationError, DatabricksClaudeError


class DatabricksClaudeCore:
    """Core functionality for DB Claude Bridge - lightweight bridge for Claude Code CLI and Databricks Foundation Models."""
    ...
    def __init__(
        ...
        self.config_dir = Path.home() / '.db-claude-bridge'
    ...
    def _setup_logging(self) -> logging.Logger:
        ...
        logger = logging.getLogger('db-claude-bridge')
    ...
    def _install_claude_macos(self) -> bool:
        ...
        try:
            ...
            installer_url = (
                "https://storage.googleapis.com/claude-release/claude-installer.sh"
            )
            ...
            print(
                "  📝 Please install Claude CLI manually from: https://claude.ai/download"
            )
            return False
    ...
    def update_claude_config(
        self, token: str, base_url: Optional[str] = None, model: Optional[str] = None
    ) -> None:
    ...
    def prompt_for_configuration(self) -> Tuple[str, str]:
        ...
        print(
            "   Other options: databricks-claude-opus-4-1, databricks-claude-sonnet-4"
        )
    ...
    def is_authenticated(self) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            assert self.databricks_host is not None
            result = subprocess.run(
                [
                    'databricks',
                    'current-user',
    ...
    def refresh_token_and_execute(self, claude_args: list) -> int:
        ...
        try:
            ...
            if not claude_args:
                ...
                result = subprocess.run(
                    ['claude'], stdin=sys.stdin, stdout=sys.stdout, stderr=sys.stderr
                )
            else:
