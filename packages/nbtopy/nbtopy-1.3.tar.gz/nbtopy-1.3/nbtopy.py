...
def write_file(opath: Path, out: str) -> bool | None:
...
def main() -> None:
    ...
    opt.add_argument(
        '-f', '--force', action='store_true', help='force overwrite existing file[s]'
    )
    opt.add_argument(
        '-r',
        '--recurse',
    ...
    opt.add_argument(
        '-o', '--out', help="alternative output file name, or '-' for stdout"
    )
    opt.add_argument(
        '-d',
        '--dir',
    ...
    for ipath in args.ipynb_path:
        ...
        if not path.exists():
            ...
            if not args.no_warnings:
                print(
                    f'Skipping {path} : does not have .ipynb suffix.', file=sys.stderr
                )
        else:
            count += convert_file(path, dirout, args)
