...
from .button_group import MButtonGroupData, MToolButtonGroup
from .item_model import HeaderData, MSortFilterModel, MTableModel
from .item_view import MAbstractView, MBigView, MTableView
...
class MItemViewFullSet(QtWidgets.QWidget):
    ...
    def __init__(
        ...
        data_group: list[MButtonGroupData] = []
        if table_view:
        ...
        leader_view = cast(MAbstractView, self.stack_widget.widget(0))
        self.selection_model = leader_view.selectionModel()
        for index in range(self.stack_widget.count()):
            if index == 0:
                continue
            other_view = cast(MAbstractView, self.stack_widget.widget(index))
        ...
        if data_group and len(data_group) > 1:
            self.view_button_grp.checked_changed.connect(
        ...
        self.search_line_edit = MLineEdit().search_with_close().small()
    ...
    def enable_context_menu(self):
        for index in range(self.stack_widget.count()):
            view = cast(MAbstractView, self.stack_widget.widget(index))
    ...
    def set_selection_mode(self, mode: QtWidgets.QAbstractItemView.SelectionMode):
        for index in range(self.stack_widget.count()):
            view = cast(MAbstractView, self.stack_widget.widget(index))
            view.setSelectionMode(mode)

    def tool_bar_visible(self, flag: bool):
        self.tool_bar.setVisible(flag)

    @QtCore.Slot(QtCore.QModelIndex)
    def slot_left_clicked(self, start_index: QtCore.QModelIndex):
    ...
    def tool_bar_append_widget(self, widget: QtWidgets.QWidget):
        self.top_lay.addWidget(widget)

    def tool_bar_insert_widget(self, widget: QtWidgets.QWidget):
        self.top_lay.insertWidget(0, widget)

    @QtCore.Slot()
    def setup_data(self, data_list: list[dict[str, Any]]):
        self.source_model.clear()
        if data_list:
    ...
    @QtCore.Slot(int)
    def set_record_count(self, total: int):
