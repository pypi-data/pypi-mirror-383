...
class FluTheme(QObject):
    ...
    def _refreshColors(self):
        is_dark = self._dark()
        self.primaryColor = (
            self._accentColor.lighter if is_dark else self._accentColor.dark
        )
        self.backgroundColor = (
            QColor(0, 0, 0, 255) if is_dark else QColor(255, 255, 255, 255)
        )
        self.dividerColor = (
            QColor(80, 80, 80, 255) if is_dark else QColor(210, 210, 210, 255)
        )
        self.windowBackgroundColor = (
            QColor(32, 32, 32, 255) if is_dark else QColor(237, 237, 237, 255)
        )
        self.windowActiveBackgroundColor = (
            QColor(26, 26, 26, 255) if is_dark else QColor(243, 243, 243, 255)
        )
        self.fontPrimaryColor = (
            QColor(248, 248, 248, 255) if is_dark else QColor(7, 7, 7, 255)
        )
        self.fontSecondaryColor = (
            QColor(222, 222, 222, 255) if is_dark else QColor(102, 102, 102, 255)
        )
        self.fontTertiaryColor = (
            QColor(200, 200, 200, 255) if is_dark else QColor(153, 153, 153, 255)
        )
        self.itemNormalColor = (
            QColor(255, 255, 255, 0) if is_dark else QColor(0, 0, 0, 0)
        )
        self.frameColor = (
