...
# ruff: noqa: N815 N802 N801 N806 N816
...
class FluFrameless(QQuickItem, QAbstractNativeEventFilter):
    ...
    def nativeEventFilter(self, event: ..., message: int | None):
        if not Tools.isWin():
            return False
        from ctypes import POINTER, byref, cast
        from ctypes.wintypes import MSG, POINT, RECT, RECTL

        if event != qtNativeEventType or message is None:
            return False
