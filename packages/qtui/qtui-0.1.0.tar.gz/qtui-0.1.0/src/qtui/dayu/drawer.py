...
class MDrawer(QtWidgets.QWidget):
    ...
    def add_widget_to_bottom(self, button: QtWidgets.QWidget):
        self._bottom_lay.addWidget(button)

    def add_widget_to_top(self, button: QtWidgets.QWidget):
    ...
    def _set_proper_position(self):
        ...
        if self._position == MDrawer.RightPos:
            pos = parent.mapToGlobal(parent_geo.topRight())
            self.setFixedHeight(parent_geo.height())
        ...
        if self._position == MDrawer.BottomPos:
            pos = parent.mapToGlobal(parent_geo.bottomLeft())
            self.setFixedWidth(parent_geo.width())
            target_x = pos.x()
    ...
    def set_dayu_position(self, value: str):
        """
        ...
        if value in [MDrawer.BottomPos, MDrawer.TopPos]:
            self.setFixedHeight(int(200 * scale_x))
        else:
            self.setFixedWidth(int(200 * scale_x))
