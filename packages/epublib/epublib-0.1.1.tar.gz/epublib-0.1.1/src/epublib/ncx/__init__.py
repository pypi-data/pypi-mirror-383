...
class NCXHead(XMLParent[NCXMeta, NCXSoup]):
    ...
    def depth(self) -> int:
        """
        Return the depth of the navigation map strucutre.
        """
        try:
            meta = self["dtb:depth"]
        except KeyError as error:
    ...
    def total_page_count(self) -> int | None:
        """
        Return the total page count of the publication. If there are no navigable
    ...
    def max_page_number(self) -> int | None:
        """
        Return the largest value attribute on page targets in the page list. If
...
class NCXPageList(
    ...
    def reset(self, entries: Sequence[PageBreakData]) -> None:
        ...
        for index, pagebreak in enumerate(
            sorted(entries, key=attrgetter("page")), start=1
        ):
            __ = self.add_item(
                NCXPageTarget(
                    soup=self.soup,
