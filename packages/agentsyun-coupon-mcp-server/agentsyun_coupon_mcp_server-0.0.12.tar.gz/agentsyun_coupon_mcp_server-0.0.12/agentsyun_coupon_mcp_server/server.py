...
# 上线要求替换成实际的配置信息
CATEGORY_DICT = {
    "takeaway_coupons": 4,  # 外卖商品券
    "restaurant_coupons": 3,  # 美食优惠券
    "entertainment_coupons": 29,  # 休闲娱乐
    "hotel_coupons": 9,  # 酒店民宿
    "travel_coupons": 32,  # 门票度假
    "medical_coupons": 37,  # 医药
}
# 渠道
SOURCE_DICT = {
    "meituan": 1,
...
def filter_empty_params(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.debug(f"Filtered params: {filtered_params}")
    logger.debug(f"Filtered out {len(params) - len(filtered_params)} empty parameters")
