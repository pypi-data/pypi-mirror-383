    ...
    def searchfile(
        ...
        while nowdate <= enddate:
            for row, col in zip(rowid, colid) :
                # 20250708 官方更新了文件名规则，将结束日期0101改为了1231
                if nowdate.year >= 2024:
                    edate = nowdate.strftime('%Y1231')
                else:
    ...
    def download(self, outdir, url, tries=3, timeout=5*60, skip_download=False, cover=True, continuing=False, **kwargs):
        try:
            ...
            if isinstance(url, list):
                filename = []
                for url1 in url:
                    filename1 = wget(outdir, url1, tries=tries, timeout=timeout, skip_download=skip_download, cover=cover, continuing=continuing, **kwargs)
                    filename.append(filename1)
            elif isinstance(url, str):
                filename = wget(outdir, url, tries=tries, timeout=timeout, skip_download=skip_download, cover=cover, continuing=continuing, **kwargs)
