...
class downloadByESA :
    ...
    def getToken(self, username, password):
        ...
        if resp.status_code == 200 :
            ...
            self.logger.info('成功创建token【%s】' %(token))

            return token
        else:
            self.logger.error('创建token失败,请注册或传入正确的账号密码')
            self.logger.info('注册地址【%s】' %'https://identity.dataspace.copernicus.eu/auth/realms/CDSE/login-actions/registration?client_id=account-console&tab_id=kSJ8msgpvaw')
            return None
    ...
    def download(self, outdir, url, checksum=True, nodefilter=None, skip_download=False, **kwargs):
        ...
        if isinstance(url, list):
            ...
            for item in url :
                cnt -= 1
                print('='*100)
                self.logger.info('正在下载【%d】【%s】' %(cnt, item['title']))
    ...
    def format_query(self, collection, startdate, enddate,
        ...
        for attr, value in sorted(keywords.items()) :
            ...
            if key.lower() not in converters:
                self.logger.error('类型不支持【%s】' %(attributes[collection.upper()][attr]))
                continue
...
class Downloader:
    ...
    def download(self, product_info, directory=".", *, stop_event=None, skip_download=False):
        ...
        if skip_download :
            self.logger.info('跳过下载【%s】' %(product_info["path"]))
            return product_info
