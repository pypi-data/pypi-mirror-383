...
class spiderdownload(object):
    ...
    def _download(self, outdir, url, timeout=5*60, chunk_size=1024, skip_download=False, cover=False, continuing=False):
        ...
        if os.path.isfile(local_filename) :
            if cover :
                os.remove(local_filename)
                logger.warning('文件已存在，删除该文件后重新下载【%s】' %(local_filename))
            else:
                logger.info('文件已存在，跳过下载该文件【%s】' %(local_filename))
                return local_filename
        ...
        if continuing and os.path.isfile(tempfile) and file_size > 0:
            ...
            if already_downloaded_bytes < file_size :
                ...
                logger.warning('将继续断点续传：原数据【%.2f】MB, 已下载【%.2f】MB' %(file_size/1000000.0,
                                                                        already_downloaded_bytes/1000000.0))
            else:
                already_downloaded_bytes = 0
                continuing = False
        else:
        ...
        try:
            ...
            if file_size == os.path.getsize(tempfile):
                shutil.move(tempfile, local_filename)
                logger.info('成功下载【%s】' %(local_filename))
            elif file_size == -1 :
                shutil.move(tempfile, local_filename)
                logger.info('成功下载【%s】' %(local_filename))
            elif file_size <  os.path.getsize(tempfile) :
                logger.warning('该文件下载大小不一致【%d】【%d】【%s】' %(file_size,
                                                   os.path.getsize(tempfile),
                                                   local_filename))
                shutil.move(tempfile, local_filename)
            else:
                logger.error('下载失败【%s】' %(local_filename))
        except requests.exceptions.Timeout:
            logger.error("连接超时【{}】".format(timeout))
            return None
