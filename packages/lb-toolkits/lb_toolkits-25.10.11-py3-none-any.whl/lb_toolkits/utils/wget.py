...
def wget(outdir, url, username=None, password=None, token=None,
    ...
    if os.path.isfile(local_filename) :
        if cover :
            logger.warning('文件已存在，将执行删除文件【%s】' %(local_filename))
            os.remove(local_filename)
        else:
            logger.info('文件已存在，跳过下载【%s】' %(local_filename))
            return local_filename
    ...
    cmd.append('--wait=3 --no-check-certificate -e robots=off -np -R .html,.tmp -nH --cut-dirs=10')
    ...
    if os.path.isfile(tempfile) :
        ...
        try:
            os.rename(tempfile, local_filename)
        except BaseException :
            time.sleep(3)
            try:
                os.rename(tempfile, local_filename)
            except BaseException :
                logger.error('处理失败【%s】' %(tempfile))
