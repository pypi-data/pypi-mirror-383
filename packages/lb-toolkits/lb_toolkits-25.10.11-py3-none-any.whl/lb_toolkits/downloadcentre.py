...
from lb_toolkits.download import downloadGFS
from lb_toolkits.download import downloadNOAA
...
class DownloadCentre :
    ...
    def __init__(self, collection, username=None, password=None):
        ...
        elif collection.api_id in ['GFS'] :
            self.down = downloadGFS()
        elif collection.api_id in ['NOAA'] :
            self.down = downloadNOAA(emailId=username)
        else :
    ...
    def searchfile(
        ...
        if version is not None and version.lower() in ["Not provided".lower() ] :
            logger.error('暂不支持该数据集下载【%s】' %(collection))
            return None

        if description is not None :
            logger.info('正在查找【%s】' %(description))

        if self.collection.api_id in ['CMR'] :
            return self.down.searchfile(shortname=shortname,
                                        starttime=startDate,
                                        endtime=endDate,
                                        provider=provider,
                                        version=version,
            ...
            return self.down.searchfile(product=collection,
                                        startdate=startDate,
                                        enddate=endDate,
                                        bbox=bbox,
            ...
            return self.down.searchfile(startdate=startDate,
                                        enddate=endDate,
        ...
        elif self.collection.api_id in ['GFS'] :
            return self.down.searchfile(**kwargs)

        elif self.collection.api_id in ['NOAA'] :
            return self.down.searchfile(startdate=startDate,
                                        enddate=endDate,
                                        datatype=collection,
                                        **kwargs)
    ...
    def download(
        ...
        #     from lb_toolkits.utils.multiprocess import SmartThreadPool
        #
        #     pool = SmartThreadPool(min_workers=numthred, timeout=timeout)
        #     for url1 in url :
        #         pool.addtask(self.down.download, outdir=outdir,
        #                      url=url1,
        #                      tries=tries,
        #                      timeout=timeout,
        #                      skip_download=skip_download,
