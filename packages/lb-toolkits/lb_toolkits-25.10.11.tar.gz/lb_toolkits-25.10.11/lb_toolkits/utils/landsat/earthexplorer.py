...
class EarthExplorer(object):
    ...
    def createToken(self):
        ...
        "error": false,
        "message": "",
        "token": "Q5ygM1up5UDcYUcksdWs7@pwA!!guw@uFbsTRFMhnlRShWDhzOnLdCwUtponohQ1",
        "tokenCount": 2
    }
        '''

        url = 'https://ers.cr.usgs.gov/password/appgenerate?operation=generate'

        data = {
            'date_expires' : datetime.datetime.now().strftime('%Y-%m-%d'),
            'scopes':'M2M',
            'token_description': 'xx'
        }
        rsp = self.session.post(url, data=data, allow_redirects=True)
    ...
    def searchToken(self):
        ...
        text = json.dumps(rsp.text)


        return text

    def deleteToken(self, tokenid):
        url = 'https://ers.cr.usgs.gov/password/appremove'
        data = {
            'token_id' : tokenid
        }
        rsp = self.session.post(url, data=data, allow_redirects=True)
        if rsp.status_code != 200:
            raise EarthExplorerError("EE: delete token failed.")
