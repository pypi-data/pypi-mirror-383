...
import datetime
import logging
logger = logging.getLogger('lb_toolkits')
...
class cmr() :
        ...
        try:
            ...
            while True:
                ...
                if not cmr_search_after :
                    ...
                    if hits > 0:
                        logger.info('共匹配【{count}】个文件'.format(count=hits))
                    else:
                        logger.error('未查询到满足相应条件的文件')
                        break
    ...
    def searchtoken(self, username, password):
        ...
        if resp.status_code != 200 :
            logger.error('账号【%s】密码【%s】错误' % (username, password))
            return None
        ...
        for item in res :
            ...
            if validdate > datetime.datetime.utcnow() :
                logger.info('【%s】' %(token))
                logger.info('该token有效时间至%s' %(validdate.strftime('%Y-%m-%d')))
                return token

            if validdate <= datetime.datetime.utcnow() :
                logger.warning('该token有效时间至%s，当前已失效，将其删除后重新创建新的token' %(validdate.strftime('%Y-%m-%d')))
                self.deletetoken(username, password, token)
                token = self.createtoken(username, password)

        if token is None :
            logger.error('账号【%s】未存在token,请使用createtoken接口进行token创建' %(username))
    ...
    def deletetoken(self, username, password, token):
        ...
        if resp.status_code == 200 :
            logger.info('成功删除token【%s】' %(token))
            return True
        else:
            logger.error('删除token失败【%s】' %(token))
            return False
    ...
    def cmr_check_provider(self, shortname, provider=None, version=None):
        ...
        if len(Dict_Match_Info) == 1:
            ...
            logger.info('相关详细信息参考："https://cmr.earthdata.nasa.gov/search/site/'
            ...
            for item in Dict_Match_Info :
                ...
                logger.info('请参考Short Name【%s】"' %(CMR_ProviderURL))
                print('~'*100)

            logger.info('该shortname【{}】有多个生产单位或版本，请指定生产单位或版本'.format(shortname))
            return False

        logger.info('在JSON文件中未匹配到相应的产品信息，进行在线搜索方式查询')
        for prodid in CMR_Provider :
            ...
            logger.info(CMR_Provider_Json)
            writejson(CMR_Provider_Json, dict_data)

        if len(Dict_Match_Info) == 1:
            print('='*100)
            logger.info('本产品【%s】生产单位：【%s】，版本【%s】' %(shortname, provider, version))
            logger.info('相关详细信息参考："https://cmr.earthdata.nasa.gov/search/site/'
            ...
            for item in Dict_Match_Info :
                logger.info('生产单位【%s】, 版本【%s】, 产品描述【%s】' %(item['Provider'], item['Version'], item['Collection']))
                CMR_ProviderURL = 'https://cmr.earthdata.nasa.gov/search/site/' \
                                  'collections/directory/{Provider}/gov.nasa.eosdis'.format(Provider=provider)
        ...
        logger.info('未匹配到shortname【%s】相关信息，可通过接口【self.searchShortName】进行模糊查询获取' %(shortname))
