...
import os
import json
from evotool.core import BaseRunStateDict, Solution
...
class FunSearchRunStateDict(BaseRunStateDict):
    ...
    def save_current_history(self) -> None:
        ...
        # 元数据：包含island信息等
        metadata = {
            'valid_count': sum(1 for s in self.current_batch_solutions if s.evaluation_res and s.evaluation_res.valid)
        }
