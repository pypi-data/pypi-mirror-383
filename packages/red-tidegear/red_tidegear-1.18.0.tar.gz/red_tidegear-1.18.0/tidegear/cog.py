...
class Cog(commands.Cog):
    ...
    async def _tidegear_session_on_request_start(
        self, session: ClientSession, trace_config_ctx: SimpleNamespace, params: TraceRequestStartParams
    ) -> None:
        self._rest_logger.trace("-> %s %s", params.method, params.url)
    ...
    async def _tidegear_session_on_request_end(
        ...
        self._rest_logger.trace(
            "<- %s %s -> %s (%s)", params.method, params.url, params.response.status, self._tidegear_trace_get_duration(end_time, trace_config_ctx)
        )
