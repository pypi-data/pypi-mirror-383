...
class DataProcessingAndMessaging:
    ...
    def wx_to_excel(self, docid, sheet_id, excel_filename):
        """
        ...
        """
        self._wechat_doc_log(f"开始导出文档[{docid}]子表[{sheet_id}]的记录到Excel：{excel_filename}")
        self._wechat_doc_refresh_token_if_needed()
        ...
        for record in all_records[:1]:  # 取第一条记录即可（所有记录字段结构一致）
            if record.get("values"):
                ...
                break
        # 系统字段 + 自定义字段 = 所有字段
        all_fields = ["记录ID", "创建时间", "更新时间", "创建者", "最后编辑者"] + list(custom_fields)
        ...
        msg = f"成功导出{len(rows)}条记录到Excel：{excel_filename}（{len(date_related_columns)}个日期相关列已转为北京时间）"
        self._wechat_doc_log(msg)
        print(f"[企业微信文档] {msg}")
