...
class TestRunScript(unittest.TestCase):
    def setUp(self):
        ...
        config = ToucanConfig(
            ...
            result_path=Path(f"results/{mock_apk_app_name('apk')}")
        )
    ...
    def test_previous_run_supported(self, os_isfile, os_exists, os_isdir, os_listdir):
    ...
    @patch('os.path.exists')
    @patch('os.path.isfile')
    def test_previous_run_unsupported(self, os_isfile, os_exists, os_isdir, os_listdir):
        os_exists.return_value = True
        os_listdir.return_value = ['dummy_name_unsupported.txt']
        os_isfile.return_value = False
    ...
    def test_toucan_config(self):
        ...
        self.assertIn("6 validation errors", str(context.exception))

        with self.assertRaises(ValidationError) as context:
            ToucanConfig(
                'alice', None, None, None, None, None
            )
        self.assertIn("5 validation errors", str(context.exception))

        with self.assertRaises(ValidationError) as context:
            ToucanConfig(
                'alice', 'bob', None, None, None, None
            )
        self.assertIn("4 validation errors", str(context.exception))

        with self.assertRaises(ValidationError) as context:
            ToucanConfig(
                'alice', 'bob', True, None, None, None
            )
        self.assertIn("3 validation errors", str(context.exception))

        with self.assertRaises(ValidationError) as context:
            ToucanConfig(
                'alice', 'bob', True, 'debug', None, None
            )
        self.assertIn("2 validation errors", str(context.exception))
        with self.assertRaises(ValidationError) as context:
           ToucanConfig(
                'alice', 'bob', True, 'debug', 60, None
            )

        self.assertIn("1 validation error", str(context.exception))

        config = ToucanConfig(
            'alice', 'bob', True, 'debug', 60, Path("path")
        )
        ...
        self.assertEqual(config.result_path, Path("path"))
