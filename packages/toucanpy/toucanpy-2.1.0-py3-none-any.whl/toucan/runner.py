...
from toucan.utils import RESULT_DIR_KEY, versions
...
class ToucanRunner:
    def __init__(self, formats: list[ToucanFormat], reporter: Reporter, config: ToucanConfig):
        ...
        self.app_version_result_folder: Path = Path()
        ...
        self.result_root = self.config.result_path
        # Set result path as environment variable os it can be accessed from anywhere
        os.environ[RESULT_DIR_KEY] = str(self.result_root)
    ...
    def _store_support(self, toucan_format: ToucanFormat, file_results: dict):
        ...
        with open(self.app_version_result_folder / f'{toucan_format.app_name}_supported.txt', 'w'):
            pass
    ...
    def _execute_actions(self, toucan_format: ToucanFormat):
        ...
        try:
            ...
            self.alice.adb_device._command(
                f'exec-out screencap -p > {str(self.result_root / "log" / f"{screenshot_name}_self.alice.png")}'
            )
            self.bob.adb_device._command(
                f'exec-out screencap -p > {str(self.result_root / "log" / f"{screenshot_name}_bob.png")}'
            )
            raise e
        logger.info(
    ...
    def _install_latest_version(self, toucan_format: ToucanFormat):
        apk_path = self.app_version_result_folder / f'{toucan_format.package_name}_v{self.latest_version}.apk'
    ...
    def _handle_no_new_version(self, toucan_format):
        ...
        self.app_version_result_folder = (
            self.result_root / toucan_format.app_name / self.latest_version / self.timestamp
        )
        self.app_version_result_folder.mkdir(exist_ok=True, parents=True)
    ...
    def _version_supported(self, app_name: str) -> bool:
        ...
        """
        if os.path.exists(self.app_version_result_folder):
            result_folders = sorted(
                [
                    os.path.join(self.app_version_result_folder.parent, file)
                    for file in os.listdir(self.app_version_result_folder.parent)
                ],
                reverse=True,
            )
            result_folders = [f for f in result_folders if os.path.isdir(f)]
            return any(
                os.path.isfile(Path(result_folders[i]) / f'{app_name}_supported.txt')
    ...
    def _get_file_results(self, toucan_format: ToucanFormat, file_results):
        for toucan_file in toucan_format.toucan_files:
            ...
            destination_file = get_file_from_emulator(
                self.alice,
                toucan_file,
                str(self.app_version_result_folder),
                toucan_format.app_name,
                self.latest_version,
            )
    ...
    def _store_results(self, recognition_result: str, parsing_result: str, file_name: str):
        ...
        """
        # TODO TOUCAN-167: report failures
        with open(self.app_version_result_folder / f'{file_name}_recognition_result.txt', 'w') as parsing_file:
            parsing_file.write(recognition_result)
        with open(self.app_version_result_folder / f'{file_name}_parsing_result.txt', 'w') as parsing_file:
