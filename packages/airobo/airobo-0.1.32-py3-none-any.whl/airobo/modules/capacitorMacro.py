...
def prepare_capacitor_app(app_path):
    ...
    try:
        ...
        result = subprocess.run(["npm", "install"], check=True, capture_output=True, shell=True, text=True)
        
        print("Building web assets...")
        result = subprocess.run(["npm", "run", "build"], check=True, capture_output=True, shell=True, text=True)
