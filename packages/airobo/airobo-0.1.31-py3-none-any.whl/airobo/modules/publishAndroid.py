...
def build_android_bundle(app_path):
    ...
    try:
        ...
        print("🔄 Build Status: Starting Gradle build process...")
        
        # Run build in separate terminal (always)
        gradle_cmd = "gradlew.bat" if os.name == 'nt' else "./gradlew"
        ...
        # Show wrapper interface while waiting
        print("⏳ Waiting for build to complete... (detailed output in separate terminal)")
        print("   You can monitor the build progress in the opened terminal window")
        ...
        if build_result_code == 0:
            ...
            print("❌ Gradle build failed - check the separate terminal for details")
