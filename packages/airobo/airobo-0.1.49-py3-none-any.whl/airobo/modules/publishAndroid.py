...
def build_android_bundle(app_path):
    ...
    try:
        ...
        # Ensure we're using the current Java version and fresh Gradle daemon
        env = os.environ.copy()
        env['JAVA_HOME'] = r'C:\jdk-17.0.0.1'
        env['PATH'] = r'C:\jdk-17.0.0.1\bin;' + env.get('PATH', '')
        
        # Stop any existing Gradle daemons that might be using wrong Java version
        subprocess.run([gradle_cmd, "--stop"], cwd=android_path, shell=(os.name == 'nt'), 
                      capture_output=True, env=env)
        
        # Run Gradle build with explicit Java 17 environment
