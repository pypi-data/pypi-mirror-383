...
def build_android_bundle(app_path):
    ...
    try:
        ...
        _ensure_java_config(android_path)
...
def _force_java_compatibility(android_path, java_version):
    """
    Force Java compatibility in build.gradle files
    """
    # Update app/build.gradle
    app_build_gradle = os.path.join(android_path, "app", "build.gradle")
    if os.path.exists(app_build_gradle):
        ...
        # Add compileOptions if not present, or update existing ones
        compile_options = f"""
    compileOptions {{
        ...
        print(f"✅ Updated app/build.gradle for Java {target_java_version} compatibility")