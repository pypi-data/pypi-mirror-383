...
def prepare_capacitor_app(app_path):
    ...
    try:
        ...
        subprocess.run(["npm", "install"], check=True, capture_output=True, shell=True, text=True, 
                      stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        
        print("Building web assets...")
        # Run npm build silently  
        subprocess.run(["npm", "run", "build"], check=True, capture_output=True, shell=True, text=True,
                      stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
