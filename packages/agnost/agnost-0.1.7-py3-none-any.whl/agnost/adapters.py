...
class FastMCPAdapter(ServerAdapter):
    ...
    def _patch_list_tools(self) -> None:
        """Patch the list_tools handler to create session when client first lists tools.
        ...
        try:
            ...
            if not hasattr(mcp_server, 'list_tools'):
                logger.debug("MCP server has no list_tools method")
                return
            ...
            async def patched_list_tools(*args, **kwargs):
                ...
                if not session_created[0] and adapter_self._session_callback:
                    ...
                    logger.debug("tools/list called - creating initial session with tools")
                    try:
            ...
            mcp_server.list_tools = patched_list_tools
            logger.debug("list_tools handler patched for session creation")
        except Exception as e:
            logger.debug(f"Could not patch list_tools: {e}")
