...
class JibitService:
    ...
    async def generate_token(self) -> JibitTokens:
        ...
        print(f'generate new tokens {data}')
        return JibitTokens(
    ...
    # ---------- Public APIs ----------
    async def card_to_shaba(
        self, card_number: str, tokens: Optional[JibitTokens]
    ) -> Tuple[Card, Optional[JibitTokens]]:
    ...
    async def _handle_response(self, resp: aiohttp.ClientResponse) -> dict:
        ...
        if code:
            ...
            if error_code != JibitErrorCode.UNKNOWN_ERROR:
                raise JibitException(error_code, code, http_status=resp.status)

        status_map = {
            400: (JibitErrorCode.INVALID_CARD, "Card invalid"),
