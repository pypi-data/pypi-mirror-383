...
class MicronautParser(BaseParser):
    ...
    def _extract_methods(self, content: str) -> List[Dict[str, Any]]:
        ...
        # Patterns for Micronaut HTTP annotations with paths
        # Match: @Get("/path"), @Get(value="/path"), @Get(uri="/path")
        mapping_patterns = [
        ...
        # Match annotations without path: @Post(consumes=...), @Get(produces=...), etc.
        no_path_patterns = [
            (r'@Get\s*\((?!\s*(?:value|uri)\s*=)(?:[^)]*)\)', HTTPMethod.GET),
            (r'@Post\s*\((?!\s*(?:value|uri)\s*=)(?:[^)]*)\)', HTTPMethod.POST),
            (r'@Put\s*\((?!\s*(?:value|uri)\s*=)(?:[^)]*)\)', HTTPMethod.PUT),
            (r'@Delete\s*\((?!\s*(?:value|uri)\s*=)(?:[^)]*)\)', HTTPMethod.DELETE),
            (r'@Patch\s*\((?!\s*(?:value|uri)\s*=)(?:[^)]*)\)', HTTPMethod.PATCH),
        ]
        ...
        # First, find all methods with explicit paths
        ...
        # Then find methods without paths (use empty path, will use controller base path)
        for pattern, http_method in no_path_patterns:
            for match in re.finditer(pattern, content):
                if match.start() in found_positions:
                    continue
        ...
        # Handle annotations without parentheses
        for pattern, http_method in no_paren_patterns:
            for match in re.finditer(pattern, content):
                if match.start() in found_positions:
                    continue
                found_positions.add(match.start())
