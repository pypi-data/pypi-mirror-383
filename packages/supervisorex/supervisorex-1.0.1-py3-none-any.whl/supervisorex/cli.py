...
def check_supervisor_installed():
    """Check if supervisor is installed, install if not."""
    try:
        subprocess.run(["supervisord", "-v"], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    except subprocess.CalledProcessError:
        print("[INFO] Installing supervisor...")
...
"""
    ...
    # Move the temp conf file to proper directory with sudo
...
def start_supervisor_process(process_name):
    ...
    subprocess.run(["sudo", "supervisorctl", "start", process_name], check=True)
    print(f"Supervisor Process Started\n\nProcess Name: {process_name}")

def main():
    if len(sys.argv) < 2:
        print("Usage: python3 supervisorex <command>")
        sys.exit(1)
