...
class PortWidgetManager(BasePortManager):
    ...
    def _clean_page_data(self, page_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        metadata_fields = ['ok', 'error', 'message', 'details', 'createdAt', 'updatedAt', 'createdBy', 'updatedBy', 'page', 'type', 'sidebar', 'section', 'showInSidebar', 'protected', 'requiredQueryParams']
    ...
    def setup_dashboard_widget(self, page_data: Dict[str, Any]) -> bool:
        ...
        self.logger.info(f"Merged {len(existing_widgets)} existing widgets with {len(new_widgets)} new widgets → {len(merged_widgets)} total")
    ...
    def _setup_page_widgets(self, page_id: str, widget_infos: List[Dict[str, Any]]) -> bool:
        ...
        if dashboard_widget and 'widgets' in dashboard_widget:
            ...
            for widget_info in widget_infos:
                ...
                widget_exists = any(existing_widget.get('id') == widget_id for existing_widget in dashboard_nested_widgets)
