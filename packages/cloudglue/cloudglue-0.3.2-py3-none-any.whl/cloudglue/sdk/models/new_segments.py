...
class NewSegments(BaseModel):
    ...
    NewSegments
    """ # noqa: E501
    url: StrictStr = Field(description="Input video URL. Supports URIs of files uploaded to Cloudglue Files endpoint, public HTTP URLs, and S3 or Dropbox URIs which have been granted access to Cloudglue via data connectors.  **⚠️ Important: YouTube URLs are NOT supported for the segments API.** This includes both shot-based and narrative-based segmentation. For S3 and Dropbox URI support see our documentation on data connectors for setup information.")
    criteria: StrictStr = Field(description="Segmentation criteria: • **shot**: Detect scene changes and shot boundaries using computer vision • **narrative**: Identify logical narrative segments and chapters using AI analysis")
