...
export class NotebookTools {
   ...
   */
  private async getOrCreateNotebookUniqueId(notebookPath: string): Promise<string> {
    const contentManager = AppStateService.getContentManager();
    if (!contentManager) {
    ...
    try {
      ...
      if (nbFile?.content?.metadata?.sage_ai?.unique_id) {
        ...
        console.log(`[getOrCreateNotebookUniqueId] Found existing unique_id: ${existingId}`);
        return existingId;
      }
    } catch (error) {
      console.log(`[getOrCreateNotebookUniqueId] Notebook doesn't exist or error reading: ${error}`);
    }

    // Generate new unique_id if none exists
    const newId = this.generateNotebookUniqueId();
    console.log(`[getOrCreateNotebookUniqueId] Generated new unique_id: ${newId}`);
    return newId;
  }
   ...
   */
  private async createNewNotebook(path: string, uniqueId: string): Promise<boolean> {
    const contentManager = AppStateService.getContentManager();
    if (!contentManager) {
      console.error('Content manager not available');
      return false;
    ...
    try {
      ...
      console.log(`[createNewNotebook] Created notebook at: ${path} with unique_id: ${uniqueId}`);
      return true;
    } catch (error) {
  ...
  private async copyThreadToNotebook(
    ...
    if (copiedThread) {
      ...
      console.log('[copyThreadToNotebook] No launcher thread to copy, or copy failed');
      return false;
    }
  }
  ...
  async open_notebook(options: {
    ...
    try {
      ...
      if (!notebookExists && create_new) {
        ...
        console.log(`[open_notebook] Creating new notebook with unique_id: ${uniqueId}`);
        
        const created = await this.createNewNotebook(path_of_notebook, uniqueId);
        if (!created) {
      ...
      const notebookUniqueId = await this.getOrCreateNotebookUniqueId(path_of_notebook);
      ...
      if (opened) {
        console.log(`[open_notebook] Successfully opened notebook: ${path_of_notebook}`);
      } else {
        console.log(
