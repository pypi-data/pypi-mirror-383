...
# 1. 创建 MCP Server 实例
# 这是整个应用的核心，它将管理工具和底层的 FastAPI 应用
server = Server("mcp-novelaigen", host="0.0.0.0", port=8000)

# 2. 定义工具实现
# 使用 @server.tool 装饰器来注册一个工具。
# MCP 框架将根据这个函数的签名和类型提示自动生成输入模式 (inputSchema)。
@server.tool(
    name="NovelAIGen",
...
async def novelaigen(prompt: str, resolution: str, negative_prompt: str | None = None) -> Sequence[TextContent | ImageContent]:
    """
    Generates an image using NovelAI based on the provided parameters.
    This function is registered as an MCP tool.
    """
    api_key = os.environ.get("NOVELAI_API_KEY")
    if not api_key:
        # In the new MCP structure, we raise an exception for configuration errors.
        raise McpError(code=-32001, message="服务器配置错误: 环境变量 NOVELAI_API_KEY 未设置。")
    ...
    filename, debug_logs = await generate_image_from_novelai(arguments, api_key)
    log_text = "\n".join(debug_logs)

    if filename:
        # Construct the full URL from environment variables for stability
        base_url = os.environ.get("PUBLIC_BASE_URL", f"http://{server.host}:{server.port}")
...
# 3. 添加自定义路由以提供图片文件
# 我们使用 server.app 来访问底层的 FastAPI 实例并添加我们的路由。
IMAGE_SERVE_DIR = Path(os.environ.get("PROJECT_BASE_PATH", ".")) / "generated_images"

@server.app.get("/generated_images/{filename}")
...
# 4. 定义服务器入口点
# 这个函数将由 __init__.py 中的 main 函数调用。
def serve():
    """
    Starts the MCP server.
    This function will block and run the server, handling both stdio and HTTP modes.
    """
    logging.info("Starting MCP server...")
    try:
        # server.run() 是 MCP 框架提供的标准启动方法
        server.run()
