...
async def call_tool(
    ...
    if tool_request.name != "NovelAIGen":
    ...
    filename, debug_logs = await generate_image_from_novelai(tool_request.arguments, api_key)
    ...
    if filename:
        ...
        image_url = f"{str(http_request.base_url).rstrip('/')}/generated_images/{filename}"
