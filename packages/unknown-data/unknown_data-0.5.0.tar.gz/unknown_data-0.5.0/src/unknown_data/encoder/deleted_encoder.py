...
class DeletedDataEncoder(BaseDataEncoder):
    ...
    def convert_data(self, data: dict) -> bool:
        ...
        for first_depth in list(self.data.keys()):  # dictionary iteration 문제 해결
            match first_depth:
                ...
                case "data_sources":
                    df = self._dict_data_to_df(first_depth)
                    self._update_datas("data_sources", df)
                case "statistics":
                    df = self._dict_data_to_df(first_depth)
                    self._update_datas("statistics", df)
                case "mft_deleted_files":
                    for df_chunk in self._list_data_to_df(first_depth):
                        self._update_datas("mft_deleted_files", df_chunk)
                case "recycle_bin_files":
                    for df_chunk in self._list_data_to_df(first_depth):
                        self._update_datas("recycle_bin_files", df_chunk)
                case _:
                    pass
