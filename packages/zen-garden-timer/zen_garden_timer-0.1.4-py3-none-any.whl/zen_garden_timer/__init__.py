...
SHORT_BREAK_SECONDS = 1 if TEST_MODE else 300  # 5 minutes
LONG_BREAK_SECONDS = 2 if TEST_MODE else 900   # 15 minutes
...
class MusicPlayer:
    ...
    def play_in_background(self):
        if self.is_initialized and not self.is_muted:
            threading.Thread(
...
class Plant:
    ...
    def resume(self):
        ...
        if self.is_paused and self.pause_start_time:
            ...
            self.last_event_time += pause_duration
            self.is_paused = False
            self.pause_start_time = None
    ...
    def get_time_until_next_event(self) -> int:
        ...
        if self.is_paused:
            # When paused, show the time that was remaining when pause started
            elapsed = self.pause_start_time - self.last_event_time
        else:
...
class GardenApp:
    ...
    def _handle_input(self):
        ...
        # Global keybind for mute (works in any state)
        if key == 'm' and self.music_player:
    ...
    def _create_garden_display(self) -> Panel:
        ...
        grid.add_row("")
        controls = Text("Press [S] to start | [M] to mute | [Q] to quit", style="bold")
    ...
    def _create_plant_selection_display(self) -> Panel:
        ...
        grid.add_row("")
        footer = Text("Press [M] to mute | [Q] to return to the Garden.", style="")
        grid.add_row(footer)
    ...
    def _create_break_display(self, break_type: str, remaining_seconds: int) -> Panel:
        """Create the break screen display."""
        grid = Table.grid(expand=False)
        grid.add_column(justify="center", no_wrap=False, width=70)

        # Title
        if break_type == "long_break":
            title = Text("☕ L O N G   B R E A K ☕", style="bold magenta")
            subtitle = Text("Time for a longer break! Walk around and recharge.", style="dim")
        else:
            title = Text("🌿 S H O R T   B R E A K 🌿", style="bold green")
            subtitle = Text("Quick break! Stretch and rest your eyes.", style="dim")
        ...
        timer = Text(f"⏱️  {minutes:02d}:{seconds:02d}", style="bold yellow")
        grid.add_row(timer)
        grid.add_row("")

        # Instructions
        instructions = Text("Timer is paused during break", style="dim italic")
        grid.add_row(instructions)
        grid.add_row("")

        quit_text = Text("Press [M] to mute | [Q] to skip break", style="dim")
    ...
    def _create_session_display(self) -> Panel:
        ...
        grid.add_row("")
        quit_text = Text("Press [M] to mute | [Q] to quit early", style="dim")
        grid.add_row(quit_text)
        ...
        with Live(layout, console=console, screen=True, refresh_per_second=30) as live:
            while self.session_running and not self.session_plant.is_fully_grown():
                ...
                if event_type:
                    ...
                    # Pause the timer for break duration
                    self.session_plant.pause()
                    break_duration = LONG_BREAK_SECONDS if event_type == "long_break" else SHORT_BREAK_SECONDS

                    # Send notification for break
                    if event_type == "short_break":
                        send_notification(
                            "Zen Garden - Short Break",
                            "Time for a 5 minute break! Stretch and rest your eyes. 🌿"
                        )
                    elif event_type == "long_break":
                        send_notification(
                            "Zen Garden - Long Break",
                            "Time for a 15 minute break! Walk around and recharge. 🌟"
                        )

                    # Show break screen for the duration
                    break_end_time = time.time() + break_duration
                    while time.time() < break_end_time and self.session_running:
                        self._handle_input()
                        remaining = int(break_end_time - time.time())
                        base_panel = self._create_break_display(event_type, remaining)
            ...
            if self.session_plant.is_fully_grown():
                ...
                plant_info = PLANT_TYPES[self.session_plant.type]
                send_notification(
                    "Zen Garden - Session Complete! 🎉",
                    f"Congratulations! Your {plant_info['bloom']} {plant_info['name']} has bloomed!"
                )
...
def main():
    ...
    # Find music file in package directory
    music_file = Path(__file__).parent / "background_music.mp3"
    music_player = MusicPlayer(str(music_file))
    try:
        music_player.play_in_background()
        app = GardenApp(music_player)
        app.run()
        console.print(
