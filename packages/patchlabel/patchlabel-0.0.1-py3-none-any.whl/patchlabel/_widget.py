...
def _is_image_file(p: Path):
    return p.suffix.lower() in {".png", ".jpg", ".jpeg", ".tif", ".tiff", ".bmp", ".gif"}
...
class ImageLabeler(QWidget):
    ...
    def _on_load(self):
        ...
        if not text:
            QMessageBox.warning(self, "No folder", "Please provide a folder path.")
            return
        p = Path(text)
        if not p.exists() or not p.is_dir():
            QMessageBox.warning(self, "Invalid folder", "The path is not a valid directory.")
            return
        self.folder = p
        # discover images
        self.images = sorted([f for f in p.iterdir() if f.is_file() and _is_image_file(f)])
    ...
    def _on_show(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "Read error", f"Could not read image: {e}")
            return
    ...
    def _assign_label(self, label: str):
        if self.current_image is None:
            QMessageBox.information(self, "No image", "Select an image to label.")
            return
        fname = self.current_image.name
        self.labels[fname] = label
        self._save_labels_file(append=False)  # rewrite entire file so last label wins
        QMessageBox.information(self, "Saved", f"Assigned label {label} to {fname}")
    ...
    def _save_labels_file(self, append: bool = True):
        ...
        if self.folder is None:
            QMessageBox.warning(self, "No folder", "Load a folder before saving labels.")
            return
        ...
        try:
            ...
            QMessageBox.critical(self, "Error", f"Could not write labels file: {e}")
    ...
    def _clear_labels(self):
        self.labels.clear()
        QMessageBox.information(self, "Cleared", "Labels cleared from memory.\nSave to overwrite file.")
