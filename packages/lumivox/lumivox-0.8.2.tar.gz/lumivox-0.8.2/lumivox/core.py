...
class LED_GUI:
    ...
    def animate(self):
        ...
        for color, led in self.led_shapes.items():
            ...
            self.glow[color] = new
            b = int(50 + 205 * new)
            color_code = {"red": f"#{b:02x}0000", "green": f"#00{b:02x}00", "blue": f"#0000{b:02x}"}[color]
...
def start_mic(model):
    ...
    try:
        ...
        # safer initialization delay for Windows
        time.sleep(0.5)
        ...
        stream.start()
        print("[mic] ✅ Microphone stream started safely.")
        return rec, q
    except Exception as e:
