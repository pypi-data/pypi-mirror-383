...
_lib.enqueue_find_symbols_in_text.argtypes = [FIND_CALLBACK, C.c_void_p, C.c_char_p, C.c_bool]
...
def find_symbols_in_text(text: str, callback=None, data=None, sync: bool = False) -> bool:
    if callback is None:
        c_callback, data_ptr = C_NO_CALLBACK, 0
    else:
    ...
    _lib.enqueue_find_symbols_in_text(c_callback, data_ptr, text.encode(), sync)
