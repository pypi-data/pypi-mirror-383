...
#_______________________________Quantum Computing(will be in 1.2.3)_______________________________
#__________________________________Quantum Computation (will be in 1.2.3)__________________________
...
class Web_Scrapper:
    ...
    - Extract tags, text, images, links
    - Download and save media
    - Handle headers, user agents, and timeouts
    ...
    def get_links(self):
        ...
        for a in self.soup.find_all("a", href=True):
            href = urljoin(self.last_url, a["href"])
            links.append(href)
    ...
    def get_images(self):
        ...
        for img in self.soup.find_all("img", src=True):
            src = urljoin(self.last_url, img["src"])
            imgs.append(src)
    ...
    def download_all_images(self, folder="images"):
        """Download all images from the last fetched page."""
    ...
    def search(self, pattern: str):
        """Search pattern inside the page text."""
        if not self.content: return []
        return re.findall(pattern, self.content, re.IGNORECASE)

    def clear_cache(self):
        """Clear the stored content and soup."""
        self.content = None
        self.soup = None
        self.last_url = None
...
class PhysicalSensors:
    ...
    def motion_detected(self, camera_index=0, threshold=300000):
        """Detect movement using webcam frame difference."""
        print("🎥 Starting motion detection (Ctrl+C to stop)...")
    ...
    # ---------------- PROXIMITY (via camera intensity) ----------------
    def proximity_detect(self, camera_index=0, frames=10):
        """Rough estimate of object proximity using brightness."""
        try:
            cap = cv2.VideoCapture(camera_index)
            avg_brightness = 0
            ...
            return f"Proximity detection error: {e}"
