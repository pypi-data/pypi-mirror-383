...
template<typename T>
std::vector<T> bogo_sort(const std::vector<T>& data, 
                         const std::function<void(const std::vector<T>&)>& callback) {
    std::vector<T> result = data;
    std::random_device rd;
    std::mt19937 g(rd());
    while (!std::is_sorted(result.begin(), result.end())) {
        std::shuffle(result.begin(), result.end(), g);
        callback(result); // Call the Python function with the current state
    }
    return result;
}

template<typename T>
std::vector<T> bozo_sort(const std::vector<T>& data,
                         const std::function<void(const std::vector<T>&)>& callback) {
    std::vector<T> result = data;
    ...
    while (!std::is_sorted(result.begin(), result.end())) {
        std::swap(result[dist(g)], result[dist(g)]);
        callback(result); // Call the Python function with the current state
    }
    return result;
}
...
template<typename T>
std::vector<T> miracle_sort(const std::vector<T>& data, int max_attempts, const std::function<void()>& prey) {
    std::vector<T> result = data; 
