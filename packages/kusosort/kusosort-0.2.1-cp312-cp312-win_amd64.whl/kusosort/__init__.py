...
def miracle(data: List[T], max_attempts: int = 10, pray: Callable[[], None] = None) -> List[T]:
    """
    Waits for a miracle to sort the list.
    Returns the final state of the list (sorted or not).
    """
    def default_pray(): print("The miracle did not happen...")
    user_pray = pray if pray is not None else default_pray
    func_set = _get_sort_function_set(data)
    ...
    result = miracle_fn(data, max_attempts, user_pray)
