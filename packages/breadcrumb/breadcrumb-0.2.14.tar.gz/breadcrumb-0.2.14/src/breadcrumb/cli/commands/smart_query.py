...
- --fuzzy <term>: Search recent trace events for a substring
...
from breadcrumb.storage.smart_queries import gaps_query, call_query, flow_query, fuzzy_query
...
def execute_smart_query(
    ...
    Args:
        ...
        fuzzy: Substring to search across recent trace events
        ...
        disable_truncation: Disable truncation when falling back to SQL output
    ...
    Example:
        ...
        exit_code = execute_smart_query(fuzzy="breadcrumbs")
    """
    if verbose:
    ...
    if query_types_selected == 0:
        error_msg = format_error(
            ...
            message="No query type specified. Use --gaps, --call, --flow, --fuzzy, or --sql",
    ...
    if query_types_selected > 1:
        error_msg = format_error(
            ...
            message="Multiple query types specified. Please use only one: --gaps, --call, --flow, --fuzzy, or --sql",
    ...
    try:
        ...
        if gaps:
            ...
            if verbose:
                print(f"Executing fuzzy query for term: {fuzzy}", file=sys.stderr)
            result = fuzzy_query(db_path=effective_db_path, term=fuzzy)
        ...
        if not has_results and result.get('status') != 'not_implemented':
            if format == "json":
                ...
                print("No results")
            # Empty results from a successful query should return success (0)
            return EXIT_SUCCESS
