...
def _generate_run_report(db_path: Optional[str] = None, config: Optional[str] = None) -> None:
    """
    ...
    try:
        ...
        with manager.get_connection_context() as conn:
            ...
            if total_events <= 100:
                ...
                print(f"       High call counts? Start with minimal tracing (['__main__'])")
                print(f"       and expand based on --gaps analysis.")
            ...
            # get just the filename (not path, and no extension) if possible
            if config:
                config = os.path.splitext(os.path.basename(os.path.abspath(config)))[0]
            else:
                config = os.path.splitext(os.path.basename(os.path.expanduser("~/.breadcrumb/config.yaml")))[0]
                if not os.path.exists(config):
                    config = None
            print()
            print("Recommendations on what to do next:")
            print(f"  Detailed Flow: breadcrumb query -c {config} --flow")
            print(f"  View specific call: breadcrumb query -c {config} --call <call_id>")
            if top:
                print(f"     Example '{top}': breadcrumb query -c {config} --call {top}")
            print()

            print("If information is missing:")
            print(f"  Find untraced calls: breadcrumb query -c {config} --gaps")
            print(f"  Add untraced calls to include config: breadcrumb config edit {config} --add-include '<fully.qualified.name>'")
            print(f"  Rerun trace: breadcrumb run -c {config} -t 60 python your_script.py")
...
def run_command(
    ...
    Examples:
        ...
        # Trace multiple modules (30 second timeout)
        breadcrumb run -t 30 --include "myapp.*" --include "flock.*" python main.py
        ...
        # Increase captured payload size (5,000 characters)
        breadcrumb run -t 60 --max-chars 5000 python main.py
    """
    ...
    if max_repr_length is not None:
        init_args.append(f"max_repr_length={max_repr_length}")
...
"""
            ...
            try:
                ...
                try:
                    ...
                    time.sleep(0.5)
                    _generate_run_report(db_path, config_file)  # Show run report on success
