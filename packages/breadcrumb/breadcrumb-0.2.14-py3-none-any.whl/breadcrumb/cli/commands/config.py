...
# Shared defaults from core config module
from breadcrumb.config import DEFAULT_EXCLUDE
...
# Helpful defaults skip noisy instrumentation frameworks that can break tracing
exclude:
{chr(10).join(f"  - '{pattern}'" for pattern in config.get('exclude', []))}
...
max_repr_length: {config.get('max_repr_length', 2000)}
"""
...
def config_edit(
    ...
    remove_exclude: Optional[List[str]] = None,
    sample_rate: Optional[float] = None,
    db_path: Optional[str] = None,
    ...
    if add_include:
        ...
        # if any pattern contains "*" return error
        if any("*" in pattern for pattern in add_include):
            typer.echo("Error: only fully qualified names are allowed in include patterns", err=True)
            raise typer.Exit(1)
...
def config_validate(name: str):
    ...
    if not isinstance(exclude, list):
        ...
        typer.echo(f"    [OK] {len(exclude)} pattern(s)")
