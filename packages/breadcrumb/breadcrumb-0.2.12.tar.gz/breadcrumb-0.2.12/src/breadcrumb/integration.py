...
from breadcrumb.storage.value_truncation import MAX_VALUE_SIZE as DEFAULT_MAX_VALUE_SIZE
...
class TracingIntegration:
    ...
    def __init__(
        ...
        if writer is None:
            writer_max_value_size = max_value_size or DEFAULT_MAX_VALUE_SIZE
            writer = TraceWriter(
                db_path=db_path,
                backend_ref=backend,  # Pass backend reference for diagnostic reports
                max_value_size=writer_max_value_size,
            )
            writer.start()
