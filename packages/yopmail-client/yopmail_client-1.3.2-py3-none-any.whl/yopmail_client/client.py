...
from .license_verifier import validate_license_and_config, LicenseError
...
class YOPMailClient:
    ...
    def __init__(
        ...
        Args:
            ...
            skip_license_check: Skip license verification (for development only)
            
        Raises:
            LicenseError: If license validation fails
        """
        # Validate license and load user configuration
        if not skip_license_check:
            try:
                user_config = validate_license_and_config()
                logger.info("License validation successful")
                
                # Merge user proxy settings with provided config
                if user_config.get("proxy_url") or user_config.get("proxy_list"):
                    if config is None:
                        config = {}
                    
                    # Add proxy settings from user config
                    if user_config.get("proxy_url"):
                        config["proxy_url"] = user_config["proxy_url"]
                    if user_config.get("proxy_list"):
                        config["proxy_list"] = user_config["proxy_list"]
                    if user_config.get("proxy_username"):
                        config["proxy_username"] = user_config["proxy_username"]
                    if user_config.get("proxy_password"):
                        config["proxy_password"] = user_config["proxy_password"]
                    if user_config.get("proxy_rotation"):
                        config["proxy_rotation"] = user_config["proxy_rotation"]
                    
                    # Enable proxy if any proxy settings are provided
            ...
            logger.warning("License check skipped - this should only be used for development")
        ...
        self.cookie_manager = CookieManager(self.client, use_dynamic_cookies=use_dynamic_cookies)
    ...
    def open_inbox(self) -> None:
        ...
        try:
            ...
            if 'yp_token' in detected_cookies:
                ...
                self.yp_token = "ZAGplZmp0ZmR3ZQN4ZGx1ZGR"  # nosec B105
