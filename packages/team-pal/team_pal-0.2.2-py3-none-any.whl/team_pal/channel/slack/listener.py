...
class SlackInstructionListener(InstructionListener):
    ...
    def _process_event(self, event: SlackEvent) -> None:
        ...
        if self._project_resolver and self._project_resolver.is_selection_required():
            channel_id = payload.get("channel")
            if channel_id:
                self._post_project_selection_prompt(str(channel_id))
            self._safe_ack(event)
            return
    ...
    def _try_handle_command(self, event: SlackEvent, payload: Mapping[str, object]) -> bool:
        ...
        if (
            self._project_resolver
            and self._project_resolver.is_selection_required()
            and verb in {"$new", "$new_session", "$continue", "$cs", "$status", "$st", "$list", "$ls"}
        ):
            self._post_project_selection_prompt(str(channel_id))
            return True
    ...
    def _post_project_selection_prompt(self, channel_id: str) -> None:
        if self._project_resolver is None:
            return
        projects = self._project_resolver.list_projects()
        lines = ["No project selected. Please choose a project before continuing."]
        if projects:
            ...
            lines.append("No projects discovered under the configured root directory.")
        lines.append("Use `$ps <project_name>` to select a project or `$pl` to view details.")
        self._post_command_response(channel_id, "\n".join(lines))
