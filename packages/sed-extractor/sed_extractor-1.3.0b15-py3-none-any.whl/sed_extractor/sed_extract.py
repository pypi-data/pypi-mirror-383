    ...
    if not have_wcs and (not region.have_mask() or region.mask.image_file_format != ImageFormat.WCS):
        ...
        for i in range(len(maps_healp)):
            ...
            if obj[i] == 'PARTIAL':
                ...
                if region.shape == ShapeRegion.POINT:
                    im_sliced = row_dict.get(valid_pix[0])
                else:
                    im_sliced_numpy = np.array([row_dict.get(valid_indice) for valid_indice in valid_pix[0]])
                    im_sliced = csr_matrix(im_sliced_numpy.reshape(1, -1))
            ...
            if rm_bckgd is not None and region.shape != ShapeRegion.POINT:
                if obj[i] == 'PARTIAL':
                    ...
                    im_sliced = np.array([row_dict.get(vpix_bckgd) for vpix_bckgd in valid_pix_bckgd[0]])
                    im_sliced_sparse = csr_matrix(im_sliced.reshape(1, -1))
                    bckgd_st[maps_healp[i]] = (im_sliced_sparse, valid_pix_bckgd, vvalid_pix_bckgd)
                else:
