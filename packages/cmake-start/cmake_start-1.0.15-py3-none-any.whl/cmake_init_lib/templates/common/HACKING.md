...
to setup `PATH` to run tests.{% else %}you need to set up a [Conan profile][profile].

If this is your first time using Conan, you need to create a default profile by
running:
...
This will automatically detect your system's compiler and settings. Once your
profile is set up, download the dependencies and generate the necessary CMake
...
#### Using Conan with different development environments

If you work with multiple compiler environments (e.g., native system compiler
vs. Nix-provided GCC), you have several options:

**Option 1: Use the automated setup script (easiest)**

The project includes a `conan-setup.sh` script that automatically detects your
environment and selects the appropriate profile:

```sh
# Run with default Debug build
./conan-setup.sh

# Or specify a build type
./conan-setup.sh Release
```

The script will:
- Detect if you're in a Nix environment (checks `$IN_NIX_SHELL` and `$NIX_BUILD_TOP`)
- Use the `nix` profile for Nix environments, `default` for native
- Create the default profile if it doesn't exist
- Run `conan install` with the correct settings

**Option 2: Use named profiles manually**

Create environment-specific profiles instead of just using `default`:

```sh
# For your native environment (apple-clang, etc.)
conan profile detect --force --name native

# For Nix environment with GCC, create a profile manually
conan profile detect --force --name nix-gcc
# Then edit ~/.conan2/profiles/nix-gcc to match your Nix environment
```
...
```sh
# Native build
conan install . -pr native -s build_type=Debug -b missing

# Nix build
conan install . -pr nix-gcc -s build_type=Debug -b missing
```

**Option 3: Override compiler settings**

Keep the default profile and override settings per-build:

```sh
# Override compiler on command line
conan install . -s compiler=gcc -s compiler.version=13 \
  -s compiler.libcxx=libstdc++11 -s build_type=Debug -b missing
```

**Option 4: Conditional profile selection**

You can detect the environment and select profiles automatically in a script:

```sh
if [ -n "$IN_NIX_SHELL" ]; then
  CONAN_PROFILE="nix-gcc"
else
  CONAN_PROFILE="default"
fi
conan install . -pr $CONAN_PROFILE -s build_type=Debug -b missing
```{% end %}
