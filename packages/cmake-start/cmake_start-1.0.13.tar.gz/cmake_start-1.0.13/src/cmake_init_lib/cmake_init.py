        ...
        try:
            ...
            if validation_func is not None:
                ...
                if is_valid:
                    ...
                    if no_prompt:
                        raise ArgumentError(f"'{in_value}' is not an acceptable value: {error_msg}")
                    print(f"❌ {error_msg}")
                    continue
...
def validate_project_name(name):
    ...
    if not re.match("^[a-z]", name):
        if name[0].isupper():
            return False, f"Project name must start with a lowercase letter, not '{name[0]}'.\n       Try: '{name.lower()}' instead."
        elif name[0].isdigit():
            return False, "Project name cannot start with a number.\n       Try: 'my-{name}' or '{name}-project'."
        else:
            return False, f"Project name must start with a lowercase letter (a-z), not '{name[0]}'."
    
    # Check for uppercase letters
    uppercase_chars = [c for c in name if c.isupper()]
    if uppercase_chars:
        return False, f"Project name cannot contain uppercase letters: {', '.join(set(uppercase_chars))}\n       Try: '{name.lower()}' instead."
    
    # Check for invalid characters
    if not re.match("^[a-z][0-9a-z-_]*$", name):
        invalid_chars = [c for c in name if not re.match("[a-z0-9-_]", c)]
        if invalid_chars:
            return False, f"Project name contains invalid characters: {', '.join(set(invalid_chars))}\n       Only lowercase letters, numbers, hyphens (-), and underscores (_) are allowed."
    ...
    if "__" in name:
        return False, "Project name cannot contain consecutive underscores (__).\n       Use single underscores instead."
...
def get_substitutes(cli_args, name):
    ...
    if no_prompt and not is_valid:
        ...
        print("   • Can contain: lowercase letters, numbers, hyphens (-), underscores (_)", file=sys.stderr)
        print("   • Cannot contain: uppercase letters, spaces, or special characters", file=sys.stderr)
        print("   • Cannot be: 'test' or 'lib' (reserved names)", file=sys.stderr)
        print("   • Cannot have: consecutive hyphens (--) or underscores (__)", file=sys.stderr)
...
def create(args, templates_root):
    ...
    if isinstance(templates_root, zipfile.ZipFile):
        ...
        for template_path in (f"templates/{p}" for p in template_paths):
            write_dir(path, d, args.overwrite, zipfile.Path(templates_root, template_path))
        ...
        raise TypeError(f"templates_root must be zipfile.ZipFile or pathlib.Path, got {type(templates_root)}")
...
def main(templates_root, template_compiler):
    ...
    create_flags = \
        ["type_id", "std", "use_clang_tidy", "use_cppcheck", "examples", "openmp"]
