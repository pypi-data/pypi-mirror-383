...
class AsyncPushClient:
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        Args:
            ...
            force_fcm_v1: If True, send Android notifications via FCM V1, regardless
                of whether a valid credential exists.
            max_concurrent_requests: Maximum number of concurrent batch requests.
                Controls rate limiting and prevents overwhelming the server.
                Default: 10 (allows ~1000 notifications/sec with 100 per batch)
        """
        self.host: str = host if host else self.DEFAULT_HOST
        self.api_url: str = api_url if api_url else self.DEFAULT_BASE_API_URL
        self.force_fcm_v1: Optional[bool] = force_fcm_v1
    ...
    async def publish_multiple(self, push_messages: List[PushMessage]) -> List[PushTicket]:
        """Sends multiple push notifications at once asynchronously with concurrent batching.
        ...
        Returns:
           An array of PushTicket objects which contains the results.
        """
        async def send_chunk(chunk: List[PushMessage]) -> List[PushTicket]:
            async with self.semaphore:
                return await self._publish_internal(chunk)
        ...
        for start in itertools.count(0, self.max_message_count):
            ...
            if not chunk:
                break
            tasks.append(send_chunk(chunk))

        results = await asyncio.gather(*tasks)
        return [ticket for batch in results for ticket in batch]

    async def check_receipts_multiple(self, push_tickets: List[PushTicket]) -> List[PushReceipt]:
        """Check receipts in batches of 1000 as per expo docs (asynchronously with concurrent batching)"""
        async def check_chunk(chunk: List[PushTicket]) -> List[PushReceipt]:
            async with self.semaphore:
                return await self._check_receipts_internal(chunk)
        ...
        results = await asyncio.gather(*tasks)
        return [receipt for batch in results for receipt in batch]
