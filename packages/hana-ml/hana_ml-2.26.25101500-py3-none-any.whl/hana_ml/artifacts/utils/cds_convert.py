...
def create_cds_artifacts(data, namespace, context, entity, primary_key=None, create_view=True, annotation=None, hdbtable_name=None, hdbview_name=None, cds_version=8):
    """
    For internal use.
    """
    anno_prefix = ''
    ...
    if cds_version >= 9:
        # CDS 9+ 使用hdbtable
        persistence_annotation = ['@cds.persistence.table']
    else:
        # CDS 8及以下使用hdbcds
        persistence_annotation = ['@cds.persistence.exists']

    if annotation:
        annotation = persistence_annotation + annotation
    else:
        annotation = persistence_annotation
