...
def process_line(dirty: bool, line: str) -> tuple:
    line = line.strip()
    ...
    # ID if a line has been configured in soups.toml
    is_configured = lineType in set(get_mapping().keys())

    # Skip the line if not in mapping, if dirty=true map just to text instead
    if not is_configured and not dirty:
        return (1, lineType)

    # Based on type, assign all the values to the skeleton

    # Run regular pipeline for configured lines, if unconfigured + dirty then just fill in type and text
    if is_configured:
        ...
        skeletor["type"] = f"({lineType})"

    # All line types get text field
    skeletor["text"] = " ".join(spl[get_mapping()["default"]["type"]["index"] + 1 :])

    return (0, skeletor)


def convert_to_sql(dirty: bool, full_file_path: str):
    ...
    with gzip.open(full_file_path, "rt") as f:
        ...
        with click.progressbar(
            ...
            for line in bar:
                status, processed = process_line(dirty, line)
