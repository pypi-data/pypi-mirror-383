...
class HookX:
    ...
    def scan(
        ...
        try:
            ...
            if output_format and self.reporter:
                # Add datetime to filename and save in current directory
                if output_file and output_format != "table":
                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                    output_path = Path(output_file)
                    # Save in current directory with datetime
                    output_file = Path.cwd() / f"{output_path.stem}_{timestamp}{output_path.suffix}"
    ...
    def export_for_ci(
        ...
        **kwargs
        ...
        if self.reporter:
            ...
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            output_path = Path(output_file)
            final_output = Path.cwd() / f"{output_path.stem}_{timestamp}{output_path.suffix}"
            
            result_bool = self.reporter.export_for_ci(result, final_output)
            print(f"\n📄 CI report saved to: {final_output.resolve()}")
            return result_bool
        else:
    ...
    def full_scan_report(self, path: Union[str, Path], save_to: Optional[Union[str, Path]] = None) -> dict:
        ...
        if save_to:
            ...
            final_path = Path.cwd() / f"{save_path.stem}_{timestamp}{save_path.suffix}"
            
            with open(final_path, 'w', encoding='utf-8') as f:
                json.dump(report, f, indent=2, ensure_ascii=False)
            
            print(f"\n📄 Full report saved to: {final_path.resolve()}")
