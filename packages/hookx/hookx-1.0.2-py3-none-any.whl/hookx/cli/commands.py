"""
CLI command implementations for HookX with comprehensive functionality.
...
def scan_command(ctx, path, site, output_format, output, min_severity, strict, no_cache, max_workers):
    ...
    try:
        ...
        if result.has_critical_conflicts():
            ...
            for c in result.conflicts:
                try:
                    ...
                    continue
            console.print(f"\n❌ Scan completed with {result.total_conflicts} conflicts ({critical_count} critical)", style="red bold")
        ...
        # Output file message is handled by the API itself
        pass
...
def check_command(ctx, path, exit_code, critical_only, strict, quiet):
    ...
    try:
        ...
        # Perform check
        if critical_only:
            has_issues = hookx.has_critical_conflicts(path)
            issue_type = "critical conflicts"
        else:
            has_issues = hookx.check_conflicts_only(path)
...
def validate_command(ctx, path, fix):
    ...
    try:
        ...
        # Initialize HookX
        hookx = HookX(console_output=True)
        ...
        if path_obj.is_file() and path_obj.name == "hooks.py":
            ...
            issues = hookx.validate_hooks(path_obj)
            ...
            for app_path in apps:
                ...
                if hooks_file:
                    app_issues = hookx.validate_hooks(hooks_file)
...
def export_command(ctx, path, output_file, export_format, site, fail_on_conflicts, fail_on_critical):
    ...
    try:
        ...
        # Initialize HookX
        hookx = HookX(console_output=False)
        ...
        # Success message (file path printed by API)
        console.print(f"✅ Export completed", style="green")
