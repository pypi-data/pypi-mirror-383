"""
Advanced reporting system for HookX with multiple output formats.
...
class HookReporter:
    ...
    def generate_report(self, scan_result: ScanResult, format_type: str = "table", output_file: Optional[Union[str, Path]] = None) -> str:
        ...
        if output_file:
            # If output_file doesn't have datetime, add it and save in current directory
            output_path = Path(output_file)
            if "_" not in output_path.stem or not any(char.isdigit() for char in output_path.stem.split("_")[-1]):
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                output_path = Path.cwd() / f"{output_path.stem}_{timestamp}{output_path.suffix}"
    ...
    def _print_conflicts_table(self, conflicts: List[ConflictReport]):
        ...
        for conflict in conflicts:
            ...
            if isinstance(severity, str):
                ...
                # Convert string to enum for icons/colors
            ...
            severity_text = Text(
                f"{self.severity_icons[severity_enum]} {severity_value.upper()}",
                style=self.severity_colors[severity_enum]
            )
            
            # Format hook type
            hook_type = conflict.hook_type
            if isinstance(hook_type, str):
                hook_type_value = hook_type
            else:
                try:
                    hook_type_value = hook_type.value
                except AttributeError:
                    hook_type_value = str(hook_type)
    ...
    def _print_hooks_summary(self, hooks: List[HookDefinition]):
        ...
        if by_type:
            ...
            for hook_type, count in sorted(by_type.items()):
                # Ensure hook_type is string
                if hasattr(hook_type, 'value'):
                    hook_type_str = hook_type.value
                else:
                    hook_type_str = str(hook_type)
                type_table.add_row(hook_type_str, str(count))
    ...
    def _generate_csv_report(self, scan_result: ScanResult) -> str:
        ...
        for conflict in scan_result.conflicts:
            ...
            severity_val = conflict.severity.value if hasattr(conflict.severity, 'value') else str(conflict.severity)
            hook_type_val = conflict.hook_type.value if hasattr(conflict.hook_type, 'value') else str(conflict.hook_type)
    ...
    def _generate_html_report(self, scan_result: ScanResult) -> str:
        ...
        html_parts = [
            ...
            "<html>",
            "<head>",
            "<title>HookX Scan Report</title>",
            "<style>",
            self._get_html_styles(),
            "</style>",
            "</head>",
            "<body>",
            f"<h1>HookX Scan Report</h1>",
    ...
    def _generate_markdown_report(self, scan_result: ScanResult) -> str:
        ...
        if scan_result.conflicts:
            ...
            for conflict in scan_result.conflicts:
                ...
                md_parts.append(
                    f"| {severity_val.upper()} | {hook_type_val} | "
