...
def _unbroadcast(grad: np.ndarray, target_shape: tuple[int, ...]) -> np.ndarray:
    ...
    Drops leading fake dims and sums over axes that were broadcast (size-1 in `target_shape`).
    ...
    # first, drop fake leading dims
    while grad.ndim > len(target_shape):
        grad = grad.sum(axis=0)
    # ^
    # Given x = [ x0, x1, x2 ],
    # Given f(x) = x * [[ y0, y1, y2 ], [ y3, y4, y5 ]] -> [[ x0y0, x1y1, x2y2 ], [ x0y3, x1y4, x2y5 ]]
    # So, forward-pass view of x:
    # [[ x₀, x₁, x₂ ],      ← copy #1
    #  [ x₀, x₁, x₂ ]]      ← copy #2
    # And, gradient of f w.r.t. x is [[ y0, y1, y2 ], [ y3, y4, y5 ]]
    # But, how much did x0 contribute overall? The answer is y0 + y3; likewise you sum following the same logic for all x_i
    # Overall gradient of f w.r.t. x is NOT [[ y0, y1, y2 ], [ y3, y4, y5 ]], but now IS [ y0 + y3, y1 + y4, y2 + y5 ]
    # and x will be updated as follows: x - lr * [ y0 + y3, y1 + y4, y2 + y5 ]

    # -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

    # now, sum out any size-1 dims that were broadcast following the same logic as above (aggregating contributions)
    for axis, (g_dim, t_dim) in enumerate(zip(grad.shape, target_shape)):
        if t_dim == 1 and g_dim != 1:
            grad = grad.sum(axis=axis, keepdims=True)

    out = grad.reshape(target_shape) # <-- safety net, just in case (it doesn't alter the data)
