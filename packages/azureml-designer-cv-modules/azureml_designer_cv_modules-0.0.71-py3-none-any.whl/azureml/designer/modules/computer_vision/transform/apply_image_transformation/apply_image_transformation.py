...
from azureml.studio.internal.error import ErrorMapping, InvalidTransformationDirectoryError, InvalidDatasetError
...
def entrance(
    ...
    try:
        ...
        if "required crop size" in str(e.args).lower():
            ErrorMapping.rethrow(e, InvalidDatasetError(
                dataset1=input_image_dir_path.name,
                reason=f"Got exception when transforming images: {ErrorMapping.get_exception_message(e)}",
                troubleshoot_hint="Please adjust parameter 'crop size' value or input image size."))
