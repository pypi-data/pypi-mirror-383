...
#5) repeat until all probes are processed.
    ...
    def add_parameter_key(self):
        ...
        recordings = self.schema.EphysRecording.ProbeSetting() & dict(self.dataset_key)
        sortings = self.schema.SpikeSorting() & dict(self.dataset_key, parameter_set_num = self.parameter_set_num)
    ...
    def _secondary_parse(self,arguments,parameter_number = None):
        ...
        if not parameter_number is None:
            self.parameters = ((self.schema.SpikeSortingParams() & f'parameter_set_num = {parameter_number}')).fetch(as_dict = True)
