...
Can submit job on slurm, some of these can be long or take resources.
    ...
    def apply(self):
        ...
        if not comparison:
            print('CHECKSUM FAILED for {0}'.format(Path(self.src_paths.src_path.iloc[0]).parentS))
        ...
        try:
            ...
            print(err)
            self.set_job_status(job_status = 'FAILED',job_log = f'ERROR uploading {traceback.print_exc(err)}')
            return
        try:
            ...
            print(err)
            self.set_job_status(job_status = 'FAILED',job_log = f'POST-UPLOAD: {traceback.print_exc(err)}')
            return 
