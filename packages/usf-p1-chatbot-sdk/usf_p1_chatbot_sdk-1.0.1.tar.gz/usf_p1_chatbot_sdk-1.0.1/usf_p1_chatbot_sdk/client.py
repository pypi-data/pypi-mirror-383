...
class ChatbotClient:
    ...
    def _get_headers(self) -> Dict[str, str]:
        """Get base headers (Authorization only). Content-Type is added per-request as needed."""
        headers = {
    ...
    def chat(
        ...
        response = self.client.post(f"{self.base_url}/api/chat", json=data, headers=headers)
        return self._handle_response(response)
    ...
    async def chat_stream(
        ...
        headers = self._get_headers()
        headers["Content-Type"] = "application/json"
        async with self.async_client.stream("POST", f"{self.base_url}/api/chat/stream", json=data, headers=headers) as response:
            response.raise_for_status()
    ...
    def ingest_pdfs(
        ...
        Args:
            ...
            files: List of file paths to PDF files
            patient_user_name: Optional patient name to associate files with
            
        Returns:
            Dict containing request_id and ingestion details
            
        Raises:
            FileNotFoundError: If a file path doesn't exist
            ChatbotClientError: If the API returns an error
        """
        file_objects = []
        opened_files = []
        ...
        try:
            for file_path in files:
                ...
                file_objects.append(("files", (file_path.name, f, "application/pdf")))
            
            data = {"collection_id": collection_id}
            if patient_user_name:
                data["patient_user_name"] = patient_user_name
            
            # Note: Content-Type header is NOT set here - httpx will automatically
            # set it to multipart/form-data with proper boundary
            headers = self._get_headers()
            response = self.client.post(
                f"{self.base_url}/api/data/async/pdfs", 
                data=data, 
                files=file_objects,
                headers=headers
            )
    ...
    def ingest_urls(
        ...
        headers = self._get_headers()
        headers["Content-Type"] = "application/json"
        response = self.client.post(f"{self.base_url}/api/data/async/urls", json=data, headers=headers)
        return self._handle_response(response)
    ...
    def ingest_default(
        ...
        try:
            ...
            headers = self._get_headers()
            response = self.client.post(
                f"{self.base_url}/api/data/async", 
                data=data, 
    ...
    def create_collection(self, collection_name: str, description: Optional[str] = None) -> Dict[str, Any]:
        ...
        headers = self._get_headers()
        headers["Content-Type"] = "application/json"
        response = self.client.post(f"{self.base_url}/api/collections", json=data, headers=headers)
        return self._handle_response(response)
    ...
    def register_patient(
        ...
        **additional_data
        ...
        headers = self._get_headers()
        headers["Content-Type"] = "application/json"
        response = self.client.post(f"{self.base_url}/api/patient/register", json=data, headers=headers)
        return self._handle_response(response)
    ...
    def validate_patient(self, collection_id: str, patient_name: str) -> Dict[str, Any]:
        ...
        headers = self._get_headers()
        headers["Content-Type"] = "application/json"
        response = self.client.post(f"{self.base_url}/api/patient/validate", json=data, headers=headers)
        return self._handle_response(response)
