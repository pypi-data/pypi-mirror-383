...
int collect_wavs(const char *dir, char ***out_list, int *out_count) {
    if (!dir || !out_list || !out_count) {
        if (debug) fprintf(stderr, "DEBUG: collect_wavs argument invalide\n");
        return -1;
    }
    ...
    do {
        ...
        if (fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
            ...
            if (!tmp) {
                FindClose(hFind);
                if (debug) fprintf(stderr, "DEBUG: realloc collect_wavs failed\n");
                return -1;
            }
            *out_list = tmp;
            ...
            if (debug) fprintf(stderr,"DEBUG: wav trouve %s\n", fullpath);
            (*out_count)++;
        }
