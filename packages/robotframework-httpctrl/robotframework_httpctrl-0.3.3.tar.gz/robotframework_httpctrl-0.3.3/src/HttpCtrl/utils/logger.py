...
class LoggerAssistant:
    ...
    def get_body(body):
        if body is not None:
            if (LoggerAssistant.__MAX_BODY_SIZE_TO_LOG is None) or (
                len(body) < LoggerAssistant.__MAX_BODY_SIZE_TO_LOG
            ):
                return body
            else:
                return "%s...\n...\n<display only the first '%d' from '%d' symbols>" % (
                    body[: LoggerAssistant.__MAX_BODY_SIZE_TO_LOG],
                    LoggerAssistant.__MAX_BODY_SIZE_TO_LOG,
                    len(body),
                )
