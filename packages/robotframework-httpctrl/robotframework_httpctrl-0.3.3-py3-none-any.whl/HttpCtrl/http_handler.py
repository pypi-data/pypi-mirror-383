...
from HttpCtrl.internal_messages import IgnoreRequest, TerminationRequest
...
class HttpHandler(SimpleHTTPRequestHandler):
    ...
    def __default_handler(self, method):
        ...
        if stub is not None:
            ...
            if isinstance(response, TerminationRequest) or isinstance(
                response, IgnoreRequest
            ):
                return
        ...
        try:
            ...
            logger.info(
                "Response was not sent to client due to reason: '%s'." % str(exception)
            )
