...
@mcp.tool(
    description="查询指定时间范围内，每个场地所有可用的具体时间段。这会返回一个详细的时间段列表，而不是简单的“可用/不可用”状态。注意：场馆的有效预订时间为每天 07:00 至 22:00。",
)
...
@mcp.tool(
    description="查询指定时间段内可用的场地（重要提示：1. 场馆每天的有效预订时间为 07:00 至 22:00；2. 查询时间段应尽可能精确，与实际需求一致，避免查询过长的时段）",
)
...
@mcp.tool(
    description="预订指定时间段的指定类型场地（重要提示：1. 场馆每天的有效预订时间为 07:00 至 22:00；2. 预订前必须先查询可用场地；3. 单次预订时长限制为1小时；4. 预订操作不可撤销，请确认信息正确）",
)
