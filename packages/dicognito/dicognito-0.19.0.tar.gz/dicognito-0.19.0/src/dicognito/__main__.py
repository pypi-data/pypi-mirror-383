...
def _get_datasets_from_sources(sources: Iterable[str]) -> Iterable[pydicom.dataset.Dataset]:
    for filename in list(_get_filenames_from_sources(sources)):
        try:
            ...
            logger.info("File %s appears not to be DICOM. Skipping.", filename)


def main(main_args: Sequence[str]) -> None:
    """Run the anonymizer."""
    ...
    for dataset in _get_datasets_from_sources(args.sources):
        try:
            ...
            logger.exception("Error occurred while converting %s. Aborting.", dataset.filename)
            sys.exit(1)
