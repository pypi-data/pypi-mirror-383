...
from azpaddypy.mcp.services import base
from azpaddypy.mcp.services import resources
from azpaddypy.mcp.services import storage
from azpaddypy.mcp.services import cosmos
from azpaddypy.mcp.services import subscription
...
class TestAzureServiceContext:
    ...
    def test_initialize_from_mgmt_config_success(self):
        ...
        with patch.dict("sys.modules", {
            ...
            assert ctx._credential == mock_credential
            assert ctx._subscription_id == "subscription-from-kv"
            assert ctx._initialized is True

    def test_initialize_from_mgmt_config_import_error(self):
        """Test initialization when mgmt_config not found."""
        with patch.dict("sys.modules", {"mgmt_config": None}):
            ctx = base.get_context()

            with pytest.raises(base.AzureMCPError, match="Failed to import mgmt_config"):
                ctx.initialize_from_mgmt_config()
