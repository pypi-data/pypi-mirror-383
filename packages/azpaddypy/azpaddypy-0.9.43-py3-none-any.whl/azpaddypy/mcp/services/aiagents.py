    ...
    try:
        ...
        for resource in resources:
            ...
            ai_projects.append({
                ...
                "endpoint": f"https://{resource_name}.services.ai.azure.com/api/projects/{resource_name}"
            })
        ...
        raise AzureMCPError(f"Failed to list AI Foundry projects: {e}") from e


def _get_ai_project_client(project_endpoint: Optional[str] = None) -> AIProjectClient:
    ctx = get_context()
    ...
    try:
        ...
        raise AzureMCPError(f"Failed to create AI Project client: {e}") from e


def list_ai_agents() -> List[Dict[str, Any]]:
    try:
        client = _get_ai_project_client()
        ...
        raise AzureMCPError(f"Failed to list AI agents: {e}") from e


def get_ai_agent(name: str, project_endpoint: Optional[str] = None) -> Dict[str, Any]:
    try:
        client = _get_ai_project_client(project_endpoint)
        agents = client.agents.list_agents()
        ...
        for agent in agents:
            if agent.name == name:
                return {
                    ...
                    "created_at": agent.created_at.isoformat() if agent.created_at else None,
                    "available": True,
                    "project_endpoint": project_endpoint or os.getenv("AZURE_AI_FOUNDRY_PROJECT_ENDPOINT")
                }
        ...
        raise AzureMCPError(f"Failed to get AI agent: {e}") from e
...
def create_ai_agent(
    ...
    try:
        ...
        raise AzureMCPError(f"Failed to create AI agent: {e}") from e


def delete_ai_agent(agent_id: str) -> Dict[str, Any]:
    try:
        client = _get_ai_project_client()
        ...
        raise AzureMCPError(f"Failed to delete AI agent: {e}") from e
...
def invoke_ai_agent(
    ...
    try:
        ...
        while run.status in ["queued", "in_progress"]:
            import time
            time.sleep(1)
            run = client.agents.runs.retrieve(thread_id=thread_id, run_id=run.id)
        ...
        raise AzureMCPError(f"Failed to invoke AI agent: {e}") from e
