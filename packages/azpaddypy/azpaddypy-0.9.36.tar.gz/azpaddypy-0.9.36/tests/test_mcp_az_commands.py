...
class TestInitialization:
    ...
    def test_initialize_from_mgmt_config_success(self):
        ...
        with patch.dict("sys.modules", {
            "mgmt_config": Mock(keyvaults=mock_keyvaults)
        }), patch.object(az_commands, "DefaultAzureCredential") as mock_default_cred_class:
            mock_default_cred_class.return_value = mock_credential

            az_commands.initialize_from_mgmt_config()

            # Verify that DefaultAzureCredential is created directly
            mock_default_cred_class.assert_called_once()
            assert az_commands._credential == mock_credential
...
class TestResourceGroups:
    ...
    def test_list_resource_groups(self, setup_clients):
        ...
        with patch.object(az_commands, "ResourceManagementClient") as mock_client_class:
            mock_client = Mock()
            mock_client.resource_groups.list.return_value = [mock_rg1, mock_rg2, mock_rg3]
            ...
            assert result[2]["name"] == "rg-3"
            assert result[2]["application"] == "None"
