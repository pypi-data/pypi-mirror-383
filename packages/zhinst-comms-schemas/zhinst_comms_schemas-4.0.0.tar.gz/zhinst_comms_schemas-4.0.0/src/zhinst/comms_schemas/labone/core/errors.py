...
class LabOneCoreError(LabOneError):
    ...
    def __eq__(self, other: object):
        if not isinstance(other, LabOneCoreError):
            return False
        return (
            super().__eq__(other)
    ...
    def __hash__(self):
        return hash((self.args, self.code, self.category))
...
def convert_streaming_error(err: hpk_schema.Error) -> LabOneCoreError:
    """Create a LabOneCoreError from a labone error struct.
    ...
    """
    return _ZI_ERROR_MAP.get(err.kind, LabOneCoreError)(  # type: ignore[call-overload]
        err.message,
        code=err.code,
...
def raise_streaming_error(err: hpk_schema.Error) -> None:
    ...
    Raises:
        LabOneCoreError: The converted error.
    """
    raise convert_streaming_error(err)
