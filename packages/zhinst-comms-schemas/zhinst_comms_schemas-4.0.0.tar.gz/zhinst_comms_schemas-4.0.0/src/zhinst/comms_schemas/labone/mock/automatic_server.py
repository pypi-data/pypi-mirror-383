...
from zhinst.comms_schemas.labone.core import ListNodesFlags, ListNodesInfoFlags
from zhinst.comms_schemas.labone.core.errors import LabOneCoreError
from zhinst.comms_schemas.labone.core.value import (
    AnnotatedValue,
    Value,
)
from zhinst.comms_schemas.labone.mock.errors import LabOneMockError
from zhinst.comms_schemas.labone.mock.session import LabOneServerBase, Subscription
...
class AutomaticLabOneServer(LabOneServerBase):
    ...
    async def set_with_expression(
        ...
        if isinstance(value, AnnotatedValue):
            ...
            value = value.value
        result = [
            await self.set(value=value, path=p)
            for p in await self.list_nodes(path)  # type: ignore[arg-type]
        ]
        if not result:
