            ...
            for i, batch in enumerate(batches):
                ...
                for attempt in range(self.num_retries + 1):
                    try:
                        ...
                        prediction = pd.DataFrame(**resp.prediction)

                        # Cast 'ENTITY' to correct data type:
                        if 'ENTITY' in prediction:
                            pkey_map = self._graph_store.pkey_map_dict[
                                query_def.entity.pkey.table_name]
                            prediction['ENTITY'] = prediction['ENTITY'].astype(
                                type(pkey_map.index[0]))

                        # Cast 'ANCHOR_TIMESTAMP' to correct data type:
                        if ('ANCHOR_TIMESTAMP' in prediction
                                and not pd.api.types.is_datetime64_any_dtype(
                                    prediction['ANCHOR_TIMESTAMP'])):
                            prediction['ANCHOR_TIMESTAMP'] = pd.to_datetime(
                                prediction['ANCHOR_TIMESTAMP'], unit='ms')
