# tests/test_sa_repo_sync.py
import pytest
from typing import Optional
from sqlalchemy import create_engine, String, Integer, Boolean, Text
from sqlalchemy.orm import sessionmaker, DeclarativeBase, Mapped, mapped_column

from SARepo.sa_repo import SARepository  # твой синхронный класс
from SARepo.base import NotFoundError    # если нужно в других тестах
...
def seed(session, repo: SARepository):
    """Наполняем базу различными записями для агрегатов/фильтров."""
    data = [
        {"title": "alpha", "city": "Almaty",   "amount": 10, "age": 20, "note": "A"},
        {"title": "beta",  "city": "Astana",   "amount": 20, "age": 30, "note": "B"},
        {"title": "gamma", "city": "Almaty",   "amount": 30, "age": 40, "note": "G"},
        {"title": "delta", "city": "Shymkent", "amount": 40, "age": 50, "note": "D"},
        {"title": "omega", "city": None,       "amount": 50, "age": 60, "note": "O"},
    ]
    for row in data:
        repo.add(row)  # add ожидает dict и делает INSERT ... RETURNING id + commit
    # repo.add коммитит сам; отдельный session.commit() не обязателен
...
def test_restore(session, repo):
    ...
    # убедимся в известном состоянии
    beta.is_deleted = False
    session.commit()

    # remove делает soft-delete при наличии is_deleted
    ok_del = repo.remove(id=beta.id)
