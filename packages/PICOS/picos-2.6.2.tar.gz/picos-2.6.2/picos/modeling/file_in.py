...
from ..expressions import (AffineExpression, Constant, IntegerVariable,
...
def _read_cbf_block(P, blocname, f, parsed_blocks):
    ...
    if blocname == "OBJSENSE":
        ...
        for i in range(n):
            ni = int(f.readline())
            vardims.append(ni)
            Xi = SymmetricVariable("X[" + str(i) + "]", (ni, ni))
            XX.append(Xi)
        ...
        for i in range(ncones):
            ...
            if tp == "F":
                xi = RealVariable("x[" + str(i) + "]", dim)
            elif tp == "L+":
                xi = RealVariable("x[" + str(i) + "]", dim, lower=0)
            elif tp == "L-":
                xi = RealVariable("x[" + str(i) + "]", dim, upper=0)
            elif tp == "L=":
                xi = RealVariable("x[" + str(i) + "]", dim, lower=0, upper=0)
            elif tp == "Q":
                xi = RealVariable("x[" + str(i) + "]", dim)
        ...
        for i in ints:
            if len(ints[i]) == x[i].size[0]:
                x[i] = IntegerVariable(
                    x[i].name, x[i].shape, lower=x[i]._lower, upper=x[i]._upper)
            else:
                x.append(
                    IntegerVariable("x_int[" + str(i) + "]", len(ints[i]))
                )
