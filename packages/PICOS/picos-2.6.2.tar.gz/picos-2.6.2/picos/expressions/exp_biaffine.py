...
class BiaffineExpression(Expression, ABC):
    ...
    def __or__(self, other):
        ...
        result = self.vec.H * other.vec
        result._symbStr = glyphs.clever_dotp(
            self.string, other.string, self.complex, self.scalar)
        return result
        ...
        def _kron(A, B):
            if isinstance(A, cvxopt.spmatrix) or isinstance(B, cvxopt.spmatrix):
                try:
                    ...
                    A_B = load_data(
                        numpy.kron(cvx2np(A), cvx2np(B)), typecode=tc
                    )[0]
                else:
                    from .data import cvx2csc
                    A_B = load_data(
                        scipy.sparse.kron(cvx2csc(A), cvx2csc(B), format='csc'),
                        typecode=tc, sparse=True
                    )[0]
            else:
                A_B = load_data(
                    numpy.kron(cvx2np(A), cvx2np(B)), typecode=tc
    ...
    @cached_property
    def opreal(self):
        r"""The real part of a Hermitian operator.
        ...
        """
        if not self.square:
            raise TypeError("Cannot take opreal of non-square {}.".format(self))

        return (self + self.H)/2

    @cached_property
    def opimag(self):
        r"""The imaginary part of a Hermitian operator.

        For a square (complex) affine expression :math:`A`, this is
        :math:`\frac{1}{2i}(A - A^H)`.
        ...
        """
        if not self.square:
            raise TypeError("Cannot take opimag of non-square {}.".format(self))

        return (self - self.H)/(2j)
    ...
    def hermitianized(self):
        ...
        This is the same as :meth:`opreal`.
        """
        return self.opreal
    ...
    def sum(self):
        ...
        # NOTE: 1 on the left side in case self is complex.
        return (1 | self)
    ...
    def tr(self):
        ...
        # NOTE: "I" on the left side in case self is complex.
        return ("I" | self)
