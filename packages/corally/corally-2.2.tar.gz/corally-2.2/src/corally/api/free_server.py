...
# Startup logging is now handled in the lifespan function
...
async def lifespan(app: FastAPI):
    # Startup
    logging.info("=== Free API Server Starting ===")
    ...
    yield
    # Shutdown
    logging.info("=== Free API Server Shutting Down ===")
    ...
    logging.info("=== Free API Server Shutdown Complete ===")

    # Final flush
    for handler in logging.getLogger().handlers:
        handler.flush()

app = FastAPI(title="Free Currency Converter with CSV Cache", lifespan=lifespan)
...
async def convert(
    ...
    print(f"DEBUG: Convert request: {amount} {from_currency} -> {to_currency}", file=sys.stderr)
    ...
    try:
        ...
        logging.error(f"HTTP error getting exchange rate: {e.detail}")
        raise
    except Exception as e:
