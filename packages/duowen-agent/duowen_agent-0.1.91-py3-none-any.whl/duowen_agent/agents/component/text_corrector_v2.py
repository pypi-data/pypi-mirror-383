        ...
        for tag, i1, i2, j1, j2 in s.get_opcodes():
            if tag == 'insert':
                ...
                if j2 - j1 > 20:  # 大段插入（超过20字符）
                    ...
                    large_operations.append(f"大段插入({j2-j1}字符): '{inserted_text[:30]}...'")
                    
            elif tag == 'delete':
                delete_chars += i2 - i1
                if i2 - i1 > 20:  # 大段删除（超过20字符）
                    deleted_text = target[i1:i2]
                    large_operations.append(f"大段删除({i2-i1}字符): '{deleted_text[:30]}...'")
                ...
                if max(i2 - i1, j2 - j1) > 20:  # 大段替换（超过20字符）
                    ...
                    large_operations.append(f"大段替换({max(i2-i1, j2-j1)}字符): '{original_text[:15]}...' → '{new_text[:15]}...'")
    ...
    def _correct_line(self, **chunk_data) -> Dict[str, Any]:
        ...
        try:
            ...
            for attempt in range(max_retries + 1):
                ...
                if not self._validate_word_level_similarity(chunk_data["text"], corrected):
                    if attempt == max_retries:
                        chunk_info = f"块{chunk_data['chunk_num']}(行{chunk_data['start_line']}-{chunk_data['end_line']})"
                        self.call_back_func(
