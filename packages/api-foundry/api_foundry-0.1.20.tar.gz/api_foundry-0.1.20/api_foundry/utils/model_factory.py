...
SUPPORTED_TYPES = {
    ...
    "date-time",
    "time",
    # Note: OpenAPI uses type 'number' with format 'float'/'double'.
    # Types 'float' and 'double' are intentionally NOT supported here.
    "uuid",
    "array",
    "object",
}
    ...
    def __init__(self, schema_name: str, name: str, prop: Dict[str, Any]):
        ...
        if raw_type is not None and raw_type not in {
            ...
            raise ApplicationException(
                500,
                (
                    f"Property: {name} in schema object: {schema_name} of type: "
                    + f"{raw_type} is not a valid type"
                ),
            )
...
class SchemaObject(OpenAPIElement):
    ...
    def _resolve_property(
        ...
        # Validate raw OpenAPI base type early to catch invalid types like 'float'
        raw_type = prop.get("type")
        if raw_type is not None and raw_type not in {
            "string",
            "integer",
            ...
            raise ApplicationException(
                500,
                (
                    f"Property: {property_name} in schema object: {self.api_name} "
