...
@router.post("/compounds/batch", response_model=List[models.CompoundResponse])
def get_compounds_by_any_synonyms(
    request: models.SynonymBatchRequest,
    db: Session = Depends(get_db),
):
    compounds = crud.get_compounds_by_synonyms(db, request.property_values, request.property_name)
    if not compounds:
        raise HTTPException(status_code=404, detail="No compounds found for the provided synonyms")
    return compounds
