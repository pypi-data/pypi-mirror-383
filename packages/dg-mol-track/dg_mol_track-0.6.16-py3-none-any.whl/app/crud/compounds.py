...
def _query_compounds_by_synonym_filters(
    db: Session,
    property_filter,
    property_name: str = None,
):
    filters = [models.Property.semantic_type_id == 1, property_filter]
    if property_name:
...
def get_compound_by_synonym(
    db: Session,
    property_value: str,
    property_name: str = None,
    enrich: bool = True,
):
    query = _query_compounds_by_synonym_filters(db, models.CompoundDetail.value_string == property_value, property_name)
    compound = query.first()
...
def get_compounds_by_synonyms(
    db: Session,
    property_values: list[str],
    property_name: str = None,
    enrich: bool = True,
):
    query = _query_compounds_by_synonym_filters(
        db, models.CompoundDetail.value_string.in_(property_values), property_name
    )
    results = query.all()

    value_to_compound = {
        detail.value_string: compound
        for compound in results
        for detail in compound.compound_details
        if detail.property.semantic_type_id == 1
        and (property_name is None or detail.property.name == property_name)
        and detail.value_string in property_values
    }

    return [
        (
            enrich_model(value_to_compound[v], models.CompoundResponse, "compound_details", "compound_id")
            if v in value_to_compound and enrich
            else value_to_compound.get(v)
        )
