...
from typing import Annotated, List, Optional, Self, TypeGuard, Union, overload
...
class DateFilter(
    ...
    from_date: Annotated[OptionalDatetime, Field(None, description="From date")] = None
    to_date: Annotated[OptionalDatetime, Field(None, description="To date")] = None
...
OptionalDateFilter = Optional[DateFilter]
ListOfDateFilters = List[DateFilter]
OptionalListOfDateFilters = Optional[OptionalDateFilter]


def is_date_filters(
    filters: Union[ListOfDateFilters, ListOfStrings],
...
class Filters(BaseModel):
    ...
    @classmethod
    def validate_filters_pattern(cls, value: ListOfStrings) -> ListOfStrings:
        for v in value:
...
class DateFilters(BaseModel):
    date_filters: Annotated[
        ListOfDateFilters,
        Field(
            default_factory=list[DateFilter], description="Date filters to be applied"
        ),
    ] = list[DateFilter]()

    @classmethod
    def from_filters(cls, filters: ListOfStrings) -> "DateFilters":
...
@overload
def convert(filters: ListOfDateFilters) -> ListOfStrings: ...
@overload
def convert(filters: ListOfStrings) -> ListOfDateFilters: ...
def convert(
    filters: Union[
        ListOfDateFilters,
        ListOfStrings,
