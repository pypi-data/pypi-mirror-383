...
from zsynctech_studio_sdk.core.errors.step import StepFinalizedError
...
def is_a_finalized_step(method):
    @wraps(method)
    def wrapper(self, *args, **kwargs):
        if self.model.status in STEP_FINALIZED_STATUS:
            raise StepFinalizedError(
                f"Cannot perform '{method.__name__}' "
                f"on a finished step with status '{self.model.status.value}'."
            )
