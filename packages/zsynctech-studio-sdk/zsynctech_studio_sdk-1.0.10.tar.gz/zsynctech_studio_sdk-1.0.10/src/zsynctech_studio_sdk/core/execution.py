...
def is_a_finished_execution(method):
    ...
    def wrapper(self, *args, **kwargs):
        if self.model.status in EXECUTION_FINISHED_STATUS:
            logger.debug(
                f"Ignorando '{method.__name__}' — execução já finalizada com status '{self.model.status.value}'."
            )
            return self
...
def send_execution(method):
    ...
    def wrapper(self, *args, **kwargs):
        if self.model.status in EXECUTION_FINISHED_STATUS:
            logger.debug(
                f"Ignorando envio de '{method.__name__}' — execução já finalizada ({self.model.status.value})."
            )
            return self
        ...
        try:
            ...
            logger.exception(f"Falha ao atualizar execução: {e}")
