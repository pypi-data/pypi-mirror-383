...
from bear_dereth.collections.lru_cache import LRUCache
...
from bear_dereth.query import QueryProtocol, where_obj as where
...
class Table(TableProtocol[Record]):
    ...
    def set(self, key: str, value: Record) -> None:
        ...
        """
        records: Records = self.get(key=key, cond=where(key=key))
    ...
    def search(self, query: QueryProtocol) -> Records:
        ...
        if cached is not None:
            if cached is NullRecords:
                return NullRecords
            return cached
        results: Records = Records.from_list([rec for rec in self.table_data.records if query(rec)])
    ...
    def all(self) -> list[Record]:
        ...
        """
        return list(self.table_data.records)
