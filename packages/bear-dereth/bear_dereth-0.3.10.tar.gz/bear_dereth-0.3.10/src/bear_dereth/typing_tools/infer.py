...
def eval_to_type_str(value: Any, cond: Callable[[PossibleStrs], bool]) -> PossibleStrs | Any:
    ...
    if isinstance(value, str):
        try:
            ...
            return v if cond(v) else CONTINUE
...
@lru_cache(maxsize=128)
def eval_to_native(v: Any, cond: Callable[[Any], bool]) -> Any:
    """Uses ast.literal_eval to convert a string wrapped value to its native typ or string type.

    Args:
        value (Any): The value to convert.
        cond (Callable[[Any], bool]): A callable that takes a value and returns a bool
...
class Inference:
    ...
    def __init__(
        self, value: Any, path_as_str: bool = False, arb_types_allowed: bool = False, prime_types_only: bool = False
    ) -> None:
        ...
        self.prime_types_only: bool = prime_types_only

    def infer_type(self) -> str:
        """Infer the inner type of an array-like structure (list, tuple, set)."""
        if isinstance(self.value, str):
            value: Any = eval_to_native(self.value, lambda x: isinstance(x, ACCEPTABLE_TYPES))
            if str_to_bool(self.value, as_str=True) == "bool":
                return "bool"
            if self.path_check(self.value):
                return "path"
            if value is CONTINUE:
                return self.str_type(self.value)
            self.value = value
        if self.empty_collection:
            return type_to_str(type(self.value))
        if isinstance(self.value, tuple):
            return self._tuple_work(self.value)
        if isinstance(self.value, list):
            return self._general_work("list")
        if isinstance(self.value, set):
            return self._general_work("set")
        if isinstance(self.value, dict):
            return self._dict_work(self.value)
        try:
            return type_to_str(type(self.value))
        except Exception:
            return "Any"  # Should basically never get here

    def path_check(self, value: str) -> Literal["path", False]:
        """Check if a string represents a valid filesystem path."""
        with suppress(Exception):
            if not self.path_as_str:
                return "path" if Path(value).exists() else False
        return False
    ...
    def str_type(value: str) -> str:
        ...
        """
        string_value: PossibleStrs = eval_to_type_str(value=value, cond=lambda x: x in ACCEPTABLE_TYPE_STRS)
    ...
    def _dict_work(self, value: Any) -> str:
        """Handle special cases for dicts."""
        if self.prime_types_only:
            return "dict"
    ...
    def _general_work(self, type_str: str) -> str:
    ...
    def _tuple_work(self, value: Any) -> str:
    ...
    def first_value(self) -> str:
        ...
        return self.str_type(
            next(iter(self.value.values())) if isinstance(self.value, dict) else next(iter(self.value))
        )
    ...
    def all_value_types(self) -> set[str]:
        ...
        if not isinstance(self.value, dict):
            return {self.str_type(i) for i in self.value}
        return {self.str_type(v) for v in self.value.values()}
...
@lru_cache(maxsize=128)
def infer_type(value: str | Any, path_as_str: bool = False, arb_types_allowed: bool = False) -> str:
    """Infer the type of a str wrapped value and return it as a string.
    ...
    """
    inf = Inference(value, path_as_str=path_as_str, arb_types_allowed=arb_types_allowed)
    return inf.infer_type()
