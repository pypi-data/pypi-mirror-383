...
from .constants import ECHO_APP_ID


def _get_openai_client(api_key: str):
    """Initialize OpenAI client with Echo support.

    Automatically detects Echo API keys (starting with 'echo_') and uses
    the Echo router base URL. Otherwise uses the standard OpenAI API.

    Args:
        api_key: The OpenAI or Echo API key
...
def _register_commands():
    ...
    def generate(
        prompt: str = typer.Argument(
            ..., help="Base prompt describing the video concept"
        ),
        ...
        if not api_key:
            console.print(
                ...
                f"You can get an API key at https://echo.merit.systems/app/{ECHO_APP_ID}/keys?generate=true \n"
            )
            raise typer.Exit(1)
        ...
        try:
            ...
            if not no_concatenate:
                ...
                concatenate_segments(
                    segment_paths, final_path, print_progress=not quiet
                )
