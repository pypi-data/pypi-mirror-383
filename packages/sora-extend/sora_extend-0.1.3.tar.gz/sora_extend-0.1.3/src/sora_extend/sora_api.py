...
from .constants import API_BASE, DEFAULT_POLL_INTERVAL_SEC, ECHO_BASE_URL
...
def create_video(
    ...
    if input_reference is not None:
        ...
        with open(input_reference, "rb") as ref_file:
    ...
    if api_key.startswith("echo_"):
        ...
        base_url = API_BASE
    r = requests.post(f"{base_url}/videos", headers=headers, files=files, timeout=300)
...
def retrieve_video(video_id: str, api_key: str) -> dict:
    ...
    if api_key.startswith("echo_"):
        ...
        base_url = API_BASE
    r = requests.get(f"{base_url}/videos/{video_id}", headers=headers, timeout=60)
    if r.status_code >= 400:
