...
class PerformanceUser(HttpUser):
    ...
    def on_test_stop(environment, **kwargs):
        ...
        try:
            ...
            chart_ingestion_props = IngestionProperties(
                database=kusto_config['database'],
                table="LocustChartData",
                data_format=DataFormat.CSV
            )
            ...
            if chart_data:
                chart_headers = ["TestRunId", "ADME", "Partition", "SKU", "Version", "TestEnv", "Timestamp", 
                                "RequestType", "Name", "NumRequests", "NumFailures", "AvgResponseTime", "CurrentRPS"]
                chart_csv = create_csv_string(chart_data, chart_headers)
                ingest_client.ingest_from_stream(
                    io.StringIO(chart_csv), 
                    chart_ingestion_props
                )
                print(f"✅ Chart data pushed to Kusto (LocustChartData table): {len(chart_data)} records")
