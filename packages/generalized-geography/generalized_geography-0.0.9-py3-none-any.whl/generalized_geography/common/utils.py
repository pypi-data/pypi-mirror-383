...
def split_nodes_by_resolved_type(type_map: dict) -> tuple[List[BipartiteNode], List[BipartiteNode]]:
    lose_nodes = [node for node, ntype in type_map.items() if resolve_node_type(ntype) == LOSE]
    win_nodes = [node for node, ntype in type_map.items() if resolve_node_type(ntype) == WIN]
    
    return lose_nodes, win_nodes



def nodes_in_pos(nodes: List[BipartiteNode], pos: 0 | 1) -> List[BipartiteNode]:
