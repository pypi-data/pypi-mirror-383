...
def deg_is_critical_edge(graph: DiGraph, u: Hashable, v: Hashable, weight_key: str = "num") -> bool:
    ...
    if another is None:
        return False
    if out_degree == 2 and graph[u][another][weight_key] == 1 and u == another:
        return True
    return False


def deg_get_critical_edges(graph: DiGraph, weight_key: str = "num") -> List[Tuple[Hashable, Hashable]]:
    critical_edges = []
    for u in graph.nodes:
        if graph.out_degree(u) > 2:
            continue
        for v in graph.successors(u):
            if deg_is_critical_edge(graph, u, v, weight_key=weight_key):
