...
class GetDataClient(DataTransferClient):
    """СМЭВ клиент сервиса DataTransfer (метод GetData)"""
    ...
    def GetData(self, session_id, encoded_file_name, filename):
        method = 'GetData'
        ...
        request.MessageData.AppData.GetDataRequest.AuthorizationKey = (
            dt_settings.DATATRANSFER_AUTHORIZATION_KEY)
        request.MessageData.AppData.GetDataRequest.SessionID = session_id
        ...
        error = None
        try:
            self.service.GetData(request.Message, request.MessageData)
