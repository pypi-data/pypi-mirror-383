...
def find_files(target_path_str: str, recursive: bool = False) -> List[Path]:
    """
    ...
    Args:
        ...
        recursive: If True and target_path_str is a directory, searches recursively
                  through all subdirectories. Default is False.
    ...
    if target_path.is_file():
        ...
        logging.info(f"Scanning directory {search_type}: {target_path_str} for supported files...")

        for ext in SUPPORTED_EXTENSIONS:
            # Choose glob pattern based on recursive flag
            pattern = f"**/*{ext}" if recursive else f"*{ext}"
            found_files.extend(target_path.glob(pattern))
            if ext.lower() != ext.upper():  # e.g. for .tif vs .TIF
                upper_pattern = f"**/*{ext.upper()}" if recursive else f"*{ext.upper()}"
                found_files.extend(target_path.glob(upper_pattern))
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        "-r",
        action="store_true",
        help="When processing a directory, search recursively through all subdirectories.",
    )
    ...
    files_to_process = find_files(args.input_path, recursive=args.recursive)
