...
- **ALL tests MUST go in existing `tests/frontend/` directory structure**
...
- **Unit tests**: `tests/frontend/unit/[component].test.tsx` or `tests/frontend/unit/[component].spec.ts`
- **E2E tests**: `tests/frontend/e2e/[flow-name].e2e.test.ts` or `tests/frontend/e2e/[flow-name].spec.ts`
- **Visual tests**: `tests/frontend/visual/[component].visual.test.ts` (if visual directory exists)
- **Accessibility tests**: `tests/frontend/accessibility/[page].a11y.spec.ts` (if accessibility directory exists)

**FORBIDDEN Actions:**
- ❌ Creating `__tests__/` directories
- ❌ Creating `components/` in root or tests/
- ❌ Creating `e2e/` in root or tests/
- ❌ Creating duplicate directories that already exist
- ❌ Placing test files in root directory
- ❌ Creating framework-specific directories outside tests/frontend/
...
test_files = [f for f in root_files if 'test' in f.lower() and (f.endswith('.ts') or f.endswith('.js') or f.endswith('.spec.ts'))]
if test_files:
    print('❌ STOP: Found test files in root:', test_files)
    print('   Move to tests/frontend/unit/ or tests/frontend/e2e/')
    sys.exit(1)
...
- **ALWAYS check existing tests/frontend/ directory structure first**
- **ONLY place files in existing subdirectories of tests/frontend/**
