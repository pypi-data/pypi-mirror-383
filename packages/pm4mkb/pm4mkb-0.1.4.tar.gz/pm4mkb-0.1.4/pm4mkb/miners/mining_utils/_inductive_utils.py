...
from pm4mkb.visual import create_dfg
...
def _get_connected_components(graph, connection):
    ...
    matrix = zeros((len(node2ind), len(node2ind)))

    for n1, n2 in graph.edges.keys():
        matrix[node2ind[n1], node2ind[n2]] = 1
    matrix = csr_matrix(matrix)

    comp_number, comps = connected_components(
        csgraph=matrix, directed=True, connection=connection, return_labels=True
    )
...
def cut_graph(graph, start_nodes, end_nodes, node_groups):
    ...
    for i, node_group in enumerate(node_groups):
        snodes = {node for node in start_nodes if node in node_group}
        enodes = {node for node in end_nodes if node in node_group}
...
def create_inverted_graph(graph):
    ...
    for i in range(len(nodes)):
        ...
        for j in range(i + 1, len(nodes)):
            n2 = nodes[j]
            if (n1, n2) not in graph.edges or (n2, n1) not in graph.edges:
...
def check_each_node_group_has_start_end_nodes(node_groups, start_nodes, end_nodes):
    ...
    """
    return not any(
        len(start_nodes.intersection(group)) == 0 or (end_nodes.intersection(group)) == 0 for group in node_groups
    )
...
def get_graph_with_grouped_nodes(graph, start_nodes, end_nodes, node_groups):
    ...
    new_start_nodes = {node2group[node] for node in start_nodes}
    new_end_nodes = {node2group[node] for node in end_nodes}
