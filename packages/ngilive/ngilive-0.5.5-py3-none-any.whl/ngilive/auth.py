"""
Different utility classes to get access tokens.

With these classes you can customize how `NGILive`
will authenticate the call. You can choose between multiple
strategies, including `AuthorizationCode` and `ClientCredentials`.

The default strategy is `AuthorizationCode`.

To modify this, you can for example do...
...
# Now you don't have to think about auth no more!
```
"""
...
class Auth(ABC):
    ...
    def get_token(self) -> str:
        """
        All Auth strategies can use the get_token method to obtain a token.

        This is useful if you want to control the HTTP client yourself,
        for example using `requests` or `httpx` libraries.
        ...
        response = httpx.get(
            "http://api.ngilive.no/projects/20190539/sensors",
...
class AuthorizationCode(Auth):
    ...
    def get_token(self) -> str:
        """
        Example: get a token using Authorization Code Flow
        ```python
        from ngilive.auth import AuthorizationCode

        auth = AuthorizationCode()

        ### Your browser will open here! Sign in! ###
        access_token = auth.get_token()
        ```
        """
...
class ClientCredentials(Auth):
    def __init__(
        ...
        """
        With Client Credentials, you can get a token using a
        provided `client_id` and `client_secret`. This is useful
        if you are going to call the API automatically from
        a server, where you cannot open a browser to sign in.

        Contact NGI if you need to get a client configured for
        this purpose.
        """
        self._client_id = client_id
    ...
    def get_token(self) -> str:
        """
        Example: get a token using Client Credentials
...
__all__ = ["Auth", "AuthorizationCode", "ClientCredentials"]
