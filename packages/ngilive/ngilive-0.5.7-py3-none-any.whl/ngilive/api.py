...
class NGILive:
    ...
    def event_file(self, event_id: str, file_name: str) -> BytesIO:
        ...
        **Example:**
        ...
        Get the file as a `BytesIO` file-like object and saves it to disk using `os`.

        ```python
        filename = "known_filename.parquet"

        response_bytestream: BytesIO = nl.event_file("<event_id>", filename)

        with open(filename, "wb") as f:
            f.write(response_bytestream.read())
        ```
        """
        res = self._httpx.get(
