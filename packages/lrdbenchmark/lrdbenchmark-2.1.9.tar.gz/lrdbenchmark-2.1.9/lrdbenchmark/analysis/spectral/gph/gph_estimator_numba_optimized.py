#!/usr/bin/env python3
"""
NUMBA-Optimized GPH Estimator for LRDBench

This module provides a NUMBA-optimized version of the GPH estimator
...
@jit(nopython=True, parallel=True, cache=True)
def _numba_calculate_gph_statistics(log_frequencies, log_periodogram, weights):
    """
    NUMBA-optimized GPH statistics calculation.
    ...
    sum_wx = np.sum(weights * log_frequencies)
    sum_wy = np.sum(weights * log_periodogram)
    sum_wxy = np.sum(weights * log_frequencies * log_periodogram)
    sum_wx2 = np.sum(weights * log_frequencies**2)
    ...
    ss_res = np.sum(weights * (log_periodogram - y_pred)**2)
    ss_tot = np.sum(weights * (log_periodogram - np.mean(log_periodogram))**2)
...
class NumbaOptimizedGPHEstimator(BaseEstimator):
    """
    NUMBA-Optimized Geweke-Porter-Hudak (GPH) Estimator for analyzing long-range dependence.
    ...
    def estimate(self, data: np.ndarray) -> Dict[str, Any]:
        """
        Estimate Hurst parameter using NUMBA-optimized GPH method.
        ...
        if len(valid_periodogram) < 10:
            raise ValueError("Insufficient valid data points for GPH analysis")
        
        # Calculate log values
        log_frequencies = np.log(valid_frequencies)
        log_periodogram = np.log(valid_periodogram)
        
        # Calculate weights (Bartlett window)
        weights = np.ones(len(log_frequencies))
        
        # Use NUMBA-optimized regression if available
        if NUMBA_AVAILABLE:
            slope, intercept, r_squared = _numba_calculate_gph_statistics(
                log_frequencies, log_periodogram, weights
            )
        else:
...
def benchmark_gph_performance():
    """Benchmark the performance difference between original and NUMBA-optimized GPH."""
    import time
    ...
    print("🚀 GPH NUMBA Optimization Benchmark")
    print("=" * 50)
    ...
    for size in data_sizes:
        ...
        try:
            ...
            original_gph = GPHEstimator()
            
            start_time = time.time()
            result_orig = original_gph.estimate(data)
            time_orig = time.time() - start_time
            
            print(f"Original GPH: {time_orig:.4f}s")
        except Exception as e:
            print(f"Original GPH: Failed - {e}")
            time_orig = None
        
        # Test NUMBA-optimized GPH
        try:
            numba_gph = NumbaOptimizedGPHEstimator()
            
            start_time = time.time()
            result_numba = numba_gph.estimate(data)
            time_numba = time.time() - start_time
            
            print(f"NUMBA-Optimized GPH: {time_numba:.4f}s")
            ...
            print(f"NUMBA-Optimized GPH: Failed - {e}")


if __name__ == "__main__":
    benchmark_gph_performance()
