...
This module provides a NUMBA-optimized version of the Wavelet Log Variance estimator for significantly
improved performance in wavelet log variance calculations.
"""
...
def _numba_calculate_wavelet_log_variance(coeffs: List[np.ndarray]) -> np.ndarray:
    """
    NUMBA-optimized calculation of wavelet log variance at each scale.
    
    Args:
        coeffs: List of wavelet coefficients
        
    Returns:
        Array of wavelet log variances
    """
    n_scales = len(coeffs) - 1  # Exclude the approximation coefficients
    log_variances = np.empty(n_scales, dtype=np.float64)
    ...
    for i in prange(n_scales):
        ...
        if variance > 0:
            log_variances[i] = np.log(variance)
        else:
            log_variances[i] = np.nan
...
class NumbaOptimizedWaveletLogVarianceEstimator(BaseEstimator):
    """
    NUMBA-Optimized Wavelet Log Variance Analysis estimator.

    This estimator uses NUMBA-optimized wavelet log variance analysis to estimate the Hurst parameter
    ...
    def estimate(self, data: np.ndarray) -> Dict[str, Any]:
        ...
        if len(data) < 100:
            raise ValueError("Data length must be at least 100 for wavelet log variance analysis")
        ...
        # NUMBA-optimized log variance calculation
        log_variance_array = _numba_calculate_wavelet_log_variance(self.wavelet_coeffs)

        # Create scales array (powers of 2)
        self.scales = np.array([2**i for i in range(1, len(log_variance_array) + 1)])

        # Store wavelet log variances
        for i, scale in enumerate(self.scales):
            self.wavelet_log_variances[scale] = log_variance_array[i]

        # Prepare data for linear regression (log scales vs log variances)
        log_scales = np.log10(self.scales)
        
        # Remove NaN values from log variances
        valid_indices = ~np.isnan(log_variance_array)
        if np.sum(valid_indices) < 2:
            raise ValueError("Insufficient valid log variance values for analysis")
        
        log_scales_valid = log_scales[valid_indices]
        log_variances_valid = log_variance_array[valid_indices]

        # NUMBA-optimized linear regression
        slope, intercept, r_squared = _numba_linear_regression(log_scales_valid, log_variances_valid)
        ...
        self.confidence_interval = self._calculate_confidence_interval(
            log_scales_valid, log_variances_valid, slope, intercept
        )
        ...
        return {
            ...
            'slope': slope,
            'intercept': intercept,
            'wavelet_log_variances': self.wavelet_log_variances,
            'scales': self.scales,
            'wavelet_coeffs': self.wavelet_coeffs,
            'level': self.level,
            'optimization_level': self.optimization_level,
            'execution_time': self.execution_time,
            'method': 'Wavelet Log Variance (NUMBA-Optimized)'
        }
    ...
    def plot_scaling_behavior(self, save_path: Optional[str] = None) -> None:
        ...
        try:
            ...
            # Plot 1: Log-log plot of scale vs log variance
            scales = list(self.wavelet_log_variances.keys())
            log_variances = list(self.wavelet_log_variances.values())
            
            # Filter out NaN values for plotting
            valid_scales = []
            valid_log_variances = []
            for scale, log_var in zip(scales, log_variances):
                if not np.isnan(log_var):
                    valid_scales.append(scale)
                    valid_log_variances.append(log_var)
            
            if valid_scales:
                ax1.loglog(valid_scales, np.exp(valid_log_variances), 'bo-', label='Data')
                ax1.set_xlabel('Scale')
                ax1.set_ylabel('Variance')
                ax1.set_title('Wavelet Log Variance Scaling Behavior')
                ax1.grid(True, alpha=0.3)
                ax1.legend()
                
                # Add regression line
                log_scales_valid = np.log10(valid_scales)
                slope, intercept, _ = _numba_linear_regression(log_scales_valid, valid_log_variances)
                y_pred = slope * log_scales_valid + intercept
                ax1.loglog(valid_scales, np.exp(y_pred), 'r--', 
