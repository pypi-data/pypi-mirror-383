...
class ToolResponse(Response):
    ...
    def model_post_init(self, __context: Any) -> None:
        ...
        if self.metadata:
            self.metadata = self._additional_validate(self.metadata)

    def _additional_validate(self, obj):
        if isinstance(obj, dict):
            try:
                a = Ref.model_validate(obj)
                a.value = self._additional_validate(a.value)
                return a
            except ValidationError:
                return {
                    k: self._additional_validate(v)
                    for k, v in obj.items()
                }
        elif isinstance(obj, list):
            return [
                self._additional_validate(i)
                for i in obj
            ]
        else:
            return obj

    def get_refs(self) -> Dict[str, Ref]:
        results = {}
        self._get_refs(self.metadata, "", results)
        return results

    def _get_refs(self, obj, path: str, results: Dict[str, Ref]):
        if isinstance(obj, Ref):
            results[path] = obj
            self._get_refs(obj.value, path, results)
        elif isinstance(obj, dict):
            for k, v in obj.items():
                self._get_refs(v, f"{path}/{k}", results)
        elif isinstance(obj, list):
            for i, v in enumerate(obj, start=1):
                self._get_refs(v, f"{path}/{i}", results)

    def render_metadata(self, start_level: int = 0) -> str:
        refs = self.get_refs()
        buffer = io.StringIO()
        for path, ref in refs.items():
            ...
            if ref.readable:
                buffer.write(f"```{ref.format}\n")
                buffer.write(ref.value)
                buffer.write("\n```")
                buffer.write("\n")
            buffer.write("\n")
