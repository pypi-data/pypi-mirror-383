...
client = SolviceVrpSolver(
    api_key=os.environ.get("SOLVICE_API_KEY"),  # This is the default and can be omitted
)

on_route_response = client.vrp.sync_solve(
    jobs=[{"name": "Job-1"}],
    resources=[
        {
            "name": "vehicle-1",
...
print(on_route_response.id)
```

While you can provide an `api_key` keyword argument,
we recommend using [python-dotenv](https://pypi.org/project/python-dotenv/)
to add `SOLVICE_API_KEY="My API Key"` to your `.env` file
...
async def main() -> None:
    on_route_response = await client.vrp.sync_solve(
        jobs=[{"name": "Job-1"}],
        resources=[
            {
                "name": "vehicle-1",
...
print(on_route_response.custom_distance_matrices)
```
