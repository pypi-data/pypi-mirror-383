...
class LDATrainer:
    ...
    def _prepare_gensim_data_from_matrix(self):
        ...
        # Create Gensim dictionary from genus IDs (column names)
        genus_names = list(self.sampletable_genusid.columns)
        self.gensim_dictionary = Dictionary([genus_names])
        
        # Create corpus directly from the matrix
        self.gensim_corpus = []
        for idx, row in self.sampletable_genusid.iterrows():
            doc_bow = []
            for genus_id, count in row.items():
                if count > 0:
                    word_id = self.gensim_dictionary.token2id[genus_id]
                    doc_bow.append((word_id, int(count)))
