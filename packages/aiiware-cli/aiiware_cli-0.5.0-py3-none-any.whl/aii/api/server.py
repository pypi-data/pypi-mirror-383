...
async def websocket_execute(websocket: WebSocket):
    ...
    try:
        ...
        # Execute function via execution engine (same as REST API)
        result = await server.engine.execution_engine.execute_function(
            recognition_result=recognition_result,
        ...
        # Extract clean_output from result data
        content = ""
        if result.success and result.data and isinstance(result.data, dict):
            content = result.data.get('clean_output', '')

        # Fallback to message if clean_output not found
        if not content:
            content = result.message

        # Send as a single token (simulating streaming for now)
        if content:
            await websocket.send_json({
                "type": "token",
                "data": content
