...
class Repository:
    ...
    def __init__(  # noqa: PLR0913, PLR0917
        ...
        if cache_file_outdated(self._cache_content_file):
            obj = None
            content_file_url = HttpUrl(posixpath.join(str(self.url), "dists", self.dist, f"Contents-{arch}"))

            if obj := http.download_compressed(content_file_url, self.auth, self.verify):
                content_file_data = obj
            else:
                content_file_url = HttpUrl(posixpath.join(str(self.url), f"Contents-{arch}"))
                content_file_data = (
