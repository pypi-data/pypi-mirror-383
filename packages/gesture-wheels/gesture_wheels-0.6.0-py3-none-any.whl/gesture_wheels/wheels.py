...
def detect_fingers():
    ...
    cv2.rectangle(frame, box_start, box_end, (255, 255, 255), -1)
    cv2.rectangle(frame, box_start, box_end, color, -1)

    # Centered Text (e.g. "1 : FORWARD")
    cmd_text = f"{count} : {action}" if action != "NONE" else " "
...
def send_command(robot, action):
    """Send a single-character command via Serial."""
    if not robot:
