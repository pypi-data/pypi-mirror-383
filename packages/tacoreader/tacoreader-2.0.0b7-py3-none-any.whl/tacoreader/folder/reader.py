...
async def detect_max_level_remote(store: ObjectStore, prefix: str = "") -> int:
    """
    Detect maximum level in remote FOLDER by probing files.
...
async def download_folder_bytes(path: str) -> tuple[bytes, list[bytes]]:
    """
    Download all bytes from remote FOLDER (COLLECTION.json + all Avro levels).
    ...
    Args:
        path: Remote FOLDER URL (s3://, gs://, etc.)

    Returns:
        Tuple of (collection_bytes, [level0_bytes, level1_bytes, ...])

    Examples:
        >>> collection, levels = await download_folder_bytes("s3://bucket/dataset/")
    ...
    # Detect max level
    max_level = await detect_max_level_remote(store)

    # Download all Avro files concurrently
    async def download_level(level: int) -> bytes:
        result = await obs.get_async(store, f"METADATA/level{level}.avro")
        avro_bytes = await result.bytes_async()
        return bytes(avro_bytes)

    # Use asyncio.gather for concurrent downloads
    avro_bytes_list = await asyncio.gather(
        *[download_level(lvl) for lvl in range(max_level + 1)]
    )

    return bytes(collection_bytes), list(avro_bytes_list)


def parse_avro_from_bytes(avro_bytes: bytes) -> pl.DataFrame:
    """
    Parse Avro bytes into Polars DataFrame.

    PHASE 2 worker function: Processes bytes WITHOUT using obstore.
    Safe to run in forked processes.

    Args:
        avro_bytes: Raw Avro file bytes
    ...
    Examples:
        ...
        >>> df = parse_avro_from_bytes(avro_bytes)
        >>> len(df)
        1000

    Notes:
        - Does NOT use obstore (only polars)
        - Safe to run in ProcessPoolExecutor workers
        - Can be used with fork() start method
    """
    # Polars can read Avro from bytes directly
    return pl.read_avro(io.BytesIO(avro_bytes))
