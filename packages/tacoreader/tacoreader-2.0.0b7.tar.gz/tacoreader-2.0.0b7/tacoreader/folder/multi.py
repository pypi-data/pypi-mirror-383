...
from tacoreader.folder.reader import (
    download_folder_bytes,
    parse_avro_from_bytes,
...
def load_multiple_folders(
    ...
    """
    Load multiple FOLDER datasets and validate compatibility (SEQUENTIAL).
...
def _worker_load_single_folder_local(path: str) -> tuple[dict, list[pl.DataFrame]]:
    """
    Worker function: Load single local FOLDER.
    ...
    # Read all levels (no obstore)
    dataframes = read_all_levels(path)

    return collection, dataframes


def _worker_process_folder_bytes(
    data: tuple[bytes, list[bytes]],
) -> tuple[dict, list[pl.DataFrame]]:
    """
    Worker function: Process downloaded FOLDER bytes.

    This runs in a worker process and does NOT use obstore.
    Safe to use with fork().

    Args:
        data: Tuple of (collection_bytes, [avro_bytes, ...])

    Returns:
        Tuple of (collection dict, list of DataFrames)
    """
    collection_bytes, avro_bytes_list = data

    # Parse COLLECTION.json
    collection = parse_collection_from_bytes(collection_bytes)

    # Parse all Avro levels
    dataframes = [parse_avro_from_bytes(ab) for ab in avro_bytes_list]

    return collection, dataframes


def _load_multiple_folders_local_parallel(
    paths: list[str],
    safe_mode: bool,
    n_workers: int,
    batch_size: int,
    ...
    """
    Load multiple local FOLDERs in parallel using ProcessPoolExecutor.

    Single phase: workers read local files directly (no obstore).

    Args:
        paths: List of local FOLDER paths
    ...
    results = process_in_batches(
        normalized_paths,
        worker_fn=_worker_load_single_folder_local,
        n_workers=n_workers,
...
async def _download_all_folders_async(
    paths: list[str],
) -> list[tuple[bytes, list[bytes]]]:
    """
    Download all FOLDER bytes concurrently using asyncio.

    PHASE 1 for remote: Downloads all folders using obstore/tokio.
    Runs in main process only.

    Args:
        paths: List of remote FOLDER URLs

    Returns:
        List of (collection_bytes, avro_bytes_list) tuples
    """
    # Download all FOLDERs concurrently
    download_tasks = [download_folder_bytes(path) for path in paths]
    all_bytes = await asyncio.gather(*download_tasks)
    return list(all_bytes)


def _load_multiple_folders_remote_parallel(
    paths: list[str],
    safe_mode: bool,
    n_workers: int,
    batch_size: int,
    ...
    """
    Load multiple remote FOLDERs in parallel using two-phase approach.
    ...
    for batch in batches:
        ...
        batch_bytes = asyncio.run(_download_all_folders_async(batch))
        all_downloaded_bytes.extend(batch_bytes)

    # PHASE 2: Process bytes in parallel (workers without tokio)
    results = process_in_batches(
        all_downloaded_bytes,
        worker_fn=_worker_process_folder_bytes,
        n_workers=n_workers,
        batch_size=batch_size,
    )
...
def load_multiple_folders_parallel(
    paths: list[str],
    safe_mode: bool = False,
    ...
    """
    Load multiple FOLDER datasets with optional parallel processing.

    Supports both local and remote folders with automatic detection.
    ...
    Args:
        paths: List of FOLDER paths (local or remote, can include pathlib.Path)
        safe_mode: If True, only load common columns
        n_workers: Number of parallel workers (None = sequential)
        batch_size: Number of folders to process per batch
    ...
    Examples:
        >>> # Sequential (default)
        >>> schema, dfs = load_multiple_folders_parallel(paths)
        
        >>> # Parallel with 8 workers
        >>> schema, dfs = load_multiple_folders_parallel(paths, n_workers=8)
        
        >>> # Parallel with custom batch size
        >>> schema, dfs = load_multiple_folders_parallel(
    ...
    if optimal_workers == 1:
        return load_multiple_folders(paths, safe_mode)
    ...
    if first_is_local:
        return _load_multiple_folders_local_parallel(
            paths, safe_mode, optimal_workers, batch_size
        )
    else:
        return _load_multiple_folders_remote_parallel(
            paths, safe_mode, optimal_workers, batch_size
        )