...
class MetaOPT(Model):
    ...
    def _initialize_model(self):
        ...
        The transformer architecture uses the following config parameters:
        - vocab_size: Size of the vocabulary
        - hidden_size: Dimension of the model's hidden layers
        - num_attention_heads: Number of attention heads
        - intermediate_size: Size of the feedforward layer (defaults to 4x hidden_size)
        ...
        self._model = torch.nn.ModuleDict({
            ...
            "transformer": torch.nn.TransformerEncoder(
                torch.nn.TransformerEncoderLayer(
                    ...
                    dim_feedforward=self.config.get("intermediate_size", self.config["hidden_size"] * 4),  # Default to 4x hidden size if not specified
                    dropout=self.config.get("hidden_dropout_prob", 0.1),  # Default dropout probability of 0.1
                    activation="gelu",
                    batch_first=True
                ),
