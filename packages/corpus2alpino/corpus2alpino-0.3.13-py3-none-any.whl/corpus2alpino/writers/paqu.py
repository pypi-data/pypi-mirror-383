...
class PaQuWriter(Writer):
    ...
    def output_utterances(self, utterances: Iterable[Utterance], doc_metadata):
        ...
        for utterance in utterances:
            ...
            for key in prev_metadata:
                if key not in utterance.metadata:
                    metadata[key] = MetadataValue("")

            metadata = {**metadata, **doc_metadata, **utterance.metadata}
            metadata_display = (
                "\n".join(self.output_metadata_items(metadata, prev_metadata)) + "\n"
                if metadata
                else ""
            )
            yield "{0}{1}|{2}\n\n".format(
                metadata_display, utterance.id, utterance.text
            )
    ...
    def output_metadata_items(self, metadata, prev_metadata=None):
        for key, item in sorted(metadata.items()):
            if (
                prev_metadata == None
                or key not in prev_metadata
                or prev_metadata[key].value != item.value
            ):
