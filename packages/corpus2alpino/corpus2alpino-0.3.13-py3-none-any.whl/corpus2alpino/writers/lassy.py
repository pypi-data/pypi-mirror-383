...
class LassyWriter(Writer):
    ...
    def start_merged(self, target: Target) -> None:
        ...
        if self.merge_treebanks:
            target.write_merged('<?xml version="1.0" encoding="UTF-8"?>\n<treebank>')
    ...
    def write(self, document: Document, target: Target):
        if self.merge_treebanks:
            ...
            for utterance in document.utterances:
                self.write_utterance(
                    document, target, utterance, "{0}.xml".format(index)
                )
                index += 1

    def write_utterance(
        self, document: Document, target: Target, utterance: Utterance, filename=None
    ):
        try:
            annotation = utterance.annotations[ANNOTATION_KEY]
        except KeyError:
            logging.getLogger().warning(
                "Annotation missing for: {0}|{1} ({2})".format(
                    utterance.id, utterance.text, document.subpath
                )
            )
            annotation = f'<alpino_ds version="1.6" id="{self.escape_xml_attribute(document.subpath)}">\n<sentence sentid="{self.escape_xml_attribute(utterance.id)}">{escape(utterance.text).strip()}</sentence>\n</alpino_ds>'

        target.write(
            document,
            self.render_annotation(document, utterance, annotation, not filename),
            filename,
        )
    ...
    def render_annotation(
        ...
        if metadata:
            lines.insert(
                existing_metadata,
                self.render_metadata(metadata, existing_metadata == -1),
            )
    ...
    def render_metadata(self, metadata: Dict[str, MetadataValue], wrap: bool) -> str:
        return (
            ("<metadata>\n" if wrap else "")
            + "\n".join(
                self.render_metadata_value(key, item)
    ...
    def render_metadata_value(self, key: str, item: MetadataValue) -> str:
        return '<meta type="{0}" name="{1}" value="{2}" />'.format(
            item.type, key, self.escape_xml_attribute(item.value)
        )
