...
class SecCompany:
    ...
    def __get_xml_file(self):
        ...
        file_url = files["Base File"] + "/" + [item["Name"] for item in files["Sub File"] if "Name" in item and "_htm.xml" in item["Last Modified"]][0]
        file_date = [item["Name"] for item in files["Sub File"] if "Name" in item and "_htm.xml" in item["Name"]][0]
