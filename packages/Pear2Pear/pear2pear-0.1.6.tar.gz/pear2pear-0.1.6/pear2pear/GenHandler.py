...
def load_dataset(filename: str) -> pd.DataFrame:
    """
    Load a CSV dataset included in the package.

    Parameters
    ----------
    filename : str
        Name of the CSV file inside the package.
    ...
    pd.DataFrame
        Loaded CSV data
    """
    with pkg_resources.open_text("Pear2Pear", filename) as f:
...
def train_process(
    ...
    while True:
        ...
        if oldBestFit == bestFit:
            ...
            TESTFILEPATH = "dataset_new_1h"
            dataset_filename = f"{TESTFILEPATH}.csv"
            testDataResult = load_dataset(dataset_filename)
...
def genetic_algorithm(
    ...
    financial_data = load_dataset(dataset_filename)
