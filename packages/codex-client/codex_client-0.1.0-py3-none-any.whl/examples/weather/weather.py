#!/usr/bin/env python3
"""
Weather data fetching helpers built on top of the wttr.in API.
"""
    ...
    async def get_current_weather(location: str = "") -> Dict[str, Any]:
        ...
        try:
            async with httpx.AsyncClient(
                ...
                headers={"User-Agent": "Codex-Client-Weather-Demo/1.0"},
            ) as client:
            ...
            return {
                ...
                "success": True,
                "timestamp": current.get("observation_time", "unknown"),
            }

        except httpx.TimeoutException:
            return {"error": "Weather service request timed out", "location": location, "success": False}
        except httpx.HTTPStatusError as exc:
            return {
                "error": f"Weather service returned status {exc.response.status_code}",
                "location": location,
                "success": False,
            }
        except httpx.RequestError as exc:
            return {"error": f"Network error: {exc}", "location": location, "success": False}
        except json.JSONDecodeError:
            return {"error": "Invalid response from weather service", "location": location, "success": False}
        except Exception as exc:  # noqa: BLE001
            return {"error": f"Unexpected error: {exc}", "location": location, "success": False}
        ...
        try:
            ...
            for day_data in data.get("weather", [])[:days]:
                day_forecast = {
                    ...
                    "condition": day_data.get("hourly", [{}])[0]
                    .get("weatherDesc", [{"value": "Unknown"}])[0]["value"],
                ...
                for hour_data in day_data.get("hourly", []):
                    ...
                    day_forecast["hourly_forecast"].append(
                        {
                            "time": formatted_time,
            ...
            return {"error": f"Error getting forecast: {exc}", "location": location, "success": False}
        except Exception as exc:  # noqa: BLE001
            return {"error": f"Unexpected error: {exc}", "location": location, "success": False}

    @staticmethod
    async def get_weather_summary(location: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            return {"location": location or "Current Location", "summary": summary, "success": True}

        except (httpx.TimeoutException, httpx.HTTPStatusError, httpx.RequestError) as exc:
            return {"error": f"Error getting weather summary: {exc}", "location": location, "success": False}
        except Exception as exc:  # noqa: BLE001
            return {"error": f"Unexpected error: {exc}", "location": location, "success": False}
