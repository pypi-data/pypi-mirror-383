...
class HostsConfig(BaseModel):
    ...
    def to_docker_hosts_format(self) -> List[Tuple[str, str, int]]:
        ...
        for alias, host_config in self.get_enabled_hosts().items():
            # Create SSH alias for the host (alias is the hostname)
            ssh_alias = host_config.get_ssh_alias(alias)
            hosts.append((ssh_alias, alias, host_config.port))
        return hosts

    def get_host_by_hostname(self, hostname: str) -> Optional[Tuple[str, HostConfig]]:
        """Find host by hostname (which is the dict key/alias).

        Returns tuple of (alias, HostConfig) or None if not found.
        """
        if hostname in self.hosts:
            return (hostname, self.get_host_config(hostname))
        return None
...
# The dict key (e.g., 'prod') is used as the hostname for SSH connections
