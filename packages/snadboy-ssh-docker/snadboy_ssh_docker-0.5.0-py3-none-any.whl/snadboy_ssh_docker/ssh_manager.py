...
class SSHManager:
    ...
    def get_ssh_alias(self, host_alias: str) -> str:
        ...
        Args:
            host_alias: Host alias from configuration (used as hostname)
        ...
        return host.get_ssh_alias(host_alias)
    ...
    def test_connections(self) -> List[Dict[str, Any]]:
        ...
        for alias, host_config in enabled_hosts.items():
            ...
            print(f"Testing connection to {alias}:{host_config.port}")
            ...
            try:
                ...
                if success:
                    print(f"✓ Successfully connected to {alias} (Tailscale)")
                else:
                    print(
                        f"✗ Failed to connect to {alias}: {result.stderr}"
                    )
                ...
                print(f"✗ Connection timeout to {alias}")
