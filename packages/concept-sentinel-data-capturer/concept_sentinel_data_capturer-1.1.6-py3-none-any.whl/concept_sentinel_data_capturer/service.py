...
class Prompt:
    ...
    def generate_agent_context_prompt(agent_name, agent_description) -> str:
        template=f"""
            You are an assistant that enumerates all distinct, relevant TASKS (called contexts) an agent can perform.
...
async def create_context(prompt: str, response: str, prompt_context: str, response_context: str,
                    ...
                    accuracy: str = None, intent_satisfied: str = None, hallucination: str = None,
...
async def insertion_with_context(payload: ContextRequest):
    ...
    try:
        ...
        if agent_flag:
            ...
            # bias_result = await ResponsibleAIExplain.bias_prompt(
            #     prompt=input_prompt,
            #     response=llm_response,
            #     agent_metadata=payload.agent_metadata,
            #     agent_name=agent_name
            # )
            # bias_json = ResponsibleAIExplain.llm_response_to_json(bias_result.replace("\n", " "))
