...
class SunSwapService:
                    ...
                    for attempt in range(max_attempts):
                        ...
                        if tx_info:
                            ...
                            if actual_amount_out is not None:
                                ...
                                logger.debug(f"第 {attempt + 1} 次轮询未获取到实际兑换数量，继续等待...")
    ...
    async def purchase_sunpump_token(
        self, wallet_client: TronWalletBase, parameters: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        tx_hash = (
            tx_result.get("hash")
            or tx_result.get("txid")
            or tx_result.get("transaction_id")
            or tx_result.get("id")
        )
        status = tx_result.get("status", "unknown")
        if status is True:
        ...
        if tx_hash and tx_hash != "unknown" and status == "success":
            actual_amount_out = await self._get_sunpump_actual_amount(
                wallet_client, tx_hash, token_address, decimals
            )
    ...
    async def _get_sunpump_actual_amount(
        ...
        try:
            ...
            for attempt in range(max_attempts):
                ...
                if tx_info:
                    # 从日志中提取实际兑换数量
                    actual_amount_base = self._extract_swap_amount_from_logs(
                        tx_info, token_address, None
                    )
                    
                    if actual_amount_base is not None:
                        # 转换为 human-readable 格式
                        amount_human = str(Decimal(actual_amount_base) / (Decimal(10) ** decimals))
                        logger.info(f"成功获取 SunPump 实际兑换数量，轮询次数: {attempt + 1}")
                        return amount_human
                    else:
                        logger.debug(f"第 {attempt + 1} 次轮询未获取到实际兑换数量，继续等待...")
            ...
            logger.warning(f"获取 SunPump 实际兑换金额失败: {e}")
            return None
