...
class ArmMEClient:
    ...
    def _ensure_status(
        ...
        raise ArmMERequestError(
            f"{method.upper()} {path} -> {response.status_code}: {detail}"
        )
    ...
    def set_zone_bypass(
        ...
        *,
        ...
        """POST /api/AlarmSystem/ZoneBypass to toggle bypass state."""
        payload: dict[str, Any] = {
            "DeviceTypeId": int(device_type_id),
            "DeviceId": int(device_id),
            "PanelCode": panel_code,
        ...
        if accuracy is not None:
            payload["Accuracy"] = float(accuracy)
        data = self.post_json("/api/AlarmSystem/ZoneBypass", payload=payload)
        if not isinstance(data, dict):
            raise ArmMERequestError(f"Unexpected ZoneBypass payload: {data!r}")
        return OperationResult.from_json(data)
    ...
    def bypass_zone(
        ...
        *,
        ...
        """Convenience wrapper to bypass a single zone."""
        return self.set_zone_bypass(
            device_type_id,
            device_id,
            panel_code,
            partition=partition,
    ...
    def get_device_notification_subscriptions(
        self,
        device_id: int,
        mobile_device_id: str,
        *,
        fsk_device_id: int | None = None,
    ) -> DeviceNotificationSubscriptions:
        """POST /api/Push/GetDeviceNotificationSubscriptions for a device."""
        payload = {
            "DeviceId": int(device_id),
            "FSKDeviceId": int(fsk_device_id or device_id),
            "MobileDeviceId": mobile_device_id,
            "MobileClientId": self.client_id,
        }
        data = self.post_json(
            "/api/Push/GetDeviceNotificationSubscriptions", payload=payload
        )
        if not isinstance(data, dict):
            raise ArmMERequestError(
                f"Unexpected device notification subscriptions payload: {data!r}"
            )
        return DeviceNotificationSubscriptions.from_json(data)
    ...
    def set_com_output(
        ...
        *,
        ...
        """POST /api/AlarmSystem/ComOutputSet to control relays."""
        payload = {
            "DeviceTypeId": int(device_type_id),
            "DeviceId": int(device_id),
        ...
        data = self.post_json("/api/AlarmSystem/ComOutputSet", payload=payload)
        if not isinstance(data, dict):
            raise ArmMERequestError(f"Unexpected ComOutputSet payload: {data!r}")
        return OperationResult.from_json(data)

    def set_device_notification_subscriptions(
        self,
        device_id: int,
        mobile_device_id: str,
        *,
        g1_emergency: bool,
        g2_other: bool,
        g3_power_monitoring: bool,
        g4_device_state: bool,
        g5_health_check: bool,
        g6_zone_bypass: bool,
        partition_filter: Iterable[int] | None = None,
        source: str | None = None,
    ) -> None:
        """POST /api/Push/SetDeviceNotificationSubscriptions to update settings."""
        payload: dict[str, object] = {
            "MobileClientId": self.client_id,
            "MobileDeviceId": mobile_device_id,
            "DeviceId": int(device_id),
            "Source": source or self.client_id,
            "G1_Emergency": bool(g1_emergency),
            "G2_Other": bool(g2_other),
            "G3_PowerMonitoring": bool(g3_power_monitoring),
            "G4_DeviceState": bool(g4_device_state),
            "G5_HealthCheck": bool(g5_health_check),
            "G6_ZoneBypass": bool(g6_zone_bypass),
        }
        if partition_filter is not None:
            payload["PartitionFilter"] = [int(x) for x in partition_filter]
        response = self.post(
            "/api/Push/SetDeviceNotificationSubscriptions", json=payload
        )
        if response.status_code != 200:
            raise ArmMERequestError(
                "POST /api/Push/SetDeviceNotificationSubscriptions -> "
                f"{response.status_code}: {self._format_error_detail(response)}"
            )

    def get_account_subscriptions(self) -> AccountSubscriptions:
        """GET /api/Account/Subscriptions/list for the current user."""
        data = self.get_json("/api/Account/Subscriptions/list")
        if not isinstance(data, dict):
            raise ArmMERequestError(f"Unexpected account subscriptions payload: {data!r}")
        return AccountSubscriptions.from_json(data)
    ...
    def rename_zone(
        ...
        """POST /api/mydevices/config/rename/zone to rename a zone."""
        payload = {
            "DeviceId": int(device_id),
            "ZoneNumber": int(zone_number),
            "Name": name,
        }
        data = self.post_json("/api/mydevices/config/rename/zone", payload=payload)
        if not isinstance(data, dict):
            raise ArmMERequestError(f"Unexpected rename zone payload: {data!r}")
        return OperationResult.from_json(data)
    ...
    def unbypass_zone(
        ...
        *,
        ...
        """Convenience wrapper to restore a zone from bypass."""
        return self.set_zone_bypass(
            device_type_id,
            device_id,
            panel_code,
    ...
    def away_arm(
        ...
        *,
        ...
        """POST /api/AlarmSystem/AwayArm to arm the panel in away mode."""
        payload = {
            "DeviceTypeId": int(device_type_id),
            "DeviceId": int(device_id),
            "PanelCode": panel_code,
        ...
        data = self.post_json("/api/AlarmSystem/AwayArm", payload=payload)
        if not isinstance(data, dict):
            raise ArmMERequestError(f"Unexpected AwayArm payload: {data!r}")
        return OperationResult.from_json(data)
    ...
    def disarm(
        ...
        *,
        ...
        """POST /api/AlarmSystem/Disarm to disarm the panel."""
        payload = {
            "DeviceTypeId": int(device_type_id),
            "DeviceId": int(device_id),
            "PanelCode": panel_code,
        ...
        data = self.post_json("/api/AlarmSystem/Disarm", payload=payload)
        if not isinstance(data, dict):
            raise ArmMERequestError(f"Unexpected Disarm payload: {data!r}")
        return OperationResult.from_json(data)
    ...
    def stay_arm(
        ...
        *,
        ...
        """POST /api/AlarmSystem/StayArm to arm the panel in stay mode."""
        payload = {
            "DeviceTypeId": int(device_type_id),
            "DeviceId": int(device_id),
            "PanelCode": panel_code,
        ...
        data = self.post_json("/api/AlarmSystem/StayArm", payload=payload)
        if not isinstance(data, dict):
            raise ArmMERequestError(f"Unexpected StayArm payload: {data!r}")
        return OperationResult.from_json(data)
    ...
    def poll_loop(
        ...
        *,
        ...
        if isinstance(zones, list):
            for name, active, bypassed in self.map_zone_status_to_names(
                zones, zone_names
            ):
