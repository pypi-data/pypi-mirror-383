...
class DeviceSummary:
    ...
    def from_json(data: dict[str, Any]) -> DeviceSummary:
        ...
        return DeviceSummary(
            ...
            panel_type_id=(
                int(data["PanelTypeId"])
                if data.get("PanelTypeId") is not None
                else None
            ),
...
class ApiFailure:
    ...
    def from_json(data: dict[str, Any]) -> ApiFailure:
        ...
        return ApiFailure(
            succeeded=bool(data.get("Succeeded", False)),
            apn_error_code=(
                int(data["ApnErrorCode"])
                if data.get("ApnErrorCode") is not None
                else None
            ),
            custom_error_code=(
...
class AppVersionInfo:
    ...
    def from_json(data: dict[str, Any]) -> AppVersionInfo:
        ...
        return AppVersionInfo(
            ...
            terms_version=(
                str(data["TermsVersion"])
                if data.get("TermsVersion") is not None
                else None
            ),
            privacy_version=(
...
@dataclass(frozen=True)
class DeviceNotificationSubscriptions:
    """Push notification subscription settings for a device."""

    g1_emergency: int
    g2_other: int
    g3_power_monitoring: int
    g4_device_state: int
    g5_health_check: int
    g6_zone_bypass: int
    f1_cancel: int
    ...
    @staticmethod
    def from_json(data: dict[str, Any]) -> DeviceNotificationSubscriptions:
        """Parse a DeviceNotificationSubscriptions payload."""
        return DeviceNotificationSubscriptions(
            g1_emergency=int(data.get("G1_Emergency", 0)),
            g2_other=int(data.get("G2_Other", 0)),
            g3_power_monitoring=int(data.get("G3_PowerMonitoring", 0)),
            g4_device_state=int(data.get("G4_DeviceState", 0)),
            g5_health_check=int(data.get("G5_HealthCheck", 0)),
            g6_zone_bypass=int(data.get("G6_ZoneBypass", 0)),
            f1_cancel=int(data.get("F1_Cancel", 0)),
            f2_panic=int(data.get("F2_Panic", 0)),
            partition_filter=data.get("PartitionFilter"),
        )


@dataclass(frozen=True)
class AccountSubscriptions:
    """Account-level communication preferences and acceptance records."""

    newsletter_emails: bool
    product_update_emails: bool
    terms_version: str | None
    terms_accepted_date: str | None
    privacy_version: str | None
    privacy_accepted_date: str | None
    has_cancel_code: bool
    has_duress_code: bool

    @staticmethod
    def from_json(data: dict[str, Any]) -> AccountSubscriptions:
        """Parse an AccountSubscriptions payload."""
        return AccountSubscriptions(
            newsletter_emails=bool(data.get("NewsletterEmails", False)),
            product_update_emails=bool(data.get("ProductUpdateEmails", False)),
            terms_version=data.get("TermsVersion"),
            terms_accepted_date=data.get("TermsAcceptedDate"),
            privacy_version=data.get("PrivacyVersion"),
            privacy_accepted_date=data.get("PrivacyAcceptedDate"),
            has_cancel_code=bool(data.get("HasCancelCode", False)),
            has_duress_code=bool(data.get("HasDuressCode", False)),
        )
