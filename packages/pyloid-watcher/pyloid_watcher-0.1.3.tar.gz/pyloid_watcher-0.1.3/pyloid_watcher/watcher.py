...
from watchdog.observers import Observer, FSEventsObserver
...
class PyloidWatcher(FileSystemEventHandler):
    ...
    def on_modified(self, event: FileSystemEvent) -> None:
        ...
        if self.should_restart(event):
            self.log(f"[FILE] {event.src_path}", "file")
            self.restart()
    ...
    def start(self) -> None:
        ...
        self.observer = FSEventsObserver() if platform.system() == "Darwin" else Observer()
