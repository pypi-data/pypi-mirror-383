...
class PyloidWatcher(FileSystemEventHandler):
    ...
    def should_restart(self, event: FileSystemEvent) -> bool:
        ...
        if not event.src_path.endswith(self.file_pattern.lstrip("*")):
            if self.verbose:
                self.log(f"[DEBUG] Pattern mismatch: {event.src_path} (pattern: {self.file_pattern})", "warning")
            return False

        # Check if file is within watch directory
        try:
            event_path = Path(event.src_path).resolve()
            watch_path = self.watch_path.resolve()
            event_path.relative_to(watch_path)
            if self.verbose:
                self.log(f"[DEBUG] Path match: {event_path}", "success")
            return True
        except ValueError:
            if self.verbose:
                self.log(f"[DEBUG] Path outside watch dir: {event.src_path}", "warning")
            return False

    def on_modified(self, event: FileSystemEvent) -> None:
        """Handle file modification events."""
        if self.verbose:
            self.log(f"[EVENT] Modified: {event.src_path} (is_dir: {event.is_directory})", "info")
    ...
    def on_created(self, event: FileSystemEvent) -> None:
        ...
        if self.verbose:
            self.log(f"[EVENT] Created: {event.src_path} (is_dir: {event.is_directory})", "info")
        if not event.is_directory and self.should_restart(event):
    ...
    def on_moved(self, event: FileSystemEvent) -> None:
        ...
        if self.verbose:
            self.log(f"[EVENT] Moved: {event.src_path} (is_dir: {event.is_directory})", "info")
        if not event.is_directory and self.should_restart(event):
    ...
    def start(self) -> None:
        ...
        watch_path_absolute = self.watch_path.resolve()
        self.observer.schedule(self, path=str(watch_path_absolute), recursive=True)
        self.observer.start()
        
        if self.verbose:
            self.log(f"[DEBUG] Watching absolute path: {watch_path_absolute}", "info")
