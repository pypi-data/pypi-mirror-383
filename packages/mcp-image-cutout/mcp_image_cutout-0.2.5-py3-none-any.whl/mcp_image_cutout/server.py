...
class VolcImageCutter:
    ...
    async def upload_image_from_file(
        ...
        Args:
            file_path: 文件路径
            filename: 上传时使用的文件名（默认为原文件名）
            mime_type: 自定义MIME类型，默认根据图片格式自动推断
...
async def image_cutout(
    ...
    """
    对图像进行显著性分割抠图，可接受本地文件或远程URL。

    Args:
        image_urls: 图像URL列表，支持HTTP/HTTPS协议。
        image_paths: 本地图片文件路径列表。将自动上传到服务器并转换为可访问的URL后再进行抠图。
    ...
    使用说明：
        1. 直接提供本地图片路径或已经托管的图像URL
        2. 本地图片会自动上传到服务器并生成可访问URL
        3. 工具将使用这些URL执行抠图，并把结果再次上传后返回链接
    """
    if not image_urls and not image_paths:
        raise MCPImageCutoutError("必须提供图像URL或本地图片路径")
    ...
    try:
        ...
        # 先处理本地图片：上传到服务器获取URL
        if image_paths:
            uploaded_input_urls: List[str] = []
            for idx, raw_path in enumerate(image_paths, start=1):
                try:
                    path = Path(raw_path).expanduser()
                    if not path.exists():
                        status_messages.append(f"❌ 输入图像 {idx}: 文件不存在 - {path}")
                        continue
                    if not path.is_file():
                        status_messages.append(f"❌ 输入图像 {idx}: 路径不是文件 - {path}")
                        continue

                    upload_result = await cutter.upload_image_from_file(path)
                    if upload_result.get('success') and upload_result.get('url'):
                        uploaded_input_urls.append(upload_result['url'])
                        status_messages.append(f"✅ 输入图像 {idx}: 上传成功")
                    else:
                        status_messages.append(
                            f"❌ 输入图像 {idx}: {upload_result.get('error', '上传失败')}"
                        )
                except Exception as e:
                    status_messages.append(f"❌ 输入图像 {idx}: 上传过程中出错 - {e}")

            resolved_urls.extend(uploaded_input_urls)

        if image_urls:
            resolved_urls.extend(image_urls)

        if not resolved_urls:
            error_msg = "没有可用于抠图的图片URL"
            if status_messages:
                error_msg += "\n" + "\n".join(status_messages)
            raise MCPImageCutoutError(error_msg)

        # 进行抠图处理
        temp_files = await cutter.saliency_segmentation(resolved_urls)
        ...
        for i, temp_file in enumerate(temp_files):
            try:
                ...
                upload_result = await cutter.upload_image_from_file(
                    temp_file,
                    filename=filename,
                    mime_type="image/png"
                )
                ...
                if upload_result.get('success'):
                    ...
                    output_upload_results.append(f"✅ 第 {i+1} 张: 处理成功")
                    logger.info(f"第 {i+1} 张图片处理完成: {upload_result['url']}")
                else:
                    output_upload_results.append(f"❌ 第 {i+1} 张: {upload_result.get('error', '未知错误')}")
                    logger.error(f"第 {i+1} 张图片上传失败: {upload_result.get('error')}")

            except Exception as e:
                output_upload_results.append(f"❌ 第 {i+1} 张: 处理失败 - {str(e)}")
                logger.error(f"处理第 {i+1} 张图片时出错: {e}")
            finally:
                # 清理临时文件
                try:
        ...
        if uploaded_urls:
            ...
            if status_messages:
                final_messages.extend(status_messages)
            final_messages.extend(output_upload_results)
