...
def compare_data_powder( q_values, chi_values, data2d_stack, q_powder, I_powder, hkl ):
    ...
    slider_ymin = Slider(ax_ymin, 'I min', data2d_stack.min(), data2d_stack.max(), valinit=ax2.get_ylim()[0])
    slider_ymax = Slider(ax_ymax, 'I max', data2d_stack.min(), data2d_stack.max(), valinit=ax2.get_ylim()[1]) 
    ...
    def update_idx(val):
        ...
        if not block_slider:
            slider_ymin.set_val(0)
            slider_ymax.set_val(data2d_stack[idx].max())
            update_lim(0)
    ...
    def update_lim(val):
        global block_slider
        ax2.set_ylim(slider_ymin.val, slider_ymax.val)
        pcm.set_clim(slider_ymin.val, slider_ymax.val)
        block_slider = True
        fig.canvas.draw_idle()
