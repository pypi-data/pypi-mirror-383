...
class Builder:
    ...
    def __init__(self, neo4j_config, openai_config, embedder, logger: Optional[Any] = None):
        ...
        try:
            ...
            # Property name mapping per level (unchanged)
            self.NAMES = {
                "model": ("model_full_context_text", "model_summary_text", "emb_model_full_context"),
                "participant": ("participant_full_context_text", "participant_summary_text", "emb_participant_full_context"),
                "process": ("process_full_context_text", "process_summary_text", "emb_process_full_context"),
                "lane": ("lane_full_context_text", "lane_summary_text", "emb_lane_full_context"),
                "flownode": ("node_full_context_text", "emb_node_full_context"),
            }

            self.PROP_MAP = {
                "flownode":    ("textFull", "textSummary", "vector"),
                "lane":        ("laneTextFull", "laneTextSummary", "laneVector"),
                "process":     ("procTextFull", "procTextSummary", "procVector"),
                "participant": ("partTextFull", "partTextSummary", "partVector"),
                "model":       ("modelTextFull", "modelTextSummary", "modelVector"),
            }
    ...
    def build_flownode_texts(
        ...
        *,
        ...
        # Names and metrics
        try:
            full_prop, emb_prop = self.NAMES["flownode"]
    ...
    def _make_artifact(self, node_id: int, node_name: str, level: str, full: str, summary: str) -> dict:
        full_prop, sum_prop, emb_prop = self.PROP_MAP[level]
        return {
            "node_id": node_id,
            "node_name": node_name,
            "full_prop": full_prop,
