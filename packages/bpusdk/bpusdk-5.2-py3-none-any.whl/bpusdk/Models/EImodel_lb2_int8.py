...
class Exponential(bp.Projection): 
  def __init__(self, population_sizes, pre, post, delay, prob, g_max, tau, E, method,allow_multi_conn):
    super().__init__()
...
class EINet(bp.DynamicalSystem):
    def __init__(self, population_sizes, conn, method, allow_multi_conn=True):
        ...
        tauRef = 1 
        self.E = bp.dyn.LifRef(population_sizes[0], V_rest=2, V_th=20, V_reset=2, tau=1, tau_ref=tauRef,
                               V_initializer=bp.init.Uniform(-15.,18),method=method)
        self.I = bp.dyn.LifRef(population_sizes[1], V_rest=2, V_th=20, V_reset=2, tau=1, tau_ref=tauRef,
                               V_initializer=bp.init.Uniform(-15.,18),method=method)

        self.E2E = Exponential(self.population_sizes, self.E, self.E, delay=0,
                               prob=conn, g_max=1, tau=1, E=1,method=method,allow_multi_conn=allow_multi_conn)
    ...
    def dump(self,download_path,nStep,inpS=None,inpI=0,jit=True,save=True, txt=False,offset=""): 
        ...
        if save == True:
          ...
          np.save(download_dir / f"N_V{offset}.npy", V)
          np.save(download_dir / f"N_spike{offset}.npy", S)
          np.save(download_dir / f"N_wacc1{offset}.npy", wacc1+wacc2)
          # np.save(download_dir / f"N_wacc2{offset}.npy", np.zeros((nStep+1, np.sum(self.population_sizes))))
    ...
    def dump_LRConn(self,download_path,nStep,inpS=None,inpI=0,jit=True,save=True, txt=False): 
        inpS = np.zeros((int(nStep), np.sum(self.population_sizes))).astype(bool) if inpS==None else inpS
        V_init = np.concatenate((bm.floor(self.E.V.value), bm.floor(self.I.V.value)), axis=0)
        V_init = np.expand_dims(V_init, axis=0)
        S_init = np.zeros((1, np.sum(self.population_sizes)))
