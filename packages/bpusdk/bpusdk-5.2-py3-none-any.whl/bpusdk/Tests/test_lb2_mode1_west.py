...
# 0: other, 1: left coloum, 2: bottom left
def determinePopolationPos(iPopulation,nRow,nCol,Y_first):
    if Y_first:
        if iPopulation == nRow*16-1:
            return 2
        if (iPopulation < nRow*16) and (iPopulation%16==0):
            return 1
        else:
            return 0
    else:
        if iPopulation == (nCol*(nRow-1))*16+15:
            return 2
        if (iPopulation % (nCol*16)) == 0:
            return 1
        else:
            return 0

# 0: other, 1: left coloum, 2: bottom left 3: top left, 4:right colum
...
def createConn(nNeuron, nRow, nCol,Y_first):
    ...
    for iPopulation in range(nPopulation):
        ...
        if Y_first:
            match case: # 0: other, 1: left coloum, 2: bottom left
                case 0:
                    if iPopulation%16==0:
...
def createRouter(download_dir,nRow,nCol,Y_first,hex=False):
    ...
    for iTile in range(nCol*nRow):
        ...
        match case: # 0: other, 1: left coloum, 2: bottom left 3: top left, 4:right colum
            case 0:
                ...
                if Y_first:
                    element_hex0 = gethexString(iTile, '10010')
...
if __name__ == "__main__":
    download_dir = "../data7/Lb2_mode1_576k_west"
    upload_dir = "../upload7/ResLb2_mode1_576k_west"
    nExc = 288*1024
    nInh = 288*1024
    nStep = 10
    nRow = 6
    ...
    inpI = 100.                                       
    bpuset = lb2_SNN(net, inpI, mode=1,config={"nRow":nRow,"nCol":nCol})
