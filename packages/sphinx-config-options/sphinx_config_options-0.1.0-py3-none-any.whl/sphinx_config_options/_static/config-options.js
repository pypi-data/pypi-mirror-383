$(document).ready(function () {
  ...
  if ($(location).attr("hash")) {
    $(
      "div#" +
        $.escapeSelector($(location).attr("hash").substr(1)) +
        " > .details",
    ).removeAttr("hidden");
  }
  ...
  $(".configoption").before(
    '<div class="expand-collapse"><span class="expand-all" title="Expand all">⤋</span><span class="collapse-all" title="Collapse all">⤊</span></div>',
  );
  ...
  $(".configoption div.basicinfo").click(function () {
    if ($(this).nextAll(".configoption .details").first().prop("hidden")) {
      ...
      $(this)
        .nextAll(".configoption .details")
        .first()
        .prop("hidden", "until-found");
    }
  });
  ...
  $(".configref").click(function () {
    if ($(this).attr("href").substr(0, 1) == "#") {
      $(".configoption div.details").prop("hidden", "until-found");
      $(
        "#" + $.escapeSelector($(this).attr("href").substr(1)) + " .details",
      ).removeAttr("hidden");
    }
  });
