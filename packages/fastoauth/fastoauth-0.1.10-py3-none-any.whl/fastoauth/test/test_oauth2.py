...
def test_create_access_token():
    ...
    access_token = OAuth2.create_access_token({'user_id': 1, 'user_name': 'foo', 'tenant_id': 23}, expire_seconds=6)
    print(f'{access_token = }')
    print(f'{OAuth2._decode(access_token.access_token) = }')
    print(f'{OAuth2._decode(access_token.refresh_token) = }')

def test_decode():
    OAuth2.SECRET_KEY = 'c1cc1a59d0d64ea04c4d62bc738dee398cd78348d18ef57b66ebad5b88e39d66'
    access_token='eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyX2lkIjoxLCJ1c2VyX25hbWUiOiJmb28iLCJ0ZW5hbnRfaWQiOjIzLCJleHAiOjE3NjAxNTQyMTF9.HfXfQzug7VtpbEzP3m9Ay1HvsdHlcyCcXg-TGcm3i6Y'
    refresh_token='eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyX2lkIjoxLCJ1c2VyX25hbWUiOiJmb28iLCJ0ZW5hbnRfaWQiOjIzLCJleHAiOjE3NjI3NDYyMDUsIl9yZWZyZXNoIjp0cnVlfQ.PTweJdz_mpyKH9jbYnn8NDHV_RPkYN05IyjS1ZehqaY'
    # print(f'{OAuth2._decode(access_token) = }')
    print(f'{OAuth2._decode(refresh_token) = }')
...
def test_refresh_token():
    ...
    # 之前运行出来的十年有效期的refresh_token
    refresh_token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyX2lkIjoxLCJ1c2VyX25hbWUiOiJmb28iLCJ0ZW5hbnRfaWQiOjIzLCJleHAiOjIwNzU1MDYzNjEsIl9yZWZyZXNoIjp0cnVlfQ.V_BPU8URiz_eHrcvl_1nEJ3rFxfnTcgc7gnrbOuf-8U'
...
def test_redis_create_token():
    # 需要先启动redis
    OAuth2.REDIS_URL = 'redis://localhost:6379/oauth2'


def test_redis_decode_token():
    # 需要先启动redis
    OAuth2.REDIS_URL = 'redis://localhost:6379/oauth2'
    OAuth2.EXPIRE_SECONDS = 1
    indata = {'user_id': 1, 'user_name': 'aaaa', 'tenant_id': 23}
    access_token = OAuth2.create_access_token(indata)
    print(f'{access_token = }')
    sleep(OAuth2.EXPIRE_SECONDS+1)
    with pytest.raises(HTTPException) as ex:
        print(f'{OAuth2._decode(access_token.access_token) = }')     # 超时
    print(f'{OAuth2._decode(access_token.refresh_token) = }')
