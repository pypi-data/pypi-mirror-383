...
def compile_model(
    ...
    # Auto-detect Stream-HLS repository path
    streamhls_repo = os.environ.get('STREAMHLS_REPO')
    
    if not streamhls_repo:
        # Try common locations
        possible_paths = [
            '/home/aykumar/streamhls/Stream-HLS',
            os.path.expanduser('~/streamhls/Stream-HLS'),
            os.path.expanduser('~/Stream-HLS'),
            '/opt/streamhls/Stream-HLS',
        ]
        ...
        for path in possible_paths:
            if os.path.exists(path) and os.path.isdir(path):
                streamhls_repo = path
                if verbose:
                    print(f"✅ Auto-detected Stream-HLS at: {streamhls_repo}")
                break
        
        if not streamhls_repo:
            raise RuntimeError(
                "Stream-HLS repository not found. Please set STREAMHLS_REPO environment variable:\n"
                "  export STREAMHLS_REPO=/path/to/Stream-HLS\n"
                f"Or install Stream-HLS in one of: {', '.join(possible_paths)}"
            )
