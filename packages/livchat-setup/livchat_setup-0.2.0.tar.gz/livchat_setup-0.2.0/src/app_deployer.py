...
class AppDeployer:
    ...
    async def deploy(self, server: Dict[str, Any], app_name: str,
        ...
        try:
            ...
            # v0.2.0: Validation 2 - base-infrastructure must be deployed (except if deploying it itself)
            if app_name != "base-infrastructure":
                ...
                if "base-infrastructure" not in apps:
                    return {
                        "success": False,
                        "app": app_name,
                        "error": "Base infrastructure (Traefik + Portainer) not deployed. This is required for all applications.",
                        "hint": "Deploy base-infrastructure first: deploy-app(server_name='...', app_name='base-infrastructure')"
                    }

                logger.info(f"Base infrastructure validation passed for {server['name']}")
