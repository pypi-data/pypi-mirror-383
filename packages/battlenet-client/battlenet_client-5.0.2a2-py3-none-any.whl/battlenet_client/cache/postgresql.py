...
class PostgresCache(BaseCache):
    ...
    def __init__(self, db_host: Optional[str], db_port: Optional[int], db_name: Optional[str], db_user: Optional[str],
                 db_password: Optional[str], db_url: Optional[str]) -> None:
        if db_url:
            self.connector = psycopg.connect(db_url)
        else:
            self.connector = psycopg.connect(host=db_host, port=db_port, user=db_user, password=db_password,
                                             dbname=db_name)

    def __del__(self) -> None:
        self.connector.close()

    def check(self, region_tag: str, category_tag: str, object_id: Union[str, int], locale: str) -> bool:
        ...
        hash_value = self.get_hash(region_tag, category_tag, object_id, locale)
        try:
...
unixepoch('now') - cache.last_updated <= categories.duration;""",
    ...
    def select(self, region_tag: str, category_tag: str, object_id: Union[str, int], locale: str) -> Union[bytes|None]:
        ...
        hash_value = BaseCache.get_hash(region_tag, category_tag, object_id, locale)
...
FROM cache, categories
WHERE cache.object_hash = ? and categories.name = ? and cache.category = categories.cat_id and 
unixepoch('now') - cache.last_updated <= categories.duration order by seq;""", (hash_value, category_tag)).fetchall()
        except TypeError:
            return None
        else:
            if len(res) == 0:
    ...
    def upsert(self, region_tag: str, category_tag: str, object_id: Union[str, int], locale: str,
               data: bytes, chunk_size: int=32 * 1024) -> int:
...
ON CONFLICT (object_hash, seq) DO UPDATE SET data = excluded.data, category=excluded.category, last_updated = excluded.last_updated;""",
    ...
    def delete(self, region_tag: str, category_tag: str, object_id: Union[str, int], locale: str) -> int:
        ...
        delete_dict = {"hash": self.get_hash(region_tag, category_tag, object_id, locale)}
