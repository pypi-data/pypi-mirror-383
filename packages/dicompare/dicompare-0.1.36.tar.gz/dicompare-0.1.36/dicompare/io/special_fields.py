...
def get_field_categorization_summary(
    field_definitions: List[Dict[str, str]]
) -> Dict[str, Any]:
    """
    Get a summary of field categorization for display.

    Args:
        field_definitions: List of field metadata

    Returns:
        Dictionary with counts and lists of fields by category

    Example:
        >>> fields = [
        ...     {'name': 'TR', 'tag': '0018,0080'},
        ...     {'name': 'MultibandFactor', 'tag': ''}
        ... ]
        >>> summary = get_field_categorization_summary(fields)
        >>> summary['counts']['standard']
        1
        >>> summary['counts']['handled']
        1
    """
    categorized = categorize_fields(field_definitions)
    ...
    return {
        'counts': {
            ...
            'handled': len(categorized['handled']),
            'unhandled': len(categorized['unhandled']),
            'total': len(field_definitions)
        },
        'fields': categorized,
        'has_unhandled': len(categorized['unhandled']) > 0
    }
