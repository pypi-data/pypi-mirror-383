...
class HybridStorage:
    ...
    def merge_roots_recursive(
        ...
        *,
        ...
        dedup: str = "hash",          # "hash" | "payload" | "none"
        compact: bool = True,
        progress_every: int = 1000,   # print a line every N processed objects
        quiet: bool = False,          # if True, suppress progress prints
    ) -> dict:
        """
        Recursively merge *all* HybridStorage databases found under `root_dir`
        into a single destination database.

        A HybridStorage database root is any directory containing both:
            - db_index.sqlite
            - db_objects.h5
        ...
        ----------
        root_dir : str
            Root path to scan (recursively) for HybridStorage databases.
        dst_root : Optional[str], default None
            Destination root directory for the merged database. If None, a fresh
            directory named "_merged_YYYYmmdd_HHMMSS" is created inside `root_dir`.
            Destination must not already contain 'db_index.sqlite' or 'db_objects.h5'.
        dedup : {"hash","payload","none"}, default "hash"
            De-duplication mode:
                - "hash": skip objects if their content hash is already present.
                           (hash is taken from object_hashes or meta_json["hash"/"content_hash"/"sha256"])
                - "payload": skip objects if their pickled-byte SHA256 was seen.
                - "none": copy everything.
        compact : bool, default True
            If True, compact the destination HDF5 at the end.
        progress_every : int, default 1000
            Print progress every N processed objects.
        ...
        -------
        dict
            Summary report with counts and paths:
            {
              "sources_found": int,
              "sources_merged": int,
              "objects_scanned": int,
        ...
        assert dedup in ("hash", "payload", "none"), "dedup must be 'hash', 'payload', or 'none'"

        root_dir = os.path.abspath(root_dir)

        # ---------- identify all HybridStorage roots ----------
        ...
        for dirpath, dirnames, filenames in os.walk(root_dir):
            # Skip the destination if the user points it inside root_dir and it already exists (safety)
            if dst_root and os.path.abspath(dirpath) == os.path.abspath(dst_root):
                continue
            if _is_hybrid_root(dirpath):
                sources.append(dirpath)

        # If destination wasn't provided, create a fresh one under root_dir
        if dst_root is None:
        ...
        # Remove the destination from sources list if walk found it for any reason
        sources = [s for s in sources if os.path.abspath(s) != dst_root]
        sources.sort()

        if not quiet:
            print(f"[merge] Found {len(sources)} source DBs under: {root_dir}")
            if not sources:
                print("[merge] Nothing to do.")
                return {
                    "sources_found": 0,
                    "sources_merged": 0,
                    "objects_scanned": 0,
                    ...
                    "dst_root": dst_root,
                    "dst_sqlite": os.path.join(dst_root, "db_index.sqlite"),
                    "dst_h5": os.path.join(dst_root, "db_objects.h5"),
                    "sources": [],
                }
            print(f"[merge] Destination: {dst_root}")

        # ---------- helpers reused from classmethod logic ----------
        def _extract_src_hash_from_meta(meta: dict) -> Optional[str]:
            for k in ("hash", "content_hash", "sha256"):
                h = meta.get(k)
                if isinstance(h, str) and h:
        ...
        # ---------- open destination (RW) ----------
        dst = HybridStorage(dst_root, access="rw")

        # Preload seen sets to minimize DB chatter
        seen_hashes: set[str] = set(dst.get_all_hashes()) if dedup == "hash" else set()
        seen_payloads: set[str] = set() if dedup == "payload" else set()
        ...
        try:
            for si, src_root in enumerate(sources, start=1):
                # Open source (RO)
                try:
                    src = HybridStorage(src_root, access="ro")
                ...
                # Fast id->hash map if available
                id2hash: Dict[int, Optional[str]] = {}
                if dedup == "hash":
                    try:
                        pairs = src.get_all_hashes(with_ids=True)  # List[(id, hash)]
                ...
                try:
                    for oid, obj in src.iter_objects(batch_size=1000):
                        ...
                        if dedup == "hash":
                            h = id2hash.get(oid)
                            if h is None:
                                # fall back to meta lookup
                                try:
                                    meta = src.get_meta(oid)
                                    h = _extract_src_hash_from_meta(meta)  # may remain None
                                except Exception:
                                    h = None
                            if isinstance(h, str) and h:
                                if (h in seen_hashes) or dst.has_hash(h):
                                    is_dup = True
                        elif dedup == "payload":
                            ph = _payload_sha256(obj)
                            if ph in seen_payloads:
                        ...
                        # Keep dedup registries and meta_json consistent
                        if dedup == "hash" and isinstance(h, str) and h:
                            # ensure object_hashes row
                            try:
                                dst._conn.execute(
                                    "INSERT OR REPLACE INTO object_hashes(object_id, hash) VALUES (?,?);",
                                    (new_id, h)
                                )
                                # ensure meta_json carries 'hash'
                                cur = dst._conn.cursor()
                                cur.execute("SELECT meta_json FROM objects WHERE id=?;", (new_id,))
                                row = cur.fetchone()
                                mjson = row[0] if row else None
                        ...
                        # Optionally backfill free_energy from source meta_json if not extracted
                        try:
                            meta = src.get_meta(oid)
                            src_F = meta.get("free_energy", None)
                            if src_F is not None:
                ...
                if not quiet:
                    print(
                        f"    -> source summary: scanned={scanned_here}, added={added_here}, skipped={skipped_here} "
                        f"(cumulative: scanned={total_scanned}, added={total_added}, skipped={total_skipped})"
                    )

            # Optional HDF5 compaction
            if compact:
                try:
                    dst.compact_hdf5()
                    if not quiet:
            ...
            if not quiet:
                print("[merge] DONE.", json.dumps(report, indent=2))
            return report
