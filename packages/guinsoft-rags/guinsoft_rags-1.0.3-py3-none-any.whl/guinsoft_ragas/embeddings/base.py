...
import numpy as np
from langchain_community.embeddings import AzureOpenAIEmbeddings as BaseAzureOpenAIEmbeddings
...
class OpenAIEmbeddings(BaseOpenAIEmbeddings, RagasEmbeddings):
    ...
    def __init__(self, api_key: str = NO_KEY):
        ...
        if proxy_from_env:
            http_client = httpx.Client(proxies=proxy_from_env)
            http_async_client=httpx.AsyncClient(proxies=proxy_from_env)
        else:
            http_client = None
