...
from guinsoft_ragas.async_utils import run_async_tasks
from guinsoft_ragas.exceptions import AzureOpenAIKeyNotFound, OpenAIKeyNotFound
from guinsoft_ragas.llms.base import RagasLLM
from guinsoft_ragas.llms.langchain import _compute_token_usage_langchain
from guinsoft_ragas.utils import NO_KEY, get_debug_mode
...
class OpenAIBase(RagasLLM):
    def __init__(self, model: str, _api_key_env_var: str, timeout: int = 6000) -> None:
        self.model = model
...
class OpenAI(OpenAIBase):
    ...
    def _client_init(self):
        proxy_from_env = os.getenv("OPENAI_PROXY", "")
        if proxy_from_env:
            http_client = httpx.AsyncClient(proxies=proxy_from_env)
