...
from guinsoft_ragas._analytics import EvaluationEvent, track
from guinsoft_ragas.metrics._answer_recall_guinsoft import AnswerRecallGuinsoft
from guinsoft_ragas.metrics.base import Metric
from guinsoft_ragas.metrics.critique import AspectCritique
from guinsoft_ragas.validation import (
    remap_column_names,
...
def evaluate(
    ...
    if metrics is None:
        from guinsoft_ragas.metrics import (
            answer_relevancy,
            context_precision,
            context_recall,
            faithfulness,
    ...
    for metric in metrics:
        ...
        if isinstance(metric, AnswerRecallGuinsoft):
            all = metric.score(dataset)
            if "gt_split_point" not in dataset.features:
                dataset = dataset.add_column("gt_split_point", all["gt_split_point_out"])
            else:
                dataset = dataset.remove_columns("gt_split_point")
                dataset = dataset.add_column("gt_split_point", all["gt_split_point_out"])
            dataset = dataset.add_column("analyse", all["analyse"])
            scores.append(all.select_columns("guinsoft_recall"))
        else:
