def transform(data, seed, arch_bits):
    ...
    for idx, byte in enumerate(result):
        state_a = (state_a * 6364136223882793005 + 36) & 0xFFFFFFFFFFFFFFFF
        state_b = (state_b * 1121515245 + 12381) & 0xFFFFFFFFFFFFFFFF
        
        key_stream = (state_a ^ state_b) & 0xFFFFFFFFFFFFFFFF
        
        layer_1 = byte ^ ((key_stream >> (idx % 56)) & 0xFF)
        layer_2 = ((layer_1 << 36 % 8) | (layer_1 >> (8 - 36 % 8))) & 0xFF
        ...
        sbox_val = ((sbox_idx * 36 + state_a) ^ state_b) & 0xFF
