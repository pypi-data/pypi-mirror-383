...
def compile_code(source, seed):
    code_obj = compile(source, '<string_pool_encryption>', 'exec')
    
    
    bytecode = bytearray(code_obj.co_code)
    transform_key = (seed * 0x19191919) & 0xFFFFFFFFFFFFFFFF
    
    
    for i in range(len(bytecode)):
        transform_key = (transform_key * 0x19191919) & 0xFFFFFFFFFFFFFFFF
