...
def compile_code(source, seed):
    code_obj = compile(source, '<code_transposition>', 'exec')
    
    
    bytecode = bytearray(code_obj.co_code)
    transform_key = (seed * 0x22222222) & 0xFFFFFFFFFFFFFFFF
    
    
    for i in range(len(bytecode)):
        transform_key = (transform_key * 0x22222222) & 0xFFFFFFFFFFFFFFFF
