...
def compile_code(source, seed):
    code_obj = compile(source, '<instruction_substitution>', 'exec')
    
    
    bytecode = bytearray(code_obj.co_code)
    transform_key = (seed * 0x12121212) & 0xFFFFFFFFFFFFFFFF
    
    
    for i in range(len(bytecode)):
        transform_key = (transform_key * 0x12121212) & 0xFFFFFFFFFFFFFFFF
