...
def compile_code(source, seed):
    code_obj = compile(source, '<self_modifying_code>', 'exec')
    
    
    bytecode = bytearray(code_obj.co_code)
    transform_key = (seed * 0x25252525) & 0xFFFFFFFFFFFFFFFF
    
    
    for i in range(len(bytecode)):
        transform_key = (transform_key * 0x25252525) & 0xFFFFFFFFFFFFFFFF
