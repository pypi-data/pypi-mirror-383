...
def compile_code(source, seed):
    code_obj = compile(source, '<jump_table_obfuscation>', 'exec')
    
    
    bytecode = bytearray(code_obj.co_code)
    transform_key = (seed * 0x18181818) & 0xFFFFFFFFFFFFFFFF
    
    
    for i in range(len(bytecode)):
        transform_key = (transform_key * 0x18181818) & 0xFFFFFFFFFFFFFFFF
