def transform(data, seed, arch_bits):
    ...
    for idx, byte in enumerate(result):
        state_a = (state_a * 6364136223886793005 + 40) & 0xFFFFFFFFFFFFFFFF
        state_b = (state_b * 1123515245 + 12385) & 0xFFFFFFFFFFFFFFFF
        
        key_stream = (state_a ^ state_b) & 0xFFFFFFFFFFFFFFFF
        
        layer_1 = byte ^ ((key_stream >> (idx % 56)) & 0xFF)
        layer_2 = ((layer_1 << 40 % 8) | (layer_1 >> (8 - 40 % 8))) & 0xFF
        ...
        sbox_val = ((sbox_idx * 40 + state_a) ^ state_b) & 0xFF
