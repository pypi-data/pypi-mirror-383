...
from typing import Any, Callable, MutableMapping, Optional, overload
...
class ColumnarDataset(Dataset):
    ...
    def __init__(
        ...
        self.columns: dict[str, np.ndarray | list | Feat] = columns  # type: ignore
    ...
    @overload
    def __getitem__(self, idx: slice) -> ColumnarDataset: ...
...
class EmbeddingFeat(Feat):
    ...
    def from_encoded_single_numpy_array(
        ...
        if len(array.value) == 0:
            array_decoder = np.asarray(array.decoder)
        else:
            array_decoder = emb_fn(array.decoder)
        return EmbeddingFeat(array.value, array_decoder, array.decoder)
