...
def initialize_components(data_dir=None):
    ...
    try:
        ...
        # Allow access to common mount points and current directory
        # In Docker, users typically mount volumes at /data, /workspace, /code, etc.
        allowed_dirs = [
            "/",  # Allow all paths in Docker (security is handled by container isolation)
        ]
...
def index_codebase(path: str = None) -> str:
    ...
    Args:
        path: Path to the codebase directory to index.
              ...
              Common paths: /workspace (Docker mount), /data, /code
...
Options:
1. Provide path explicitly: index_codebase(path="/workspace")
2. Set WORKSPACE_PATH environment variable when starting the container:
   docker run -e WORKSPACE_PATH=/workspace ...
...
DO NOT index the MCP server's own code directory (/app)!
"""
    ...
    try:
        # Safety check: Never allow indexing the MCP server's own code
        if path.strip().rstrip('/') in ['/app', '/app/src', '/app/semanticscout']:
            error_msg = f"""❌ ERROR: Cannot index MCP server's own code directory!

You attempted to index: {path}

This is the MCP server's internal code directory. You must index a mounted volume instead.
...
Make sure you mounted your project directory when starting the Docker container:
  docker run -v /path/to/your/project:/workspace semanticscout:latest
"""
            logger.error(f"Rejected attempt to index MCP server directory: {path}")
            return error_msg
...
def search_code(query: str, collection_name: str, top_k: int = 5) -> str:
    ...
    Returns:
        Human-readable search results with code snippets and metadata
    """
    logger.info(f"Tool called: search_code(query={query[:50]}..., collection={collection_name}, top_k={top_k})")
