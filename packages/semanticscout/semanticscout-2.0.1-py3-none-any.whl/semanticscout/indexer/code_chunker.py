...
class ASTCodeChunker:
    ...
    def _ast_chunk(
        ...
        self._extract_chunks_recursive(
            root_node, content, file_path, language, chunk_types, code_chunks, root_node
        )
    ...
    def _extract_chunks_recursive(
        ...
        for child in node.children:
            self._extract_chunks_recursive(
                child, content, file_path, language, chunk_types, chunks, root_node
            )
    ...
    def _extract_file_context(
        self, root_node: Node, content: str, file_path: Path, language: str
    ) -> Optional[CodeChunk]:
        """
