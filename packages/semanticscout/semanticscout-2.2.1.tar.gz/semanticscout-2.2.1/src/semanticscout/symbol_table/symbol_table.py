...
class SymbolTable:
    ...
    def __init__(self, db_path: Optional[Path] = None, collection_name: str = "default"):
        ...
        # Migrate existing tables FIRST (before creating indexes/triggers)
    ...
    def _migrate_existing_schema(self):
        """
        Migrate existing database schema before creating new tables/indexes.

        This must run BEFORE _create_tables() to ensure old databases have
        the collection_name column before we try to create indexes on it.
        """
        # Check if tables exist (if not, this is a new database, skip migration)
        cursor = self.conn.execute("""
            SELECT name FROM sqlite_master
            WHERE type='table' AND name IN ('symbols', 'dependencies', 'file_metadata')
        """)
        existing_tables = {row[0] for row in cursor.fetchall()}

        if not existing_tables:
            # New database, no migration needed
            logger.debug("New database detected, skipping migration")
            return

        logger.info("Existing database detected, checking for schema migrations...")

        # Migrate each table
        if 'symbols' in existing_tables:
            self._migrate_symbols_table()
        if 'dependencies' in existing_tables:
            self._migrate_dependencies_table()
        if 'file_metadata' in existing_tables:
            self._migrate_file_metadata_table()
    ...
    def _migrate_symbols_table(self):
        """
        Migrate existing symbols table to add collection_name column.
        ...
        try:
            ...
            if "collection_name" not in columns:
                logger.info("Migrating symbols table: adding collection_name column")
                self.conn.execute("""
                    ALTER TABLE symbols
                    ADD COLUMN collection_name TEXT NOT NULL DEFAULT 'default'
                """)
                self.conn.commit()
                logger.info("✓ Symbols table migration complete")

        except sqlite3.OperationalError as e:
            logger.warning(f"Could not migrate symbols table: {e}")

    def _migrate_dependencies_table(self):
        """
        Migrate existing dependencies table to add collection_name column.
        ...
        try:
            # Check if collection_name column exists
            cursor = self.conn.execute("PRAGMA table_info(dependencies)")
            columns = {row[1] for row in cursor.fetchall()}

            if "collection_name" not in columns:
                logger.info("Migrating dependencies table: adding collection_name column")
                self.conn.execute("""
                    ALTER TABLE dependencies
                    ADD COLUMN collection_name TEXT NOT NULL DEFAULT 'default'
                """)
                self.conn.commit()
                logger.info("✓ Dependencies table migration complete")

        except sqlite3.OperationalError as e:
            logger.warning(f"Could not migrate dependencies table: {e}")

    def _migrate_file_metadata_table(self):
        """
        Migrate existing file_metadata table to add embedding model columns.

        This ensures backward compatibility with existing databases.
        """
        try:
            # Check if columns already exist
            cursor = self.conn.execute("PRAGMA table_info(file_metadata)")
            columns = {row[1] for row in cursor.fetchall()}

            # Add collection_name column if missing (for old databases)
            if "collection_name" not in columns:
                logger.info("Migrating file_metadata table: adding collection_name column")
                self.conn.execute("""
            ...
            if "embedding_model" not in columns:
                logger.info("Migrating file_metadata table: adding embedding_model column")
                self.conn.execute("""
                    ALTER TABLE file_metadata
            ...
            # Add embedding_dimensions column if missing
            if "embedding_dimensions" not in columns:
                logger.info("Migrating file_metadata table: adding embedding_dimensions column")
                self.conn.execute("""
                    ALTER TABLE file_metadata
            ...
            logger.debug("File metadata table migration complete")

        except sqlite3.OperationalError as e:
            logger.warning(f"Could not migrate file_metadata table: {e}")
