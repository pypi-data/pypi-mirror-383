...
class ASTProcessor:
    ...
    def _create_import_dependency(self, node: Node, content: str, file_path: str, language: str) -> Optional[Dependency]:
        ...
        try:
            ...
            if language == "c_sharp" and node.type == "using_directive":
                return self._create_csharp_using_dependency(node, content, file_path)
    ...
    def _create_csharp_using_dependency(self, node: Node, content: str, file_path: str) -> Optional[Dependency]:
        ...
        try:
            ...
            # For C#, the namespace is both the "to_file" and the imported symbol
            # We use namespace import type since C# using imports entire namespaces
            return Dependency(
                from_file=file_path,
                to_file=namespace,
                ...
                line_number=node.start_point[0] + 1,
                is_type_only=False,
                metadata={"node_type": node.type, "language": "c_sharp"}
            )
        except Exception as e:
