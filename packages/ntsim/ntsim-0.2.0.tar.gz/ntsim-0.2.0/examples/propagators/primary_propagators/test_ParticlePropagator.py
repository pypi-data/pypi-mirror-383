...
def ParticleGenerator_options(opts: NestedNamespace) -> ParticleGeneratorConfig:
    return ParticleGeneratorConfig(
...
def ParticleGenerator(opts: ParticleGeneratorConfig) -> gParticles:
    particles = gParticles('primary_particle')
    
    gen                = 0
    pdgid              = opts.particle_pdgid
    position_m         = opts.position_m
    time_ns            = 0.
    mass               = Particle.from_pdgid(pdgid).mass*units.MeV/units.GeV
    kin_energy_GeV     = opts.kin_energy_GeV
    abs_momentum_GeV   = np.sqrt(kin_energy_GeV*(kin_energy_GeV+2*mass))
    momentum_GeV       = np.array(opts.direction)*abs_momentum_GeV
    
    n_gun_particles    = opts.n_gun_particles
    
    for _ in range(n_gun_particles):
        particles.add_particles(gen,pdgid,position_m,time_ns,momentum_GeV,kin_energy_GeV)
    return particles
...
if __name__ == '__main__':
    ...
    particle = ParticleGenerator(ParticleGenerator_options(opts))
    event    = gEvent()
