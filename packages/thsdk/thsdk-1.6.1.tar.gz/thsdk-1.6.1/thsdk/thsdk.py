...
快速开始:
    ...
    with THS({"username": "xxx", "password": "xxx"}) as ths:
        r = ths.klines("USHA600000", count=100)
        if r.is_success():
            print(r.get_result())
...
class ErrorCodeMap:
    ...
    @classmethod
    def get_err_msg(cls, code: int) -> str:
        """
        根据错误码获取错误信息。
        ...
        Returns:
            str: 错误信息
        """
        return cls.ERR_MAP.get(code, f"未知错误码: {code}")
...
class THS:
    ...
    def call(self, method: str, params: Optional[Union[str, dict, list]] = "",
        ...
        if result_code == 0:
            ...
            return error_response(
                f"错误代码: {result_code},{ErrorCodeMap.get_err_msg(result_code)}, 未找到方法: {method}, 参数:{params}")
    ...
    def connect(self, max_retries: int = 5) -> Response:
        ...
        if self._login:
            ...
            return error_response("❌ 已处于登录状态，请先断开连接（disconnect）后再重新连接。")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if result_code != 0:
                    logger.error(f"❌ 错误代码: {result_code},{ErrorCodeMap.get_err_msg(result_code)}, 连接失败 account:{self.ops.get('username', '')}")
                    return error_response(f"错误代码: {result_code},{ErrorCodeMap.get_err_msg(result_code)}, 连接失败 account:{self.ops.get('username', '')}")
                response = Response(result)
        ...
        while attempt < max_attempts:
            ...
            if result_code == 0:
                ...
                if attempt == max_attempts:
                    return error_response(
                        f"达到最大尝试次数，错误代码: {result_code},{ErrorCodeMap.get_err_msg(result_code)} "
                        f"请求: {params}, 最终缓冲区大小: {buffer_size}")
            else:
                return error_response(
                    f"错误代码: {result_code},{ErrorCodeMap.get_err_msg(result_code)}, 未找到请求数据: {params}")
    ...
    def call_auction_anomaly(self, market: str = "USHA") -> Response:
        ...
        if len(market) != 4 or market not in MARKETS:
            return error_response("市场代码必须为4个字符，并且属于有效市场代码")
