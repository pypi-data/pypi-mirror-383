...
class DatasetProcessor:
    ...
    def save_dataset_file(
        ...
        save_extension = [".csv", ".json", ".parquet", ".arrow", ".pkl"]
        if save_path is None:
            base_name = f"{date}_anonymised"
            # changed this
            save_format = (
                self._last_input_format
                if hasattr(self, "_last_input_format")
                and self._last_input_format
                in save_extension
                else ".csv"
            )
            save_path = os.path.join(f"{base_name}{save_format}")

        # if save_path is in save_extension, use it directly
        elif any(save_path.endswith(ext) for ext in save_extension):
            save_format = next(
                ext for ext in save_extension if save_path.endswith(ext)
            )
        else:
            save_format = ".csv"
