        ...
        if cache:
            ...
            try:
                if save_format == ".csv":
                    ...
                    target_df.to_csv(save_path, index=False)
                    self.logger.warning(f"Unsupported format '{save_format}', saved as CSV.")

                self.logger.info(f"Saved dataset to {save_path} ({save_format}).")

            except Exception as e:
                self.logger.error(f"Failed to save dataset to {save_path}: {e}. Aborting, saving output as petharbor_anonymised.csv to current directory")
