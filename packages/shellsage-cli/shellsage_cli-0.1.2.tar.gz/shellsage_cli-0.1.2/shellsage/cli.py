...
@click.option("--explain-only", is_flag=True, help="Only explain the command, don't run it")
@click.option("--run-only", is_flag=True, help="Only run the command, don't explain it")
def main(command, explain_only, run_only):
    """ShellSage — run and explain Linux commands with AI."""
    cmd = " ".join(command)

    # Run the command (unless explain-only is set)
    if not explain_only:
    ...
    # Explain the command (unless run-only is set)
    if not run_only:
