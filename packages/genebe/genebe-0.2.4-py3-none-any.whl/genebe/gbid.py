...
class VariantIdEncoder:
    ...
    def encodeSpdi(self, chromosome: str, position: int, del_length: int, ins: str):
        ...
        if len(ins) > self.MAX_INS_LENGTH or del_length > self.MAX_DEL_LENGTH:
            useChangeHash = True
        ...
        if "N" in ins:
            ...
            encodedAlt = self.encode_bases(ins)

        if positionId == -1:
            return hash(chromosomeNormalized, position, del_length, ins)
        else:
            if useChangeHash:
                encoded = 0
                ...
                encoded = self.set_value(encoded, self.MASK_POS, positionId)
                # print('4'+bin(encoded))
                encoded = self.set_value(encoded, self.MASK_INS_LEN, len(ins))
                # print('5'+bin(encoded))
                encoded = self.set_value(encoded, self.MASK_DEL, del_length)
                # print('6'+bin(encoded))
