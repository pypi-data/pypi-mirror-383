...
class EvexIoc:
    ...
    def read_wb_exp_to_df(cls, kwargs: TyDic) -> TnPdDf:
        _pddf: TnPdDf = PdPathKIoiWb.read_wb_to_df(Cfg.InPathnm.evex, kwargs, Cfg.sheet_exp)
        return _pddf

    @classmethod
    def read_wb_exp_to_aod(cls, kwargs: TyDic) -> TnAoD:
        _aod: TnAoD = PdPathKIoiWb.read_wb_to_aod(Cfg.InPathnm.evex, kwargs, Cfg.sheet_exp)
        return _aod
