{% macro watsonx_spark__can_clone_table() %}
    {{ return(True) }}
{% endmacro %}

{% macro watsonx_spark__create_or_replace_clone(this_relation, defer_relation) %}
    create or replace table {{ this_relation }} shallow clone {{ defer_relation }}
{% endmacro %}

{%- materialization clone, adapter='watsonx_spark' -%}
