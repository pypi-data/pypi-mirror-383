...
class GraphRegistryWithAuth:
    ...
    async def build_graph(
        ...
        if force_rebuild or cache_key not in self._compiled_graphs:
            ...
            instance = await self.get_graph_instance(user_id, graph_type, config)
            instance_duration = int((time.time() - instance_start) * 1000)
            logger.info(f"graph_instance_created | user_id={user_id} | graph_type={graph_type} | duration_ms={instance_duration}")
            ...
            logger.info(f"graph_compiled | user_id={user_id} | graph_type={graph_type} | duration_ms={compile_duration}")

        build_duration = int((time.time() - build_start) * 1000)
        logger.info(f"graph_build_complete | user_id={user_id} | graph_type={graph_type} | cached={cache_key in self._compiled_graphs} | duration_ms={build_duration}")
