...
class SessionService:
    ...
    async def create_session(self, session_id: Optional[str] = None, user_id: str = "anonymous", title: Optional[str] = None) -> str:
        ...
        if user_id != "anonymous" and self.auth_token:
            try:
                ensure_result = user_service.ensure_user(
                    user_id=user_id,
    ...
    async def get_session(self, session_id: str, user_id: Optional[str] = None) -> Optional[Dict]:
        ...
        try:
            result = user_service.get_user_sessions(
                user_id=user_id,
                limit=100,  # Get enough sessions to find ours
                auth_token=self.auth_token
            )
    ...
    async def update_session_activity(self, session_id: str, user_id: str = "anonymous"):
        ...
        try:
            user_service.update_session_activity(
    ...
    async def add_message_to_session(
        ...
        try:
            result = user_service.add_session_message(
                session_id=session_id,
                role=role,
                content=content,
    ...
    async def clear_session(self, session_id: str):
        ...
        # Delete via User Service API
        try:
            result = user_service.delete_session(
                session_id=session_id,
                auth_token=self.auth_token
            )
            
            if result.get("success"):
                api_logger.info(f"🗑️ Session {session_id} deleted via API")
            else:
                api_logger.warning(f"⚠️ Failed to delete session via API: {result.get('error')}")
