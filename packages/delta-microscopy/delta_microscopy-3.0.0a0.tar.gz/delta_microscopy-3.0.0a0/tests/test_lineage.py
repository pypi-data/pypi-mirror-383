...
def test_plot():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with savepath.open("rb") as file:
            hash_actual = hashlib.file_digest(file, "sha256")

    with path.with_suffix(".png").open("rb") as file:
        hash_expected = hashlib.file_digest(file, "sha256")

    assert hash_actual.digest() == hash_expected.digest()
