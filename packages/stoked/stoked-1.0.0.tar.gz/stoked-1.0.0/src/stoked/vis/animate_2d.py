...
def animation_2d(
    ...
    **kwargs,
    ...
    """
    if fading_kwargs is None:
        fading_kwargs = {}
    if trail_kwargs is None:
        trail_kwargs = {}
    ...
    if trail_type not in trail_types:
        raise ValueError(f"trail_type '{trail_type}' is not valid. Choose from {trail_types}")
    ...
    for i in range(Nparticles):
        ...
        if patches is not None:
            if isinstance(patches_type[i], mpl.collections.PatchCollection):
                ...
                particles.append(
                    patches_type[i](
                        pos, *patches_args[i], fill=False, fc=color, edgecolor=color, animated=True, **circle_properties
                    )
                )
            ...
            if patches_type[i] is plt.Circle and angles is not None:
                ...
                lines[i] = plt.Line2D(
                    ...
                    [pos[1], pos[1]],
                    lw=circle_properties["linewidth"],
                    color=color,
                    animated=True,
                    **line_properties,
                )
        ...
        if number_labels:
            ...
            text[label].set_path_effects([path_effects.Stroke(linewidth=5, foreground="white"), path_effects.Normal()])
    ...
    def _get_artist(obj):
        """Extract the Artist from an object, handling colored_line wrappers."""
    ...
    def update(t):
        ...
        for i in range(Nparticles):
            ...
            if time is not None:
                text["clock"].set_text(rf"{time[t]:.2f} {time_unit}")
        ...
        trail_artists = [_get_artist(t) for t in trails]
        return trail_artists + particles + list(lines.values()) + list(text.values()) + dots
...
def trajectory_animation(trajectory, patches=None, *args, **kwargs):
    ...
    xlim = np.array([np.min(trajectory.position[..., 0]), np.max(trajectory.position[..., 0])])
    ylim = np.array([np.min(trajectory.position[..., 1]), np.max(trajectory.position[..., 1])])
    if patches:
        if isinstance(patches, list):
