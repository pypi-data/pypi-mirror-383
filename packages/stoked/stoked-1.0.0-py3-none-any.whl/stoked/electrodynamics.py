...
from stoked.solver import interactions
...
class point_dipole_electrodynamics(interactions):
    ...
    def force(self):
        ...
        for i in range(self.Nparticles):
            ...
            F[i] = 0.5 * np.real(np.einsum("xy,y,zx", np.conj(self.alpha_t[i]), np.conj(self.sol[i]), dE))
    ...
    def torque(self):
        ...
        for i in range(self.Nparticles):
            ...
            T[i] = 0.5 * np.real(np.cross(np.conj(p), np.dot(a0_inverse, p)))
