...
def trajectory_animation_3d(
    ...
    p = Process(
        target=_trajectory_animation_3d,
        kwargs=dict(
...
def _trajectory_animation_3d(
    ...
    for i in range(Nparticles):
        ...
        particle = patches_type[i](pos=pos, color=color, opacity=opacity, **patches_args[i])
        ...
        if axes:
            arrow_x = vpython.arrow(pos=pos, axis=vec(1, 0, 0), scale=axes_length, color=vec(0, 0, 0), shaftwidth=5)
            arrow_y = vpython.arrow(pos=pos, axis=vec(0, 1, 0), scale=axes_length, color=vec(0, 0, 0), shaftwidth=5)
            arrow_z = vpython.arrow(
                pos=pos, axis=vec(0, 0, 1), scale=axes_length, color=vpython.color.red, shaftwidth=5
            )
    ...
    if grid:
        vpython.arrow(
            pos=vpython.vector(0, 0, 0), axis=vpython.vector(300, 0, 0), shaftwidth=2, color=vpython.color.black
        )
        vpython.arrow(
            pos=vpython.vector(0, 0, 0), axis=vpython.vector(0, 300, 0), shaftwidth=2, color=vpython.color.black
        )
        vpython.arrow(
            pos=vpython.vector(0, 0, 0), axis=vpython.vector(0, 0, 300), shaftwidth=2, color=vpython.color.black
        )
...
if __name__ == "__main__":
    ...
    trajectory_animation_3d(coordinates, patches, opacity=0.5, colors="C0", trail=100, wait=True)
