...
def clear_logs_befor(cutoff_date: str):
    """Delete log entries before the given date from log files.
    
    cutoff_date: ISO-like date string, e.g. '2025-01-31' or '2025-01-31 12:34:56'.
    """
    import os
    import re
    from datetime import datetime
    ...
    files = ["service.log", "errors.log", "debug.log"]
    ...
    for fname in files:
        ...
        try:
            ...
            kept_lines = []
            for ts, entry_lines in entries:
                if ts is None or ts >= cutoff_dt:
                    kept_lines.extend(entry_lines)
