...
from panshi2task.grpc_gen.task_llm_pb2 import EntityMiniingRequest, EntityMiniingResponse, FAQMiniingRequest, \
    QuestionGenRequest, SummaryRequest, TopicMiningRequest
...
class LlmTaskClient:
    ...
    def entity_mining(self, schema: List[Dict[str, str]], text: str) -> Dict[str, List[str]]:
        req = ParseDict({"schema": schema, "text": text}, EntityMiniingRequest())
    ...
    def topic_mining(self, summary: str, titles: List[str] | None = None) -> List[str]:
        req = ParseDict({"summary": summary, "titles": titles}, TopicMiningRequest())
