...
from panshi2task.grpc_gen import task_embedding_pb2
...
class EmbeddingTaskClient:
    ...
    def embedding(self, text: str, max_length: int=512) -> List[float]:
        pb2_req = task_embedding_pb2.EmbedRequest(input=text, max_length=max_length)
        pb2_resp: task_embedding_pb2.EmbedResponse = self.stub.Embed(pb2_req)
        resp = MessageToDict(pb2_resp)
        return resp["embeddings"]

    def batch_embedding(self, texts: List[str], max_length: int=512, batch_size: int = 10) -> List[List[float]]:
        pb2_req = task_embedding_pb2.BatchEmbedRequest(inputs=texts, max_length=max_length,batch_size=batch_size)
        pb2_resp: task_embedding_pb2.BatchEmbedResponse = self.stub.BatchEmbed(pb2_req)
        resp = MessageToDict(pb2_resp)
