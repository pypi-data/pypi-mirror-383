...
from panshi2task.base import GrpcServerInfo
...
class PanshiTaskClient:
    ...
    def image2text(self, image_base64: str) -> str:
        assert self.llm_client is not None, "请提供llm_server_url地址"
        return self.llm_client.image2text(image_base64)

    def image2text_batch(self, image_base64s: List[str]) -> List[str]:
        assert self.llm_client is not None, "请提供llm_server_url地址"
        return self.llm_client.image2text_batch(image_base64s)
    ...
    # ========================rerank====================
    def rerank(self, query: str, texts: List[str], normalize: bool = True,return_text: bool = True) -> List[RankItem]:
        assert self.rerank_client is not None, "请提供rerank_server_url地址"
        return self.rerank_client.rerank(query, texts, normalize, return_text)
