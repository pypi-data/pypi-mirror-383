...
class KubeflowPlugin:
    ...
    def get_served_models(
        ...
        def is_404_error(exc: Exception) -> bool:
            """Return True if this or a nested exception represents an HTTP 404."""
            # Handle nested exceptions (e.g. Tenacity wraps)
            cause = getattr(exc, "__cause__", None) or getattr(exc, "__context__", None)
            if cause and cause is not exc:
                if is_404_error(cause):
                    return True

            # Only inspect ApiException instances
            if isinstance(exc, ApiException):
                # Try attributes first
                if getattr(exc, "code", None) == 404 or getattr(exc, "status", None) == 404:
                    return True

                # Then reason/message text
                reason_text = str(getattr(exc, "reason", "")).lower()
                if "not found" in reason_text or "404" in reason_text:
                    return True

                # Finally, parse JSON body if available
                try:
            ...
            # As a last fallback, check message text
        ...
        try:
            if isvc_name:
                try:
                    ...
                    if is_404_error(e):
                        # clean exit — service doesn't exist
                        return None
                    raise
