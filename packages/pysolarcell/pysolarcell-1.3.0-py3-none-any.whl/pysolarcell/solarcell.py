...
class Layer:
    def __init__(self, name, bandgap, iqe=1, thickness: float=0, k=None, area=100, Rs=0, Rsh=np.inf, T=298, n=1, real_voc=np.inf, real_jsc=np.inf, fr=2):
        ...
        self.fr = fr  # Emission area fraction (fr = 2 allows radiative emission from both side)
    ...
    def to_solar_cell(self, light_spectrum: Lamp):
        ...
        if self.real_voc == np.inf:
            ...
            J0 = self.fr * (15 * q * sigma * self.T ** 3) / (k * pi ** 4) * integral(u) / 10
        else:
