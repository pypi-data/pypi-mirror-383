    ...
    def __output_response(__obj: requests.Response):
        ...
        if __obj.request.body:
            if "application/json" in __obj.request.headers.get("Content-Type", ""):
                ...
                # 按行逐行解析 body，当内容为 Content-Type: application/octet-stream 时，忽略内容输出。其余情况都直接输出
                content_type = __obj.request.headers.get("Content-Type", "")
                boundary = content_type.split("boundary=")[-1]
                # boundary 可能被引号包裹
                boundary = boundary.strip('"')
                # multipart/form-data 的分隔符前面通常有两个连字符
                boundary_marker = f"--{boundary}"
                if isinstance(__obj.request.body, bytes):
                    body_str = __obj.request.body.decode("utf-8", errors="ignore")
                else:
                    body_str = str(__obj.request.body)
                body_params: list[list[str]] = list()
                # 去除空行
                for param in body_str.split(boundary_marker):
                    if param.strip():
                        param_lines = param.splitlines()
                        param_lines = [_line for _line in param_lines if _line.strip()]
                        # 去掉最末尾的--
                        if len(param_lines) > 1:
                            body_params.append(param_lines)
                for param in body_params:
                    for _line in param:
                        if "Content-Type: application/octet-stream" in _line:
                            _logger(f"请求内容：{_line.strip()}")
                            _logger(f"请求内容：********忽略二进制内容********")
                            # 忽略后面的二进制内容
                            break
                        else:
                            _logger(f"请求内容：{_line.strip()}")
