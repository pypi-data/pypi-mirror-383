...
class Module(ABC):
    ...
    def _initialize_helpers(self, context: ExecutionContext):
        ...
        if not api_client:
            ...
            # Allow offline mode for local development/serve
            offline = bool(context.get("offline")) or str(os.getenv("HLA_COMPASS_OFFLINE", "")).lower() in {"1", "true", "yes"}
            if not (has_token or has_api_key):
                if offline:
                    self.logger.info("Offline mode: skipping API client initialization (no credentials found)")
                    api_client = None
                else:
                    raise AuthError(
