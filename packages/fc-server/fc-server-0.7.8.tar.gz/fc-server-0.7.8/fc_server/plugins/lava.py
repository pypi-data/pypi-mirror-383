...
class Plugin(FCPlugin, Lava):
            ...
            if devices:
                ...
                for info in used_possible_resources:
                    if not info["current_job"]:
                        await driver.return_resource(info["hostname"], self)
    ...
    async def __get_device_tags(self, device, semaphore):
        """
    ...
    async def __seize_resource(
        self, driver, job_id, candidated_non_available_devices, semaphore
    ):
        """
        ...
        async with semaphore:
            device_tags_semaphore = asyncio.Semaphore(20)
            non_available_device_tags_list = await asyncio.gather(
                *[
                    self.__get_device_tags(
                        candidated_non_available_device, device_tags_semaphore
                    )
    ...
    async def schedule(
        ...
        async def schedule_prepare():
            ...
            for device in devices:
                if device["hostname"] in driver.managed_resources and device[
                    ...
                    # if default framework, this is the only entry to assign lava to this resource
                    ...
                    if device["current_job"] and await driver.is_resource_available(
                        ...
                        driver.accept_resource(
                            device["hostname"], self, owner=device["current_job"]
                        )
                        asyncio.create_task(
        ...
        job_tags_semaphore = asyncio.Semaphore(20)
        ...
        for queued_job in queued_jobs:
            ...
            available_device_tags_list = await asyncio.gather(
                *[
                    self.__get_device_tags(
                        candidated_available_device, device_tags_semaphore
                    )
            ...
            def lava_seize_resource(*_):
                ...
                if (
                    ...
                    if self.seize_semaphore is None:
                        self.seize_semaphore = asyncio.Semaphore(20)
        ...
        if not driver.is_default_framework(self) and possible_resources:
            # let lava dispatch
