...
class WordAutoNumHelper:
    ideographTraditional = "甲乙丙丁戊己庚辛壬癸"
    ideographZodiac = "子丑寅卯辰巳午未申酉戌亥"


    def __init__(self, file_path, gap_text="\t"):
    ...
    def get_paragraph_num(self, paragraph: Paragraph):
        if not self.numId2style:
            return ""
        flag1 = hasattr(paragraph, "_element")
        flag2 = hasattr(paragraph._element, "pPr")
        flag3 = hasattr(paragraph._element.pPr, "numPr")
        if flag1 and flag2 and flag3:
            return self.get_number_text(paragraph._element.pPr.numPr)
        else:
            return ""
            ...
            for abstractNumIdTag in numbering_part.findall(qn("w:abstractNum")):
                ...
                for lvlTag in abstractNumIdTag.findall(qn("w:lvl")):
                    ...
                    if "numFmt" not in style:
                        numFmtVal = lvlTag.xpath(
                            "./mc:AlternateContent/mc:Fallback/w:numFmt/@w:val",
                            namespaces=numbering_part.nsmap
                        )
                        if numFmtVal and numFmtVal[0] == "decimal":
                            numFmt_format = lvlTag.xpath(
    ...
    def int2ordinalText(self, num):
        ...
        if thousand > 0:
            if num == 0:
                return f"{self.int2cardinalText(thousand)} thousandth"
            result.append(f"{self.int2cardinalText(thousand)} thousand")
    ...
    def int2ChineseCounting(self, num):
        return self.int2Chinese(num, ch_num='〇一二三四五六七八九', units='个十百千')

    def int2ChineseLegalSimplified(self, num):
        return self.int2Chinese(num, ch_num='零壹贰叁肆伍陆柒捌玖', units='个拾佰仟')
