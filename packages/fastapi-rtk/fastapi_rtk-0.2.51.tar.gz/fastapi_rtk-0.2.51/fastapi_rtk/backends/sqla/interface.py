...
class SQLAInterface(AbstractInterface[ModelType, Session | AsyncSession, Column]):
    ...
    def _generate_schema_from_dict(self, schema_dict):
        # Allow await for deferrable columns such as relationships
        relationship_columns = [
            key for key in schema_dict.keys() if self.is_relation(key)
        ]
        model_name = schema_dict.get("__name__", "UnknownModel")
        if relationship_columns:

            async def fill_relationship(model: Model, col: str):
                try:
            ...
            def fill_relationships(data):
                if isinstance(data, Model):
                    for col in relationship_columns:
                        safe_call_sync(fill_relationship(data, col))
                return data

            decorated_func = pydantic.model_validator(mode="before")(fill_relationships)
            schema_dict["__validators__"] = schema_dict.get("__validators__", {})
            schema_dict["__validators__"][fill_relationships.__name__] = decorated_func
