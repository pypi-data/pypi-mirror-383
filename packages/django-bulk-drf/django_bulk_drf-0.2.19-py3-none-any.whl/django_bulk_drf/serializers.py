...
class BulkModelSerializer(BulkSerializerMixin, serializers.ModelSerializer):
    ...
    Automatically converts standard relational fields to bulk-safe versions:
    - PrimaryKeyRelatedField → BulkPrimaryKeyRelatedField  
    - SlugRelatedField → BulkSlugRelatedField
    
    This ensures no N+1 queries during validation without runtime patching.
    """
    
    def __init__(self, *args, **kwargs):
        """
        Initialize serializer and auto-convert relational fields to bulk-safe versions.
        
        This happens once during construction, not at runtime during validation.
        """
        super().__init__(*args, **kwargs)
        
        # Auto-convert standard relational fields to Bulk* versions
        # This prevents N+1 queries during validation
        self._convert_to_bulk_fields()
    
    def _convert_to_bulk_fields(self):
        """
        Convert standard DRF relational fields to bulk-safe equivalents.
        
        This is done at serializer construction time, not during validation,
        so there's no runtime patching.
        """
        ...
        for field_name, field in list(self.fields.items()):
            # Skip if already a Bulk* field
            if isinstance(field, (BulkPrimaryKeyRelatedField, BulkSlugRelatedField)):
                continue
            
            # Convert PrimaryKeyRelatedField → BulkPrimaryKeyRelatedField
            if isinstance(field, DRFPrimaryKeyRelatedField):
                # Create bulk-safe version with same configuration
            ...
            # Convert SlugRelatedField → BulkSlugRelatedField  
    ...
    def _extract_field_kwargs(self, field):
        ...
        Returns:
            Dict of kwargs safe to pass to new field
        """
        kwargs = {}
