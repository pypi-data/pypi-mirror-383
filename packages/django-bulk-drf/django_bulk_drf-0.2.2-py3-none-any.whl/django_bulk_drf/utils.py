...
class FieldConverter:
    ...
    @staticmethod
    def resolve_fk_ids(model, fk_field_name, identifiers):
        """
        ...
        if to_field == 'pk':
            ...
            # Lookup by to_field
            lookup = {f'{to_field}__in': identifiers}
            queryset = related_model.objects.filter(**lookup)
            return {getattr(obj, to_field): obj.pk for obj in queryset}
    
    @staticmethod
    def convert_related_fields_batch(validated_data, model):
        """
...
class M2MHandler:
    ...
    def set_m2m_relations(self, instances, m2m_data_list):
        ...
        if len(instances) != len(m2m_data_list):
            raise ValueError(
                f'Mismatch: {len(instances)} instances but {len(m2m_data_list)} M2M data items'
            )
