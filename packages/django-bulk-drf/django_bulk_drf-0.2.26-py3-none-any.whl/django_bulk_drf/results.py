...
class BulkResponseFormatter:
    ...
    def format_full(result, serializer=None, optimized_instances=None):
        ...
        if serializer:
            logger.debug(f"[FORMAT RESPONSE] Serializer provided, will serialize data")
            # Use optimized instances if provided, otherwise get from result
            all_instances = optimized_instances if optimized_instances is not None else result.get_all_instances()
            
            logger.debug(f"[FORMAT RESPONSE] Using optimized_instances: {optimized_instances is not None}")
            logger.debug(f"[FORMAT RESPONSE] Total instances to serialize: {len(all_instances)}")
            logger.debug(f"[FORMAT RESPONSE] Serializer type: {type(serializer).__name__}")
            ...
            if all_instances:
                ...
                logger.debug(f"[FORMAT RESPONSE] About to serialize with: {ser}")
                logger.debug(f"[FORMAT RESPONSE] Instance types: {[type(i).__name__ for i in all_instances[:3]]}")
                
                response_data["data"] = ser(all_instances, many=True).data
                
                logger.debug(f"[FORMAT RESPONSE] Serialization complete")
        else:
            logger.debug(f"[FORMAT RESPONSE] No serializer provided, skipping data serialization")
