...
class BulkTransactionManager:
    ...
    def handle_partial_failure(self, successful, failed, errors):
        ...
        if self.failure_strategy == PartialFailureStrategy.ROLLBACK_ALL:
            ...
            logger.info(f"Partial failure: {len(successful)} succeeded, {len(failed)} failed. Committing successful operations.")
            ...
            logger.info(f"Continuing despite failures: {len(successful)} succeeded, {len(failed)} failed.")
            return successful, failed, True
