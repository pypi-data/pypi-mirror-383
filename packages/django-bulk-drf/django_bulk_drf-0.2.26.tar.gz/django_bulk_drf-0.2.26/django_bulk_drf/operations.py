...
class BulkUpdateOperation(BulkOperation):
    ...
    def _fetch_existing(self, validated_data):
        ...
        Applies select_related/prefetch_related from context to prevent N+1 queries.
        ...
        # Apply query optimizations from context to prevent N+1 queries
        select_related_fields = self.context.get("select_related_fields")
        if select_related_fields:
            queryset = queryset.select_related(*select_related_fields)
        
        prefetch_related_fields = self.context.get("prefetch_related_fields")
