...
class FieldConverter:
    ...
    def get_model_fk_fields(model):
        ...
        logger.debug(f"[GET MODEL FK FIELDS] Model: {model.__name__}, Total fields: {len(all_fields)}")
        logger.debug(f"[GET MODEL FK FIELDS] All field names: {[f.name for f in all_fields]}")
        
        for field in all_fields:
            if isinstance(field, models.ForeignKey):
                fk_fields.append(field.name)
                logger.debug(f"[GET MODEL FK FIELDS] Found FK: {field.name} -> {field.related_model.__name__}")
        
        logger.debug(f"[GET MODEL FK FIELDS] FK fields: {fk_fields}")
        return fk_fields
    ...
    def get_fk_field_info(model, fk_field_name):
        ...
        try:
            ...
            logger.debug(f"[GET FK INFO] Model: {model.__name__}, Field: {fk_field_name}, Field type: {type(field).__name__}")
            
            if not isinstance(field, models.ForeignKey):
                logger.warning(f"[GET FK INFO] Field {fk_field_name} on {model.__name__} is not a ForeignKey (type: {type(field).__name__})")
                return None
            ...
            logger.debug(f"[GET FK INFO] FK info for {fk_field_name}: related_model={field.related_model.__name__}, to_field={result['to_field']}")
            return result
        except Exception as e:
            logger.error(f"[GET FK INFO] Exception getting field info for {model.__name__}.{fk_field_name}: {e}")
            import traceback
            logger.error(f"[GET FK INFO] Traceback: {traceback.format_exc()}")
            return None
    ...
    def collect_fk_identifiers(validated_data, model):
        ...
        logger.debug(f"[COLLECT FK IDS] Model: {model.__name__}, FK fields from model: {fk_fields}")
        
        identifiers_map = {fk_field: set() for fk_field in fk_fields}

        for data in validated_data:
            logger.debug(f"[COLLECT FK IDS] Data keys: {list(data.keys())}")
            for fk_field in fk_fields:
                if fk_field in data and data[fk_field] is not None:
                    identifiers_map[fk_field].add(data[fk_field])

        logger.debug(f"[COLLECT FK IDS] Identifiers collected: {identifiers_map}")
    ...
    def resolve_fk_ids(model, fk_field_name, identifiers, slug_field=None):
        ...
        field_info = FieldConverter.get_fk_field_info(model, fk_field_name)
        if not field_info:
            logger.error(f"[RESOLVE FK IDS] No field info found for {model.__name__}.{fk_field_name}")
            return {}

        related_model = field_info["related_model"]
        to_field = field_info["to_field"]

        logger.debug(f"[RESOLVE FK IDS] Field: {fk_field_name}, Parent Model: {model.__name__}, Related Model: {related_model.__name__}, to_field: {to_field}, slug_field: {slug_field}, identifiers: {identifiers}")
        ...
        if all_ints and to_field == "pk" and not slug_field:
            ...
            logger.debug(f"[RESOLVE FK IDS] Direct PK lookup, no query needed")
        ...
        if slug_field:
            ...
            logger.debug(f"[RESOLVE FK IDS] Using serializer's slug_field: {slug_field}")
        ...
        if lookup_field == "pk":
            logger.debug(f"[RESOLVE FK IDS] Querying: {related_model.__name__}.objects.filter(pk__in={list(identifiers)})")
            queryset = related_model.objects.filter(pk__in=identifiers)
            logger.debug(f"[RESOLVE FK IDS] SQL Query: {queryset.query}")
            results_list = list(queryset)
            logger.debug(f"[RESOLVE FK IDS] Query returned {len(results_list)} objects: {results_list}")
            result = {obj.pk: obj.pk for obj in results_list}
        else:
            # Lookup by slug/to_field
            lookup = {f"{lookup_field}__in": identifiers}
            logger.debug(f"[RESOLVE FK IDS] Querying: {related_model.__name__}.objects.filter({lookup})")
        ...
        logger.debug(f"[RESOLVE FK IDS] Resolution map for {fk_field_name}: {result}")
        
        if len(result) < len(identifiers):
            missing = set(identifiers) - set(result.keys())
            logger.warning(f"[RESOLVE FK IDS] Missing identifiers for {fk_field_name}: {missing}")
    ...
    def convert_related_fields_batch(validated_data, model, serializer=None):
        ...
        logger.debug(f"[FK CONVERT] Starting conversion for model: {model.__name__}")
        logger.debug(f"[FK CONVERT] Serializer provided: {serializer is not None}")
        logger.debug(f"[FK CONVERT] Data sample: {validated_data[0] if validated_data else 'empty'}")
        
        # Extract FK field metadata from serializer
        fk_metadata = {}
        if serializer:
            logger.debug(f"[FK CONVERT] Serializer fields: {list(serializer.fields.keys())}")
            # Lazy import to avoid circular dependency
            from .serializers import BulkSlugRelatedField
            for field_name, field in serializer.fields.items():
                logger.debug(f"[FK CONVERT] Field {field_name}: {type(field).__name__}")
                if isinstance(field, BulkSlugRelatedField):
                    fk_metadata[field_name] = {
                        'slug_field': field.slug_field
                    }
                    logger.debug(f"[FK CONVERT] Found BulkSlugRelatedField: {field_name}, slug_field={field.slug_field}")
        
        logger.debug(f"[FK CONVERT] FK Metadata extracted: {fk_metadata}")
        
        # Collect all FK identifiers
        identifiers_map = FieldConverter.collect_fk_identifiers(validated_data, model)
        logger.debug(f"[FK CONVERT] Identifiers collected: {identifiers_map}")
        ...
        for fk_field, identifiers in identifiers_map.items():
            if identifiers:
                ...
                logger.debug(f"[FK CONVERT] Resolving {fk_field}: identifiers={identifiers}, slug_field={slug_field}")
                resolution_maps[fk_field] = FieldConverter.resolve_fk_ids(
                    model, fk_field, identifiers, slug_field=slug_field
                )
                logger.debug(f"[FK CONVERT] Resolution map for {fk_field}: {resolution_maps[fk_field]}")
            ...
            for fk_field, resolution_map in resolution_maps.items():
                if fk_field in data and data[fk_field] is not None:
                    ...
                    if identifier in resolution_map:
                        ...
                        logger.debug(f"[FK CONVERT] Converted {fk_field}={identifier} -> {id_field}={resolution_map[identifier]}")
                        # Remove original FK field
                        del data[fk_field]
                    else:
                        logger.warning(f"[FK CONVERT] Identifier {identifier} not found in resolution map for {fk_field}")

        logger.debug(f"[FK CONVERT] Final data sample: {validated_data[0] if validated_data else 'empty'}")
