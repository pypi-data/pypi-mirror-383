...
class BulkSettings:
    ...
    def validate_settings(self):
        ...
        if not isinstance(default_batch, int) or default_batch <= 0:
            raise ImproperlyConfigured(
                f'DEFAULT_BATCH_SIZE must be a positive integer, got {default_batch}'
            )
        ...
        if default_batch > max_batch:
            raise ImproperlyConfigured(
                f'DEFAULT_BATCH_SIZE ({default_batch}) cannot exceed '
        ...
        for setting_key in bool_settings:
            ...
            if value is not None and not isinstance(value, bool):
                raise ImproperlyConfigured(
                    f'{setting_key} must be a boolean, got {type(value).__name__}'
                )
    ...
    def signal_dispatch(self):
        """Check if signal dispatch is enabled."""
        return self.get_setting('SIGNAL_DISPATCH')
