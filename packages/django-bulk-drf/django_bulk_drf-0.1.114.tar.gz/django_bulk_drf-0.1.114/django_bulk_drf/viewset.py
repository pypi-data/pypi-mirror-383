...
class BulkModelViewSet:
    ...
    def _handle_array_upsert(self, payload, update_fields, unique_fields):
        ...
        if not update_fields:
            serializer_class = self.get_serializer_class()
            model_class = serializer_class.Meta.model
            update_fields = self._get_update_fields(payload, unique_fields, model_class)
    ...
    def _validate_and_prepare_instances(self, payload):
        ...
        if list_serializer.is_valid():
            # Use standard DRF API - .save() calls .create() which returns transformed instances
    ...
    def _get_update_fields(self, payload, unique_fields, model_class):
