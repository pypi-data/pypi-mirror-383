...
class BulkTransactionManager:
    ...
    def handle_partial_failure(self, successful, failed, errors):
        ...
        if self.failure_strategy == PartialFailureStrategy.ROLLBACK_ALL:
            ...
            logger.warning(
                f'Partial failure detected: {len(successful)} succeeded, {len(failed)} failed. '
            ...
            logger.info(
                f'Partial failure: {len(successful)} succeeded, {len(failed)} failed. '
                f'Committing successful operations.'
            )
            ...
            logger.info(
                f'Continuing despite failures: {len(successful)} succeeded, {len(failed)} failed.'
            )
            return successful, failed, True
