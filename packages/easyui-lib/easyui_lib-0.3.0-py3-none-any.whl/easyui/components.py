...
def make_font(family: str, size: int, bold: bool = False, italic: bool = False, underline: bool = False) -> tkfont.Font:
    """
    ...
    return tkfont.Font(family=family, size=size, weight=weight, slant=slant, underline=underline)
...
class UIComponent(ABC):
    ...
    def width(self, value): self._configure(width=value); return self
    def height(self, value): self._configure(height=value); return self
    def padding(self, value): self._configure(padx=value, pady=value); return self
    def margin(self, value): self._configure(padx=value, pady=value); return self
    def background(self, color): self._configure(bg=color); return self
    def color(self, color): self._configure(fg=color); return self
    def font_size(self, size): self._configure(font=('Arial', size)); return self
    def font_family(self, family): self._configure(font=(family, 12)); return self
    def font_weight(self, weight): self._configure(font=('Arial', 12, weight)); return self
    def border(self, width): self._configure(borderwidth=width); return self
    def text_align(self, align): self._configure(anchor=align); return self
    def position(self, x, y): self.place(x=x, y=y); return self
...
class Input(UIComponent):
    ...
    def _configure(self, **kwargs):
        ...
        if 'fg' not in kwargs and self.widget.get() and hasattr(self, '_placeholder_text'):
...
class Image(UIComponent):
    ...
    def __init__(self, path: str, width: Optional[int] = None, height: Optional[int] = None, keep_aspect: bool = True, **kwargs):
        self.path = path
...
class Icon(UIComponent):
    ...
    def __init__(self, name: Optional[str] = None, path: Optional[str] = None, size: Optional[int] = 16, **kwargs):
        self.name = name
        self.path = path
        self.size = size
        self._img_ref = None
    ...
    def _load_icon(self):
        ...
        if self.size:
            img = img.resize((self.size, self.size), getattr(PILImage, "LANCZOS", PILImage.BICUBIC))
        self._img_ref = ImageTk.PhotoImage(img)
        self.widget.configure(image=self._img_ref)
...
class ListView(UIComponent):
    ...
    def __init__(self, items: Optional[List[str]] = None, selectmode: str = "browse", **kwargs):
...
class Table(UIComponent):
    ...
    def __init__(self, columns: List[str] = None, data: Optional[List] = None,
                 show_row_numbers: bool = False, sortable: bool = True,
    ...
    def _setup_widget(self):
        ...
        for col in self.columns:
            self.widget.heading(col, text=col, command=lambda c=col: self._sort_by_column(c))
    ...
    def _sort_by_column(self, column):
        ...
        data.sort(key=lambda x: x[col_index] if col_index < len(x) else "", reverse=self._sort_reverse)
    ...
    def _on_double_click(self, event):
        ...
        if item and column:
            ...
            current_value = self.widget.item(item, 'values')[self.columns.index(column)-1 if self.show_row_numbers else self.columns.index(column)]
            ...
            self._edit_entry.bind('<Return>', lambda e: self._confirm_edit(item, column))
