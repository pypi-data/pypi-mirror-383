...
class Window:
    ...
    def show_async(self):
        """
        Muestra la ventana en un hilo separado con soporte para tareas en background.
        """
        from .threading import run_in_background

        def run_ui():
            # Configurar procesamiento periódico de la cola de UI
            def process_queue():
                from .threading import process_ui_queue

                process_ui_queue()
                self.root.after(10, process_queue)  # Procesar cada 10ms
        ...
        run_in_background(run_ui, daemon=False)
