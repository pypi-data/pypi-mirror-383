...
from typing import List, Dict, Any, cast, Set, Callable
...
from sirius.market_data import Stock, Option, MarketData
...
class IBKRMarketData(MarketData):
    ...
    async def __get_ohlc_data(contract_id: int, from_timestamp: datetime.datetime, to_timestamp: datetime.datetime) -> List[Dict[str, float]]:
        ...
        try:
            response = await session.get(
                ...
                query_params={
                    ...
                    "period": "999d",
                    "bar": "1d",
                    "startTime": to_timestamp.strftime(DATE_FORMAT),
                    "direction": "-1"
                }
            )
        ...
        response_from_time = datetime.datetime.strptime(response.data["startTime"], DATE_FORMAT)

        if response_from_time > from_timestamp:
            more_data = await IBKRMarketData.__get_ohlc_data(contract_id, from_timestamp, response_from_time)
    ...
    async def _get(abstract_stock: Stock, from_timestamp: datetime.datetime, to_timestamp: datetime.datetime) -> List["MarketData"]:
        stock: IBKRStock = cast(IBKRStock, abstract_stock if isinstance(abstract_stock, IBKRStock) else (await IBKRStock.find(abstract_stock.ticker))[0])
        raw_ohlc_data_list: List[Dict[str, float]] = await IBKRMarketData.__get_ohlc_data(stock.contract_id, from_timestamp, to_timestamp)
