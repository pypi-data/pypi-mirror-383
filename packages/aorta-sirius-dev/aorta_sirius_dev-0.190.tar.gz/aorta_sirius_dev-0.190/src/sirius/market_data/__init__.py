...
class Stock(Asset, ABC):
    ...
    @classmethod
    async def get(cls, ticker: str) -> "Stock":
        stock: Stock = await cls._find(ticker)
        if not stock:
...
class StockPerformanceAnalysis(DataClass):
    ...
    async def get(stock: Stock, number_of_days_invested: int, number_of_days_to_analyse: int, analysis_end_timestamp: datetime.datetime | None = None) -> "StockPerformanceAnalysis":
        ...
        analysis_dates_list: List[datetime.datetime] = [analysis_end_timestamp - datetime.timedelta(days=day) for day in range(number_of_days_to_analyse)]
