...
from dooservice.instance.infrastructure.driving_adapter.cli.interactive_selector import (  # noqa: E501
    InteractiveSelector,
)
...
def list(config: str):
    ...
    try:
        ...
        if instances:
            selector.display_instances_table(instances)
        else:
...
def create(name: Optional[str], interactive: bool, config: str):
    ...
    if interactive or not name:
        try:
            ...
            instances = asyncio.run(list_use_case.execute(config_data))
            name = selector.select_instance(instances, only_created=False)
            if not name:
...
def start(name: Optional[str], interactive: bool, config: str):
    ...
    if interactive or not name:
        try:
            ...
            list_use_case = composer.list_instances_use_case
            instances = asyncio.run(list_use_case.execute(config_data))
            name = selector.select_instance(instances, only_created=True)
            if not name:
                click.echo("No instance selected")
                return
        except Exception as e:
