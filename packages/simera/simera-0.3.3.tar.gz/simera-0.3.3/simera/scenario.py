...
class Scenario:
    ...
    def solve(self, keep_cost_details: bool = True):
        ...
        for activity, activity_objects in self.activity_objects.items():
            ...
            cost_summary_df, activities_with_cost, activities_without_ratesheets = self.calculate_cost(activity_objects, activity_ratesheets, must_match=must_match, keep_cost_details=keep_cost_details)
    ...
    @staticmethod
    def calculate_cost(activities_input, ratesheets_input, must_match: list[str] | None = None, keep_cost_details=True):
        """
        ...
        if keep_cost_details:
            ...
            for activity, rs in tqdm(
                    valid_combinations,
                    desc=f"    Calculating activity-ratesheet costs (keep=True)",
                    mininterval=1,
            ):
            ...
            activities_with_cost = 'keep_cost_details=False'
            activities_summarise = []
            for activity, rs in tqdm(
                    valid_combinations,
                    desc=f"    Calculating activity-ratesheet costs (keep=False)",
                    mininterval=1,
            ):
                activities_summarise.append(Cost(activity, rs).cost_summary)
            print('    Creating cost summary dataframe... ', end='')
            cost_summary_df = pd.DataFrame([activity for activity in activities_summarise])
...
if __name__ == "__main__":
    ...
    scen0 = Scenario('00', 'Baseline', df, activity_config=activity_config_input, merge_activities_config=merge_activities_config, scenario_items=scenario_items, solve_immediately=True, merge_immediately=True, keep_cost_details=True)
