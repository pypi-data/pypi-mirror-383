...
from asyncflow import AsyncExecutor, ExecutorShutdownError
...
class TestAsyncExecutor:
    ...
    async def test_map_ordered(self):
        ...
        async with AsyncExecutor(max_workers=3) as executor:
            results = []
            async for result in executor.map(simple_task, range(10), preserve_order=True):
                results.append(result)
            assert results == [i * 2 for i in range(10)]

    @pytest.mark.asyncio
    async def test_map_unordered(self):
        """Test map without order preservation."""
        async with AsyncExecutor(max_workers=3) as executor:
            results = []
            async for result in executor.map(simple_task, range(10), preserve_order=False):
                results.append(result)
