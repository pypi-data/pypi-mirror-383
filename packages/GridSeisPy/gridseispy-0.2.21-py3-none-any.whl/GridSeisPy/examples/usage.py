...
def infer_pos_L(dens, wds, step, eps=1e-9):
    ...
    pos = i_left * step + wds - y[i_left]
    L = (i_right * step + y[i_right]) - pos

    # 可选：校验
    L_sum = step * dens.sum()
    assert L_sum == L, '数据计算错误!'
    return float(pos), float(L)
...
wds = 100
step = 50
assert not wds % step, '滑动窗口必须是步长的整数倍！'
dens = []
for i in range((1000-wds) // step):
    d = log[i*step:i*step+wds].mean()
    dens.append(d)

pos_, l_ = infer_pos_L(dens, wds, step)
print(dens)



data = io.loadgrid(r"E:\桌面\ModelMultPORE.dat", skiprows=1, dimens=(114, 559, 100))
data[..., 80:] = 0
io.savegrid(r"E:\桌面\ModelMultPORE_cut80.dat", data)
exit()
patn = r"G:\02proj\胜利剩余油项目-二期\08项目工区\ST2Q-cut_d.sgy"
sgy = SeisData(patn).load()
sgy[1, 1, 3]
sgy[:10, :10, 3]
exit()
