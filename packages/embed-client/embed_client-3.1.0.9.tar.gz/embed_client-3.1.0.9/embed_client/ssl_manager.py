...
class ClientSSLManager:
    ...
    def create_client_ssl_context(self) -> Optional[ssl.SSLContext]:
        ...
        try:
            # ✅ ИСПРАВЛЕНИЕ: Использовать правильный метод SecurityManager
            ssl_config = self.config.get("ssl", {})
            
            # Проверяем, есть ли SecurityManager и правильный метод
            if self.ssl_manager and hasattr(self.ssl_manager, 'create_ssl_context'):
                # ✅ ИСПРАВЛЕНИЕ: Используем правильный метод SecurityManager с context_type="client"
                return self.ssl_manager.create_ssl_context(context_type="client")
            else:
