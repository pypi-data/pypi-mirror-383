...
def write_spatialite_layer(
    ...
    with conn or commit_and_close(db=db_path, spatial=True) as conn:
        ...
        for col in cols:
            s += f"{col} TEXT PRIMARY KEY," if col == "id" else f"{col} INTEGER," if col in ["srid"] else f"{col} TEXT,"
        ...
        insert_sql = f"INSERT INTO {table_name}({','.join(cols + ['geo'])}) VALUES ({','.join(['?'] * (len(cols)))},GeomFromWKB(?, ?))"
