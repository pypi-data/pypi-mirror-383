...
if LooseVersion(h5py.__version__) >= LooseVersion("3.5.0"):  # new file locking
    ...
    logging.error(
        "Qkit HDF file handling: In h5py between 3.0 and 3.5, there are problems with file locking handling. Please update to h5py==3.5.0"
    )
...
class H5_file:
    ...
    def set_base_attributes(self):
        ...
        self.hf.attrs.create("qkit", b"1.0")  # qkit version
        ...
        self.hf.attrs.create("NeXus_version", b"4.3.0")

        self.entry = self.hf.require_group("entry")
        self.entry.attrs.create("NX_class", b"NXentry")
        ...
        self.dgrp.attrs.create("NX_class", b"NXdata")
        self.dgrp.attrs.create("NX_class", b"NXdata")
    ...
    def create_dataset(
        ...
        **kwargs
        ...
        if folder == "data":
            ...
            logging.error(
                "please specify either: folder = 'data' , folder = 'analysis' or folder ='view' "
            )
            raise ValueError
        ...
        if name in self.grp.keys():
            if folder == "data":
                logging.error(
                    "Dataset '%s' in 'data' already exists. Cannot overwrite datasets in 'data'!"
                    % (name)
                )
                raise ValueError
            else:
        ...
        if ds_type == ds_types["txt"]:
            ds = self.grp.create_dataset(
                name, shape, maxshape=maxshape, chunks=chunks, dtype=dtype
            )
        else:
            ds = self.grp.create_dataset(
                name,
        ...
        if len(ds.shape) == 2:
            ...
            if len(data) == 1 and pointwise:
                ...
                if reset:
                    ds[
                        dim0 - 1, :
                    ] = data  # reset overwrites last data series (last row matrix)
