...
class dataset_view:
    ...
    def __init__(
        ...
        if not x or not y:
            raise ValueError(
                __name__
                + ": Error creating view '{!s}' in file {!s}: x or y axis not given.".format(
                    name, hdf_file.hf.attrs["_filepath"]
                )
            )
    ...
    def _setup_metadata(self, init=True):
        ...
        ds.attrs.create(
            "xy_" + str(self.view_num),
            (str(self.x_object) + ":" + str(self.y_object)).encode(),
        )
        if self.error_object:
            ds.attrs.create(
                "xy_" + str(self.view_num) + "_error", str(self.error_object).encode()
            )
        ds.attrs.create("overlays", self.view_num)
        ds.attrs.create(
            "xy_" + str(self.view_num) + "_filter", str(self.filter).encode()
        )
