...
class Data:
    ...
    def __init__(self, name=None, mode="r+", copy_file=False):
        ...
        if os.path.isfile(self._name):
            ...
            try:
                ...
                logging.debug(
                    f"Could not add newly generated h5 File '{name}' to qkit.fid database: {e}"
                )
        else:
            self._filepath = os.path.abspath(self._name)
            self._folder, self._filename = os.path.split(self._filepath)
        ...
        try:
            ...
            raise OSError(
                "File does not exist. Use argument \"mode='a'\" to create a new h5 file."
            )
        if self.hf.newfile:
    ...
    def add_comment(self, comment, folder="data"):
        if folder == "data":
            ...
            logging.warning(
                "Foler muset be either 'data' (default) or 'analysis': '%s' provided"
                % (folder)
            )
            raise ValueError
