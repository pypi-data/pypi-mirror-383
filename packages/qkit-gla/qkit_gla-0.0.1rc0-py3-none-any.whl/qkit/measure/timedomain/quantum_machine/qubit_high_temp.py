...
class QubitHighTemperatureExperiments(QmQkitWrapper):
    ...
    def adc_data(self, plot=True):
        ...
        self.coords = {
            "time_sig": [np.arange(adcs.size), "ns"],
            "time_ref": [np.arange(adcr.size), "ns"],
        }
        self.values = {
            "adcs": [adcs, ["time_sig"], "V"],
            "adcr": [adcr, ["time_ref"], "V"],
        }
    ...
    def iq_clouds(self, n_points, plot=True):
        with program() as clouds:
            ...
            with for_(N, 0, N < n_points, N + 1):
                ...
                align("signal", "reference")
                measure(
                    "readout_sig", "signal", None, ("integW_Is", Is), ("integW_Qs", Qs)
                )
                measure(
                    "readout_ref",
                    "reference",
    ...
    @QmQkitWrapper.measure
    def power_rabi(
        self, a_vec, n_pulses=1, threshold=0.0, state=False, avg=100, plot=True
    ):
        a_list = a_vec.tolist()
        ...
        with program() as powerRabi:
            ...
            with for_(N, 0, N < avg, N + 1):
                with for_each_(a, a_list):
                    ...
                    if state:
                        play(
                            "pi_pulse", "qubit", condition=I > threshold
                        )  # excited state
                    else:
    ...
    def pi_pulse_calibration(self, n_pulses, dn=1, avg=100, plot=True):
        ...
        self.I = np.reshape(
            res.variable_results.I.values, (avg, (n_pulses - 1) // dn + 1)
        )
        self.Q = np.reshape(
            res.variable_results.Q.values, (avg, (n_pulses - 1) // dn + 1)
        )
        self.I_avg = np.mean(self.I, axis=0)
        self.Q_avg = np.mean(self.Q, axis=0)
    ...
    def t1_qndness(self, n_measure, t_vec, avg=100, plot=True):
        ...
        ts_nsec_I = np.reshape(
            res.variable_results.I.ts_nsec, (avg, t_vec.size, n_measure)
        )
        ts_nsec_Q = np.reshape(
            res.variable_results.Q.ts_nsec, (avg, t_vec.size, n_measure)
        )
