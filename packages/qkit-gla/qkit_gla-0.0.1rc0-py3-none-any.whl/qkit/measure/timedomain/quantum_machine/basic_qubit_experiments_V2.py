...
class BasicQubitExperiments(QmQkitWrapper):
    def __init__(self, qm_config):
        ...
        self.wait_before = (
            250000  # time 4 for ns   # TODO wir nicht automatisch abgespeichert
        )
    ...
    def adc_data_test(self, f_list, avg=1000):
        ...
        self.values = {
            "I": [self.I.T, ["QB_IF_freq", "avg"], "V"],
            "Q": [self.Q.T, ["QB_IF_freq", "avg"], "V"],
            "I_avg": [self.I_avg, ["QB_IF_freq"], "V"],
            "Q_avg": [self.Q_avg, ["QB_IF_freq"], "V"],
        }
    ...
    def adc_data(self, plot=True):
        ...
        self.coords = {
            "time_i": [np.arange(adci.size), "ns"],
            "time_q": [np.arange(adcq.size), "ns"],
        }
    ...
    def qubit_spectroscopy(self, f_vec, a=1.0, avg=100, plot=True):
        if np.min(f_vec) < 50e6 or np.max(f_vec) > 100e6:
            ...
            f_vec = f_vec[b]
            print(
                "IF frequency needs to be between 50 and 100 MHz. \nFrequency vector was adjusted."
            )
        ...
        self.I = np.reshape(
            job.result_handles.get("I").fetch_all()["value"], (avg, len(f_list))
        )
        self.Q = np.reshape(
            job.result_handles.get("Q").fetch_all()["value"], (avg, len(f_list))
        )
        self.I_avg = np.mean(self.I, axis=0)
        self.Q_avg = np.mean(self.Q, axis=0)
        ...
        with program() as qbSpec:
            ...
            with for_(N, 0, N < avg, N + 1):
                with for_each_(f, f_list):
                    ...
                    assign(
                        I,
                        (Is << bit_shift) * (Ir << bit_shift)
                        + (Qs << bit_shift) * (Qr << bit_shift),
                    )
                    assign(
                        Q,
                        (Is << bit_shift) * (Qr << bit_shift)
                        - (Qs << bit_shift) * (Ir << bit_shift),
                    )
    ...
    def power_rabi(self, a_vec, n_pulses=1, avg=100, plot=True):
        ...
        # Get data
        self.I = np.reshape(
            job.result_handles.get("I").fetch_all()["value"], (avg, len(a_list))
        )
        self.Q = np.reshape(
            job.result_handles.get("Q").fetch_all()["value"], (avg, len(a_list))
        )
        self.I_avg = np.mean(self.I, axis=0)
        self.Q_avg = np.mean(self.Q, axis=0)
        ...
        self.values = {
            "I": [self.I.T, ["amp", "avg"], "V"],
            "Q": [self.Q.T, ["amp", "avg"], "V"],
            "I_avg": [self.I_avg, ["amp"], "V"],
            "Q_avg": [self.Q_avg, ["amp"], "V"],
        }
    ...
    def pi_pulse_calibration(self, n_pulses, dn=1, avg=100, plot=True):
        ...
        # Get data
        self.I = np.reshape(
            job.result_handles.get("I").fetch_all()["value"],
            (avg, (n_pulses - 1) // dn + 1),
        )
        self.Q = np.reshape(
            job.result_handles.get("Q").fetch_all()["value"],
            (avg, (n_pulses - 1) // dn + 1),
        )
        self.I_avg = np.mean(self.I, axis=0)
        self.Q_avg = np.mean(self.Q, axis=0)
        ...
        self.coords = {
            "pi_pulses": [np.arange(0, n_pulses, dn), "1"],
            "avg": [np.arange(avg), "#"],
        }
        self.values = {
            "I": [self.I.T, ["pi_pulses", "avg"], "V"],
            "Q": [self.Q.T, ["pi_pulses", "avg"], "V"],
            "I_avg": [self.I_avg, ["pi_pulses"], "V"],
            "Q_avg": [self.Q_avg, ["pi_pulses"], "V"],
        }
    ...
    def prepare_waiting_list(self, t_vec):
        ...
        if np.min(t_vec) < 4:
            ...
            t_vec[b] = 4
            print(
                "Minimum waiting time 4 clock cycles, which corresponds to 16ns. \nWaiting time vector was adjusted"
            )
    ...
    def t1(
                ...
                with for_each_(t, t_list):
                    ...
                    if active_reset:
                        ...
                        measure(
                            ...
                            None,
                            dual_demod.full(
                                "integ_w1_I", "out1", "integ_w2_I", "out2", I
                            ),
                            dual_demod.full(
                                "integ_w1_Q", "out1", "integ_w2_Q", "out2", Q
                            ),
                        )
        ...
        # Get data
        self.I = np.reshape(
            job.result_handles.get("I").fetch_all()["value"], (avg, t_vec.size)
        )
        self.Q = np.reshape(
            job.result_handles.get("Q").fetch_all()["value"], (avg, t_vec.size)
        )
        self.I_avg = np.mean(self.I, axis=0)
        self.Q_avg = np.mean(self.Q, axis=0)
        ...
        self.values = {
            "I": [self.I.T, ["t_wait", "avg"], "V"],
            "Q": [self.Q.T, ["t_wait", "avg"], "V"],
            "I_avg": [self.I_avg, ["t_wait"], "V"],
            "Q_avg": [self.Q_avg, ["t_wait"], "V"],
        }
    ...
    def t2(self, t_vec, echo=0, avg=100, plot=True):
        ...
        if echo == 0:
            ...
            self.exp_name = f"T2_echo_{echo}_pulses"
        self.coords = {"t_wait": [4 * t_vec / 1000, "us"], "avg": [np.arange(avg), "#"]}
        self.values = {
            "I": [self.I.T, ["t_wait", "avg"], "V"],
            "Q": [self.Q.T, ["t_wait", "avg"], "V"],
    ...
    def t2_cpmg(self, t, dt2, zrot=np.pi / 2, n_points=100, avg=100, plot=True):
        ...
        # qkit data storage
        self.exp_name = f"T2_cpmg_{4 * dt / 1000:.3f}us".replace(".", "p")
        self.coords = {"t_wait": [4 * t_vec / 1000, "us"], "avg": [np.arange(avg), "#"]}
        self.values = {
            "I": [self.I.T, ["t_wait", "avg"], "V"],
            "Q": [self.Q.T, ["t_wait", "avg"], "V"],
    ...
    def t1_qndness(self, n_measure, t_vec, avg=100, plot=True):
        ...
        ts_nsec_I = np.reshape(
            job.result_handles.get("I").fetch_all()["timestamp"],
            (avg, t_vec.size, n_measure),
        )
        ts_nsec_Q = np.reshape(
            job.result_handles.get("Q").fetch_all()["timestamp"],
            (avg, t_vec.size, n_measure),
        )
        ...
        self.coords = {
            "avg": [np.arange(avg), "#"],
            "t_wait": [4 * t_vec / 1000, "us"],
            "n_measure": [np.arange(n_measure), "#"],
        }
        self.values = {
            "I": [np.moveaxis(self.I, 0, 2), ["t_wait", "n_measure", "avg"], "V"],
    ...
    @QmQkitWrapper.measure
    def active_reset_from_thermal_state(
        self, state, threshold, n_points=1000, plot=True
    ):
        with program() as activeReset:
            ...
            with for_(N, 0, N < n_points, N + 1):
                ...
                wait(
                    self.wait_end, "resonator"
                )  # wait until qubit is thermally populated again
        ...
        self.coords = {"n_points": [np.arange(n_points), "#"]}
        self.values = {
            "I": [self.I.T, ["n_points"], "V"],
            "Q": [self.Q.T, ["n_points"], "V"],
        }
    ...
    def t1_stabilized_V2(
        ...
        # qkit data storage
        self.exp_name = f"T1_stabilized_{n_stab}_{stab_state}_{start_state}_{4 * t_wait / 1000}us".replace(".", "p")
        self.coords = {
            "t_wait": [4 * t_vec / 1000, "us"],
            "avg": [np.arange(avg), "#"],
    ...
    @QmQkitWrapper.measure
    def t1_measurement_heat(
        self, t_vec, n_stab, threshold, start_state=False, avg=1000, plot=True
    ):
        t_vec = self.prepare_waiting_list(t_vec)
        t_list = t_vec.tolist()
        ...
        # qkit data storage
        self.exp_name = f"T1_measurement_heat_{n_stab}_{start_state}"
        self.coords = {
            "t_wait": [4 * t_vec / 1000, "us"],
            "avg": [np.arange(avg), "#"],
