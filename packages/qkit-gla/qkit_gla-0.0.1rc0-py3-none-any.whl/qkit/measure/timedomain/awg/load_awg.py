...
def update_sequence(
    ...
    if show_progress_bar:
        p = Progress_Bar(
            len(ts) * (2 if "Tektronix" in awg.get_type() else 1), "Load AWG"
        )  # init progress bar
        ...
        for chan in [0, 1]:
            ...
            if "Tektronix" in awg.get_type():
                wfm_fn[chan] = "ch%d_t%05d" % (
                    chan + 1,
                    ti,
                )  # filename is kept until changed
                if len(wfm_samples) == 1 and chan == 1:
                    wfm_pn[chan] = f"{drive}{path}\\{np.zeros_like(wfm_fn[0])}"  # create empty array
                else:
                    wfm_pn[chan] = f"{drive}{path}\\{wfm_fn[chan]}"
