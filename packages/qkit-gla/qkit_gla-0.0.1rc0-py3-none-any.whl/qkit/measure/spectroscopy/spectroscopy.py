...
class spectrum:
    ...
    def __init__(self, vna, exp_name="", sample=None):
        self.vna = vna
        self.averaging_start_ready = (
            "start_measurement" in self.vna.get_function_names()
            and "ready" in self.vna.get_function_names()
        )
        if not self.averaging_start_ready:
            logging.warning(
                __name__
                + ": With your VNA instrument driver ("
                + self.vna.get_type()
        ...
        self.vna_poll_interval = (
            0.1  # interval in seconds in which the vna is queried to be ready.
        )
    ...
    def set_y_parameters(
        self, y_vec, y_coordname, y_set_obj, y_unit="", delete_landscape=True
    ):
        """
        Sets parameters for sweep. In a 3D measurement, the x-parameters will be the "outer" sweep.
        For every x value all y values are swept
        ...
        if self.landscape.xzlandscape_func and delete_landscape:
            self.landscape.delete_landscape_function_xz()
            logging.warning(
                "xz landscape has been deleted. You can switch this off if you whish."
            )
    ...
    def _prepare_measurement_file(self):
        ...
        if not self._scan_time:
            ...
            self._data_time.add(
                np.arange(0, self._nop, 1) * self.vna.get_sweeptime() / (self._nop - 1)
            )
            sweep_vector = self._data_time

        if self._scan_dim == 1:
            self._data_real = self._data_file.add_value_vector(
                "real", x=sweep_vector, unit="", save_timestamp=True
            )
            self._data_imag = self._data_file.add_value_vector(
                "imag", x=sweep_vector, unit="", save_timestamp=True
            )
            self._data_amp = self._data_file.add_value_vector(
                "amplitude", x=sweep_vector, unit="arb. unit", save_timestamp=True
            )
            self._data_pha = self._data_file.add_value_vector(
                "phase", x=sweep_vector, unit="rad", save_timestamp=True
            )
        ...
        if self._scan_dim == 2:
            ...
            self._data_pha = self._data_file.add_value_matrix(
                "phase", x=self._data_x, y=sweep_vector, unit="rad", save_timestamp=True
            )
            ...
            if self._nop < 10:
                ...
                self._views = [
                    self._data_file.add_view(
                        ...
                        view_params=dict(
                            transpose=True,
                            default_trace=self._nop // 2,
                            linecolors=[(200, 200, 100)],
                        ),
                    ),
                    self._data_file.add_view(
                        "phase_midpoint",
        ...
        if self._scan_dim == 3:
            ...
            if (
                self._nop == 0
            ):  # saving in a 2D matrix instead of a 3D box HR: does not work yet !!! test things before you put them online.
                self._data_amp = self._data_file.add_value_matrix(
                    "amplitude",
                    x=self._data_x,
                    y=self._data_y,
            ...
            if self.log_function_2D != None:  # use 2D logging
                ...
                for i in range(len(self.log_y_name_2D)):
                    if (
                        self.log_y_name_2D[i] not in self.log_y_name_2D[:i]
                    ):  # add y coordinate for 2D logging
                        self._log_y_value_2D.append(
                            self._data_file.add_coordinate(
                                self.log_y_name_2D[i],
                                unit=self.log_y_unit_2D[i],
                                folder="data",
                            )
                        )  # possibly use "data1"
                ...
                for i in range(len(self.log_function_2D)):
                    self._log_value_2D.append(
                        self._data_file.add_value_matrix(
                            self.log_name_2D[i],
                            x=self._data_x,
                            y=self._log_y_value_2D[i],
                            unit=self.log_unit_2D[i],
    ...
    def measure_1D(self, rescan=True, web_visible=True):
        ...
        if self.open_qviewkit:
            self._qvk_process = qviewkit.plot(
                self._data_file.get_filepath(), datasets=["amplitude", "phase"]
            )
            ...
            if self.averaging_start_ready:
                ...
                if self.progress_bar:
                    self._p = Progress_Bar(
                        self.vna.get_averages(), self.dirname, self.vna.get_sweeptime()
                    )
                qkit.flow.sleep(0.2)
                ...
                self.vna.avg_clear()
                if (
                    self.vna.get_averages() == 1 or self.vna.get_Average() == False
                ):  # no averaging
                    if self.progress_bar:
                        self._p = Progress_Bar(
                            1, self.dirname, self.vna.get_sweeptime()
                        )
        ...
        logging.info(
            "Measurement finished, data saved in " + self._data_file.get_filepath()
        )

        file = self._end_measurement()
        if self._fit_resonator:  # self._do_fit_resonator()
            self.cir = circuit.notch_port(
                self._freqpoints, data_amp * np.exp(1j * data_pha)
            )
    ...
    def measure_2D(self, web_visible=True):
        ...
        if not self.x_set_obj:
            ...
            return
        if (
            self.landscape.xzlandscape_func
        ):  # The vna limits need to be adjusted, happens in the frequency wrapper
        ...
        """opens qviewkit to plot measurement, amp and pha are opened by default"""
        if self._nop < 10:
            self._data_file.hf.hf.attrs["default_ds"] = [
                "views/amplitude_midpoint",
                "views/phase_midpoint",
            ]
        else:
            self._data_file.hf.hf.attrs["default_ds"] = [
                "data0/amplitude_midpoint",
                "data0/phase_midpoint",
            ]
        ...
        # Execute measurement and collect results
        self._measure_internal()

        # Return the filepath after all measurements are complete
    ...
    def measure_3D(self, web_visible=True):
        ...
        if len(self.x_vec) * len(self.y_vec) == 0:
            logging.error(
                "No points to measure given. Check your x ad y vector... aborting"
            )
            return
        if (
            self.landscape.xzlandscape_func
        ...
        if self.progress_bar:
            if self.landscape.xylandscapes:
                truth = np.full(
                    (len(self.y_vec), len(self.x_vec)), False
                ...
                for e in self.landscape.xylandscapes:
                    if not e["blacklist"]:
                        truth = np.logical_or(
                            truth,
                            (
                                np.abs(self.y_vec[:, np.newaxis] - e["center_points"])
                                <= e["y_span"] / 2
                            )
                            * (e["x_range"][0] <= self.x_vec)  # check y span
                            * (self.x_vec <= e["x_range"][1]),
                        )  # check x range

                for e in self.landscape.xylandscapes:
                    if e["blacklist"]:  # exclude blacklisted areas
                        truth = np.logical_and(
                            truth,
                            np.logical_not(
                                (
                                    np.abs(
                                        self.y_vec[:, np.newaxis] - e["center_points"]
                                    )
                                    <= e["y_span"] / 2
                                )
                                * (e["x_range"][0] <= self.x_vec)
                                * (self.x_vec <= e["x_range"][1])
                            ),
                        )
                points = np.sum(truth)
    ...
    def _measure_internal(self):
        """
        Internal method that performs the actual measurement but doesn't return anything.
        This allows measure_2D to control when the file path is returned.
        """
        qkit.flow.start()
        try:
            """
            ...
            for ix, x in enumerate(self.x_vec):
                ...
                if self.log_function != None:
                    for i, f in enumerate(self.log_function):
                        self._log_value[i].append(float(f(ix)))
                ...
                if self._scan_dim == 3:
                    for y in self.y_vec:
                        ...
                        if (
                            self.landscape.xylandscapes
                            and not self.landscape.perform_measurement_at_point(
                                x, y, ix
                            )
                        ):
                            ...
                            data_pha = np.full(
                                int(self._nop), np.NaN, dtype=np.float16
                            )  # fill with NaNs
                        else:
                            self.y_set_obj(y)
                            sleep(self.tdy)
                            if self.averaging_start_ready:
                                ...
                                while not self.vna.ready():
                                    qkit.flow.sleep(
                                        min(
                                            self.vna.get_sweeptime_averages(query=False)
                                            / 11.0,
                                            self.vna_poll_interval,
                                        )
                                    )
                ...
                if self._scan_dim == 2:
                    if self.averaging_start_ready:
                        ...
                        while not self.vna.ready():
                            qkit.flow.sleep(
                                min(
                                    self.vna.get_sweeptime_averages(query=False) / 11.0,
                                    0.2,
                                )
                            )
                    else:
            ...
            # Just end the flow, don't end the measurement or return anything
            qkit.flow.end()
    ...
    def _measure(self):
        ...
        """
        # Call the internal measurement function
        self._measure_internal()

        # End the measurement and return the filepath
        file_path = self._end_measurement()

        return file_path  # return the filepath of the measurement file
    ...
    def set_resonator_fit(
        self, fit_resonator=True, fit_function="", f_min=None, f_max=None
    ):
        """
    ...
    def _do_fit_resonator(self):
        ...
        if self._fit_function == 0:  # lorentzian
            ...
            self._resonator.fit_circle(
                reflection=True, f_min=self._f_min, f_max=self._f_max
            )
        ...
        # self._resonator.fit_all_fits(f_min=self._f_min, f_max = self._f_max)
            ...
            logging.error(f"Fit function {self._fit_function:d} not supported")
    ...
    def gen_fit_function(
        self, curve_f, curve_p, x_range=None, span=None, p0=[-1, 0.1, 7], units=""
    ):
        """
        ...
        """
        logging.warning(
            "This function is deprecated. Better to use landscape.generate_fit_function_xy"
        )
        self.landscape.generate_fit_function_xy(
            curve_f=curve_f,
            curve_p=curve_p,
            x_range=x_range,
            y_span=span,
    ...
    def delete_fit_function(self, n=None):
        ...
        """
        logging.warning(
            "This function is deprecated for clarity purposes."
    ...
    def measure_timetrace(self):
        logging.error(
            "This function does no longer exist. Use set_cw_time_mesurement() "
...
class Landscape:
    ...
    def __init__(self, vna, spec):
        ...
        self.z_span = (
            self.vna.get_span()
        )  # This is for the xz landscape scan i.e. span of vna is adjusted w/ resp to x

    def generate_fit_function_xy(
        self,
        curve_f,
        curve_p,
        x_range=None,
        ...
        if y_span < 0:
            ...
            blacklist = True
            logging.warning(
                "Using a negative span in generate_fit_function_xy is deprecated. Please use blacklist = True instead for more clarity."
            )
    ...
    def generate_fit_function_xz(self, curve_f, curve_p, z_span=None, p0=[-1, 0.1, 7]):
        ...
        try:
            ...
            self.xz_freqpoints = np.arange(
                np.min(self.xzlandscape_func(self.spec.x_vec)) - self.z_span / 2,
    ...
    def plot_xy_landscape(self):
        ...
        if self.xylandscapes:
            for i in self.xylandscapes:
                try:
                    arg = np.where(
                        (i["x_range"][0] <= self.spec.x_vec)
                        & (self.spec.x_vec <= i["x_range"][1])
                    )
            ...
            plt.axhspan(
                np.min(self.spec.y_vec),
                np.max(self.spec.y_vec),
                facecolor="0.5",
                alpha=0.5,
            )
    ...
    def perform_measurement_at_point(self, x, y, ix):
        ...
        for e in self.xylandscapes:
            if (
                np.abs(e["center_points"][ix] - y) <= e["y_span"] / 2
                and e["x_range"][0] <= x <= e["x_range"][1]
            ):  # The point is covered by this span
                if e["blacklist"]:
    ...
    def get_tracedata_xz(self, x):
        ...
        startarg = np.argmin(
            np.abs(self.xz_freqpoints - (self.xzlandscape_func(x) - self.z_span / 2))
        )
    ...
    def f_transmon(self, x, w_max, L, I_ext, djj):
        ...
        return (
            w_max
            * (
                np.abs(np.cos(np.pi / L * (x - I_ext)))
                * (1 + djj**2 * np.tan(np.pi / L * (x - I_ext)) ** 2) ** 0.5
            )
            ** 0.5
        )
