...
class pointtracker:
    ...
    def _track_points(self, search_indices, direction):
        ...
        if (
            search_indices[0] < 0
            or search_indices[0] >= len(self.xdata)
            or search_indices[1] - self.span / 2 < 0
            or search_indices[1] + self.span / 2 >= len(self.ydata)
        ):
        ...
        search_data = (
            ...
            * self.data[
                ...
                int(search_indices[1] - self.span / 2) : int(
                    search_indices[1] + self.span / 2
                ),
            ]
        )

        indexes_found = peakutils.indexes(
            search_data, thres=self._thres, min_dist=self._min_dist
        )
        ...
        if direction == 0:
            ...
            self._track_points(
                [search_indices[0] + direction, search_indices[1]], direction=direction
            )
    ...
    def plot(self, all=True, amount=1, log=False):
        ...
        if log == False:
            ...
            plt.pcolormesh(
                ...
                self.data.T,
                cmap="viridis",
                norm=LogNorm(vmin=self.data.min(), vmax=self.data.max()),
            )
        ...
        for i in range(0, n):
            plt.plot(
                self.xdata[self.x_results[i]],
                self.ydata[self.y_results[i]],
                col[i] + "x",
                label="Trace %d" % (i),
            )
