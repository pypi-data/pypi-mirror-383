...
class QFIT:
    def __init__(self):
        self.cfg = {
            ...
            "spline_knots": 1.0e-3,  # spacing between knots used during spline fitting, smaller number = more accurate
            "figsize": (15, 7),  # figure size setting
            "debug": False,  # debug mode
            "analysis_folder": "analysis",  # folder in h5 file where data is to be stored
            "datafolder_structure": "day_time",
        }  # day_time or epoche_key
        ...
        try:
            ...
            if (
                qkit.cfg.get("datafolder_structure", 1) == 2
            ):  # new data structure with epoche key in qkit settings
                try:
                    ...
                    logging.error(
                        "No run_id and user entry found in qkit.cfg. Falling back to old style data folder structure."
                    )
            else:
    ...
    def __f_Lorentzian_sqrt(self, f, f0, k, a, offs):
        return (
            np.sign(a)
            * np.sqrt(np.abs(a**2 * (k / 2) ** 2 / ((k / 2) ** 2 + (f - f0) ** 2)))
            + offs
        )
    ...
    def __f_Skewed_Lorentzian(self, f, f0, k, a, offs, tilt, skew):
        return (
            (a + skew * (f - f0)) * k / (2 * np.pi) / ((k / 2) ** 2 + (f - f0) ** 2)
            + offs
            + tilt * (f - f0)
        )
    ...
    def __f_damped_sine(self, t, fs, Td, a, offs, ph):
        if a < 0:
            return np.NaN  # constrict amplitude to positive values
        if fs < 0:
            return np.NaN
        if Td < 0:
            return np.NaN
    ...
    def __f_exp_sine(self, t, fs, Td, a, offs, ph, d):
        return (
            a * np.exp(-t / Td) * 0.5 * (1 + d * np.cos(2 * np.pi * fs * t + ph)) + offs
        )
    ...
    def __get_parameters(self, fit_function):
        ...
        """
        return {
            self.__f_Lorentzian_sqrt: ["f0", "k", "a", "offs"],
    ...
    def read_hdf_data(self, return_data=False):
        ...
        if len(self.urls) == 2:  # for only two urls, it is clear what to do
            ...
            len(self.urls) > 2
        ):  # if more than two, read out the data_column set in the config
            self.coordinate = data[0]
            self.data = data[self.cfg["data_column"]]
    ...
    def find_latest_file(self, ftype):
        ...
        try:
            nfile = max(
                glob.iglob(str(self.data_dir) + "/*/*." + ftype), key=os.path.getctime
            )  # find newest file in directory
        except ValueError:
            logging.error(
                f"No .{str(ftype):s} file located in directory {str(self.data_dir):s}."
            )
            return

        self.file_name = str(nfile).replace("\\", "/")
        if self.cfg["show_output"]:
            print(f"Latest file: {self.file_name:s}")
    ...
    def discover_hdf_data(self, entries=None):
        ...
        if self.cfg["show_output"]:
            print("Available data entries:", keys)
            ...
            for k in keys:  # go through all keys
                try:
                    ...
                    if (
                        str(k[:4]).lower() == "freq"
                        or str(k[:2]).lower() == "f "
                        or str(k[:4]).lower() == "puls"
                        or str(k[:4]).lower() == "dacf"
                        or str(k[:5]).lower() == "delay"
                        or str(k[:8]).lower() == "pi pulse"
                    ):
                ...
                for e in entries:
                    for k in self.hf["/entry/" + et].keys():
                        try:
                            ...
                            logging.error(
                                f"Entries cannot be identified. No data for >> {str(e):s} << found. Aborting."
                            )
                            return
        ...
        if self.cfg["show_output"]:
            print("Entries identified:", self.urls)
    ...
    def spline_smooth(self):
        ...
        """
        self.data = scipy.interpolate.UnivariateSpline(
            self.coordinate, self.data, s=self.cfg["spline_knots"]
        )(self.coordinate)
    ...
    def rotate_IQ_plane(self, entries=None):
                ...
                dist_proj[i] = [
                    np.abs(
                        np.vdot(
                            ...
                            [np.real(cr) - np.real(c[i]), np.imag(cr) - np.imag(c[i])],
                        )
                    )
                    for cr in ci
                ]
        ...
        if self.cfg["show_complex_plot"] and self.cfg["matplotlib"]:
            if len(self.c_raw.shape) > 1:
                ...
                ax1.set_xlim(
                    np.min(data_opt) - 0.05 * prange, np.max(data_opt) + 0.05 * prange
                )

                ax2.plot(np.real(c), np.imag(c), ".")
                ax2.plot(
                    np.real(c)[:10], np.imag(c)[:10], ".", color="r"
                )  # show first 10 data points in red
                ax2.plot(
                    np.real(cmax), np.imag(cmax), "*", color="black", markersize=15
                )
        ...
        if self.urls is not None:
            ...
            try:
                ...
                hdf_error = self.hf.add_value_vector(
                    self.data_label + "_errors", folder=self.cfg["analysis_folder"]
                )
    ...
    def _store_fit_data(self, fit_params, fit_covariance):
        ...
        try:
            ...
            hdf_x = self.hf.add_coordinate(
                self.data_label + "_coordinate", folder=self.cfg["analysis_folder"]
            )
            ...
            if (
                self.data_label + "_data_opt"
                in self.hf[
                    os.path.join("/entry/", self.cfg["analysis_folder"] + "0").replace(
                        "\\", "/"
                    )
                ].keys()
            ):
                ...
                try:
                    ...
                    joint_view = self.hf.add_view(
                        self.data_label + "_opt_fit", x=hdf_x, y=hdf_y
                    )  # fit
                    joint_view.add(
                        x=self.hf.get_dataset(self.urls[0]),
                ...
                joint_view = self.hf.add_view(
                    self.data_label + "_fit", x=hdf_x, y=hdf_y
                )  # fit
                joint_view.add(
                    x=self.hf.get_dataset(self.urls[0]),
                    y=self.hf.get_dataset(self.data_url),
                )  # data

            self.hf.close_file()
        except NameError as m:
            logging.error(
                f"Error while attempting to save fit data in h5 file: {str(m):s}"
            )
    ...
    def _guess_lorentzian_parameters(self):
        s_offs = np.mean(
            np.append(
                self.data[: int(0.1 * len(self.data))],
                self.data[int(0.9 * len(self.data)) :],
            )
        )
        # offset is calculated from the first and last 10% of the data to improve fitting on tight windows @andre20150318
        if np.abs(np.max(self.data) - np.mean(self.data)) > np.abs(
            np.min(self.data) - np.mean(self.data)
        ):
            # expect a peak
            s_a = np.abs(np.max(self.data) - np.mean(self.data))
            s_f0 = (
                self.coordinate[np.where(self.data == max(self.data))[0][0]]
                * self.freq_conversion_factor
            )
            if self.cfg["debug"]:
            ...
            s_f0 = (
                self.coordinate[np.where(self.data == min(self.data))[0][0]]
                * self.freq_conversion_factor
            )
            if self.cfg["debug"]:
                print("expecting dip")
        ...
        for dat_p in range(len(self.data) - 1):
            if np.sign(self.data[dat_p] - mid) != np.sign(
                self.data[dat_p + 1] - mid
            ):  # mid level crossing
                m.append(dat_p)  # frequency of found mid point

        if len(m) > 1:
            s_k = (
                self.coordinate[m[-1]] - self.coordinate[m[0]]
            ) * self.freq_conversion_factor
            if self.cfg["show_output"]:
                print(f"assume k = {s_k:.3g}")
        else:
            s_k = (
                0.15
    ...
    def _guess_oscillating_parameters(self, damping, asymmetric_exp=False):
        ...
        if np.max(
            np.abs(np.diff(self.data[int(0.75 * len(self.data)) :]))
        ) < 0.3 * np.abs(np.max(self.data) - np.min(self.data)) / (
            len(self.coordinate[int(0.75 * len(self.coordinate)) :])
            * (self.coordinate[-1] - self.coordinate[int(0.75 * len(self.data))])
        ):  # if slope in last part small
        ...
        if damping:
            if asymmetric_exp:
                ...
                if np.abs(np.max(self.data) - s_offs) < np.abs(
                    np.min(self.data) - s_offs
                ):  # if maximum closer to offset than minimum
                    s_a = -s_a
            else:
            ...
            # damping
            a_end = np.abs(
                np.max(self.data[int(0.7 * len(self.data)) :])
            )  # scan last 30% of values -> final amplitude
            # print a_end
            # -> calculate Td
            ...
            if np.abs(s_Td) == float("inf") and not asymmetric_exp:
                ...
                if self.cfg["show_output"]:
                    logging.warning(
                        "Consider using the sine fit routine for non-decaying sines."
                    )
            if self.cfg["debug"]:
                print(f"assume T = {round(s_Td, 4):s}")
        ...
        for dat_p in range(len(self.data) - 1):
            if np.sign(self.data[dat_p] - s_offs) != np.sign(
                self.data[dat_p + 1] - s_offs
            ):  # offset line crossing
                roots += 1
        s_fs = float(roots) / (
            2 * self.coordinate[-1]
        )  # number of roots/2 /measurement time
        ...
        if dmax < dmean:  # start on upper side -> offset phase pi/2
            ...
            if (
                np.sum(np.gradient(self.data)[: int(0.05 * len(self.coordinate))]) > 0
            ):  # positive slope -> sin
                s_ph = 0
            else:  # negative slope -> -sin
                s_ph = np.pi
        if self.cfg["debug"]:
            print(f"assume phase = {s_ph:.3g}")
    ...
    def fit(self, p0=None):
        ...
        if np.mean(self.coordinate) > 1.0e6:
            self.freq_conversion_factor = 1e-9
            if self.cfg["show_output"]:
                logging.warning("Frequency given in Hz. Conversion to GHz applied.")
        else:
        ...
        if (
            self.fit_function == self.__f_Lorentzian
            or self.fit_function == self.__f_Lorentzian_sqrt
        ):
            self.guesses = self._guess_lorentzian_parameters()
        elif self.fit_function == self.__f_sine:
            self.guesses = self._guess_oscillating_parameters(
                damping=False, asymmetric_exp=False
            )
        elif self.fit_function == self.__f_damped_sine:
            self.guesses = self._guess_oscillating_parameters(
                damping=True, asymmetric_exp=False
            )
        elif self.fit_function == self.__f_exp_sine:
            self.guesses = self._guess_oscillating_parameters(
                damping=True, asymmetric_exp=True
            )
            d_diff = np.gradient(
                self.data, self.coordinate[1] - self.coordinate[0]
            )  # true gradient
            i = 0
            for i in range(len(self.coordinate)):
                if np.sign(d_diff[i]) != np.sign(
                    d_diff[i + 1]
                ):  # go to first sign change -> extremum
                    break
                i += 1
            if self.cfg["debug"]:
                print(f"first extremum detected at {self.coordinate[i]:.4g}")
            ...
            s_offs = np.mean(
                self.data[int(0.9 * len(self.data)) :]
            )  # average over the last 10% of entries
            s_a = self.data[0] - s_offs
            # calculate gradient at t=0 which is equal to (+-)a/T
            s_Td = np.abs(
                float(s_a)
                / np.mean(
                    np.gradient(self.data, self.coordinate[1] - self.coordinate[0])[:5]
                )
            )
            self.guesses = [s_Td, s_a, s_offs]
        elif self.fit_function == self.__f_Skewed_Lorentzian:
            self.guesses = np.append(
                self._guess_lorentzian_parameters(), [1e-3, 1e-2]
            ).tolist()
            self.p0 = _fill_p0(self.guesses, p0)
            try:
                self.popt, self.pcov = curve_fit(
                    self.__f_Lorentzian,
                    self.coordinate * self.freq_conversion_factor,
                    self.data,
                    p0=self.p0[:4],
                    maxfev=10000,
                )
            ...
            self.guesses = None
            logging.warning(
                "Fit function unknown. No guesses available but I will continue with the specified fit function."
            )
        self.p0 = _fill_p0(self.guesses, p0)
        ...
        try:
            ...
            if self.cfg["show_output"] and (
                self.fit_function == self.__f_Lorentzian
                or self.fit_function == self.__f_Lorentzian_sqrt
            ):
                print(
                    f"QL = {np.abs(np.round(float(self.popt[0]) / self.popt[1])):.4g}"
                )
        except:
            ...
            self.fvalues = self.fit_function(
                self.x_vec * self.freq_conversion_factor, *self.popt
            )
            # plot
            create_plots = self.cfg["matplotlib"] and (
                self.cfg["show_plot"]
                or (
                    (self.cfg["save_png"] or self.cfg["save_pdf"])
                    and self.file_name != "data_import"
                )
            )
            if create_plots:
                if self.fit_function == self.__f_exp:
                    ...
                    # log plot
                    self.axes[1].plot(
                        self.coordinate, np.abs(self.data - self.popt[2]), "o"
                    )  # subtract offset for log plot
                    self.axes[1].plot(
                        self.x_vec,
                        np.abs(self.__f_exp(self.x_vec, *self.popt) - self.popt[2]),
                    )
                    ...
                    )  # open and address plot instance
                    plt.plot(
                        self.coordinate * self.freq_conversion_factor, self.data, "o"
                    )
                    ...
                    self.fig.suptitle(
                        str(
                            [
                                f"{p:s} = {entry:.4g}"
                                for p, entry in zip(
                                    self.__get_parameters(self.fit_function), self.popt
                                )
                            ]
                        )
                    )
                ...
                if self.cfg["save_png"] and self.file_name != "data_import":
                    plt.savefig(
                        self.file_name.strip(".h5") + ".png",
                        dpi=self.cfg.get("dpi", 200),
                    )
                if self.cfg["save_pdf"] and self.file_name != "data_import":
                    plt.savefig(
                        self.file_name.strip(".h5") + ".pdf",
                        dpi=self.cfg.get("dpi", 200),
                    )
        ...
        if (
            self.pcov is not None and self.urls is not None
        ):  # if fit successful and data based on h5 file
            self._store_fit_data(
                fit_params=self.popt, fit_covariance=np.sqrt(np.diag(self.pcov))
            )
        if self.pcov is None:
            self.std = float("inf") * np.ones(
                len(self.popt)
            )  # fill up errors with 'inf' in case fit did not converge
        else:
