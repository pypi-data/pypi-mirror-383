...
class CurveXtractor:
    ...
    def __init__(self, xdata=[0], ydata=[0], zdata=[0], smoothness=0.1):
        ...
        if plt.get_backend().find("nbagg") == -1:
            print(
                "Module requires using the widget backend for matplotlib.\nActivate with %matplotlib widget."
            )
            return
        self.xvals = 0
        self.yvals = 0
    ...
    def _click_event(self, event):
        ...
        if self._status:
            if event.button == 1:
                ...
                if (
                    (event.xdata < self._xlim[-1])
                    and (event.xdata > self._xlim[0])
                    and (event.ydata < self._ylim[-1])
                    and (event.ydata > self._ylim[0])
                ):
            ...
            if len(self._curveplots) > 0:
                ...
                if self._status:
                    ...
                    for i in range(0, len(self._curve_x)):
                        temp_plot = plt.plot(
                            np.array(self._curve_x[i])[np.argsort(self._curve_x[i])],
    ...
    def set_points(self, show_info=True):
        ...
        self._cid = self._fig.canvas.mpl_connect(
            "button_press_event", self._click_event
        )
    ...
    def _addpoint(self, x, y):
        ...
        """
        temp_plot = plt.plot(
            [x], [y], self._col[len(self._curve_x) % len(self._col)] + "."
        )
    ...
    def _addcurve(self):
        ...
        """
        temp_plot = plt.plot(
            np.array(self._points_x)[np.argsort(self._points_x)],
        ...
        for i in range(0, len(self._curve_x)):
            if len(self._curve_x[i]) == 3:
                ...
                warning += (
                    "In curve "
                    + str(i)
                    + ", only 3 points are given.\n"
                    + "A parabola is used instead of a spline.\n\n"
                )
                g = (self._curve_y[i][0] - self._curve_y[i][1]) / (
                    self._curve_y[i][2] - self._curve_y[i][1]
                )
                x0 = 0.5 * (
                    (
                        self._curve_x[i][0] ** 2
                        - self._curve_x[i][1] ** 2 * (1 - g)
                        - g * self._curve_x[i][2] ** 2
                    )
                    / (
                        self._curve_x[i][0]
                        - self._curve_x[i][1]
                        - g * (self._curve_x[i][2] - self._curve_x[i][1])
                    )
                )
                m = (self._curve_y[i][1] - self._curve_y[i][0]) / (
                    (self._curve_x[i][1] - x0) ** 2 - (self._curve_x[i][0] - x0) ** 2
                )
                ...
                warning += (
                    "In curve "
                    + str(i)
                    + ", only 2 points are given.\n"
                    + "A straight line is used instead of a spline.\n\n"
                )
                m = (self._curve_y[i][1] - self._curve_y[i][0]) / (
                    self._curve_x[i][1] - self._curve_x[i][0]
                )
                ...
                try:
                    ...
                    try:
                        ...
                        warning += (
                            "Although points were removed from curve "
                            + str(i)
                            + " a spline could not be calculated.\n"
                        )
            ...
            temp_plot = plt.plot(
                self.xvals, fct(self.xvals), self._col[i % len(self._col)] + "--"
            )
            self._curveplots.append(temp_plot)
        if len(warning) > 0:
    ...
    def find_along_splines(self, width=0, peak=False):
        ...
        for i, x in enumerate(self.xvals):
            for j, fct in enumerate(self._splines):
                val = fct(x)
                ind_min = np.argmin(np.abs(self.yvals - (val - width / 2.0)))
                ind_max = np.argmin(np.abs(self.yvals - (val + width / 2.0)))
