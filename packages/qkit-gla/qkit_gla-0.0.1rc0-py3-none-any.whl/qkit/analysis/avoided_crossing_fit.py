...
class ACF:
            ...
            if callable(fct):
                ...
                try:
                    ...
                    print(
                        "In "
                        + fct.__name__
                        + ": No x argument found. Please rename axis variable to x."
                    )
    ...
    def _validity_check(self):
        ...
        for i in range(0, self._xlen):
            if len(self.xdata[i]) != len(self.ydata[i]):
                print(
                    "In xdata/ydata: Elements "
                    + str(i)
                    + " do not have the same lengths ("
                    + str(len(self.xdata[i]))
                    + ", "
                    + str(len(self.ydata[i]))
                    + ")."
                )
                valid = 0
    ...
    def _check_p0(self):
        ...
        # If number of initial parameters is to small/large -> cut/append.
            ...
            self.p0 = np.append(
                self.p0, [1] * int(sum(self._fct_par_nums) - self._p0len)
            )
            p0_reset = True
    ...
    def _least_square_val(self, pars):
        ...
        for i, fct in enumerate(self.functions):
            dev = np.append(
                dev, self.ydata[i] - self.crossing_fct(self.xdata[i], pars)[:, i]
            )
        ...
        for data_set in self.results:
            if (data_set[0] > fct_index) and (data_set[0] <= self._flen):
                print(
                    ...
                    + str(fct_index + 1)
                    + " ("
                    + self.functions[fct_index].__name__
                    + "):"
                )
                fct_index += 1
    ...
    def _plot(self, pars):
        ...
        if (self.xdata == 0) or (self.ydata == 0):
            print("No data for plot available.")
            return
        for i in range(0, self._xlen):
            plt.plot(
                self.xdata[i], self.ydata[i], dat_cols[i], marker=mrkr, linewidth=0
            )
        xlin = np.linspace(
            np.nanmin(np.concatenate(self.xdata)),
            np.nanmax(np.concatenate(self.xdata)),
            1000,
        )
        for i in range(self._flen):
            plt.plot(xlin, self.crossing_fct(xlin, pars)[:, i], fct_cols[i])
        plt.xlim(
            np.nanmin(np.concatenate(self.xdata)), np.nanmax(np.concatenate(self.xdata))
        )
        plt.ylim(
            np.nanmin(np.concatenate(self.ydata)), np.nanmax(np.concatenate(self.ydata))
        )
        plt.show()
        return
    ...
    def plot_results(self):
        ...
        if self.fit_pars == 0:
            print(
                "Fit was not yet performed. Use self.plot_init_pars() to look at your input."
            )
            return
