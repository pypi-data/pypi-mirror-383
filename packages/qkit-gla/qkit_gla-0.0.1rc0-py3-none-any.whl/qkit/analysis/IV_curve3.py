...
from qkit.measure.json_handler import QkitJSONDecoder, QkitJSONEncoder
...
def umean(a, **kwargs):
    ...
    std = np.float64(
        np.squeeze(
            np.sqrt(
                unp.std_devs(np.mean(a, **kwargs)) ** 2
                + sem(unp.nominal_values(a), **kwargs) ** 2
            )
        )
    )
    if isinstance(avg, np.floating) and isinstance(std, np.floating):
        return ufloat(nominal_value=avg, std_dev=std)
    elif isinstance(avg, np.ndarray) and isinstance(std, np.ndarray):
        return unp.uarray(nominal_values=avg, std_devs=std)
    else:
        raise TypeError(
            "average value and standard deviation do not have compatible dtypes."
        )
...
def unanmean(a, **kwargs):
    ...
    std = np.float64(
        np.squeeze(
            np.sqrt(
                unp.std_devs(np.nanmean(a, **kwargs)) ** 2
...
def uaverage(a, axis=None):
    ...
    avg = np.sum(w * unp.nominal_values(a), axis=axis, keepdims=True) / np.sum(
        w, axis=axis, keepdims=True
    )
    var = np.sum(w * np.subtract(avg, unp.nominal_values(a)) ** 2, axis=axis) / np.sum(
        w, axis=axis
    )
    avg = np.float64(np.squeeze(avg))
    if isinstance(avg, np.floating) and isinstance(var, np.floating):
        return ufloat(nominal_value=avg, std_dev=np.sqrt(var))
    elif isinstance(avg, np.ndarray) and isinstance(var, np.ndarray):
        return unp.uarray(nominal_values=avg, std_devs=np.sqrt(var))
    else:
...
def unanaverage(a, axis=None):
    ...
    """
    w = 1 / unp.std_devs(a) ** 2
    avg = np.nansum(w * unp.nominal_values(a), axis=axis, keepdims=True) / np.nansum(
        w, axis=axis, keepdims=True
    )
    var = np.nansum(
        w * np.subtract(avg, unp.nominal_values(a)) ** 2, axis=axis
    ) / np.nansum(w, axis=axis)
    avg = np.float64(np.squeeze(avg))
    if isinstance(avg, np.floating) and isinstance(var, np.floating):
        return ufloat(nominal_value=avg, std_dev=np.sqrt(var))
    elif isinstance(avg, np.ndarray) and isinstance(var, np.ndarray):
...
class IV_curve3:
    ...
    def __init__(self):
        ...
        self.m_type, self.scan_dim, self.sweeptype, self.sweeps, self.bias = (
            None,
            None,
            None,
            None,
        ...
        self.I, self.V, self.V_corr, self.dVdI, self.R_n, self.d2VdI2 = (
            None,
            None,
            None,
            None,
        ...
        self.I_offsets, self.V_offsets, self.I_offset, self.V_offset = (
            None,
            None,
            None,
            None,
        ...
        self.scm = self.switching_current(
            sweeps=self.sweeps, settings=self.settings
        )  # subclass for switching current measurement analysis
    ...
    def load(self, uuid, dVdI="analysis0"):
        ...
        def _get_xy_parameter():
            ...
            if self.scan_dim >= 2:  # 2D or 3D scan
                ...
                if self.scan_dim == 3:  # 3D scan
                    ...
                    self.y_ds, self.y_coordname, self.y_unit, self.y_vec = (
                        None,
                        None,
                        None,
                        None,
                ...
                self.x_ds, self.x_coordname, self.x_unit, self.x_vec = (
                    None,
                    None,
                    None,
                    None,
        ...
        try:
            self.settings = dict2obj(
                json.loads(self.df.data.settings[0], cls=QkitJSONDecoder)
            )
        ...
        try:
            ...
            self.mo = dict2obj(
                json.loads(self.df.data.measurement[0], cls=QkitJSONDecoder)
            )
        self.m_type = self.mo.measurement_type  # measurement type
        if self.m_type == "transport":
            self.scan_dim = self.df.data.i_0.attrs[
                "ds_type"
            ]  # scan dimension (1D, 2D, ...)
            ...
            try:
                ...
                self.sweeptype = np.unique(
                    [self.get_sweeptype(sweeps=sweep) for sweep in self.sweeps]
                )
            ...
            shape = np.concatenate(
                [
                    ...
                    np.max(
                        [
                            self.df[f"entry/data0/i_{j:d}"].shape
                            for j in range(len(self.sweeps))
                        ],
                        axis=0,
                    ),
                ]
            )  # (number of sweeps, eventually len y-values, eventually len x-values, maximal number of sweep points)
            self.I, self.V, self.dVdI = (
                np.empty(shape=shape),
                np.empty(shape=shape),
                np.empty(shape=shape),
            )
            for j in range(shape[0]):
                i = self.df[f"entry/data0/i_{j:d}"][:]
                v = self.df[f"entry/data0/v_{j:d}"][:]
                if dVdI:
                    try:
                        dvdi = self.df[f"entry/{dVdI:s}/dvdi_{j:d}"][
                            :
                        ]  # if analysis already done during measurement
                    except KeyError:
                        dvdi = self.get_dydx(x=self.I[j], y=self.V[j])
                pad_width = np.insert(
                    np.diff([i.shape, shape[1:]], axis=0), (0,), np.zeros(self.scan_dim)
                ).reshape(self.scan_dim, 2)
                if pad_width.any():
                    self.I[j] = np.pad(
                        i, pad_width, "constant", constant_values=np.nan
                    )  # fill with current values (eventually add nans at the end, if sweeps have different lengths)
                    self.V[j] = np.pad(
                        v, pad_width, "constant", constant_values=np.nan
                    )  # fill with voltage values (eventually add nans at the end, if sweeps have different lengths)
                    if dVdI:
                        self.dVdI[j] = np.pad(
                            dvdi, pad_width, "constant", constant_values=np.nan
                        )  # fill with differential resistance values (eventually add nans at the end, if sweeps have different lengths)
                else:
                    self.I[j] = i
                    self.V[j] = v
                    if dVdI:
            ...
            self.scan_dim = self.df.data.amplitude.attrs[
                "ds_type"
            ]  # scan dimension (1D, 2D, ...)
            _get_xy_parameter()
        else:
        ...
        * 3D: values of x- and y-parameters and its corresponding sweep data are merged in the order <order>.
        ...
        def get_key(_key, _dict):
            ...
            if _key in _dict.keys():
                ...
                _key = "_".join(
                    [
                        str(int(_part) + 1) if _part.isdigit() else _part
                        for _part in _parts
                    ]
                )
                        ...
                        if j == 0:
                            ...
                            for k, v in dict(attrs[j][key]).items():
                                ...
                                if "xy_" in k and "_filter" not in k:
                                    new_key = get_key(
                                        _key=k, _dict=attrs_merged[key][0]
                                    )
                                        ...
                                        [
                                            "_".join(
                                                [
                                                    ...
                                                    for s in ulr.split("_")
                                                ]
                                            )
                                            for ulr in str(v, encoding="utf-8").split(
                                                ":"
                                            )
                                        ]
                                    ).encode()
                                # increase X of 'xy_X_filter' to the nex possible
                                    ...
                                    attrs_merged[key][0][
                                        get_key(_key=k, _dict=attrs_merged[key][0])
                                    ] = v
        else:
        ...
        # create new file
        qkit.cfg["run_id"], qkit.cfg["user"] = np.squeeze(
            np.vstack({tuple(row) for row in run_user})
        )
        ...
        for key, val in data_merged.items():
            ...
            if ds_type == ds_types["txt"]:  # measurement, settings
                txt_merged = merge_dict(
                    _listdict=[json.loads(d[0], cls=QkitJSONDecoder) for d in val]
                )
                ...
                if "settings" in key:
                    ds[key].append(
                        {
                            kk: {
                                k: remove_duplicates(_list=map(lambda x: x[k], dic))
                                for k in dic[0]
                            }
                                ...
                                txt_merged[k] = [
                                    {
                                        k: np.vstack(v)
                                        if "sweeps" in k
                                        else remove_duplicates(_list=v)
                                        for k, v in merge_dict(_listdict=v).items()
                                    }
                                ] * 2
                    ds[key].append(
                        {k: remove_duplicates(_list=v) for k, v in txt_merged.items()}
                    )
            elif ds_type == ds_types["coordinate"]:
                ds[key] = _data_file.add_coordinate(
                    name=name,
                    unit=str(
                        dict(attrs_merged[keys[key][0]][0])["unit"], encoding="utf-8"
                    ),
                )
                if np.array(
                    [
                        np.array_equal(p[0], p[1])
                        for p in itertools.combinations(
                            np.array(
                                list(itertools.zip_longest(*val, fillvalue=np.nan))
                            ).T,
                            2,
                        )
                    ]
        ...
        for key, val in data_merged.items():
            ...
            if ds_type == ds_types["vector"]:
                ds[key] = _data_file.add_value_vector(
                    name=name,
                ...
                ds[key] = _data_file.add_value_matrix(
                    name=name,
                    x=ds[str(attr["x_ds_url"], encoding="utf-8").strip("/entry")],
                ...
                ds[key] = _data_file.add_value_box(
                    name=name,
                    x=ds[str(attr["x_ds_url"], encoding="utf-8").strip("/entry")],
                    y=ds[str(attr["y_ds_url"], encoding="utf-8").strip("/entry")],
            ...
            if ds_type == ds_types["vector"]:
                ...
                if np.array(
                    [
                        np.array_equal(p[0], p[1])
                        for p in itertools.combinations(
                            list(map(list, attrs_merged[key])), 2
                        )
                    ]
                ).all():
                    for j, xy in enumerate(
                        filter(
                            lambda k: "xy_" in k and "_filter" not in k,
                            dict(attrs_merged[key][0]).keys(),
                        )
                    ):
                        x, y = (
                            i.strip("/entry")
                            for i in str(
                                dict(attrs_merged[key][0])[xy], encoding="utf-8"
                            ).split(":")
                        )
                        if j == 0:
                            ds[key] = _data_file.add_view(
                                name=name,
                                x=ds[x],
                                y=ds[y],
    ...
    def get_bias(self, df=None):
        ...
        self.bias = {"i": 0, "v": 1}[
            str(
                df.data.i_0.attrs.get(f"{chr(self.scan_dim + 119):s}_ds_url")
                and df.data.v_0.attrs.get(
                    f"{chr(self.scan_dim + 119):s}_ds_url"
                )
            ).split("/")[-1][0]
        ]
        return self.bias
    ...
    def get_dydx(self, y, x=None, mode=sig.savgol_filter, **kwargs):
        ...
        if x is None:
            ...
            if np.isnan(x).any():
                ...
                y_nans = np.isnan(y)
                dx = mode(
                    np.nan_to_num(x, copy=True, nan=0.0), **kwargs
                )  # derivation function with np.nan replaced by 0
                dy = mode(np.nan_to_num(y, copy=True, nan=0.0), **kwargs)
    ...
    def get_offsets(
        ...
        if offset is None:
            amax = np.argmax(
                1 / sp.ndimage.uniform_filter(dxdy, size=3, mode="constant")
                if dxdy is not None
                else np.grandient(y) / np.grandient(x),
                axis=-1,
            )  # maximal slope
            if len(x.shape) - 1 == 1:
                ...
                offset = np.nanmean(
                    x[np.indices(amax.shape)[0], np.indices(amax.shape)[1], amax]
                )
            elif len(x.shape) - 1 == 3:
                offset = np.nanmean(
                    x[
                        np.indices(amax.shape)[0],
                        np.indices(amax.shape)[1],
                        np.indices(amax.shape)[2],
                        amax,
                    ]
                )
        ...
        if self.sweeptype == 0:  # halfswing
            ...
            x_offsets = np.atleast_1d(
                uncert.unanaverage(
                    unp.uarray(
                        nominal_values=np.nanmean(x_const, axis=self.scan_dim),
            ...
            if yr:  # retrapping y (for JJ retrapping current)
                raise NotImplementedError(
                    "No algorithm implemented for custom sweep type"
                )
            else:
                y_cs = np.array(
                    [
                        np.nanmax(np.nanmax(y_const, axis=0), axis=self.scan_dim - 1),
        ...
        self.I_offsets, self.V_offsets = [x_offsets, y_offsets][
            :: int(np.sign(self.bias - 0.5))
        ]
    ...
    def get_offset(self, *args, **kwargs):
        ...
        """
        self.I_offset, self.V_offset = list(
            map(np.nanmean, self.get_offsets(*args, **kwargs))
        )
        return self.I_offset, self.V_offset

    def get_2wire_slope_correction(
        self, I=None, V=None, dVdI=None, peak_finder=sig.find_peaks, **kwargs
    ):
        """
        ...
        if len(V.shape) - 1 == 1:
            peaks = map(lambda dVdI1D: peak_finder(dVdI1D, **kwargs), dVdI)
            slices = map(
                lambda peaks1D: slice(
                    *np.sort(peaks1D[0][peaks1D[1]["prominences"].argsort()[-2:][::-1]])
                ),
                peaks,
            )
            popts = map(lambda I1D, V1D, s1D: lin_fit(I1D[s1D], V1D[s1D]), I, V, slices)
            self.V_corr = np.array(
                list(
                    map(
                        lambda I1D, V1D, popt1D: V1D - (popt1D[0] * I1D + popt1D[1]),
                        I,
                        V,
                        popts,
                    )
            ...
            peaks = map(
                lambda dVdI2D: map(
                    lambda dVdI1D: peak_finder(dVdI1D, **kwargs), dVdI2D
                ),
                dVdI,
            )
            slices = map(
                lambda peaks2D: map(
                    lambda peaks1D: slice(
                        *np.sort(
                            peaks1D[0][peaks1D[1]["prominences"].argsort()[-2:][::-1]]
                        )
                    ),
                    peaks2D,
                ),
            ...
            popts = map(
                lambda I2D, V2D, s2D: map(
                    lambda I1D, V1D, s1D: lin_fit(I1D[s1D], V1D[s1D]), I2D, V2D, s2D
                ),
                I,
                V,
                slices,
            ...
            self.V_corr = np.array(
                list(
                    map(
                        lambda I2D, V2D, popt2D: map(
                            lambda I1D, V1D, popt1D: V1D
                            - (popt1D[0] * I1D + popt1D[1]),
                            I2D,
                            V2D,
                            popt2D,
                        ),
                ...
                lambda peaks3D: map(
                    lambda peaks2D: map(
                        lambda peaks1D: slice(
                            *np.sort(
                                peaks1D[0][
                                    peaks1D[1]["prominences"].argsort()[-2:][::-1]
                                ]
                            )
                        ),
                        peaks2D,
    ...
    def get_Rn(
        ...
        V=None,
        dVdI=None,
        deriv_func=sig.savgol_filter,
        peak_finder=sig.find_peaks,
        mode=0,
        **kwargs
    ):
        """
        ...
        if len(V.shape) - 1 == 1:
            if peak_finder == sig.find_peaks:
                kwargs_peak_finder = np.array(
                    list(
                        map(
                            lambda d2VdI21D: {
                                "prominence": kwargs.get(
                                    "prominence", np.max(np.abs(d2VdI21D)) / 1e2
                                )
                            },
                            self.d2VdI2,
                        )
            ...
            peaks = map(
                lambda j, d2VdI21D: _peak_finder(
                    d2VdI21D * np.sign(max(d2VdI21D.min(), d2VdI21D.max(), key=abs)),
                    **kwargs_peak_finder[j]
            ...
            slcs = np.array(
                list(
                    map(
                        lambda peak1D, d2VdI21D: np.array(
                            (
                                slice(0, np.min(peak1D[1]["left_bases"])),
            ...
            if mode == 0:
                popts, pcovs = np.ones(shape=(len(self.d2VdI2), 2)), np.ones(
                    shape=(len(self.d2VdI2), 2, 2)
                )
                for j in range(2):
                    V_ohm = np.concatenate(
                        [V[k, slc] for k, slc in enumerate(slcs.T[j])]
                    )
                    I_ohm = np.concatenate(
                        [I[k, slc] for k, slc in enumerate(slcs.T[j])]
                    )
                    popts[j], pcovs[j] = np.polyfit(x=I_ohm, y=V_ohm, deg=1, cov=True)
                # self.R_n = np.mean(popts[:, 0])
                self.R_n = np.nanmean(
                    unp.uarray(
                        nominal_values=popts,
                        std_devs=np.array(
                            list(map(lambda pcov: np.sqrt(np.diagonal(pcov)), pcovs))
                        ),
                    )[:, 0]
                )
            elif mode == 1:
                ...
                self.R_n = np.array(
                    [
                        ufloat(
                            nominal_value=np.nanmean(
                                np.concatenate([dVdI[k, s] for k, s in enumerate(slc)])
                            ),
                            std_dev=np.nansem(
                                np.concatenate([dVdI[k, s] for k, s in enumerate(slc)])
                            ),
            ...
            peaks = map(
                lambda j, d2VdI22D: list(
                    map(
                        lambda k, d2VdI21D: _peak_finder(
                            d2VdI21D
                            * np.sign(max(d2VdI21D.min(), d2VdI21D.max(), key=abs)),
                            **kwargs_peak_finder[j, k]
                        ),  # sign, so that extremum is positiv
            ...
            slcs = np.array(
                list(
                    map(
                        lambda peak2D, d2VdI22D: list(
                            map(
                                lambda peak1D, d2VdI21D: np.array(
                                    (
                                        slice(0, np.min(peak1D[1]["left_bases"])),
                                        slice(
                                            max(peak1D[1]["right_bases"]), d2VdI21D.size
                                        ),
                                    )
                                ),
                                peak2D,
            ...
            if mode == 0:
                popts, pcovs = np.ones(
                    shape=(len(self.x_vec), len(self.d2VdI2), 2)
                ), np.ones(shape=(len(self.x_vec), len(self.d2VdI2), 2, 2))
                for x, slcs1D in enumerate(np.transpose(slcs, axes=(1, 0, 2))):
                    for j in range(2):
                        V_ohm = np.concatenate(
                            [V[k, x, slc] for k, slc in enumerate(slcs1D.T[j])]
                        )
                        I_ohm = np.concatenate(
                            [I[k, x, slc] for k, slc in enumerate(slcs1D.T[j])]
                        )
                        popts[x, j], pcovs[x, j] = np.polyfit(
                            x=I_ohm, y=V_ohm, deg=1, cov=True
                        )
                        ...
                        std_devs=np.array(
                            list(
                                map(
                                    lambda pcov1D: list(
                                        map(
                                            lambda pcov: np.sqrt(np.diagonal(pcov)),
                                            pcov1D,
                                        )
                                    ),
                                    pcovs,
                    ...
                    [
                        [
                            ufloat(
                                nominal_value=np.nanmean(
                                    np.concatenate(
                                        [dVdI[k, s] for k, s in enumerate(slc)]
                                    )
                                ),
                                std_dev=np.nansem(
                                    np.concatenate(
                        ...
                        lambda j, d2VdI22D: list(
                            map(
                                lambda k, d2VdI21D: _peak_finder(
                                    d2VdI21D
                                    * np.sign(
                                        max(d2VdI21D.min(), d2VdI21D.max(), key=abs)
                                    ),
                                ...
                                lambda peak2D, d2VdI22D: list(
                                    map(
                                        lambda peak1D, d2VdI21D: np.array(
                                            (
                                                slice(
                                                    0, np.min(peak1D[1]["left_bases"])
                                                ),
                                                slice(
                                                    max(peak1D[1]["right_bases"]),
                                                    d2VdI21D.size,
                                                ),
                                            )
                                        ),
            ...
            if mode == 0:
                popts, pcovs = np.ones(
                    shape=(len(self.x_vec), len(self.y_vec), len(self.d2VdI2), 2)
                ), np.ones(
                ...
                for x, slcs2D in enumerate(np.transpose(slcs, axes=(1, 2, 0, 3))):
                    for y, slcs1D in enumerate(slcs2D):
                        for j in range(2):
                            V_ohm = np.concatenate(
                                [V[k, x, y, slc] for k, slc in enumerate(slcs1D.T[j])]
                            )
                            I_ohm = np.concatenate(
                                [I[k, x, y, slc] for k, slc in enumerate(slcs1D.T[j])]
                            )
                            popts[x, y, j], pcovs[x, y, j] = np.polyfit(
                                x=I_ohm, y=V_ohm, deg=1, cov=True
                            )
                ...
                self.R_n = np.array(
                    [
                        ...
                        for x, slcs2D in enumerate(
                            np.transpose(slcs, axes=(1, 2, 3, 0))
                        )
                    ]
                )
                self.R_n = np.nanmean(self.R_n, axis=self.scan_dim - 1)
    ...
    def get_Ic_threshold(
        self, I=None, V=None, dVdI=None, threshold=20e-6, offset=None, Ir=False
    ):
        """
        ...
        if offset is None:
            if self.V_offset is None:
                amax = np.argmax(
                    1 / dVdI if dVdI is not None else np.grandient(I) / np.grandient(V),
                    axis=-1,
                )  # maximal slope
                if len(V.shape) - 1 == 1:
                    ...
                    offset = np.nanmean(
                        V[np.indices(amax.shape)[0], np.indices(amax.shape)[1], amax]
                    )
                elif len(V.shape) - 1 == 3:
                    offset = np.nanmean(
                        V[
        ...
        if self.sweeptype == 0:  # halfswing
            ...
            I_cs = np.array(
                [
                    np.nanmin(I_sc[0], axis=(self.scan_dim - 1)),
            ...
            I_rs = np.array(
                [
                    np.nanmax(I_sc[0], axis=(self.scan_dim - 1)),
            ...
            I_cs = np.array(
                [
                    np.nanmax(I_sc, axis=(self.scan_dim - 1)),
    ...
    def get_Ic_deriv(
        ...
        **kwargs
        ...
        if len(V.shape) - 1 == 1:
            peaks = np.array(
                list(map(lambda dVdI1D: _peak_finder(dVdI1D, **kwargs), dVdI)),
                dtype=object,
            )
        elif len(V.shape) - 1 == 2:
            peaks = np.array(
                list(
                    map(
                        lambda dVdI2D: list(
                            map(lambda dVdI1D: _peak_finder(dVdI1D, **kwargs), dVdI2D)
                        ),
                        dVdI,
                    )
                ),
                    ...
                    map(
                        lambda dVdI3D: list(
                            map(
                                lambda dVdI2D: list(
                                    map(
                                        lambda dVdI1D: _peak_finder(dVdI1D, **kwargs),
                                        dVdI2D,
                                    )
                                ),
                                dVdI3D,
    ...
    def get_Ic_dft(
        ...
        **kwargs
        ...
        def _get_deriv_dft(_V):
            V_fft = np.fft.fft(
                _V
            )  # Fourier transform of V from time to frequency domain
            f = np.fft.fftfreq(_V.shape[-1])  # frequency values
            kernel = 1j * np.fft.fft(
                f * np.exp(-s * f**2)
            )  # smoothed derivation function, how it would look like in time domain (with which V is convolved in the time domain)
            V_fft_smooth = (
                1j * f * np.exp(-s * f**2) * V_fft
            )  # Fourier transform of a Gaussian smoothed derivation of V in the frequency domain
            return np.fft.ifft(
                V_fft_smooth
            )  # inverse Fourier transform of the smoothed derivation of V from reciprocal to time domain
        ...
        if peak_finder is sig.find_peaks_cwt and "widths" not in kwargs.keys():
            kwargs["widths"] = np.arange(10)
        if len(V.shape) - 1 == 0:  # single trace used for in situ fit
            V_corr = V - np.linspace(
                start=V[0], stop=V[-1], num=V.shape[-1], axis=0
            )  # adjust offset slope
        ...
        if len(V.shape) - 1 == 1:
            V_corr = V - np.linspace(
                start=V[:, 0], stop=V[:, -1], num=V.shape[-1], axis=1
            )  # adjust offset slope
        elif len(V.shape) - 1 == 2:
            V_corr = V - np.linspace(
                start=V[:, :, 0], stop=V[:, :, -1], num=V.shape[-1], axis=2
            )  # adjust offset slope
        elif len(V.shape) - 1 == 3:
            V_corr = V - np.linspace(
                start=V[:, :, :, 0], stop=V[:, :, :, -1], num=V.shape[-1], axis=3
            )  # adjust offset slope
        else:
            raise ValueError("Scan dimension must be in {1, 2, 3}")
        dV_smooth = _get_deriv_dft(_V=V_corr)
        ...
        if len(V.shape) - 1 == 1:
            peaks = np.array(
                list(
                    map(
                        lambda dV_smooth1D: _peak_finder(np.abs(dV_smooth1D), **kwargs),
                        dV_smooth,
                    )
                ),
                dtype=object,
            ...
            peaks = np.array(
                list(
                    map(
                        lambda dV_smooth2D: list(
                            map(
                                lambda dV_smooth1D: _peak_finder(
                                    np.abs(dV_smooth1D), **kwargs
                                ),
                                dV_smooth2D,
                            )
                        ),
    ...
    def _classify_jump(
        self, I, V, Y, peaks, tol_offset=20e-6, window=5, Ir=False, Vg=False
    ):
        """
        ...
        def get_fwhm(y, peak, mask):
            ...
            left, right = (
                peak[1]["left_bases"][mask].item(),
                peak[1]["right_bases"][mask].item(),
            )
            y = y[slice(left, right)] - np.nanmean(
                y
            )  # use left and right bases as bounds and subtract floor (mean(y))
        ...
        Y_name = [
            key
            for key, val in sys._getframe().f_back.f_locals.items()
            if np.array_equal(val, Y)
        ][0]
        if self.V_offset is None and self.sweeptype in [
            0,
            1,
        ]:  # voltage offset to identify superconducting branch
        ...
        if self.sweeptype == 0:  # halfswing
            ...
            if len(V.shape) - 1 == 1:
                return np.array(
                    list(
                        map(
                            lambda ind1D, prop1D, I1D, V1D, Y1D: f(
                                ind1D, prop1D, I1D, V1D, Y1D
                            ),
                            *list(zip(*peaks)),
                            I,
                            V,
                ...
                return np.array(
                    list(
                        map(
                            lambda peaks2D, I2D, V2D, Y2D: list(
                                map(
                                    lambda ind1D, prop1D, I1D, V1D, Y1D: f(
                                        ind1D, prop1D, I1D, V1D, Y1D
                                    ),
                ...
                return np.array(
                    list(
                        map(
                            lambda peaks3D, I3D, V3D, Y3D: list(
                                map(
                                    lambda peaks2D, I2D, V2D, Y2D: list(
                                        map(
                                            lambda ind1D, prop1D, I1D, V1D, Y1D: f(
                ...
                raise NotImplementedError(
                    "No algorithm implemented for custom sweep type and scan_dim > 3."
                )
        if len(V.shape) - 1 == 1:
            """critical current"""
            masks_c = list(
                map(
                    lambda V_c1D, peak1D: np.logical_and(
                        np.logical_and(
            ...
            I_cs = np.array(
                list(
                    map(
                        lambda I_c1D, Y1D, peak1D, masks_c1D: ufloat(
                            nominal_value=I_c1D[peak1D[0][masks_c1D][0]],
                            std_dev=np.abs(
                                np.diff(I_c1D[get_fwhm(Y1D, peak1D, masks_c1D)])
                            ).item()
                            / 2,
                        ),
            ...
            masks_r = list(
                map(
                    lambda V_r1D, peak1D: np.logical_and(
                        np.logical_not(
                            np.logical_and(
                                (-tol_offset + self.V_offset)
                                <= np.mean(
            ...
            I_rs = np.array(
                list(
                    map(
                        lambda I_r1D, Y1D, peak1D, masks_r1D: ufloat(
                            nominal_value=I_r1D[peak1D[0][masks_r1D][0]],
                            std_dev=np.abs(
                                np.diff(I_r1D[get_fwhm(Y1D, peak1D, masks_r1D)])
                            ).item()
                            / 2,
                        ),
                        # half width half maximum
            ...
            properties = np.array(
                list(
                    map(
                        lambda ind1D, prop1D, I1D, V1D, Y1D: {
                            **prop1D,
                            **{
                                k: v
                                for k, v in zip(
            ...
            if Vg:
                ...
                V_gs = unp.uarray(
                    nominal_values=[
                        np.nanmean(V[j][slc]) for j, slc in enumerate(slcs_g)
                    ],
            ...
            I_cs = np.array(
                list(
                    map(
                        lambda I_c2D, Y2D, peaks_c2D, masks_c2D: list(
                            map(
                                lambda I_c1D, Y1D, peak1D, masks_c1D: ufloat(
                                    nominal_value=I_c1D[peak1D[0][masks_c1D][0]],
                                    std_dev=np.abs(
            ...
            I_rs = np.array(
                list(
                    map(
                        lambda I_r2D, Y2D, peaks_r2D, masks_r2D: list(
                            map(
                                lambda I_r1D, Y1D, peak1D, masks_r1D: ufloat(
                                    nominal_value=I_r1D[peak1D[0][masks_r1D][0]],
                                    std_dev=np.abs(
            ...
            I_cs = np.array(
                list(
                    map(
                        lambda I_c3D, Y3D, peaks_c3D, masks_c3D: list(
                            map(
                                lambda I_c2D, Y2D, peaks_c2D, masks_c2D: list(
                                    map(
                                        lambda I_c1D, Y1D, peak1D, masks_c1D: ufloat(
                                            nominal_value=I_c1D[
                                                peak1D[0][masks_c1D][0]
                                            ],
                                            std_dev=np.abs(
                                                np.diff(
                                                    I_c1D[
                                                        get_fwhm(Y1D, peak1D, masks_c1D)
                                                    ]
                                                )
                                            ).item()
                                            / 2,
            ...
            I_rs = np.array(
                list(
                    map(
                        lambda I_r3D, Y3D, peaks_r3D, masks_r3D: list(
                            map(
                                lambda I_r2D, Y2D, peaks_r2D, masks_r2D: list(
                                    map(
                                        lambda I_r1D, Y1D, peak1D, masks_r1D: ufloat(
                                            nominal_value=I_r1D[
                                                peak1D[0][masks_r1D][0]
                                            ],
                                            std_dev=np.abs(
                                                np.diff(
                                                    I_r1D[
                                                        get_fwhm(Y1D, peak1D, masks_r1D)
                                                    ]
                                                )
                                            ).item()
                                            / 2,
    ...
    def get_Vg(self, V=None, binwidth=2e-6, subgap=False):
        ...
        for x in range(_V.shape[1]):  # iterate x-values
            for y in range(_V.shape[2]):  # iterate y-values
                """raw histogram"""
                p, edges = np.histogram(
                    np.concatenate(_V[:, x, y]),
                    bins=int(
                        (np.nanmax(_V[:, x, y]) - np.nanmin(_V[:, x, y])) / binwidth
                    ),
                )
                # find <nop> peaks
                prominence = np.max(p)
                while True:
                    ...
                    if peaks[0].size < nop:
                        ...
                        raise ValueError(
                            "V_g: Too many peaks found. Probably adjust bin width!"
                        )
                """ fine histogram """
                for i in range(nop):
                    p1, edges1 = np.histogram(
                        np.concatenate(_V[:, x, y]),
                        bins=100,
                        range=(
                            edges[peaks[1]["left_bases"][i]],
                    ...
                    p0 = [
                        ...
                        np.median(
                            list(
                                filter(
                                    lambda val: edges[peaks[1]["left_bases"][i]]
                                    <= val
                                    <= edges[peaks[1]["right_bases"][i] + 1],
                                    np.concatenate(_V[:, x, y]),
                                )
                ...
                if subgap:
                    for i in range(nop - 1):
                        sg = np.array(
                            list(
                                filter(
                                    lambda val: edges[peaks[1]["right_bases"][i]]
                                    <= val
                                    <= edges[peaks[1]["left_bases"][i + 1] + 1],
                                    np.concatenate(_V[:, x, y]),
                                )
                            )
                        )
                        # find outliers
                        sg_median = ufloat(
                            nominal_value=np.median(sg),
                            std_dev=stats.median_absolute_deviation(sg),
                        )
                        V_sg[x, y, i] = ufloat(
                            nominal_value=[np.nanmin, np.nanmax][i](
                                sg[
                                    np.logical_and(
                                        sg_median.n - sg_median.s < sg,
    ...
    class switching_current:
        ...
        def __init__(self, sweeps, settings):
            ...
            self.P, self.P_fit, self.edges, self.edges_fit, self.bins, self.bins_fit = (
                None,
                None,
                None,
                None,
            ...
            self.Gamma, self.Gamma_fit, self.x, self.x_fit, self.y, self.y_fit = (
                None,
                None,
                None,
                None,
            ...
            self.popt, self.pcov, self.fit_res, self.I_c, self.omega_0 = (
                None,
                None,
                None,
                None,
        ...
        def fit(self, I_0, omega_0, dIdt=None, **kwargs):
            ...
            self.bins = np.convolve(
                self.edges, np.ones((2,)) / 2, mode="valid"
            )  # center of bins by moving average with window length 2
            """ escape rate """
            self.Delta_I = np.nanmean(np.diff(self.edges))
            if dIdt is None:
                self.dIdt = (
                    self.sweeps[0][2]
                    * np.squeeze(np.unique(self.settings.IVD.sense_nplc))
                    / self.settings.IVD.plc
                )
            else:
                self.dIdt = dIdt
            with np.errstate(divide="ignore"):  # raises warning due to divide by zero
                self.Gamma = (
                    ...
                    * np.array(
                        [
                            np.log(np.sum(self.P[k:]) / np.sum(self.P[k + 1 :]))
                            for k, _ in enumerate(self.P)
                        ]
                    )
                )
            ...
            with np.errstate(divide="ignore"):  # raises warning due divide by zero
                self.y = np.log(
                    unp.nominal_values(self.omega_0) / (2 * np.pi * self.Gamma)
                ) ** (2 / 3)
            self.popt, self.pcov = np.polyfit(
                x=self.x[np.isfinite(self.y)],
                y=self.y[np.isfinite(self.y)],
                cov=True,
                deg=1,
            )
            self.fit_res = unp.uarray(
                nominal_values=self.popt, std_devs=np.sqrt(np.diagonal(self.pcov))
            )
            ...
            self.T_esc = (
                ...
                * const.Phi_0
                / (3 * 2 * np.pi * const.k)
                / (self.fit_res[0] * unp.sqrt(self.fit_res[1]))
            )
            return dict2obj(
                {
                    "P": self.P,
        ...
        def plot(
            ...
            y2label=r"escape rate $ \Gamma_{esc}~(s^{-1}) $",
            y3label=r"norm. escape rate $ \ln(\omega_0/2\pi\Gamma) $",
            errors=False,
            alpha=1,
            figsize=(6, 6),
        ):
            ...
            self.edges_fit = np.linspace(
                np.min(self.edges), np.max(self.edges), self.bins.size * alpha + 1
            )
            ...
            if errors:  # this woks also for errors=False, but is much slower
                ...
                self.Gamma_fit = (
                    self.omega_0 / (2 * np.pi) * unp.exp(-self.y_fit ** (3 / 2))
                )
                self.P_fit = np.array(
                    [
                        self.Gamma_fit[k]
                        / self.dIdt
                        * unp.exp(
                            -np.sum(self.Gamma_fit[: k + 1])
                            * self.Delta_I_fit
                            / self.dIdt
                        )
                        for k, _ in enumerate(self.x_fit)
                    ]
                )
            else:
                self.y_fit = self.popt[0] * self.x_fit + self.popt[1]
                self.Gamma_fit = (
                    unp.nominal_values(self.omega_0)
                    / (2 * np.pi)
                    * np.exp(-self.y_fit ** (3 / 2))
                )
                self.P_fit = np.array(
                    [
                        self.Gamma_fit[k]
            ...
            """ histogram """
            lgd11 = self.ax1.bar(
                x=self.bins, height=self.P, width=np.mean(np.gradient(self.edges))
            )
            ...
            if errors:
                self.ax1.fill_between(
                    x=self.x_fit,
                    y1=(unp.nominal_values(self.P_fit) - unp.std_devs(self.P_fit)).clip(
                        0
                    ),
                ...
                (lgd13,) = self.ax1.fill(
                    np.NaN, np.NaN, "black", alpha=0.2
                )  # dummy for legend
                self.ax1.legend(
                    ((lgd11,), (lgd12, lgd13)), ("data", "fit"), loc="upper left"
                )
            else:
                self.ax1.legend(((lgd11,), (lgd12,)), ("data", "fit"), loc="upper left")
            # self.ax1.legend(loc='upper left')
            """ escape rate """
            (lgd21,) = self.ax2.semilogy(self.x, self.Gamma, "b.", label=r"$ \Gamma $")
            # lgd22, = self.ax2.semilogy(self.x_fit,
            #                           self.Gamma_fit,
            #                           'b-',
            #                           label='fit')
            (lgd22,) = self.ax2.semilogy(
                self.x_fit, unp.nominal_values(self.Gamma_fit), "b-", label="fit"
            )
            if errors:
                self.ax2.fill_between(
                    x=self.x_fit,
                    y1=unp.nominal_values(self.Gamma_fit)
                    - unp.std_devs(self.Gamma_fit),
                    y2=unp.nominal_values(self.Gamma_fit)
                    + unp.std_devs(self.Gamma_fit),
                    color="b",
                    alpha=0.2,
                )
            ...
            (lgd31,) = self.ax3.plot(
                self.x, self.y, "r.", label=r"$ \ln(\omega_0/2\pi\Gamma) $"
            )
            ...
            #                       label='fit')
            (lgd32,) = self.ax3.plot(
                self.x_fit, unp.nominal_values(self.y_fit), "r-", label="fit"
            )
            if errors:
                ...
                self.ax2.legend(
                    ((lgd21, lgd22), (lgd31, lgd32)),
                    (r"$ \Gamma $", r"$ \ln(\omega_0/2\pi\Gamma) $"),
                    loc="upper center",
                )
