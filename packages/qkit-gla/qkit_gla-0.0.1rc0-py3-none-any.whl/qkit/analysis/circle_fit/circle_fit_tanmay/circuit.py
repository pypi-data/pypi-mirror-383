...
class circuit:
    ...
    def plotall(self, trans=0.5):
        ...
        plt.plot(
            self.f_data * 1e-9, np.absolute(self.z_data_sim), label="fit", alpha=trans
        )
        plt.xlabel("f (GHz)")
        plt.ylabel("|S21|")
        plt.legend()
        plt.subplot(223)
        plt.scatter(
            self.f_data * 1e-9, np.angle(self.z_data_raw), label="rawdata", s=1, c="C1"
        )
        plt.plot(
            self.f_data * 1e-9, np.angle(self.z_data_sim), label="fit", alpha=trans
        )
        plt.xlabel("f (GHz)")
        plt.ylabel("arg(|S21|)")
        plt.legend()
    ...
    def plotrawdata(self):
        ...
        plt.subplot(222)
        plt.scatter(
            self.f_data * 1e-9, np.absolute(self.z_data_raw), label="rawdata", s=1
        )
        plt.xlabel("f (GHz)")
        plt.ylabel("|S21|")
        plt.legend()
