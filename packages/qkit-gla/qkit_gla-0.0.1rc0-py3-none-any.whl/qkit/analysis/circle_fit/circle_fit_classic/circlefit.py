...
class circlefit:
    ...
    def _fit_skewed_lorentzian(self, f_data, z_data):
        ...
        p_final = spopt.leastsq(
            residuals2, p0, args=(np.array(f_data), np.array(amplitude_sqr))
        )
    ...
    def _fit_entire_model(
            ...
            return np.array(
                [
                    np.absolute(
                            ...
                            * np.exp(np.complex(0, -2.0 * np.pi * delay * x[i]))
                            * (
                                1
                                - (Ql / absQc * np.exp(np.complex(0, phi0)))
                                / (np.complex(1, 2 * Ql * (x[i] - fr) / fr))
                            )
                    )
                    ** 2
        ...
        if (
            len_ydata > len(p0)
        ) and params_cov is not None:  # p_final[1] is cov_x data  #this caculation is from scipy curve_fit routine - no idea if this works correctly...
    ...
    def _residuals_notch_full(self, p, x, y):
        ...
        err = np.absolute(
            ...
            - (
                a
                * np.exp(np.complex(0, alpha))
                * np.exp(np.complex(0, -2.0 * np.pi * delay * x))
                * (
                    1
                    - (Ql / absQc * np.exp(np.complex(0, phi0)))
                    / (np.complex(1, 2 * Ql * (x - fr) / float(fr)))
                )
            )
        )
        return err
    ...
    def _residuals_transm_ideal(self, p, x, y):
        fr, Ql = p
        err = np.absolute(y - (1.0 / (np.complex(1, 2 * Ql * (x - fr) / float(fr)))))
        return err
