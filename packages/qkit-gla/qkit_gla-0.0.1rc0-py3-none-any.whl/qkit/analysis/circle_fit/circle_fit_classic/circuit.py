...
from .utilities import Watt2dBm, dBm2Watt, plotting, save_load
...
class reflection_port(circlefit, save_load, plotting, calibration):
    ...
    def get_delay(self, f_data, z_data, delay=None, ignoreslope=True, guess=True):
        ...
        if ignoreslope == True:
            ...
            z_data = (np.sqrt(np.absolute(z_data) ** 2 - A2 * (f_data - fr))) * np.exp(
                np.angle(z_data) * 1j
    ...
    def do_calibration(self, f_data, z_data, ignoreslope=True, guessdelay=True):
        ...
        delay, params = self.get_delay(
            f_data, z_data, ignoreslope=ignoreslope, guess=guessdelay
        )
        z_data = (
            np.sqrt(np.absolute(z_data) ** 2 - params[1] * (f_data - params[4]))
            * np.exp(2.0 * 1j * np.pi * delay * f_data)
            * np.exp(1j * np.angle(z_data))
        )
    ...
    def do_normalization(self, f_data, z_data, delay, amp_norm, alpha, A2, frcal):
        ...
        """
        return (
            (np.sqrt(np.absolute(z_data) ** 2 - A2 * (f_data - frcal)))
            / amp_norm
    ...
    def autofit(self):
        ...
        delay, amp_norm, alpha, fr, Ql, A2, frcal = self.do_calibration(
            self.f_data, self.z_data_raw, ignoreslope=True, guessdelay=False
        )
        self.z_data = self.do_normalization(
            self.f_data, self.z_data_raw, delay, amp_norm, alpha, A2, frcal
        )
        self.fitresults = self.circlefit(
            self.f_data, self.z_data, fr, Ql, refine_results=False, calc_errors=True
        )
...
class notch_port(circlefit, save_load, plotting, calibration):
    ...
    def get_delay(self, f_data, z_data, delay=None, ignoreslope=True, guess=True):
        ...
        if ignoreslope == True:
            ...
            z_data = (np.absolute(z_data) - A2 * (f_data - fr)) * np.exp(
                np.angle(z_data) * 1j
            )  # usually not necessary
        if delay == None:
            if guess == True:
    ...
    def autofit(self):
        ...
        """
        delay, amp_norm, alpha, fr, Ql, A2, frcal = self.do_calibration(
            self.f_data, self.z_data_raw, ignoreslope=True, guessdelay=True
        )
        self.z_data = self.do_normalization(
            self.f_data, self.z_data_raw, delay, amp_norm, alpha, A2, frcal
        )
