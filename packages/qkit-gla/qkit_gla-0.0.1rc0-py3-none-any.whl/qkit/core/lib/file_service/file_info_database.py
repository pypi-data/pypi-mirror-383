...
class fid(file_system_service):
    ...
    def __getitem__(self, key):
        with self.lock:
            ...
            try:
                ...
                raise KeyError(
                    f"Can not find your UUID '{key}' in qkit.fid database."
                )
    ...
    def update_grid_db(self):
        with self.lock:
            if qkit.cfg.get(
                "fid_init_viewer", qkit.module_available["pandas"]
            ):  # Pandas ist needed here, so if fid_init_viewer is not set, we make it dependent on pandas
                qkit.cfg["fid_init_viewer"] = True
                if not qkit.module_available["pandas"]:
                    raise ImportError(
                        "pandas not found. Pandas is needed for the fid viewer.\nInstall pandas or set qkit.cfg['fid_init_viewer']=False"
                    )
    ...
    if qkit.module_available["pandas"]:
        ...
        def _initiate_basic_df(self):
            ...
            if (
                len(self.h5_info_db) == 0
            ):  # necessary if a data directory is chosen without any h5 file
            ...
            self.df.fillna(
                "", inplace=True
            )  # Replace NAs with empty string to be able to detect changes
        ...
        def _get_settings_column(self, device, setting, uid=None, update_hdf=False):
            ...
            if len(device) != len(setting):
                raise ValueError(
                    "Please specify 'device' and 'setting' as equally long lists, where teir individual entries correspond to each other."
                )
            if uid is None:
                uid = self.df.index
            for i in uid:
                values = self._get_setting_from_set_file(
                    self.h5_db[i].replace(".h5", ".set"), device, setting
                )
                if update_hdf:
                    for p, v in values.items():
                        self._set_hdf_attribute(i, p, v)
                dfsetting = pd.concat(
                    [dfsetting, pd.DataFrame(values, index=[i])], sort=False
                )
            return dfsetting
        ...
        def add_settings_column(self, device, setting, measurement_id=None):
            ...
            :type str
            """
            settings_column = self._get_settings_column(
                device,
                setting,
                measurement_id,
                update_hdf=qkit.cfg.get("fid_scan_hdf", False),
            )
        ...
        def open_in_filemanager(self):
            ...
            if len(ids) > 10:
                logging.error(
                    "You are trying to open more than 10 files, are you sure?"
                )
                return
            ...
            if _os == "Windows":
                ...
                for i in ids:
                    Popen(fr'explorer /select,"{self.h5_db[i]}"')
            else:
        ...
        def _grid_observer(self, change):
            logging.debug(
                "grid observer called, batch_update: " + str(self._batch_update)
            )
            c = self.grid
            ...
            try:
                ...
                indices = np.where(
                    self.df.loc[uuids, keys] != changed_df.loc[uuids, keys]
                )
                logging.debug(f"I found {len(indices[0])} changes")
                for i in range(len(indices[0])):
                    ...
                    if self.df.loc[index, key] != new_value:
                        ...
                        logging.debug(
                            f"{index}[{key}] '{self.df.loc[index, key]}'-> '{new_value}'"
                        )
        ...
        def show(self, show_raw=False):
            ...
            self.wait()
            if self.found_qgrid:
                if LooseVersion(qd.__version__) < LooseVersion(
                    "1.3.0"
                ) and LooseVersion(pd.__version__) >= LooseVersion("1.0"):
                    logging.warning(
                        "qgrid < v1.3 is incompatible with pandas > v1.0. Check for a new version of qgrid or downgrade pandas to v0.25.3"
                    )
            ...
            if self.found_qgrid:
                ...
                rows = [
                    d
                    for d in self.column_sorting
                    if d in list(self.df.keys()) and d not in self.columns_ignore
                ]
                rows += [
                    d
                    for d in list(self.df.keys())
                    if d not in self.column_sorting and d not in self.columns_ignore
                ]

                _openSelected = widgets.Button(
                ...
                _batch_modifier.key_dd = widgets.Dropdown(
                    options=rows, description="Set"
                )
                _batch_modifier.value_tf = widgets.Text(
                    value="", placeholder="Value", description="to:"
                )
                ...
                display(
                    widgets.HBox(
                        [
                            _openSelected,
                            _batch_modifier.key_dd,
                            _batch_modifier.value_tf,
                            _batch_modifier,
                        ]
                    )
                )
        ...
        def get_filtered_uuids(self):
            ...
            if self.found_qgrid:
                ...
                logging.warning(
                    "Module qgrid is not installed. Filtering the database is only supported with qgrid."
                )
                return False
        ...
        def search(self, column, expression=None, value=None, bounds=None):
            ...
            if bounds is not None and value is None and expression is None:
                return self.df[
                    ((self.df[column] > bounds[0]) & (self.df[column] < bounds[1]))
                ]
            else:
        ...
        def add_column(self, colname, value=" "):
            ...
            if colname in self.df:
                raise ValueError(
                    f"Column {colname} is already in your dataset. Can not be added twice"
                )
            else:
        ...
        add_settings_column = (
            open_in_filemanager
        ) = remove_column = show = get_filtered_uuids = search = add_column = void_func
