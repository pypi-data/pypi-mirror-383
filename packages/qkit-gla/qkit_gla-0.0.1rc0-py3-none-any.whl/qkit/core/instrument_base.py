...
class Instrument:
    ...
    def add_parameter(self, name, **kwargs):
        ...
        if name in self._parameters:
            ...
            logging.error("'%s' is a reserved name, not adding parameter", name)
            return
        ...
        if options["offset"]:
            if options["type"] in (bool, bytes, str):
                raise ValueError(
                    "%s.%s: Offset not available for parameter with type %s"
                    % (self._name, name, options["type"].__name__)
                )
            else:
                func = lambda value: self._offsets.update({name: value})
                func.__doc__ = f"""Set an offset for parameter {self._name} on device {name}.
                The offset is added on every get and substracted on every set command.
                Use e.g. set_power_offset(-20) if you add 20dB attenuation at the device."""
        ...
        if options["flags"] & Instrument.FLAG_GET:
            if ch is not None:
                func = lambda query=True, **lopts: self.get(
                    name, query=query, channel=ch, **lopts
                )
            else:
            ...
            if options["get_func"] is not None:
                ...
                options["get_func"] = lambda *a, **kw: self._get_not_implemented(
                    base_name
                )
        ...
        if options["flags"] & Instrument.FLAG_SOFTGET:
            if ch is not None:
                func = lambda query=True, **lopts: self.get(
                    name, query=False, channel=ch, **lopts
                )
            else:
        ...
        if options["flags"] & Instrument.FLAG_SET:
            ...
            if options["set_func"] is not None:
                ...
                options["set_func"] = lambda *a, **kw: self._set_not_implemented(
                    base_name
                )
    ...
    def _get_value(self, name, query=True, **kwargs):
        ...
        if not query or flags & 8:  # self.FLAG_SOFTGET:
            if "value" in p:
                ...
                logging.debug(
                    "%s.%s value not cached. I try getting it with query=True."
                    % (self._name, name)
                )
                return self._get_value(name, query=True, **kwargs)
            else:
                logging.error(
                    "Trying to access cached value %s, but none available" % name
                )
                return False
        ...
        if not flags & 1:  # Instrument.FLAG_GET:
            if query:
                logging.error(
                    f"Instrument {self._name} does not support getting of {name}"
                )
            return None
    ...
    def get_threaded(self, *args, **kwargs):
        logging.error(
            "Using threading functions is not supported. Redirecting to normal get"
        )
    ...
    def _convert_value(self, value, ttype):
        if type(value) is bool and ttype is not bool:
    ...
    def _set_value(self, name, value, **kwargs):
        ...
        if "minval" in p and value < p["minval"]:
            if self._offsets[name] is not None and self._offsets[name] != 0:
                raise qkit.instruments.InstrumentBoundsError(
                    "Cannot set %s.%s to %g: With offset %g, %s at %s would be %g, which is too small (Minimum: "
                    "%g)"
                    % (
                        self._name,
                        name,
                        value + self._offsets[name],
                        self._offsets[name],
                        name,
                        self._name,
                        value,
            ...
            raise qkit.instruments.InstrumentBoundsError(
                "Cannot set %s.%s to %s: value too small (Minimum: %g)"
                % (self._name, name, value, p["minval"])
            )

        if "maxval" in p and value > p["maxval"]:
            if self._offsets[name] is not None and self._offsets[name] != 0:
                raise qkit.instruments.InstrumentBoundsError(
                    "Cannot set %s.%s to %g: With offset %g, %s at %s would be %g, which is too large (Maximum: "
                    "%g)"
                    % (
                        self._name,
                        name,
            ...
            raise qkit.instruments.InstrumentBoundsError(
                "Cannot set %s.%s to %s: value too large (Maximum: %g)"
                % (self._name, name, value, p["maxval"])
            )
        ...
        if "maxstep" in p and p["maxstep"] is not None:
            ...
            for v in np.arange(curval, value, sign * np.abs(p["maxstep"])):
                ...
                time.sleep(p.get("stepdelay", 50) / 1000.0)
        ...
        if p["flags"] & self.FLAG_GET_AFTER_SET:
            ...
            if newvalue != value:
                logging.warning(
                    "%s.%s: actual value (%s) differs from set value (%s)"
                    % (self._name, name, newvalue, value)
                )
            value = newvalue
    ...
    def reload(self):
        ...
        """
        logging.warning(
            "No warranty on reloads. Make sure to re-assign the instrument as \n %s = %s.reload()"
    ...
    def _get_not_implemented(self, name):
        logging.warning(
            f"Get not implemented for {Instrument.get_type(self)}.{name}"
        )

    def _set_not_implemented(self, name):
        logging.warning(
            f"Set not implemented for {Instrument.get_type(self)}.{name}"
        )
