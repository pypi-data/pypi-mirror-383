...
class Virtual_Coil(Instrument):
    def __init__(self, name, dacs=DAC_ROUT, dacranges=None, ivvi_instrument=None):
        ...
        if ivvi_instrument is None:
            ins_list = (
                qkit.instruments.get_instruments_by_type("IVVI")
                + qkit.instruments.get_instruments_by_type("IVVIDIG_main")
                + qkit.instruments.get_instruments_by_type("IVVIDIG_main_eth")
            )
            if len(ins_list) > 1:
                raise ImportError(
                    "I found %i instruments matching IVVI." % (len(ins_list))
                )
            else:
        ...
        if self.channels == 1:
            self.add_parameter(
                "current", type=float, flags=Instrument.FLAG_GETSET, units="mA"
            )
    ...
    def do_set_current(self, current, channel=1, verbose=True):  # current in mA
        ...
        try:
            val = np.round(
                -0.5
                * (
                    self.range_limits[channel - 1][0]
                    + self.range_limits[channel - 1][-1]
                )
                + (
                    current
                    * 1000
                    * np.power(10.0, -dac_val[self.c_ranges[channel - 1]])
                ),
                10,
            )  # dac value in mV
            if val < -2000 or val > 2000:
                ...
                if 10.0 * val != np.round(10.0 * val) and verbose:
                    logging.warning(
                        "16 bit resolution may be visible for the current value you are attempting to set."
                    )
    ...
    def do_get_current(self, channel=1):
        ...
        val = (
            float(
                self.IVVI.get_dac(self.dacs[channel - 1])
                + 0.5
                * (
                    self.range_limits[channel - 1][0]
                    + self.range_limits[channel - 1][-1]
        ...
        # print(val)
        try:
            return val * np.power(10.0, dac_val[self.c_ranges[channel - 1]])
        except IndexError as detail:
            logging.error("Electronics might be disconnected.")
            raise detail
