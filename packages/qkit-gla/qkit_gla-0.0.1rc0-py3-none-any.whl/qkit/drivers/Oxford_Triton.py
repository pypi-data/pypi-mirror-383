...
class Oxford_Triton(Instrument):
    ...
    def __init__(self, name, host, port):
        ...
        self.add_parameter("valve", type=bool, flags=Instrument.FLAG_GETSET)

        self.add_parameter("pulse_tube", type=bool, flags=Instrument.FLAG_GETSET)
        ...
        self.add_parameter("warm_up_heater", type=bool, flags=Instrument.FLAG_GETSET)
    ...
    def set_base_control(self, temperature=0, P=12.0, I=1, D=0, status=True):
        """
        ...
        if status:
            ...
            logging.debug(
                __name__
                + " : setting base temperature to %f using P %f I %i D%i"
                % (temperature, P, I, D)
            )
            if temperature >= 2.2:
                thermometer = 5
            ...
            return (
                str(
                    self._ask(
                        "SET:DEV:T%i:TEMP:LOOP:HTR:H1:MODE:ON:P:%f:I:%i:D:%i:TSET:%f"
                        % (thermometer, P, I, D, temperature)
                    )
                ).find("INVALID")
                == -1
            )
            ...
            if self._ask("READ:DEV:T5:TEMP:LOOP:MODE").strip()[27:] == "ON":
                return (
                    self._ask("SET:DEV:T5:TEMP:LOOP:MODE:OFF").strip()[35:] == "VALID"
                )
            if self._ask("READ:DEV:T8:TEMP:LOOP:MODE").strip()[27:] == "ON":
                return (
                    self._ask("SET:DEV:T8:TEMP:LOOP:MODE:OFF").strip()[35:] == "VALID"
                )
            else:
                return "False"
    ...
    def do_get_valve(self, valve):
        ...
        return status_dict[
            self._ask("READ:DEV:V%i:VALV:SIG:STATE" % valve).strip()[27:]
        ]
    ...
    def do_set_valve(self, valve, status):
        ...
        return (
            self._ask(
                "SET:DEV:V%i:VALV:SIG:STATE:%s" % (valve, status_dict[status])
            ).strip()[27:]
            == status_dict[status]
        )
    ...
    def do_set_pulse_tube(self, status):
        ...
        return (
            self._ask("SET:DEV:C1:PTC:SIG:STATE:%s" % status_dict[status]).strip()[27:]
            == status_dict[status]
        )
    ...
    def do_set_still_power(self, power):
        ...
        return (
            self._ask("SET:DEV:H2:HTR:SIG:POWR:%f" % power).strip()[37 + add :]
            == "VALID"
        )
    ...
    def do_set_base_power(self, power):
        ...
        add = len(str(power).split(".")[0])
        return (
            self._ask("SET:DEV:H1:HTR:SIG:POWR:%f" % power).strip()[37 + add :]
            == "VALID"
        )
    ...
    def _do_set_pump(self, pump="COMP", state=False):
        ...
        logging.debug(
            __name__
            + " : setting pump {} to state {}".format(pump, "ON" if state else "OFF")
        )
        return (
            self._ask(
                "SET:DEV:{}:PUMP:SIG:STATE:{}".format(pump, "ON" if state else "OFF")
            ).strip()[-6:]
            == ":VALID"
        )
    ...
    def _do_get_pump(self, pump="COMP"):
        ...
        Output:
            status (bool)
        """
        logging.debug(__name__ + f" : Getting pump {pump} state")
        return self._ask(f"READ:DEV:{pump}:PUMP:SIG:STATE").strip()[-2:] == "ON"
    ...
    def _do_set_bypass(self, state):
        ...
        if state:  # opening, do safety check
            if self.do_get_pressure(gauge=2) > 750:
                raise ValueError(
                    "{} bypass not successful. P2 is above 750mbar.".format(
            ...
            if response == "Ok":
                ...
                raise ValueError(
                    f"Opening bypass not successful '{response}' Compressor still on"
                )
        else:  # closing
    ...
    def _do_get_bypass(self):
        ...
        if response == "closed":
            ...
            raise valueError(f"get_bypass responded with '{response}'")
