...
class ZHInstQkitProperty(QkitProperty):
    def __init__(self, param: Parameter, name):
        ...
        access = (
            param._properties
        )  # A set of strings describing access: Read, Write, Setting
        ...
        if property_type == "Double" and type(test) == np.float64:
            ...
            if type(test) == np.ndarray:
                ...
                raise TypeError(
                    f"Unsupported ZIVetorData type {type(test)} of {param._path}"
                )
        else:
            raise TypeError(
                f"Documented type '{property_type}' of '{param._path}' with returned type {type(test)} is not supported!"
            )
...
class ZHInstNodeVisitor(ObjectTreeVisistor):
    def visit_instance(self, obj, path=[]):
        if hasattr(obj, "parameters"):
            for param_name in obj.parameters:
                try:
                    self.visit_property(
                        obj, param_name, getattr(obj, param_name), path=path
                    )
                except TypeError as e:
                    logging.warn(
                        f"Caught exception processing {param_name} at {path}: {e}"
                    )
                    continue
    ...
    def wrap_property(
        self, obj: Any, name: str, prop: Parameter
