...
class DC_DAC_LTC2666(Instrument):
    ...
    def init_connection(self):
        ...
        logging.debug(
            __name__
            + f": initiating connection to DAC server {self.HOST:s} on port {self.PORT:d}"
        )
        self.c = zerorpc.Client()
        self.c.connect(f"tcp://{self.HOST:s}:{self.PORT:d}")

    def do_set_voltage(self, voltage, channel):
        """
        Set dac voltage in range -5V..5V.
        """
        if voltage > 5 - 10.0 / 2**16:
            voltage = (
                5 - 10.0 / 2**16
            )  # we can not set exactly +5V, but only one bit less
        for i in range(10):
            try:
    ...
    def close_connection(self):
        logging.debug(
            __name__
            + f": closing connection to DAC server {self.HOST:s} on port {self.PORT:d}"
        )
        self.c.close()
