...
class Tabor_WX1284C(Instrument):
    ...
    def __init__(self, name, address, chpair=None, reset=False, numchannels=4):
        ...
        if visa.qkit_visa_version == 1:
            ...
            if self._visainstrument.get_visa_attribute(vc.VI_ATTR_INTF_TYPE) in (
                vc.VI_INTF_USB,
                vc.VI_INTF_GPIB,
                vc.VI_INTF_TCPIP,
            ):
                ...
                if (
                    self._visainstrument.get_visa_attribute(vc.VI_ATTR_INTF_TYPE)
                    == vc.VI_INTF_TCPIP
                ):
                    self._visainstrument.set_visa_attribute(
        ...
        self.add_parameter("reference_source", type=str, flags=Instrument.FLAG_GETSET)
        self.add_parameter(
            "reference_source_freq",
            type=float,
            flags=Instrument.FLAG_GETSET,
            minval=10e6,
            maxval=100e6,
        ...
        self.add_parameter("common_clock", type=bool, flags=Instrument.FLAG_GETSET)
        self.add_parameter(
            "amplitude",
            type=float,
            flags=Instrument.FLAG_GETSET,
        ...
        self.add_parameter(
            "sync_position", type=float, flags=Instrument.FLAG_GETSET, units="s"
        )
        ...
        if numchannels == 4:
            ...
            self.add_parameter("trigger_mode", type=str, flags=Instrument.FLAG_GETSET)
            self.add_parameter(
                "trigger_delay", type=float, flags=Instrument.FLAG_GETSET
            )
            self.add_parameter("sequence_mode", type=str, flags=Instrument.FLAG_GETSET)
            self.add_parameter("trigger_source", type=str, flags=Instrument.FLAG_GETSET)
            self.add_parameter("trigger_time", type=float, flags=Instrument.FLAG_GETSET)
            self.add_parameter("sync_output", type=bool, flags=Instrument.FLAG_GETSET)
            self.add_parameter("trigger_slope", type=str, flags=Instrument.FLAG_GETSET)
        ...
        print(
            ...
            % (numchannels, str(range(self._choff + 1, numchannels + self._choff + 1)))
        )
    ...
    def fix(self, verbose=True):
        ...
        if not self._ins_VISA_INSTR:
            ...
            self._visainstrument.timeout = 0.1
            if verbose:
            ...
            try:
                for i in range(200):
                    ...
                    if verbose:
                        print("In Buffer %i: " % i + rl)
            except visa.VisaIOError:
                if verbose:
                    print("Timeout after %i iterations" % i)
        ...
        try:
            ...
            if rv[0] == "0":
                ...
                raise ValueError(
                    ...
                    + "("
                    + self.get_name()
                    + ") : Device responded with error ->%s<-" % rv
                )
        except visa.VisaIOError:
            return False
    ...
    def ask(self, cmd):
        for i in range(15):
            try:
                logging.debug(
                    __name__ + "(" + self.get_name() + " ask command:" + cmd[:100] + ":"
                )
                ...
                logging.debug(
                    ...
                    + "("
                    + self.get_name()
                    + "): VisaIOError, retry ->%s<-" % e
                )
        raise ValueError(
            __name__
            + "("
            + self.get_name()
            + "): ask('%s') not successful after 15 retries" % (cmd[0:100])
        )
    ...
    def write(self, cmd):
        ...
        raise ValueError(
            ...
            + "("
            + self.get_name()
            + ") : write('%s') not successful after 15 retries" % (cmd[0:100])
        )
    ...
    def wait(self, max_timeouts=1, quiet=False):
        ...
        for i in range(max_timeouts):
            try:
                ...
                if not quiet:
                    print("still waiting for the awg to become ready")
                time.sleep(1)
    ...
    def do_set_marker_output(self, state, channel):
        ...
        self.write(
            ":INST%s;:MARK:SEL%i;:MARK:STAT %i"
            % (channel, ((int(channel) - 1) % 2 + 1), state)
        )
        self.check()
    ...
    def do_get_marker_output(self, channel):
        ...
        outp = self.ask(
            ":INST%s;:MARK:SEL%i;:MARK:STAT?" % (channel, ((int(channel) - 1) % 2 + 1))
        )
        if (outp == "0") | (outp == "OFF"):
            return False
        elif (outp == "1") | (outp == "ON"):
    ...
    def do_set_runmode(self, mode, channel=1):
        ...
        if mode not in ("FIX", "USER", "SEQ", "ASEQ", "MOD", "PULS", "PATT"):
            raise ValueError(
                "The selected mode is not supported, your choice was %s, but I have FIX,USER,SEQ,ASEQ,MOD,PULS,PATT"
                % mode
            )
        return self.write(
            ":INST%s;:FUNC:MODE %s"
            % ((2 * channel - 1 if self._numchannels == 4 else channel), mode)
        )
    ...
    def do_get_runmode(self, channel=1):
        ...
        return self.ask(
            ":INST%s;:FUNC:MODE?"
            % (2 * channel - 1 if self._numchannels == 4 else channel)
        )
    ...
    def do_set_sequence_mode(self, mode, channel=1):
        ...
        channel += self._choff
        mode = mode.upper()
        if mode not in ("AUTO", "ONCE", "STEP"):
            raise ValueError(
                "The selected sequence mode is not supported, your choice was %s, but I have AUTO,ONCE,STEP"
                % mode
            )
        return self.write(
            ":INST%s;:SEQ:ADV %s"
    ...
    def do_set_trigger_mode(self, runmode, channel=1):
        ...
        if runmode == "TRIG":
            ...
            if runmode == "CONT":
                ...
                if runmode == "GATE":
                    self.write(
                        ":INST%s;:INIT:CONT 0; GATE 1"
                        % (2 * channel - 1 if self._numchannels == 4 else channel)
                    )
                else:
                    logging.error(
                        __name__
                        + ' : Unable to set trigger mode to %s, expected "CONT", "TRIG" or "GATE"'
                        % runmode
                    )
    ...
    def do_set_trigger_source(self, source, channel=1):
        ...
        if source not in ("EXT", "TIM"):
            raise ValueError(
                "The selected source is not supported, your choice was %s, but I have EXT or TIM"
                % source
            )

        return self.write(
            ":INST%s;:TRIG:SOUR:ADV %s"
            % ((2 * channel - 1 if self._numchannels == 4 else channel), source)
        )
    ...
    def do_get_trigger_source(self, channel=1):
        ...
        logging.debug(
            __name__
            + " : Get source of channel %s"
            % (2 * channel - 1 if self._numchannels == 4 else channel)
        )
        return self.ask(
            ":INST%s;:TRIG:SOUR:ADV?"
            % (2 * channel - 1 if self._numchannels == 4 else channel)
        )
    ...
    def do_set_trigger_time(self, time, channel=1):
        ...
        return self.write(
            ":INST%s;:TRIG:TIM:TIME %e"
            % ((2 * channel - 1 if self._numchannels == 4 else channel), time)
        )
    ...
    def do_get_trigger_time(self, channel=1):
        ...
        logging.debug(
            __name__
            + " : Get time of channel %s"
            % (2 * channel - 1 if self._numchannels == 4 else channel)
        )
        return self.ask(
            ":INST%s;:TRIG:TIM:TIME?"
            % (2 * channel - 1 if self._numchannels == 4 else channel)
        )
    ...
    def set_trig_impedance(self, impedance):
        ...
        if (impedance == "50") | (impedance == "10k"):
            ...
            logging.error(
                __name__
                + ' : Unable to set impedance to %s Ohm, expected "50" or "10k"'
                % impedance
            )
    ...
    def do_set_trigger_impedance(self, mod):
        ...
        if mod == 10e3:
            ...
            logging.error(
                __name__
                + ' : Unable to set trigger impedance to %s, expected "10e3" or "50"'
                % mod
            )
    ...
    def do_set_trigger_slope(self, slope, channel=1):
        ...
        if slope in ["POS", "NEG", "EIT"]:
            ...
            self.write(
                ":INST%i;:TRIG:SLOP%s"
                % ((2 * channel - 1 if self._numchannels == 4 else channel), slope)
            )
    ...
    def do_set_clock(self, clock):
        ...
        if clock > 1.25e9 and self._awg_version == "WX1284C":
            self._clock = 1.25e9
            print(
                "This model of AWG has a maximum clock of 1.25GS/s. Setting to default 1.25 GS/s."
            )
        else:
            self._clock = clock
    ...
    def do_get_amplitude(self, channel):
        ...
        channel += self._choff
        logging.debug(
            __name__ + " : Get amplitude of channel %s from instrument" % channel
        )
    ...
    def do_set_amplitude(self, amp, channel):
        ...
        channel += self._choff
        logging.debug(
            __name__ + f" : Set amplitude of channel {channel} to {amp:.6f}"
        )
        if amp < 50e-3:
            amp = 50e-3
        ...
        self.write(f":INST{channel};:VOLT{amp:.3f}")
    ...
    def do_set_offset(self, offset, channel):
        ...
        logging.debug(
            __name__ + f" : Set offset of channel {channel} to {offset:.3f}"
        )
        self.write(f":INST{channel};:VOLT:OFFS{offset:.3f}")
    ...
    def do_get_marker_high(self, channel):
        ...
        logging.debug(
            __name__
            + " : Get upper bound of marker %i of channel %s"
            % (((int(channel) - 1) % 2 + 1), channel)
        )
        return float(
            self.ask(
                ":INST%s;:MARK:SEL%i;:MARK:VOLT:HIGH ?"
                % (channel, ((int(channel) - 1) % 2 + 1))
            )
        )
    ...
    def do_set_marker_high(self, high, channel):
        ...
        logging.debug(
            __name__
            + " : Set upper bound of marker%i of channel %s to %.3f"
            % (((int(channel) - 1) % 2 + 1), channel, high)
        )
        self.write(
            ":INST%s;:MARK:SEL%i;:MARK:VOLT:HIGH%.2f"
            % (channel, ((int(channel) - 1) % 2 + 1), high)
        )
        self.check()
    ...
    def do_set_status(self, status, channel):
        ...
        channel += self._choff
        logging.debug(
            __name__ + f" : Set status of channel {channel} to {status}"
        )
        if status:
    ...
    def do_set_reference_source(self, source="EXT"):
        ...
        if source == "EXT" or source == "INT":
            ...
            logging.warning(
                "Clock source needs to be one of INT or EXT. No changes made."
            )
    ...
    def do_set_reference_source_freq(self, freq=10e6):
        ...
        if freq in [10e6, 20e6, 50e6, 100e6]:
            ...
            logging.warning(
                "Clock source needs to be one of [10e6,20e6,50e6,100e6]. No changes made."
            )
    ...
    def wfm_send(self, w, m1, m2, channel, seg):
        ...
        if m1 == None:
            m1 = numpy.zeros_like(w)
        if m2 == None:
            m2 = numpy.zeros_like(w)
        if not ((len(w) == len(m1)) and (len(m1) == len(m2))):
            raise ValueError(
                "error, the length of your waveform and markes does not match"
            )
        ...
        for i in range(0, len(w)):
            ws = ws + struct.pack(
                "<H", int(8191 * w[i] + 8192 + m1[i] * 2**14 + m2[i] * 2**15)
            )
    ...
    def wfm_send2(self, w1, w2, m1=None, m2=None, channel=1, seg=1):
        ...
        channel += self._choff
        if len(w1) != len(w2):
            raise ValueError("Waveform length is not equal.")
        if len(w1) % 16 != 0:
            raise ValueError("Wfm length has to be divisible by 16")
        if len(w1) < 192:
        ...
        if m1 is None:
            m1 = numpy.zeros_like(w1)
        if m2 is None:
            m2 = numpy.zeros_like(w1)
    ...
    def define_sequence(self, channel, segments=None, loops=None, jump_flags=None):
        ...
        if loops == None:
            loops = numpy.ones(len(segments), dtype=numpy.uint)
        if jump_flags == None:
            jump_flags = numpy.zeros(len(segments), dtype=numpy.ushort)
        if not len(loops) == len(segments) or not len(jump_flags) == len(segments):
            raise ValueError(
                "Length of segments (%i) does not match length of loops (%i) or length of jump_flags(%i)"
                % (len(segments), len(loops), len(jump_flags))
            )
        if len(segments) < 3:
            raise ValueError(
                "Sorry, you need at least 3 segments. Your command has %i segments"
                % (len(segments))
            )
    ...
    def set_seq_length(self, length, chpair=1):
        self.define_sequence(
            (2 * chpair - 1 if self._numchannels == 4 else chpair), length
        )
