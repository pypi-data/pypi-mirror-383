...
class Bluefors_GHS(Instrument):
    ...
    def __init__(self, name, host, port):
        ...
        self.add_parameter(
            "flowmeter", type=float, flags=Instrument.FLAG_GET, units="mmol/s"
        )
        self.add_parameter(
            "state",
            type=bool,
            flags=Instrument.FLAG_GET,
            channel_prefix="%s_",
            channels=["v%i" % i for i in range(1, 22)]
            + [
                "compressor",
                "ext",
                "hs-mc",
    ...
    def _reconnect(self):
        ...
        for i in range(5):
            try:
                if i > 1:
                    print(
                        __name__ + ": waiting 30 seconds before next reconnect attempt."
                    )
                    time.sleep(30)
