...
import zmq
from numpy import abs, append, delete, ones, sqrt
...
class tip2_client(Instrument):
        ...
        for d in self.get_devices():
            ...
            if i["type"] == "thermometer":
                self.add_parameter(
                    ...
                    units=i["unit"],
                    get_func=lambda d=d: self.get_param(d, "temperature"),
                )
                self.add_parameter(
                    "%s_active" % d,
                    type=bool,
                ...
                if d == self.default_device:
                    self.add_parameter(
                        "P", flags=Instrument.FLAG_GETSET, type=float, units=""
                    )
                    self.add_parameter(
                        "I", flags=Instrument.FLAG_GETSET, type=float, units=""
                    )
                    self.add_parameter(
                        "D", flags=Instrument.FLAG_GETSET, type=float, units=""
                    )
    ...
    def r_set_T(self, T, thermometer=None, safety_checks=True):
        ...
        if T > 1.0 and safety_checks:
            raise ValueError(
                __name__
                + ": r_set_T cancelled. Target temperature > 1K. Make sure what you are doing and disable safety checks."
            )
