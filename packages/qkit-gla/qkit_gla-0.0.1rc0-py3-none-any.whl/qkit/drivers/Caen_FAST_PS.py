...
class Caen_FAST_PS(Instrument):
    ...
    def __init__(self, name, address, port=10001):
        ...
        self.add_parameter("status", flags=Instrument.FLAG_GET, type=bool)
    ...
    def do_set_setcurrent(self, setcurrent):
        ...
        if recv == self._ak_str:
            current = (
                self.get_current()
            )  # stores the actual current value in instrument.parameters
            return True
        else:
            return "ERROR: " + error_msg[recv.split(":")[1]]
    ...
    def do_set_floating_mode(self, mode):
        ...
        Output:
                None
        """
        logging.debug(__name__ + f" : set floating mode to {mode}")
        if mode == "N":
            ...
            print("wrong input")
            return
        recv = self._ask(f"SETFLOAT:{mode}")
        if recv == self._ak_str:
            return True
        else:
            return "ERROR: " + error_msg[recv.split(":")[1]]
    ...
    def ramp_finished(self):
        time.sleep(0.1)
        while (
            int(self._ask("MST:?").split(":")[1], 16) & 2**12 == 2**12
        ):  # MP: gets 32bit status array; 12th from last bit indicates ramping process
            time.sleep(0.1)
        return True
    ...
    def ramp_current(self, current, ramp_rate=False, wait=True):
        ...
        """
        if ramp_rate:
            self.do_set_current_ramp_rate(ramp_rate)
    ...
    def ramp_voltage(self, voltage, ramp_rate=False, wait=True):
        ...
        """
        if ramp_rate:
            self.do_set_voltage_ramp_rate(ramp_rate)
