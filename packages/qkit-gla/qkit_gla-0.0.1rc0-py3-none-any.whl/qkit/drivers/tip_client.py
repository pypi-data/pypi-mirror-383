...
from numpy import abs, append, argmin, array, delete, ones, sqrt
...
class tip_client(Instrument):
    ...
    def __init__(self, name, address="localhost", port=9999):
        ...
        self.add_parameter("T", flags=Instrument.FLAG_GETSET, type=float, units="K")
    ...
    def new_T(self, T, dT_max=0.0005):
        ...
        while True:
            ...
            if abs(rmsTs - T) > dT_max:
                print(
                    "dT > dT_max(%.5f): %.5f at Tctl: %.5f Curr T: %.5f"
                    % (dT_max, rmsTs - T, T, T_current)
                )
                qkit.flow.sleep(2)
            else:
                break
    ...
    def autorange(self):
        ...
        for y in x:
            if (y["last_Res"] / RANGE[y["range"]]) < 0.1 or (
                y["last_Res"] / RANGE[y["range"]]
            ) > 2:
                newrange = max(
                    4, argmin(abs(y["last_Res"] / array(list(RANGE.values())) - 1))
                )  # we take a minimum RANGE setting of 4
                print(
                    "%s has a R_meas/R_ref value of %.4f and is set to range %i but I would set it to %i."
                    % (
                        y["name"],
                        y["last_Res"] / RANGE[y["range"]],
                        y["range"],
