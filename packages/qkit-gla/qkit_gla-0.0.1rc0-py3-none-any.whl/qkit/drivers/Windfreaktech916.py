...
class Windfreaktech916(Instrument):
    ...
    def load_calibration(self):
        ...
        try:
            data_ChA = np.load(
                ...
                + r"\Windfreak_"
                + self._serial
                + "_ChannelA_calibration_with_freq_and_pow.npy",
                allow_pickle=True,
            )
            data_ChB = np.load(
                path_hardcode
            ...
            self._interp_amplitudeA = RectBivariateSpline(
                f_channelA, p_channelA, cal_matrix_A
            )
            self._interp_amplitudeB = RectBivariateSpline(
                f_channelB, p_channelB, cal_matrix_B
            )

        except OSError:
            raise OSError("Calibration file of Windfreak source not found")
    ...
    def set_PLL_frequency(self, frequency):
        ...
        """
        if not frequency:
            self._visainstrument.write("x1")  # unstable
        else:
            self._visainstrument.write(f"*{int(frequency / 1e6)}x0")
    ...
    def do_get_frequency(self, channel):
        self._frequency[channel - 1] = 1e6 * float(
            self._visainstrument.query(f"C{channel - 1}f?")
        )
    ...
    def do_set_frequency(self, frequency, channel):
        ...
        self._visainstrument.write(f"C{channel - 1}f{frequency / 1e6:.6f}")
    ...
    def do_set_frequency_uncalibrated(self, frequency, channel):
        ...
        self._visainstrument.write(
            f"C{channel - 1}a{self._power_level[channel - 1]}"
        )
    ...
    def do_get_power_level(self, channel):
        self._power_level[channel - 1] = self._visainstrument.query(
            f"C{channel - 1}a?"
        )
    ...
    def do_set_power_level(self, level, channel):
        ...
        self._visainstrument.write(f"C{channel - 1}a{level}")
    ...
    def do_set_power(self, power, channel):
        ...
        if channel == 0:
            ...
            helper_fn = (
                lambda x: self._interp_amplitudeA(self._frequency[channel - 1], x)
                - power
            )
            ...
            if pwr_lvl > 45000:
                print(
                    "The minimum and maximum output power of this microwave source depends on frequency. "
                    "Your desired combination of frequency and output power is not possible. "
                    f"Power is set to max for this frequency: {self._interp_amplitudeA(self._frequency[channel - 1], 45000)}.1f dBm"
                )
            else:
        ...
        if channel == 1:
            # generate a helper function to find power level for desired power in dBm by finding roots of the function
            helper_fn = (
                lambda x: self._interp_amplitudeB(self._frequency[channel - 1], x)
                - power
            )
            # by finding the zero of the helper function, the power level value is determined
            pwr_lvl = int(optimize.newton(helper_fn, 20000))
            ...
            if pwr_lvl > 45000:
                print(
                    "The minimum and maximum output power of this microwave source depends on frequency. "
                    "Your desired combination of frequency and output power is not possible. "
                    f"Power is set to max for this frequency: {self._interp_amplitudeB(self._frequency[channel - 1], 45000)}.1f dBm"
                )
            else:
                self.do_set_power_level(pwr_lvl, channel)

    def do_get_status(self, channel):
        return bool(int(float(self._visainstrument.query(f"C{channel - 1}r?"))))
    ...
    def do_set_status(self, status, channel):
        ...
        Output:
            None
        """
        logging.debug(__name__ + f' : setting status to "{status}s"')

        if status:
            self._visainstrument.write(f"C{channel - 1}r1E1")

        else:
            self._visainstrument.write(f"C{channel - 1}r0E0")
