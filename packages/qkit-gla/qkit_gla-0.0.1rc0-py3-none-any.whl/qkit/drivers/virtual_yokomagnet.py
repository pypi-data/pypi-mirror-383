...
class virtual_yokomagnet(Instrument):
    def __init__(self, name, source=None, channel=0, stepsize=2e-9, stepdelay=1):
        ...
        # Add parameters
        self.add_parameter(
            "current", type=float, flags=Instrument.FLAG_GETSET, units="A"
        )
        self.add_parameter(
            "stepsize", type=float, flags=Instrument.FLAG_GETSET, units="A"
        )
        self.add_parameter("range", type=float, flags=Instrument.FLAG_GETSET, units="A")
        self.add_parameter(
            "stepdelay", type=float, flags=Instrument.FLAG_GETSET, units="ms"
        )
    ...
    def do_set_current(self, cur):
        if np.abs(cur) > self._range:
            raise ValueError(
                "set_current(): given value exeeds the range of %f" % self._range
            )
        old = self._old
