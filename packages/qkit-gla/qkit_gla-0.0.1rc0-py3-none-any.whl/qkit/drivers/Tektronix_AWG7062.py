...
class Tektronix_AWG7062(Instrument):
    ...
    def __init__(
        self, name, address, reset=False, clock=6e9, numpoints=1000, numchannels=2
    ):
        """
    ...
    def del_waveform(self, name):
        ...
        """
        logging.debug(
            __name__ + ' : Delete the waveform "%s" from the waveform list' % name
        )
    ...
    def load_waveform(self, channel, filename, drive="Z:", path="\\"):
        ...
        logging.debug(
            __name__
            + " : Load waveform file %s%s%s for channel %s"
            % (drive, path, filename, channel)
        )
        self._visainstrument.write(
            f'SOUR{channel}:FUNC:USER "{path}/{filename}","{drive}"'
        )
    ...
    def _add_load_waveform_func(self, channel):
        ...
        """
        func = lambda filename, drive="Z:", path="\\": self.load_waveform(
            channel, filename, drive, path
        )
    ...
    def load_settings(self, filename, drive="Z:", path="\\"):
        ...
        """
        logging.debug(
            __name__ + f" : Load settings file {drive}{path}{filename}"
        )
    ...
    def do_set_runmode(self, runmode):
        ...
        if (
            ...
            logging.error(
                __name__
                + ' : Unable to set trigger mode to %s, expected "CONT", "TRIG", "GATE" or "SEQ"'
                % runmode
            )
    ...
    def do_set_trigger_mode(self, mod):
        ...
        if mod.upper() == "TRIG":
            ...
            logging.error(
                __name__
                + ' : Unable to set trigger mode to %s, expected "TRIG" or "CONT"' % mod
            )
    ...
    def do_set_numpoints(self, numpts):
        ...
        if numpts != self._numpoints:
            logging.warning(
                __name__ + " : changing numpoints. This will clear all waveforms!"
            )
    ...
    def do_set_filename(self, name, channel):
        ...
        if exists:
            ...
            logging.debug(
                __name__
                + " : File exists on instrument, loading \
            into local memory"
            )
    ...
    def do_set_offset(self, offset, channel):
        ...
        """
        logging.debug(
            __name__ + f" : Set offset of channel {channel} to {offset:.6f}"
        )
    ...
    def do_get_marker1_low(self, channel):
        ...
        return float(
            self._visainstrument.ask("SOUR%s:MARK1:VOLT:LEV:IMM:LOW?" % channel)
        )
    ...
    def do_set_marker1_low(self, low, channel):
        ...
        logging.debug(
            __name__
            + f" : Set lower bound of marker1 of channel {channel} to {low:.3f}"
        )
        self._visainstrument.write(
            f"SOUR{channel}:MARK1:VOLT:LEV:IMM:LOW {low:.3f}"
        )
    ...
    def do_get_marker1_high(self, channel):
        ...
        return float(
            self._visainstrument.ask("SOUR%s:MARK1:VOLT:LEV:IMM:HIGH?" % channel)
        )
    ...
    def do_set_marker1_high(self, high, channel):
        ...
        logging.debug(
            __name__
            + f" : Set upper bound of marker1 of channel {channel} to {high:.3f}"
        )
        self._visainstrument.write(
            f"SOUR{channel}:MARK1:VOLT:LEV:IMM:HIGH {high:.3f}"
        )
    ...
    def do_get_marker2_low(self, channel):
        ...
        return float(
            self._visainstrument.ask("SOUR%s:MARK2:VOLT:LEV:IMM:LOW?" % channel)
        )
    ...
    def do_set_marker2_low(self, low, channel):
        ...
        logging.debug(
            __name__
            + f" : Set lower bound of marker2 of channel {channel} to {low:.3f}"
        )
        self._visainstrument.write(
            f"SOUR{channel}:MARK2:VOLT:LEV:IMM:LOW {low:.3f}"
        )
    ...
    def do_get_marker2_high(self, channel):
        ...
        return float(
            self._visainstrument.ask("SOUR%s:MARK2:VOLT:LEV:IMM:HIGH?" % channel)
        )
    ...
    def do_set_marker2_high(self, high, channel):
        ...
        logging.debug(
            __name__
            + f" : Set upper bound of marker2 of channel {channel} to {high:.3f}"
        )
        self._visainstrument.write(
            f"SOUR{channel}:MARK2:VOLT:LEV:IMM:HIGH {high:.3f}"
        )
    ...
    def wfm_import(self, file, path, format="WFM"):
        ...
        """
        self._visainstrument.write(f'MMEM:IMP "{file}", "{path}", {format}')
    ...
    def set_seq_twait(self, position, status):
        ...
        """
        self._visainstrument.write(
            "SEQ:ELEM%d:TWA %s" % (position, "on" if status else "off")
        )
    ...
    def do_set_clock_source(self, source):
        ...
        if source in ["INT", "EXT"]:
            ...
            raise ValueError(
                "You wanted to set the clock source to %s, but I can only accept INT or EXT."
                % source
            )
    ...
    def do_set_trigger_source(self, source):
        ...
        if source in ["INT", "EXT"]:
            ...
            raise ValueError(
                "You wanted to set the trigger source to %s, but I can only accept INT or EXT."
                % source
            )
