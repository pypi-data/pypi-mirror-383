...
def write(devchan, data, freq=1e4, minv=-10.0, maxv=10.0, timeout=10.0):
    ...
    try:
        ...
        if data.size == 1:
            CHK(
                nidaq.DAQmxWriteAnalogScalarF64(
                    AO_TaskHandle, 1, float64(timeout), float64(data), None
                )
            )
            written = int32(1)
        else:
        ...
        logging.error(
            "NI DAQ call failed (correct channel configuration selected?): %s", str(e)
        )
    finally:
...
def read(
    ...
    if isinstance(config, str):
        if config in _config_map:
            ...
            return None
    if type(config) not in (int32, int64, uInt32, uInt64):
        return None
    if samples == 1:
        retsamples = 1
        samples = 2
    ...
    try:
        ...
        if retsamples > 1:
            ...
            CHK(
                nidaq.DAQmxReadAnalogScalarF64(
                    AI_TaskHandle,
                    float64(timeout),
                    ctypes.c_int64(data.ctypes.data),
                    None,
...
def read_counter(devchan, samples=1, freq=1.0, timeout=1.0, src=""):
    ...
    try:
        ...
        CHK(
            nidaq.DAQmxCreateCICountEdgesChan(
                Crt_TaskHandle,
                devchan.encode("ascii"),
                b"",
                DAQmx_Val_Rising,
        ...
        if src is not None and src != "":
            CHK(
                nidaq.DAQmxSetCICountEdgesTerm(
                    Crt_TaskHandle, devchan.encode("ascii"), src
                )
            )
        nread = int32()
        data = np.zeros(samples, dtype=np.float64)
...
def sync_write_read(
    ...
    if type(config) is str:
        if config in _config_map:
            ...
            return None
    if type(config) not in (int, int32, uInt32, uInt64):
        return None
    debug = False
    oversamples = 10
    sampswritten = int32()
    ...
    CHK(
        nidaq.DAQmxGetDevAITrigUsage(
            I_devchan.encode("ascii"), ctypes.byref(triggerAvailable)
        )
    )
    if not (
        (int(triggerAvailable.value) & DAQmx_Val_Bit_TriggerUsageTypes_Start)
        == DAQmx_Val_Bit_TriggerUsageTypes_Start
    ):
        raise RuntimeError(
            "Error: Input trigger for synchronization is not available on this device"
        )
    # create task handles for input and output
    AI_TaskHandle = TaskHandle(0)
    AO_TaskHandle = TaskHandle(1)
    # prepare Analog Input
    CHK(nidaq.DAQmxCreateTask(str(AI_TaskHandle.value), ctypes.byref(AI_TaskHandle)))
    CHK(
        nidaq.DAQmxCreateAIVoltageChan(
            AI_TaskHandle,
    ...
    CHK(nidaq.DAQmxCreateTask(str(AO_TaskHandle.value), ctypes.byref(AO_TaskHandle)))
    CHK(
        nidaq.DAQmxCreateAOVoltageChan(
            AO_TaskHandle,
    ...
    CHK(
        nidaq.DAQmxCfgDigEdgeStartTrig(
            AI_TaskHandle, triggerName.encode("ascii"), DAQmx_Val_Rising
        )
    )
    CHK(
        nidaq.DAQmxWriteAnalogF64(
    ...
    if debug:
        print(
            "Debug: read_write() Analog-Output buffer written:", waveform, sampswritten
        )
    # start the tasks, sampling of (samples +1) at AI_TaskHandle is started when AO_TaskHandle started
    CHK(nidaq.DAQmxStartTask(AI_TaskHandle))
    CHK(nidaq.DAQmxStartTask(AO_TaskHandle))
    CHK(nidaq.DAQmxWaitUntilTaskDone(AI_TaskHandle, float64(timeout)))
    CHK(
        nidaq.DAQmxReadAnalogF64(
            AI_TaskHandle,
            AI_samples + 1,
