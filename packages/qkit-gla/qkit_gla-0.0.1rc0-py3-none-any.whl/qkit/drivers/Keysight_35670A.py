...
class Keysight_35670A(instrument, Instrument):
    ...
    def __init__(self, name, address, reset=False, **kwargs):
        ...
        super().__init__(
            address, timeout=10, chunk_size=4096, delay=1e-3, ip=self.ip
        )
        ...
        self.data_type = {
            "power": "XFR:POW",
            "linear": "XFR:POW:LIN",
            "coherence": "XFR:POW:COH",
            "cross": "XFR:POW:CROS",
            "time": "XTIM:VOLT",
            "histogram": "XTIM:VOLT:HIST",
        }
        ...
        self.add_parameter(
            "nop", type=int, flags=Instrument.FLAG_GET, minval=101, maxval=801
        )
        self.add_parameter(
            "averages", type=int, flags=Instrument.FLAG_GETSET, minval=1, maxval=99999
        )
    ...
    def set_return_format(self, s_digits=6):
        ...
        self.write(f"FORMAT:DATA ASCii, {s_digits}")
    ...
    def auto_calibration(self, status):
        self.write(f"calibration:auto {status}")
    ...
    def set_input_channels(self, channel, status):
        ...
        if status == 0 and channel == 1:
            ...
            return
        state = ["OFF", "ON"]
        self.write(f":inp{channel} {state[status]}")
    ...
    def set_ac_coupling(self, channels=[1, 2], val=[True, True]):
        ...
        for num, i in enumerate(channels):
            self.write(f":Input{i}:Coup {mode[val[num]]}")
    ...
    def get_ac_coupling(self, channel):
        ...
        :return:
        """
        return self.ask(f":Input{channel}:Coup?")

    #### Frequency options ####

    def do_set_centerfreq(self, center_freq):  # in Hz
        """sets the center frequency"""
        self.write(f"Freq:Cent {center_freq}")
        self.frequency_changed = True
    ...
    def do_set_startfreq(self, start_freq):
        ...
        self.write(f"Freq:Start {start_freq}")
        self.frequency_changed = True
    ...
    def do_set_frequency_resolution(self, resolution):
        ...
        if resolution not in [100, 200, 400, 800]:
            ...
            self.write(f"Freq:Res {resolution}")
    ...
    def do_set_instrument_mode(self, mode):
        ...
        self.write(f"INST:NSEL {mode_number}")
    ...
    def set_autoscale(self, trace, option):
        ...
        self.write(f"disp:wind{trace}trac:y:auto " + option)
    ...
    def get_autorange(self, channels=[1, 2]):
        ...
        for i in channels:
            reply.append(
                bool(self.ask(f"sens:volt{i}:dc:range:auto?")[1])
            )  # remove newline and + before by[1]
        return reply
    ...
    def set_trace_format(self, trace, option):
        ...
        """
        self.write(f"calc{trace}:format {option}")
    ...
    def get_trace_format(self, trace):
        ...
        """
        return self.ask(f"calc{trace}:format?")[0:-1].lower()
    ...
    def set_meas_data_type(self, option, channel, trace):
        ...
        message = f"CALC{trace}:FEED '{self.data_type[option]} {channel}'"
        self.write(message)
    ...
    def get_meas_data_type(self, trace):
        ...
        """
        reply = self.ask(f"CALC{trace}:FEED?")
        meas_data = (
            self.inv_data_type[reply.split()[0][1:]] + "_ch_" + reply.split()[1][:-1]
        )  # removing the ' in the reply
        return meas_data
    ...
    def do_set_Average(self, status):
        ...
        if status not in [0, 1]:
            ...
            self.set_measurement_continous(
                0
            )  # pauses measurement so that new sequence does not start automatically
            self.write(f"AVER {int(status)}")
    ...
    def fast_averaging(self, status, displaynumber=None):
        ...
        if status not in [0, 1]:
            ...
            return
        if status == 1:
            self.write(f"AVERAGE:IRES:RATE {displaynumber}")
            self.write(f"Average:IRES:Stat {status}")
        else:
            self.write(f"Average:Ires:Stat {status}")

    def do_set_averages(self, number):
        """sets the number of averages"""
        self.write(f"AVER:COUN {number}")
    ...
    def get_freqpoints(self):
        ...
        if self.instrument_mode == "FFT":
            if self.frequency_changed:
                ...
                while (
                    not self.ready()
                ):  # but spectroscopy asks for x-values before meausurment is started
                    qkit.flow.sleep(0.1)
            self.x_values = np.fromstring(
                self.ask("CALC:X:DATA?"), dtype=np.float32, sep=","
            )
    ...
    def get_single_trace(self, trace):
        ...
        trace_values = np.fromstring(
            self.ask(f"CALC{trace}:DATA?"), dtype=np.float32, sep=","
        )
        return trace_values
    ...
    def set_y_unit(self, unit, channel):
        ...
        if channel in [1, 2, 3, 4]:  # implement string for unit
            self.write(f"calculate{channel}:unit:voltage " + unit)

    def get_y_unit(self, trace):
        """gets the y-unit of the selected trace"""
        if trace in [1, 2, 3, 4]:
            y_unit = self.ask(f"CALCulate{trace}:UNIT:VOLTage?")[:-1]
            return y_unit.strip('"')
        else:
            print("wrong input")
