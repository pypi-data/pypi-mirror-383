...
class Anritsu_VNA_MS4642B(Instrument):
    ...
    def __init__(self, name, address, channel_index=1):
        ...
        self.add_parameter(
            "avg_type", type=str, flags=Instrument.FLAG_GETSET, tags=["sweep"]
        )
        ...
        self.add_parameter(
            "sweep_mode", type=str, flags=Instrument.FLAG_GETSET, tags=["sweep"]
        )
    ...
    def get_tracedata(self, format="AmpPha", single=False):
        ...
        data = self.ask_for_values(
            "FORM:DATA REAL; FORM:BORD SWAPPED; CALC%i:SEL:DATA:SDAT?" % (self._ci),
            fmt=3,
        )
    ...
    def get_powerpoints(self):
        self._powerpoints = numpy.linspace(
            self.get_source_power_start1(),
            self.get_source_power_stop1(),
            self.get_power_nop1(),
        )
    ...
    def do_set_averages(self, av):
        ...
        if self._zerospan == False:
            ...
            self.write(f"SWE:POIN {self._ci:.1f}")
    ...
    def do_set_trigger_source(self, source):
        ...
        if source.upper() in ["AUTO", "MAN", "EXT", "REM"]:
            ...
            raise ValueError(
                "set_trigger_source(): must be AUTO | MANual | EXTernal | REMote"
            )
    ...
    def do_set_source_attenuation(self, att, channel):
        ...
        if att in (0, 10, 20, 30, 40, 50, 60):
            logging.debug(
                __name__ + " : setting attenuation on port %i to %idB " % (channel, att)
            )
    ...
    def do_set_source_power_start(self, pow, channel):
        ...
        logging.debug(
            __name__
            + " : setting starting power on port %i to %.2f dBm " % (channel, pow)
        )
    ...
    def do_set_source_power_stop(self, pow, channel):
        ...
        logging.debug(
            __name__
            + " : setting stopping power on port %i to %.2f dBm " % (channel, pow)
        )
    ...
    def do_set_power_nop(self, n, channel):
        ...
        logging.debug(
            __name__
            + " : setting number of power sweep points on channel %i to %i "
            % (channel, n)
        )
