...
class Keysight_VNA_E5080B(Instrument):
    ...
    def __init__(self, name, address, channel_index=1):
        ...
        self.add_parameter(
            "measurement_parameter", type=str, flags=Instrument.FLAG_GETSET
        )
    ...
    def get_hold(self):
        self._hold = str(
            self._visainstrument.query("SENS%i:SWE:MODE?" % self._ci)
        ).rstrip()
    ...
    def get_tracedata(self, format="AmpPha", single=False, averages=None):
        ...
        data = self._visainstrument.query_binary_values(
            "CALC%i:MEAS%i:DATA:SDAT?" % (self._ci, self._active_trace)
        )
    ...
    def get_segments(self):
        if self.get_sweep_type(query=False) == "SEGM":
            ...
            segments = [0]
            for x in numpy.reshape(
                self._visainstrument.query_binary_values(
                    "sense:segment:list? SSTOP", datatype="d"
                ),
                (-1, 8),
            ):
    ...
    def get_freqpoints(self, query=False):
        ...
        if query:
            self._freqpoints = numpy.array(
                self._visainstrument.query_ascii_values("SENS:X?")
            )
        if self.get_cw():
    ...
    def do_get_span(self):
        ...
        )  # float( self.query('SENS1:FREQ:SPAN?'))
        return span
    ...
    def do_get_edel(self, channel):  # MP 04/2017
        ...
        self._edel = float(
            self._visainstrument.query("SENS1:CORR:EXT:PORT%i:TIME?" % channel)
        )
        return self._edel
    ...
    def do_get_cw(self):
        ...
        """
        if (self.get_nop() == 1) and (self.get_sweep_type() == "LIN"):
            ret = True
        else:
            ret = False
        return ret
    ...
    def do_get_active_trace(self):
        ...
        """
        self._active_trace = int(
            self._visainstrument.query("CALC%i:PAR:MNUM?" % (self._ci)).strip()
        )
    ...
    def do_set_trigger_source(self, source):
        ...
        if source.upper() in ["EXT", "IMM", "MAN"]:
            ...
            raise ValueError(
                "set_trigger_source(): must be EXTernal | IMMediate | MANual"
            )
