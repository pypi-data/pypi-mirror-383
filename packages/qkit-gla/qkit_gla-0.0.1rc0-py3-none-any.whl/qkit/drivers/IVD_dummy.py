...
def get_IVC_JJ(x, Ic, Rn, SNR):
    ...
    return (
        Rn * x * np.heaviside(np.abs(x) - Ic, int(sign > 0))
...
class IVD_dummy(Instrument):
    ...
    def take_IV(self, sweep):
        ...
        x = np.array(
            [
                np.sign(val)
                * round(np.abs(val), -int(np.floor(np.log10(np.abs(step)))) + 1)
                for val in np.linspace(
                    start, stop, int(round(np.abs(start - stop) / step + 1))
                )
            ]
        )  # round to overcome missing precision of np.linspace
