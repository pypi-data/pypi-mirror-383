...
def draw_config(switch, config):
    ...
    for i in range(1, 7):
        ...
        if fill is None:
            ...
            draw.text(
                (pos[i][0] + 3 + add, pos[i][1] - 1), str(i), fill=not fill, font=fnt
            )
...
class Switch:
    ...
    def switch_to(self, switch, port):
        if None in self._sw_pos[switch]:
            logging.warning(
                f"Not all switch positions are known for switch {switch}. If you can tolerate heating, please use .reset_all(switch={switch})."
            )
