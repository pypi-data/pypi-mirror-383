...
import qkit
from qkit.core.lib.misc import concat, str3
...
class h5plot:
    ...
    def plt_vector(self):
        ...
        self.y_label = concat(
            self.ds.attrs.get("name", "_name_"),
            " (",
            self._unit_prefixes[self.y_exp],
            self.ds.attrs.get("unit", "_unit_"),
            ")",
        )
        try:
            ...
            self.x_label = concat(
                self.x_data.attrs.get("name", "_xname_"),
                " (",
                self._unit_prefixes[self.x_exp],
                self.x_data.attrs.get("unit", "_xunit_"),
                ")",
            )
        except Exception:
    ...
    def plt_matrix(self):
        ...
        self.x_label = concat(
            self.x_ds.attrs.get("name", "_xname_"),
            " (",
            self._unit_prefixes[self.x_exp],
            self.x_ds.attrs.get("unit", "_xunit_"),
            ")",
        )
        self.y_ds = self.hf[self.y_ds_url]
        self.y_exp = self._get_exp(np.array(self.y_ds))
        self.y_label = concat(
            self.y_ds.attrs.get("name", "_yname_"),
            " (",
            self._unit_prefixes[self.y_exp],
            self.y_ds.attrs.get("unit", "_yunit_"),
            ")",
        )
    ...
    def plt_box(self):
        ...
        self.z_label = concat(
            self.z_ds.attrs.get("name", "_zname_"),
            " (",
            self._unit_prefixes[self.z_exp],
            self.z_ds.attrs.get("unit", "_zunit_"),
            ")",
        )
        ...
        # plot
        self.ax.set_title(
            concat(self.hf._filename[:-3], " ", self.ds.attrs.get("name", "_name_"))
        )
        self.cax = self.ax.imshow(
            self.ds_data,
            extent=(x_min, x_max, y_min, y_max),
