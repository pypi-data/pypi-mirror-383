...
try:
    ...
    from ipywidgets import HTML, IntProgress
    ...
    class Progress_Bar:
        def __init__(self, max_it, name="Progress:", est_cycle_time=None, dummy=False):
            ...
            self.pi = HTML(
                ...
                % (
                    ...
                    self.max_it,
                    "-?-"
                    if est_cycle_time == None
                    else time.strftime(
                        "%Y-%m-%d (%a) %H:%M:%S",
                        time.localtime(time.time() + est_cycle_time * self.max_it),
                    ),
                    "--:--:--"
                    if est_cycle_time == None
                    else hourformat(est_cycle_time * self.max_it),
                    "00:00:00",
                    "--:--:--"
                    if est_cycle_time == None
                    else hourformat(est_cycle_time * self.max_it),
        ...
        def _update(self, param=""):
            ...
            progr_info = (
                ...
                % (
                    ...
                    hourformat(
                        ...
                        + float(time.time() - self.start_eta_time)
                        / (self.progr - (0 if self.progr == 1 else 1))
                        * (self.max_it - (0 if self.progr == 1 else 1))
                    ),  # estimated
            ...
            if self.progr == self.max_it:  # last iteration
                ...
                pb_list.append(
                    [self.name, self.pb, self.pi]
                )  # append to the list of done PBs
    ...
    class Progress_Bar:
        def __init__(self, max_it, name="Progress:", est_cycle_time=None, dummy=False):
            ...
            display(
                Javascript(
                    "if(document.getElementById('%st') !== null) document.getElementById('%st').parentNode.remove()"
                    % (self.name, self.name)
                )
            )
            ...
            outp = (
                "<table style='width:100%%;border:none'><tr style='border:none'><td style='border:none'>%s (%i/%i) </td><td style='border:none'>&#9992; %s    </td><td style='border:none'>&#128336;  %s   (estimated)</td><td style='border:none'>&#10010;  %s (elapsed) </td><td style='border:none'>&#9866;  %s (remaining)</td></tr></table>"
                % (
                    "",
                    0,
                    self.max_it,
        ...
        def abort(self):
            ...
            display(
                Javascript(
                    "$('div#%s0').css('background-color', 'red');" % (self.divid)
                )
            )
