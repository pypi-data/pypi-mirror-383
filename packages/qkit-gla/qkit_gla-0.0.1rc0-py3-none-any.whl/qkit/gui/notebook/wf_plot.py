...
def wfplot(analog_wfm, seq=1, complete_marker=None, x_range=None, sample=None):
    ...
    if sample != None:
        xval = np.linspace(
            0, sample.exc_T * 1e6, int(np.round(sample.exc_T * sample.clock) / 4) * 4
        )  # in us
        ...
        try:
            ...
            axis.plot(
                (sample.exc_T - sample.overlap * 1e-9) * 1e6 + range(len(r1)),
                r1,
                "black",
            )
            axis.plot(
    ...
    if complete_marker != None:
        ...
        if len(np.array(complete_marker).shape) > 2:  # more than one single marker
            markers = np.array(complete_marker).reshape(
                4,
                np.array(complete_marker).shape[2],
        ...
        for mi, m in enumerate(markers):
            if (m != np.zeros_like(m)).any() and m != None:
                maxis.fill_between(
                    xval, 0, float(4 - mi) / 2 * m[seq], color=clr_dict[mi], alpha=0.7
                )
                maxis.plot(
                    xval,
                    float(4 - mi) / 2 * m[seq],
