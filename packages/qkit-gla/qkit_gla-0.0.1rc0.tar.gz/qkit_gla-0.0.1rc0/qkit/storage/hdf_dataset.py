...
class hdf_dataset:
    ...
    def append(self, data, reset=False, pointwise=False):
        ...
        if self.first:
            ...
            self.ds = self.hf.create_dataset(
                self.name,
                tracelength,
                folder=self.folder,
                dim=self.dim,
    ...
    def _create_timestamp_ds(self):
        ...
        self.ds_ts = self.hf.create_dataset(
            self.name + "_ts",
            tracelength=1,
            folder=self.folder,
            dim=max(self.dim - 1, 1),
            dtype="float64",
            ds_type=ds_type,
        )
        self.ds_ts.attrs.create("name", b"measurement_time")
        self.ds_ts.attrs.create("unit", b"s")
