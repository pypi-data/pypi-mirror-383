...
class DateTimeGenerator:
    ...
    def __init__(self):
        ...
        self.returndict["_localtime"] = time.localtime(
            self.returndict["_unix_timestamp"]
        )
        self.returndict["_timestamp"] = time.asctime(self.returndict["_localtime"])
        self.returndict["_timemark"] = time.strftime(
            "%H%M%S", self.returndict["_localtime"]
        )
        self.returndict["_datemark"] = time.strftime(
            "%Y%m%d", self.returndict["_localtime"]
        )
    ...
    def new_filename(self, name=None):
        ...
        self.returndict["_folder"] = os.path.join(
            qkit.cfg["datadir"], self.returndict["_relfolder"]
        )
        self.returndict["_relpath"] = os.path.join(
            self.returndict["_relfolder"], self.returndict["_filename"]
        )
        self.returndict["_filepath"] = os.path.join(
            self.returndict["_folder"], self.returndict["_filename"]
        )
    ...
    def new_filename_v1(self, name):
        ...
        self.returndict["_relfolder"] = os.path.join(
            self.returndict["_datemark"], filename
        )
...
def decode_uuid(string):
    ...
    while string != "":
        ...
        if f == -1:
            raise ValueError(f"Can not decode this: {string[::-1]}<--")
        output += f * multiplier
        multiplier *= la
        string = string[1:]
    return output
