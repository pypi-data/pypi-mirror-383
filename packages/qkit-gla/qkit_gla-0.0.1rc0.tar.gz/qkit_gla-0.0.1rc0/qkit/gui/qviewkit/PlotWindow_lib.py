    ...
    def mouseMoved(mpos):
        ...
        if plIt.sceneBoundingRect().contains(mpos):
            ...
            if plIt.ctrl.logYCheck.isChecked():  # logarithmic scale on x axis
                yval = 10**yval
            self.PointX.setText(f"X: {xval:.6e} {units[0]}")
            self.PointY.setText(f"Y: {yval:.6e} {units[1]}")

            try:
                self.data_coord = f"{xval:e}\t{yval:e}\t{self._last_x_pos - xval:e}\t{xval / (self._last_x_pos - xval):e}"
...
def _display_2D_data(self, graphicsView):
    ...
    def mouseMoved(mpos):
        ...
        self.PointX.setText(f"X: {xval:.6e} {units[0]}")
        self.PointY.setText(f"Y: {yval:.6e} {units[1]}")
        self.PointZ.setText(f"Z: {zval:.6e} {units[2]}")
        self.data_coord = f"{xval:g}\t{yval:g}\t{zval:g}"
