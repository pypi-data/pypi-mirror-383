...
class DatasetsWindow(QMainWindow, Ui_MainWindow):
    ...
    def _disable_live_update(self):
        try:
            if not self._force_live_plot and not self.h5file["entry"].attrs.get(
                "updating", True
            ):
                self.liveCheckBox.setChecked(False)
                self._force_live_plot = (
                    True  # don't disable the live plot if it is manually enabled again
                )
        except ValueError as e:
            qkit.logging.error(
                "Qviewkit/DatasetsWindow.py: Error on checking for live plot: " + str(e)
            )
    ...
    def populate_data_list(self):
        ...
        for i, pentry in enumerate(self.h5file["/entry"].keys()):
            ...
            s = (
                "comment:\t"
                + str3(self.h5file[tree_key].attrs.get("comment", ""))
                + "\n"
            )
    ...
    def handleSelectionChanged(self):
        ...
        if getSelected[0].parent():
            ds = str(
                "/entry/"
                + getSelected[0].parent().text(0)
                + "/"
                + getSelected[0].text(0)
            )
    ...
    def update_file(self):
        ...
        try:
            ...
            title = "Qviewkit: %s" % (
                self.DATA.DataFilePath.split(os.path.sep)[-1:][0][:6]
            )
