...
class AcquisitionThread(Thread, QObject):
    ...
    def setup_remote_connection(self):
        ...
        print(
            "Connection established to %s via port %s."
            % (str(self.data.localhost.ip), str(self.data.localhost.port))
        )
        ...
        while not self.data.wants_abort:
            for p in self.data.parameters:
                ...
                if (
                    p.logging
                    and p.check_history
                    and time.time() - p.timestamps[0] < p.range * 3600
                ):
                    try:
                        ...
                        print(
                            "Error retrieving history data for parameter %s."
                            % str(p.attribute_name)
                        )
                p.check_history = (
                    False  # switch off history request after one unsuccessful attempt
                )

                # delete old data points
                data_points_out_of_range = np.where(
                    time.time() - p.timestamps > p.range * 3600
                )
        ...
        for p in [self.data.pLL, self.data.pMC]:
            ...
            for i in range(1, len(p.values) - 1):
                if len(p.values[i - 4 : i]) > 0 and len(p.values[i + 1 : i + 5]) > 0:
                    median = np.sort(p.values[i - 4 : i + 5])[
                        int(0.5 * len(p.values[i - 4 : i + 5]))
                    ]
                    if (
                        np.abs(log10(p.values[i] / median)) > 6
                        and np.abs(log10(p.values[i] / p.values[i - 1])) > 6
                    ):
                        # if jumps detected collecting more than 5 orders of magnitude back or forth
                        p.values[i] = median
                        if self.data.debug:
                            print(
                                f"runaway detected in {p.attribute_name:s}, segment #{i:d}"
                            )
            p.graph.plt.setData(
                x=np.array((p.timestamps - time.time()) / 3600), y=p.values
            )
...
class DATA:
    ...
    def __init__(self, config):
        ...
        p_instances = config.get("parameters", "p").split(
            ","
        )  # parameter instance names
        # instanciate parameter objects
        self.parameters = [
            self.PARAMETER(config, i, p) for i, p in enumerate(p_instances)
        ]  # instanciate parameter array
        for i, p_i in enumerate(
            p_instances
        ):  # create human readable aliases, such that objects are accessible from clients according to the seetings.cfg entry in []
...
def main(argv):
    ...
    parser.add_argument(
        "ConfigFile", nargs="?", default="settings.cfg", help="Configuration file name"
    )
