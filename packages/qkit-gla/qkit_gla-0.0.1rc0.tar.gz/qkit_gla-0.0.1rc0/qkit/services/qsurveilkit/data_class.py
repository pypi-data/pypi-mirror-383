    ...
    class PARAMETER:
        ...
        def get_history(self, range, nchunks=100):
            ...
            if self.url_timestamps != None:
                try:
                    ...
                    data_points_requested_mask = np.where(
                        time.time() - timestamps < range * 3600
                    )
                    ...
                    if len(data_points_requested) > nchunks:
                        timestamp_chunks = np.array(
                            np.split(
                                timestamps_requested[
                                    : (
                                        len(timestamps_requested)
                                        - len(timestamps_requested) % nchunks
                                    )
                                ],
                                nchunks,
                            )
                        )
                        timestamps = timestamp_chunks[
                            :, int(0.5 * len(timestamps_requested) / nchunks)
                        ]
                        data_chunks = np.array(
                            np.split(
                                data_points_requested[
                                    : (
                                        len(data_points_requested)
                                        - len(data_points_requested) % nchunks
                                    )
                                ],
                                nchunks,
                            )
                        ...
                        medians = np.sort(data_chunks, axis=-1)[
                            :, int(0.5 * len(data_points_requested) / nchunks)
                        ]
        ...
        def create_logfile(self):
            ...
            self.fname = os.path.join(
                self.log_path,
                self.name.replace(" ", "_") + time.strftime("%d%m%Y%H%M%S") + ".h5",
            )
            # print self.fname
