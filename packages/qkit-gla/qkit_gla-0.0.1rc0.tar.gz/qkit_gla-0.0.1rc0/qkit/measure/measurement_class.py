...
class Measurement:
    ...
    def get_JSON(self):
        ...
        """
        return json.dumps(
            obj=self._get_copydict(), cls=QkitJSONEncoder, indent=4, sort_keys=True
        )
    ...
    def save(self, filepath=None):
        ...
        if filepath:
            ...
            filepath = os.path.join(
                qkit.cfg.get("datadir"), self.hdf_relpath.replace(".h5", ".measurement")
            )

        with open(filepath, "w+") as filehandler:
            json.dump(
                obj=self._get_copydict(),
                fp=filehandler,
                cls=QkitJSONEncoder,
                indent=4,
                sort_keys=True,
    ...
    def _get_copydict(self):
        ...
        """
        if copydict["sample"]:
            copydict["sample"] = copydict[
                "sample"
            ].__dict__  # change entry to dict to make it readable for JSONEncoder
        if copydict["instruments"]:
            if not isinstance(
                copydict["instruments"], dict
            ):  # conversion needed during measurement
        ...
        for ins_name in self.instruments:
            ...
            for param_name in ins.get_parameter_names():
                ...
                param_dict.update(
                    {
                        param_name: {
                            "content": ins.get(param_name, query=False),
    ...
    def update_instrument(self, ins_name):
        ...
        try:
            ...
            logging.error(
                "Relevant instruments and attributes not properly specified. "
                + str(ins_name)
            )
        else:
            """
            ...
            for param in params_dict.keys():
                if params_dict[param]["has_setter"]:
                    ins.set(params_dict[param], params_dict[param]["content"])
