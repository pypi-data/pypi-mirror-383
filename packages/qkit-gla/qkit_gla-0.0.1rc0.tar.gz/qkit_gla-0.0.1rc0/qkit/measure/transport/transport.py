...
class transport:
    ...
    def __init__(self, IV_Device):
        ...
        )  # function to calculate numerical derivative (default: Savitzky-Golay filter)
    ...
    def add_sweep_4quadrants(self, start, stop, step, offset=0, sleep=0):
        ...
        None
        """
        self.sweeps.add_sweep(
            start=start + offset, stop=+stop + offset, step=step, sleep=sleep
        )
        self.sweeps.add_sweep(
            start=+stop + offset, stop=start + offset, step=step, sleep=sleep
        )
        self.sweeps.add_sweep(
            start=start + offset, stop=-stop + offset, step=step, sleep=sleep
        )
    ...
    def add_sweep_halfswing(self, amplitude, step, offset=0, sleep=0):
        ...
        """
        self.sweeps.add_sweep(
            start=+amplitude + offset, stop=-amplitude + offset, step=step, sleep=sleep
        )
        self.sweeps.add_sweep(
            start=-amplitude + offset, stop=+amplitude + offset, step=step, sleep=sleep
        )
    ...
    def _numerical_derivative(self, x, y):
        ...
        try:
            return self._numder_func(
                y, *self._numder_args, **self._numder_kwargs
            ) / self._numder_func(x, *self._numder_args, **self._numder_kwargs)
        except:
    ...
    def set_x_parameters(self, x_vec, x_coordname, x_set_obj, x_unit="", x_dt=None):
        ...
        if np.iterable(x_vec):
            try:
                ...
                raise type(e)(
                    f"{__name__!s}: Cannot set {x_vec!s} as x-vector"
                )
        else:
            raise TypeError(
                f"{__name__:s}: Cannot set {x_vec!s} as x-vector: iterable object needed"
            )
        # x-coordname
        if type(x_coordname) is str:
            ...
            raise ValueError(
                f"{__name__:s}: Cannot set {x_coordname!s} as x-coordname: string needed"
            )
        # x-set-object
        if callable(x_set_obj):
            ...
            raise ValueError(
                f"{__name__:s}: Cannot set {x_set_obj!s} as x-set-object: callable object needed"
            )
        # x-unit
        if type(x_unit) is str:
            ...
            raise ValueError(
                f"{__name__:s}: Cannot set {x_unit!s} as x-unit: string needed"
            )
        # x dt
        if x_dt is not None:
            self._x_dt = x_dt
    ...
    def set_y_parameters(self, y_vec, y_coordname, y_set_obj, y_unit="", y_dt=None):
        ...
        if np.iterable(y_vec):
            try:
                ...
                raise type(e)(
                    f"{__name__!s}: Cannot set {y_vec!s} as y-vector"
                )
        else:
            raise TypeError(
                f"{__name__:s}: Cannot set {y_vec!s} as y-vector: iterable object needed"
            )
        # y-coordname
        if type(y_coordname) is str:
            ...
            raise ValueError(
                f"{__name__:s}: Cannot set {y_coordname!s} as y-coordname: string needed"
            )
        # y-set-object
        if callable(y_set_obj):
            ...
            raise ValueError(
                f"{__name__:s}: Cannot set {y_set_obj!s} as y-set-object: callable object needed"
            )
        # y-unit
        if type(y_unit) is str:
            ...
            raise ValueError(
                f"{__name__:s}: Cannot set {y_unit!s} as y-unit: string needed"
            )
        # y dt
        if y_dt is not None:
            self._y_dt = y_dt
    ...
    def set_xy_parameters(
        ...
        if type(x_name) is str:
            ...
            raise ValueError(
                f"{__name__:s}: Cannot set {x_name!s} as x-name: string needed"
            )
        # x-func
        if callable(x_func):
            self._x_func = x_func
            ...
            raise ValueError(
                f"{__name__:s}: Cannot set {x_func!s} as x-function: callable object needed"
            )
        # x-vec
        if np.iterable(x_vec):
            self._x_vec = x_vec
        ...
        if type(x_kwargs) is dict:
            ...
            raise ValueError(
                f"{__name__:s}: Cannot set {x_kwargs!s} as x-kwargs: dict needed"
            )
        # y-name
        if type(y_name) is str:
            ...
            for name in y_name:
                if type(name) is not str:
                    raise ValueError(
                        f"{__name__:s}: Cannot set {name!s} as y-name: string needed"
                    )
            self._y_name = y_name
        else:
            raise ValueError(
                f"{__name__:s}: Cannot set {y_name!s} as y-name: string of iterable object of strings needed"
            )
        # y-func
        if callable(y_func):
            self._y_func = [y_func]
            ...
            for func in y_func:
                if not callable(func):
                    raise ValueError(
                        f"{__name__:s}: Cannot set {func!s} as y-function: callable object needed"
                    )
            self._y_func = y_func
        else:
            raise ValueError(
                f"{__name__:s}: Cannot set {y_func!s} as y-function: callable object of iterable object of callable objects needed"
            )
        # y-unit
        if type(y_unit) is str:
            self._y_unit = [y_unit]
            ...
            for unit in y_unit:
                if type(unit) is not str:
                    raise ValueError(
                        f"{__name__:s}: Cannot set {unit!s} as y-unit: string needed"
                    )
            self._y_unit = y_unit
        else:
            raise ValueError(
                f"{__name__:s}: Cannot set {y_unit!s} as y-unit: string of iterable object of strings needed"
            )
        # y-kwargs
        if type(y_kwargs) is dict:
            ...
            for kwargs in y_kwargs:
                if type(kwargs) is not dict:
                    raise ValueError(
                        f"{__name__:s}: Cannot set {kwargs!s} as y-kwargs: dict needed"
                    )
            self._y_kwargs = y_kwargs
        else:
            raise ValueError(
                f"{__name__:s}: Cannot set {y_kwargs!s} as y-kwargs: dict of iterable object of dicts needed"
            )
        # x-dt
        self._x_dt = x_dt
        return
    ...
    def set_log_function(self, func=None, name=None, unit=None, dtype="f"):
        ...
        if callable(func):
            ...
            for fun in func:
                if not callable(fun):
                    raise ValueError(
                        f"{__name__:s}: Cannot set {fun!s} as y-function: callable object needed"
                    )
        else:
            raise ValueError(
                f"{__name__:s}: Cannot set {func!s} as log-function: callable object of iterable object of callable objects needed"
            )
        ...
        if name is None:
            ...
            for _name in name:
                if type(_name) is not str:
                    raise ValueError(
                        f"{__name__:s}: Cannot set {_name!s} as log-name: string needed"
                    )
        else:
            raise ValueError(
                f"{__name__:s}: Cannot set {name!s} as log-name: string of iterable object of strings needed"
            )
        self.log_name = name
        # log-unit
        if unit is None:
            ...
            for _unit in unit:
                if type(_unit) is not str:
                    raise ValueError(
                        f"{__name__:s}: Cannot set {_unit!s} as log-unit: string needed"
                    )
        else:
            raise ValueError(
                f"{__name__:s}: Cannot set {unit!s} as log-unit: string of iterable object of strings needed"
            )
        self.log_unit = unit
        # log-dtype
        if dtype is None:
            ...
            for _dtype in dtype:
                if type(_dtype) is not str:
                    raise ValueError(
                        f"{__name__:s}: Cannot set {_dtype!s} as log-dtype: string needed"
                    )
        else:
            raise ValueError(
                f"{__name__:s}: Cannot set {dtype!s} as log-dtype: string of iterable object of strings needed"
            )
    ...
    def get_log_function(self):
        ...
        """
        return (
            [f.__name__ for f in self.log_function],
            self.log_name,
            self.log_unit,
            self.log_dtype,
        )
    ...
    def measure_2D(self):
        ...
        if self._x_set_obj is None:
            logging.error(f"{__name__:s}: axes parameters not properly set")
            raise TypeError(f"{__name__:s}: axes parameters not properly set")
            ...
            if self._scan_dim == 0:
                ...
                if self._fit_name:
                    for i in range(self.sweeps.get_nos()):
                        datasets.append(f"analysis/{self._fit_name:s}_{i:d}")
                elif self._scan_dim > 1:
                    for i in range(self.sweeps.get_nos()):
                        datasets.append(
                            f"{self._IV_modes[not self._bias].lower():s}_{i:d}"
                        )
            self._qvk_process = qviewkit.plot(
                self._data_file.get_filepath(), datasets=datasets
            )  # opens IV-view by default
        """ measurement """
        sys.stdout.flush()
        qkit.flow.start()
        try:
            if self._scan_dim == 0:  # single data points
                for x_val in self._x_vec:
                    ...
                    for func, kwargs, lst in zip(
                        self._y_func, self._y_kwargs, self._hdf_y
                    ):
                        lst.add(func(**kwargs))
                ...
                _rst_log_hdf_appnd = (
                    False  # variable to save points of log-function in 2D-matrix
                )
                self._rst_fit_hdf_appnd = False
                for self.ix, (x, x_func) in enumerate(
                    [(None, _pass)]
                    if self._scan_dim < 2
                    else [(x, self._x_set_obj) for x in self._x_vec]
                ):  # loop: x_obj with parameters from x_vec if 2D or 3D else pass(None)
                    x_func(x)
                    time.sleep(self._x_dt)
                    for self.iy, (y, y_func) in enumerate(
                        [(None, _pass)]
                        if self._scan_dim < 3
                        else [(y, self._y_set_obj) for y in self._y_vec]
                    ):  # loop: y_obj with parameters from y_vec if 3D else pass(None)
                        y_func(y)
                    ...
                    if self._scan_dim == 3:
                        for lst in [
                            ...
                            for k, val in enumerate(
                                [self._hdf_I, self._hdf_V, self._hdf_dVdI]
                            )
                            if k < 2 + int(self._dVdI)
                        ]:
            ...
            print(f"Measurement complete: {self._data_file.get_filepath():s}")
        return
    ...
    def _prepare_measurement_IVD(self):
        ...
        """
        self._sweep_mode = (
            self._IVD.get_sweep_mode()
        )  # 0 (VV-mode) | 1 (IV-mode) | 2 (VI-mode) | 3 (II-mode)
    ...
    def _prepare_measurement_file(self):
                ...
                list(
                    filter(
                        ...
                        (
                            "xy"
                            if self._scan_dim == 0
                            else f"{self._scan_dim:d}D_IV_curve",
                            self._filename,
                            self._expname,
                        ),
                    )
        ...
        self._measurement_object.instruments = (
            qkit.instruments.get_instrument_names()
        )  # qkit.instruments.get_instruments() #
        ...
        if self._scan_dim == 0:
            """xy"""
            # add data variables
            self._hdf_x = self._data_file.add_coordinate(
                self._x_name, unit=self._x_unit
            )
            self._hdf_y = [
            ...
            for i in range(self.sweeps.get_nos()):
                self._hdf_bias.append(
                    self._data_file.add_coordinate(
                        f"{self._IV_modes[self._bias]:s}_b_{i!s}",
                        unit=self._IV_units[self._bias],
                    )
                )
                self._hdf_bias[i].add(
                    self._get_bias_values(sweep=self.sweeps.get_sweep())
                )
                self._hdf_I.append(
                    self._data_file.add_value_vector(
                        f"I_{i!s}",
                ...
                if self._fit_func:
                    self._hdf_fit.append(
                        self._data_file.add_value_vector(
                            f"{self._fit_name:s}_{i:d}",
                            x=np.zeros(1),
                            unit=self._fit_unit,
                            save_timestamp=False,
                            folder="analysis",
            ...
            """2D scan"""
            self._hdf_x = self._data_file.add_coordinate(
                self._x_coordname, unit=self._x_unit
            )
            ...
            self._hdf_x.add(self._x_vec)
            self._hdf_y = self._data_file.add_coordinate(
                self._y_coordname, unit=self._y_unit
            )
            ...
            for i in range(self.sweeps.get_nos()):
                ...
                if self._fit_func:
                    ...
                    self._data_fit.append(
                        np.ones((len(self._x_vec), len(self._y_vec))) * np.nan
                    )
            # log-function
            self._add_log_value_vector()
            # add views
    ...
    def _prepare_progress_bar(self):
        ...
        if self.progress_bar:
            ...
            num_its = {
                ...
                * [1 if self._average is None else self._average][0],
                2: len(self._x_vec)
                * self.sweeps.get_nos()
                * [1 if self._average is None else self._average][0],
                3: len(self._x_vec)
            ...
            if self._landscape and self._scan_dim in [2, 3]:
                ...
                bias_values = np.array(
                    [
                        np.meshgrid(
                            self._get_bias_values(sweep), np.ones(len(self._x_vec))
                        )[0]
                ...
                lsc_limits = np.array(
                    [
                        ...
                        -self._lsc_vec
                        if self._lsc_mirror
                        else np.ones(shape=self._x_vec.shape)
                        * np.array(self.sweeps.get_sweeps())[:, :2],
                    ]
                )
                lsc_mask = np.logical_and(
                    bias_values
                    <= np.dot(
                        np.ones((bias_values.shape[2], 1)), [np.max(lsc_limits, axis=0)]
                    ).T,
                    bias_values
                    >= np.dot(
                        np.ones((bias_values.shape[2], 1)), [np.min(lsc_limits, axis=0)]
                    ).T,
                )
                num_its[self._scan_dim] = (
                    np.sum(lsc_mask)
                    * len(self._y_vec)
                    * [1 if self._average is None else self._average][0]
                )
                self._pb_addend = np.concatenate(
                    list(zip(*np.sum(lsc_mask, axis=2)))
                )  # value that counter has to be increased after each sweep
            self._pb = Progress_Bar(
                max_it=num_its[self._scan_dim],
                name="_".join(
                    list(
                        filter(
                            None,
    ...
    def _get_bias_values(self, sweep):
        ...
        return np.round(
            np.linspace(
                start, np.sign(stop - start) * (nop - 1) * step + start, nop
            ),  # stop is rounded down to multiples of step
            decimals=ndigits,
        )  # every element is rounded to overcome missing precision of numpy linspace
    ...
    def _get_numder_comment(self, name):
        ...
        """
        return (
            f"{self._numder_func.__module__!s}.{self._numder_func.__name__!s}("
            + ", ".join(
                np.concatenate(
                    [
                        [name],
                        np.array(self._numder_args, dtype=str)
                        if self._numder_args
                        else [],
                        [
                            f"{key:s}={val!s}"
                            for key, val in self._numder_kwargs.items()
                        ]
                        if self._numder_kwargs
                        else [],
                    ]
                )
            )
    ...
    def _get_fit_comment(self, i):
        ...
        """
        return (
            f"{self._fit_func.__module__!s}.{self._fit_func.__name__!s}("
            + ", ".join(
                np.concatenate(
                    [
                        [
                            f"I={self._hdf_I[i].name:s}",
                            f"V={self._hdf_V[i].name:s}",
                            f"dVdI={self._hdf_dVdI[i].name:s}",
                        ],
                        [
                            f"{key:s}={val!s}"
                            for key, val in self._fit_kwargs.items()
                        ]
                        if self._fit_kwargs
                        else [],
                    ]
    ...
    def _add_log_value_vector(self):
        ...
        if self.log_function != [None]:
            ...
            for i, _ in enumerate(self.log_function):
                if self._scan_dim == 1:
                    self._hdf_log.append(
                        self._data_file.add_coordinate(
                            self.log_name[i], unit=self.log_unit[i]
                        )
                    )
    ...
    def _add_views(self):
        ...
        None
        """
        self._hdf_view_IV = self._data_file.add_view(
            "IV",
            x=self._hdf_V[0],
            y=self._hdf_I[0],
            view_params={"labels": ("V", "I"), "plot_style": 1, "markersize": 5},
        )
        self._hdf_view_VI = self._data_file.add_view(
            "VI",
            x=self._hdf_I[0],
            y=self._hdf_V[0],
            view_params={"labels": ("I", "V"), "plot_style": 1, "markersize": 5},
        )
        ...
        if self._dVdI:
            self._hdf_view_dVdI = self._data_file.add_view(
                "dVdI",
                x=self._hdf_I[0],
                y=self._hdf_dVdI[0],
                view_params={"labels": ("I", "dVdI"), "plot_style": 1, "markersize": 5},
            )
            for i in range(1, self.sweeps.get_nos()):
                self._hdf_view_dVdI.add(
                    x=[self._hdf_I, self._hdf_V][self._bias][i], y=self._hdf_dVdI[i]
                )
        return
    ...
    def _get_sweepdata(self):
        ...
        if self._average is None:
            ...
            for j in range(self.sweeps.get_nos()):
                ...
                data = {
                    self._hdf_I[j]: (I_values,),
                    self._hdf_V[j]: (V_values,),
                }  # tuple in oder to use *args later
                if self._dVdI:
                    data[self._hdf_dVdI[j]] = (
                        self._numerical_derivative(I_values, V_values),
                    )
                if self._fit_func:
                    ...
                    #                                        **self._fit_kwargs)
                    if self._scan_dim == 1:
                        self._data_fit[j] = float(
                            self._fit_func(
                                data[self._hdf_I[j]][0],
                        ...
                        self._data_fit[j][self.ix] = float(
                            self._fit_func(
                                data[self._hdf_I[j]][0],
                                data[self._hdf_V[j]][0],
                                data[self._hdf_dVdI[j]][0],
                        ...
                        self._data_fit[j][self.ix, self.iy] = float(
                            self._fit_func(
                                data[self._hdf_I[j]][0],
                                data[self._hdf_V[j]][0],
                                data[self._hdf_dVdI[j]][0],
                        ...
                        data[self._hdf_fit[j]] = (
                            self._data_fit[j][self.ix],
                            self._rst_fit_hdf_appnd,
                        )
                        self._rst_fit_hdf_appnd = not bool(
                            self.iy + 1 == len(self._y_vec)
                        )
                # save data
                ...
                if self.progress_bar:
                    self._pb.iterate(
                        addend=self._pb_addend[self.ix] if self._landscape else 1
                    )
                qkit.flow.sleep()
        else:
            I_values, V_values = [], []
            for i in range(self._average):
                ...
                for j in range(self.sweeps.get_nos()):
                    ...
                    if self._IVD.get_sweep_mode() % 2 == 0:  # volt bias
                        for val, lst in zip(
                            self.take_IV(sweep=self.sweeps.get_sweep()),
                            [V_values, I_values],
                        ):
                            lst[i].append(
                                list(val)
                            )  # append as list in order to later use zip
                    elif self._IVD.get_sweep_mode() % 2 == 1:  # curr bias
                        for val, lst in zip(
                            self.take_IV(sweep=self.sweeps.get_sweep()),
                            [I_values, V_values],
                    ...
                    I_values_avg, V_values_avg = np.mean(
                        list(zip(*I_values))[j], axis=0
                    ), np.mean(
                        list(zip(*V_values))[j], axis=0
                    )  # use zip since np.mean cannot handle different shapes
                    data = {self._hdf_I[j]: I_values_avg, self._hdf_V[j]: V_values_avg}
                    if self._dVdI:
                        data[self._hdf_dVdI[j]] = self._numerical_derivative(
                            I_values_avg, V_values_avg
                        )
                    if self._fit_func:
                        data[self._hdf_fit[j]] = self._fit_func(
                            data[self._hdf_I[j]],
                            data[self._hdf_V[j]],
                            data[self._hdf_dVdI[j]],
                            **self._fit_kwargs
                    ...
                    for key, val in data.items():
                        key.append(
                            val, reset=bool(i)
                        )  # append data series or overwrite last iteration by new averaged data
                        key.ds.attrs["average"] = f"({i + 1:d}/{self._average:d})"  # add (iteration/average) as attribute
            ...
            for j in range(self.sweeps.get_nos()):
                for lst in [
                    val
                    for k, val in enumerate([self._hdf_I, self._hdf_V, self._hdf_dVdI])
                    if k < 2 + int(self._dVdI)
                ]:
    ...
    def take_IV(self, sweep):
        ...
        if self._landscape:
            ...
            if self._lsc_mirror:
                sweep_lsc = np.nanmin(
                    [np.abs(sweep), [bias_lim, bias_lim, np.nan, np.nan]], axis=0
                ) * np.sign(sweep)
            else:
                sweep_lsc = np.nanmin(
                    [sweep, [bias_lim, bias_lim, np.nan, np.nan]], axis=0
                )
            # find landscape bounds in full bias values
            bias_data = self._get_bias_values(sweep)
            mask = np.logical_and(
                bias_data >= np.min(sweep_lsc[:2]), bias_data <= np.max(sweep_lsc[:2])
            )
            sweep_lsc[:2] = (
                bias_data[np.where(mask)[0]][0],
                bias_data[np.where(mask)[0]][-1],
            )
            # take data
            data = self._IVD.take_IV(sweep=sweep_lsc)
            # fill skipped bias values with np.nan to keep shape constant
            I_values, V_values = np.array([np.nan] * len(mask)), np.array(
                [np.nan] * len(mask)
            )
