...
def erf(
    pulse, attack, decay, sample, length=None, position=None, low=0, high=1, clock=None
):
    r"""
    ...
    if (
        position == None
    ):  # automatically correct overlap only when position argument not explicitly given
        position = length
    ...
    if attack != 0:
        if attack < 2.0 / clock:
            logging.warning(
                __name__
                + " : attack too small compared to AWG sample frequency, setting to %.4g s"
                % (2.0 / clock)
            )
            attack = 2.0 / clock
    ...
    if decay != 0:
        if decay < 2.0 / clock:
            logging.warning(
                __name__
                + " : decay too small compared to AWG sample frequency, setting to %.4g s"
                % (2.0 / clock)
            )
            decay = 2.0 / clock
...
def exp(pulse, decay, sample, position=None, low=0, high=1, clock=None):
    ...
    wfm[sample_start:sample_end] += np.exp(
        -np.arange(sample_end - sample_start) / (decay * clock)
    ) * (high - low)
    return wfm


def triangle(
    attack, decay, sample, length=None, position=None, low=0, high=1, clock=None
):
    """
    ...
    wfm = low * np.ones(sample_length, dtype=dtype)
    wfm[sample_start : sample_start + sample_attack] = np.linspace(
        low, high, sample_attack
    )
...
def square(
    ...
    sample_length = (
        int(np.round(length * clock) / 4) * 4
    )  # Ensures that the number of samples is divisible by 4 @andre20150615
    # sample_length = int(np.ceil(length*clock)) #old definition
    wfm = low * np.ones(sample_length, dtype=dtype)
    if sample_start < sample_end:
        wfm[int(sample_start)] = high + (low - high) * (
    ...
    if freq == None:
        ...
        for i in range(int(sample_end) - int(np.ceil(sample_start))):
            wfm[i + int(np.ceil(sample_start))] = high * np.sin(
                2 * np.pi * freq / clock * (i)
            )
    if np.ceil(sample_end) != np.floor(sample_end):
        wfm[int(sample_end)] = low + (high - low) * (sample_end - int(sample_end))
    return wfm
...
def gauss(pulse, sample, length=None, position=None, low=0, high=1, clock=None):
    ...
    if sample_start < sample_end:
        wfm[
            int(sample_start)
        ] = 0.0  # high + (low-high)*(sample_start-int(sample_start))
    # wfm[int(np.ceil(sample_start)):int(sample_end)] = high
    pulsesamples = int(int(sample_end) - int(sample_start))
    for i in range(pulsesamples):
        wfm[int(np.ceil(sample_start) + i)] = high * np.exp(
            -((i - pulsesamples / 2.0) ** 2) / (2.0 * (pulsesamples / 5.0) ** 2)
        )
    if np.ceil(sample_end) != np.floor(sample_end):
        wfm[int(sample_end)] = low + (high - low) * (sample_end - int(sample_end))
    return wfm
...
def arb_function(function, sample, pulse, length=None, position=None, clock=None):
    ...
    times = 1.0 / clock * np.arange(0, sample_end - sample_start)
...
def t1(delay, sample, length=None, low=0, high=1, clock=None, DRAG_amplitude=None):
    ...
    if DRAG_amplitude == None:
        ...
        wfm = drag(
            sample.tpi, sample, DRAG_amplitude, length, length - delay, clock=clock
        )
...
def ramsey(
    ...
    if DRAG_amplitude == None:
        if pi2_pulse == None:
            pi2_pulse = sample.tpi2
        if delay + 2 * pi2_pulse > position:
            logging.error(__name__ + " : ramsey pulses do not fit into waveform")
        wfm = square(pi2_pulse, sample, length, position, clock=clock)
        wfm += square(
            pi2_pulse, sample, length, position - delay - pi2_pulse, clock=clock
        )
...
def spinecho(
    ...
    if round(adddelay + delay + 2 * pi2_pulse + n * pi_pulse, 10) > round(
        position, 10
    ):  # round bc of floating points arethmetic
        logging.error(
            __name__
            + " : sequence does not fit into waveform. delay is the sum of the waiting times in between the pi pulses"
        )

    if DRAG_amplitude == None:
        if readoutpulse:  # last pi/2 pulse
            wfm = square(
                pi2_pulse, sample, length, position, clock=clock, freq=freq
            ) * np.exp(
                0j
            )  # add pi/2 pulse
        else:
            wfm = square(
                pi2_pulse, sample, length, position, low, low, clock, freq=freq
            ) * np.exp(
                0j
            )  # create space (low) of the length of a pi/2 pulse
        ...
        wfm += square(
            ...
            sample,
            length,
            position - pi2_pulse - n * pi_pulse - delay - adddelay,
            clock=clock,
            freq=freq,
        ) * np.exp(
            0j
        ...
        if phase == 0:
            wfm = wfm.real  # to avoid conversion error messages

    else:
        if readoutpulse:  # last pi/2 pulse
            wfm = drag(
                pi2_pulse, sample, DRAG_amplitude, length, position, clock=clock
            ) * np.exp(
                0j
            )  # add pi/2 pulse
        else:
    ...
    if DRAG_amplitude == None:
        ...
        for ni in range(n):  # add pi pulses
            wfm += square(
                ...
                position
                - (delay + n * pi_pulse)
                * (np.sin((np.pi * (ni + 1)) / (2 * n + 2))) ** 2
                - adddelay,
                clock=clock,
                freq=freq,
            ) * np.exp(
                phase * 1j
            )  # no pi2_pulse subtracted because equation yields position of center
        wfm += square(
            pi2_pulse,
            sample,
            length,
...
def drag(pulse, sample, amplitude, length=None, position=None, clock=None):
    ...
    if (
        ...
        if hasattr(sample, "overlap"):  # if overlap exists in sample object
            ...
            logging.warning("overlap attribute not found in sample object")
    wfm = gauss(
        pulse, sample, length=np.ceil(length * 1e9) / 1e9, position=position
    ) + 1j * np.concatenate(
        [
            np.diff(
                gauss(
    ...
    wfm[
        int((position - pulse) * clock - 1) : int((position - pulse) * clock + 1)
    ] = wfm.real[
        int((position - pulse) * clock - 1) : int((position - pulse) * clock + 1)
    ]  # for smooth derivative
    wfm[int(position * clock - 1) : int(position * clock + 1)] = wfm.real[
        int(position * clock - 1) : int(position * clock + 1)
    ]
