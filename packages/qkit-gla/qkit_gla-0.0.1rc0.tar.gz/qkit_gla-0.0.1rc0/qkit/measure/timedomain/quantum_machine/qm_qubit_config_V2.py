        ...
        self.config = {
            ...
            "controllers": {
                opx_one: {
                    ...
                    "analog_inputs": {1: {"offset": 0}, 2: {"offset": 0}},  # I  # Q
                    "digital_outputs": {1: {}},
                },
            },
            "elements": {
                ...
                "resonator": {
                    ...
                    "time_of_flight": 32
                    + 4 * self.wait_res,  # multiple of 4, minimum 32 clock cycles, ns
            ...
            "waveforms": {
                ...
                "const_wf": {"type": "constant", "sample": 0.49},
                "pi_wf": {"type": "arbitrary", "samples": self.pi_wf.tolist()},
                "pi/2_wf": {"type": "arbitrary", "samples": self.pi2_wf.tolist()},
                "readout_wf": {
                    "type": "arbitrary",
            ...
            "integration_weights": {
                "integ_w1_I": {
                    ...
                    * (
                        self.readout_pulse_len // 4 + self.smearing_res
                    ),  # 1 TODO smearing ??
                    "sine": [np.sin(self.theta)]
                    * (self.readout_pulse_len // 4 + self.smearing_res),  # 0
                },
                "integ_w2_I": {
                    "cosine": [-np.sin(self.theta)]
                    * (self.readout_pulse_len // 4 + self.smearing_res),  # 0
                    "sine": [np.cos(self.theta)]
                    * (self.readout_pulse_len // 4 + self.smearing_res),  # 1
                },
                "integ_w1_Q": {
                    "cosine": [np.sin(self.theta)]
                    * (self.readout_pulse_len // 4 + self.smearing_res),  # 0
