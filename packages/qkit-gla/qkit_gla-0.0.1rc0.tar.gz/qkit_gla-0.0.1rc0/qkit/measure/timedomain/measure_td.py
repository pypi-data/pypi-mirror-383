...
class Measure_td:
        ...
        if iterations > 1:
            ...
            for j in range(self.ndev):
                ...
                hdf_amp_avg = hdf_file.add_value_matrix(
                    "amplitude_avg_%i" % i, x=self._hdf_y, y=self._hdf_x, unit="a.u."
                )
                hdf_pha_avg = hdf_file.add_value_matrix(
                    "phase_avg_%i" % i, x=self._hdf_y, y=self._hdf_x, unit="rad"
                )
    ...
    def measure_1D_ddc_time_trace(self):
        ...
        """
        time_end = (
            float(self.sample.mspec.get_samples()) / self.sample.mspec.get_samplerate()
        )
    ...
    def _prepare_measurement_file(self):
        ...
        if self.mode == 1:  # 1D
            ...
            for i in range(self.ndev):
                self._hdf_amp.append(
                    self._hdf.add_value_vector(
                        "amplitude_%i" % i, x=self._hdf_x, unit="a.u."
                    )
                )
                self._hdf_pha.append(
                    self._hdf.add_value_vector(
                        "phase_%i" % i, x=self._hdf_x, unit="rad"
                    )
                )
            if self.ReadoutTrace:
            ...
            for i in range(self.ndev):
                self._hdf_amp.append(
                    self._hdf.add_value_matrix(
                        "amplitude_%i" % i, x=self._hdf_x, y=self._hdf_y, unit="a.u."
                    )
                )
                self._hdf_pha.append(
                    self._hdf.add_value_matrix(
                        "phase_%i" % i, x=self._hdf_x, y=self._hdf_y, unit="rad"
                    )
                )
            if self.ReadoutTrace:
            ...
            for i in range(self.ndev):
                self._hdf_amp.append(
                    self._hdf.add_value_matrix(
                        "amplitude_%i" % i, x=self._hdf_y, y=self._hdf_x, unit="a.u."
                    )
                )
                self._hdf_pha.append(
                    self._hdf.add_value_matrix(
                        "phase_%i" % i, x=self._hdf_y, y=self._hdf_x, unit="rad"
                    )
                )
                if self.state_0_cal and self.state_1_cal:
                    self._hdf_p1.append(
                        self._hdf.add_value_matrix(
                            "p1_%i" % i, x=self._hdf_y, y=self._hdf_x, unit=""
                        )
                    )
            if self.ReadoutTrace:
        ...
        if self.create_averaged_data:
            ...
            for i in range(self.ndev):
                self._hdf_amp_avg.append(
                    self._hdf.add_value_vector(
                        "amplitude_avg_%i" % i, x=self._hdf_x, unit="a.u."
                    )
                )
                self._hdf_pha_avg.append(
                    self._hdf.add_value_vector(
                        "phase_avg_%i" % i, x=self._hdf_x, unit="rad"
                    )
                )

        if self.comment:
            self._hdf.add_comment(self.comment)
        self._hdf.hf.hf.attrs["default_ds"] = [
            "data0/amplitude_%i" % i for i in range(min(5, self.ndev))
        ...
        if self.open_qviewkit:
            self._qvk_process = qviewkit.plot(
                ...
                datasets=["amplitude_%i" % i for i in range(min(5, self.ndev))]
    ...
    def _append_data(self, iteration=0, ddc=None):
        ...
        if len(ampliData.shape) < 3:  # "normal" measurements
            ...
            if self.ReadoutTrace:
                if (
                    self.mode < 3
                ):  # mode 2 not yet fully supported but working for DDC timetrace experiments
            ...
            for i in range(self.ndev):
                for j in range(ampliData.T.shape[2]):
                    self._hdf_amp[i].append(
                        np.atleast_1d(ampliData.T[i, :, j]), pointwise=True
                    )
                    self._hdf_pha[i].append(
                        np.atleast_1d(phaseData.T[i, :, j]), pointwise=True
                    )
                    if self.ReadoutTrace:
        ...
        if self.create_averaged_data:
            if iteration == 0:
                ...
                for i in range(self.ndev):
                    self._hdf_amp_avg[i].append(
                        np.atleast_1d(ampliData.T[i]), pointwise=True
                    )
                    self._hdf_pha_avg[i].append(
                        np.atleast_1d(phaseData.T[i]), pointwise=True
                    )
            else:
                self.avg_complex_sum += ampliData * np.exp(1j * phaseData)
