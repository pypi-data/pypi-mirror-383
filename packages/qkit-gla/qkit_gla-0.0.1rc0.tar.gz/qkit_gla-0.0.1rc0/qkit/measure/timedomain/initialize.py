...
class InitializeTimeDomain:
    ...
    optional_params = {
        ...
        "acqu_window": [
            " position of the adc recording window as array [start, end]. No longer needed to be divisible by 32. (samples)",
            [96, 545],
        ],
        "overlap": [" Overlap between manipulation window and readout pulse.", 0],
        "readout_delay": [
            " Time offset of the readout pulse to adjust the timing of the devices",
            0,
        ],
        "readout_pha": [" Phase of the readout tone", 0],
        "readout_relamp": [" relative amplitude of the different readout tones", 1],
        "spec_input_level": [
            " Input range in mV of the spectrum adc card. Restricted to card-dependant values",
            250,
        ],
    }
    ...
    def initialize(self):
        ...
        for p in self.params:
            if p not in self._sample.__dict__:
                logging.error(
                    "Please specify '" + p + "' in your sample object:" + self.params[p]
                )
                breakpoint = True
        if breakpoint:
            raise ValueError(
                "Not all vallues in your sample file are present. I can not continue. Sorry."
            )
        ...
        if True:  # Tabor AWG in 4ch mode
            ...
            if self._sample.T_rep < 1.5 * self._sample.exc_T:
                raise ValueError(
                    "Your repetition rate T_rep is too small for the chosen window exc_T."
                )

            if (
                "Tektronix" in self._sample.readout_awg.get_type()
                and self._sample.T_rep < 170e-6
            ):
                raise ValueError(
                    "You are using the Tektronix AWG as readout. This has a bug and your repetition rate should be >= 200us."
                )
        ...
        self._sample.readout.set_LO(
            np.mean(self._sample.fr) - self._sample.readout_iq_frequency
        )  # set LO lower and use mixer as a up-converter. For multiplexing set LO to centerfreq - IQ freq
        self._sample.readout.set_tone_freq(
            np.atleast_1d(self._sample.fr)
        )  # probe tone frequency
        self._sample.readout.set_tone_pha(
            np.atleast_1d(self._sample.readout_pha)
        )  # phase shift (usually not necessary)
        self._sample.readout.set_tone_relamp(
            np.atleast_1d(self._sample.readout_relamp)
        )  # relative amplitude of tone

        self.update_timings()

        self._sample.mspec.set_gate_func(
            gate_func.Gate_Func(awg=self._sample.awg, sample=self._sample).gate_fcn
        )
        ...
        self._sample.spec_samplerate = (
            self._sample.mspec.get_samplerate()
        )  # We want to have the actual samplerate in the sample object
    ...
    def update_timings(self, zero_dac_delay=False):
        ...
        self._sample.readout.set_dac_duration(
            self._sample.readout_tone_length
        )  # length of the readout tone
    ...
    def record_single_trace(self):
        ...
        plt.grid()
        plt.xlabel(
            "samples (%.0fMHz samplerate: 1sample = %.3gns)"
            % (
                self._sample.mspec.get_samplerate() / 1e6,
                1.0 / self._sample.mspec.get_samplerate() * 1e9,
            )
        )
        ...
        plt.ylim(-128, 127)
        clips = np.size(np.where(np.array(samples).flatten() == 127)) + np.size(
            np.where(np.array(samples).flatten() == -128)
        )
        if clips > 50:
            logging.error(
                "Clipping detected, please reduce amplifier voltage (%r Clips)" % clips
            )
        elif clips > 0:
            print(
    ...
    def align_pulses(self, samples=768):
        ...
        plt.grid()
        plt.xlabel(
            f"samples ({sr * 1e3:.0f}MHz samplerate: 1 sample = {1.0 / sr:.3g}ns)"
        )
        plt.ylabel("amplitude")
        plt.twiny()
        ...
        plt.show()
        inp = raw_input(f"readout_delay ({self._sample.readout_delay:g}s) = ")
        if inp == "":
            return
    ...
    def crop_recording_window(self):
        ...
        def pltfunc(start, end, done):
            if done:
                ...
                self._dw.description = f"acqu_window set to [{start:d}:{end:d}]"
            else:
                plt.figure(figsize=(15, 5))
                plt.plot(msp)
                plt.axvspan(0, start, color="k", alpha=0.2)
                plt.axvspan(end, len(msp), color="k", alpha=0.2)
                plt.xlim(0, len(msp))
                plt.show()

        self._sw = widgets.IntSlider(
            min=0,
            max=len(msp),
            step=1,
            value=self._sample.acqu_window[0],
            continuous_update=True,
        )
        self._ew = widgets.IntSlider(
            min=0,
            max=len(msp),
            step=1,
            value=self._sample.acqu_window[1],
            continuous_update=True,
        )
        self._dw = widgets.Checkbox(value=False, description="Done!", indent=True)
        self._wgt = widgets.interact(
            pltfunc, start=self._sw, end=self._ew, done=self._dw
        )
        self._sample.mspec.set_window(*self._sample.acqu_window)
    ...
    def check_sidebands(self):
        ...
        spread = (
            np.ptp(
                np.append(np.atleast_1d(self._sample.fr), self._sample.readout.get_LO())
            )
            * 1.2
        )
        plt.xlim(
            [
                self._sample.readout.get_LO() - spread,
                self._sample.readout.get_LO() + spread,
            ]
        )
        plt.grid()
    ...
    def update_qubit_mw_src(self):
        ...
        self._sample.qubit_mw_src.set_frequency(
            self._sample.f01 - self._sample.iq_frequency
        )
