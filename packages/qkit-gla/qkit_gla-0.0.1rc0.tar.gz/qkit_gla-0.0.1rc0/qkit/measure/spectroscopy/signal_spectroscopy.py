...
class spectrum:
    ...
    def __init__(self, sig_analyzer, exp_name="", sample=None):
        ...
        self.averaging_start_ready = (
            "start_measurement" in self.sig_analyzer.get_function_names()
            and "ready" in self.sig_analyzer.get_function_names()
        )
        if not self.averaging_start_ready:
            logging.warning(
                __name__
                + ": With your signal analyzer driver ("
    ...
    def _prepare_measurement_sig_analyzer(self):
        ...
        for i in range(self.num_traces):
            self.traces_names.append(
                self.sig_analyzer.get_trace_name(i + 1)
            )  # must be implemented in driver
            self.units.append(
                self.sig_analyzer.get_y_unit(i + 1)
            )  # must also be impelmented
    ...
    def measure_1D(self, rescan=True, web_visible=True):
        ...
        if self.open_qviewkit:
            self._qvk_process = qviewkit.plot(
                self._data_file.get_filepath(), datasets=self.traces_names
            )
            ...
            if self.averaging_start_ready:
                ...
                if self.progress_bar:
                    self._p = Progress_Bar(
                        self.sig_analyzer.get_averages(),
                        self.dirname,
                ...
                if (
                    self.sig_analyzer.get_averages() == 1
                    or self.sig_analyzer.get_Average() == False
                ):  # no averaging
                    if self.progress_bar:
                        self._p = Progress_Bar(
                ...
                if self._scan_3D:
                    for y in self.y_vec:
                        ...
                        if (
                            np.min(
                                np.abs(
                                    self.center_freqs[ix]
                                    - y * np.ones(len(self.center_freqs[ix]))
                                )
                            )
                            > self.span / 2.0
    ...
    def _end_measurement(self):
        ...
        self.dirname = None
        if self.averaging_start_ready:
            self.sig_analyzer.post_measurement()
