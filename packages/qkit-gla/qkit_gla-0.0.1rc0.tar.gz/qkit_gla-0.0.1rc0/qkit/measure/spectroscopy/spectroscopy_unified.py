...
class spectrum(MeasureBase):
    ...
    def set_x_parameters(self, vec, coordname, set_obj, unit, dt=None):
        ...
        None
        """
        super().set_x_parameters(vec, coordname, set_obj, unit, dt)
    ...
    def set_y_parameters(self, vec, coordname, set_obj, unit, dt=None):
        ...
        None
        """
        super().set_y_parameters(vec, coordname, set_obj, unit, dt)
        if self.landscape.xylandscapes:
            self.landscape.delete_landscape_function_xy()
            logging.warning("xy landscape has been deleted")
        if self.landscape.xzlandscape_func:
        ...
        if self._segments:
            f = [
                self.Coordinate(
                    "frequency_%i" % i,
                    unit="Hz",
                    values=self._freqpoints[
                        0 if i == 0 else self._segments[i - 1] : self._segments[i]
                    ],
                )
            ...
            self._prepare_measurement_file(
                [
                    self.Data("real", [f], ""),
                    self.Data("imag", [f], ""),
                    self.Data("amplitude", [f], "arb. unit", save_timestamp=True),
    ...
    def measure_2D(self):
        ...
        if (
            ...
            self._x_parameter.set_function = self.landscape.vna_frequency_wrapper(
        ...
        if self._segments:
            ...
            self._prepare_measurement_file(
                [
                    self.Data(
                        P[0] % i,
                        [self._x_parameter, f[i]],
                        P[1],
                        save_timestamp=P[2] and not i,
                    )
                    for i in range(len(self._segments))
                    for P in [
                        ["amplitude_%i", "arb. unit", True],
            ...
            self._prepare_measurement_file(
                [
                    ...
                    self.Data("phase", [self._x_parameter, f], "rad"),
                ]
            )

        self._pb = Progress_Bar(
            len(self._x_parameter.values),
            "2D VNA sweep " + self.measurement_name,
        ...
        if self._nop < 10:
            ...
            self._views = [
                self._data_file.add_view(
                    "amplitude_midpoint",
                    x=self._x_parameter.hdf_dataset,
                    y=self._datasets["amplitude"],
                    view_params=dict(
                        transpose=True,
    ...
    def measure_3D(self):
        ...
        if self._segments:
            ...
            self._prepare_measurement_file(
                [
                    self.Data(
                        P[0] % i,
                        [self._x_parameter, self._y_parameter, f[i]],
                        P[1],
                        save_timestamp=P[2] and not i,
                    )
                    for i in range(len(self._segments))
            ...
            self._prepare_measurement_file(
                [
                    self.Data(
                        "amplitude",
                        [self._x_parameter, self._y_parameter, f],
                        "arb. unit",
                        save_timestamp=True,
                    ),
                    self.Data(
                        "phase", [self._x_parameter, self._y_parameter, f], "rad"
                    ),
                ]
            )
        ...
        if self.progress_bar:
            ...
            ):  # ToDo: This part could be part of the Landscape class
                truth = np.full(
                    (len(self._y_parameter.values), len(self._x_parameter.values)),
                    False,
                ...
                for e in self.landscape.xylandscapes:
                    if not e["blacklist"]:
                        truth = np.logical_or(
                            ...
                            (
                                np.abs(
                                    self._y_parameter.values[:, np.newaxis]
                                    - e["center_points"]
                                )
                                <= e["y_span"] / 2
                            )
                            * (  # check y span
                                e["x_range"][0] <= self._x_parameter.values
                            )
                            * (self._x_parameter.values <= e["x_range"][1]),
                        )  # check x range
            ...
            self._pb = Progress_Bar(
                points,
                "3D VNA sweep " + self.measurement_name,
                self.vna.get_sweeptime_averages(),
            )
        else:
    ...
    def measure_timetrace(self):
        ...
        if self.vna.get_span() > 0:
            raise ValueError(
                __name__ + ": For timetrace scans, VNA span needs to be 0 Hz"
            )
        if self.vna.get_Average():
            raise ValueError(
                __name__ + ": For timetrace scans, Averaging needs to be turned off."
            )
        ...
        t = self.Coordinate(
            "time",
            unit="s",
            values=np.arange(0, self._nop, 1)
            * self.vna.get_sweeptime()
            / (self._nop - 1),
        )
        f = self.Coordinate("frequency", unit="Hz", values=[self.vna.get_centerfreq()])
        iteration = self.Coordinate(
            "trace_number", unit="", values=np.arange(0, self.number_of_timetraces, 1)
        )

        self._prepare_measurement_file(
            [
                self.Data(
                    "amplitude", [iteration, t], "arb. unit", save_timestamp=True
                ),
                self.Data("phase", [iteration, t], "rad"),
            ],
            coords=f,
        )

        self._pb = Progress_Bar(
            self.number_of_timetraces,
            "VNA timetrace " + self.measurement_name,
            self.vna.get_sweeptime_averages(),
            dummy=not self.progress_bar,
        )
    ...
    def _acquire_vna_data(self):
        if self.averaging_start_ready:
            ...
            if self._scan_time:
                qkit.flow.sleep(
                    self.vna.get_sweeptime(query=False)
                )  # to prevent timeouts in time scan
            elif self.vna.ready():
            ...
            while not self.vna.ready():
                qkit.flow.sleep(
                    min(self.vna.get_sweeptime_averages(query=False) / 11.0, 0.2)
                )
        else:
            self.vna.avg_clear()
    ...
    def _append(self, amplitude, phase, real=None, imag=None):
        if self._segments:
            for i in range(len(self._segments)):
                self._datasets["amplitude_%i" % i].append(
                    amplitude[
                        0 if i == 0 else self._segments[i - 1] : self._segments[i]
                    ]
                )
                self._datasets["phase_%i" % i].append(
                    phase[0 if i == 0 else self._segments[i - 1] : self._segments[i]]
                )
                if real is not None and imag is not None:
                    self._datasets["real_%i" % i].append(
                        real[0 if i == 0 else self._segments[i - 1] : self._segments[i]]
                    )
                    self._datasets["imag_%i" % i].append(
                        imag[0 if i == 0 else self._segments[i - 1] : self._segments[i]]
                    )
        else:
    ...
    def generate_fit_function_xz(self, curve_f, curve_p, z_span=None, p0=[-1, 0.1, 7]):
        ...
        try:
            ...
            self.xz_freqpoints = np.arange(
                self.xzlandscape_func(self.spec.x_vec[0]) - self.z_span / 2,
