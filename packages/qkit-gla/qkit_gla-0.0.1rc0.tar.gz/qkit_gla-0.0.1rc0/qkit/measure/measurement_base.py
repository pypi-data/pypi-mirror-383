...
class MeasureBase:
    ...
    def __getattr__(self, item):
        if item == "x_vec":
            ...
            raise AttributeError(
                f"Measurement object has no attribute '{item}'"
            )
    ...
    def set_sample(self, sample):
        if sample is None:
            ...
            raise ValueError(
                __name__
                + f": The given sample '{sample!s}' is not a qkit sample object."
            )
        self._sample = sample
    ...
    def add_log_function(self, func, name, unit="", log_dtype=float):
        ...
        if not callable(func):
            raise ValueError(
                f"{__name__:s}: Cannot set {func!s} as log-function: callable object needed"
            )
        if type(name) is not str:
            raise ValueError(
                f"{__name__:s}: Cannot set {name!s} as log-name: string needed"
            )
        if type(unit) is not str:
            raise ValueError(
                f"{__name__:s}: Cannot set {unit!s} as log-unit: string needed"
            )
    ...
    class Coordinate:
        def __init__(self, name, unit="", values=None, set_function=None, wait_time=0):
            if type(name) is not str:
                raise TypeError(
                    f"{__name__:s}: Cannot set {self.name!s} as name for coordinate: string needed"
                )
            self.name = name
            self.unit = unit
            self.values = values
        ...
        def validate_parameters(self):
            ...
            if type(self.unit) is not str:
                raise TypeError(
                    f"{__name__:s}: Cannot set {self.unit!s} as unit for coordinate {self.name!s}: string needed"
                )
            if np.iterable(self.values):
                try:
                    ...
                    if len(self.values) == 0:
                        logging.warning(
                            f"{__name__:s}: Setting zero-length vector for coordinate {self.name!s}. Please check if this is intended."
                        )
                except Exception as e:
                    raise type(e)(
                        f"{__name__:s}: Cannot set {self.values!s} as vector for coordinate {self.name!s}: Array conversion failed:{e!s}"
                    )
            else:
                raise TypeError(
                    f"{__name__:s}: Cannot set {self.values!s} as vector for coordinate {self.name!s}: iterable object needed"
                )
            if not callable(self.set_function):
                raise TypeError(
                    f"{__name__:s}:  Cannot set {self.set_function!s} as function for coordinate {self.name!s}: callable object needed"
                )
            if type(self.wait_time) in [int, float]:
                self.wait_time = float(self.wait_time)
            else:
                raise TypeError(
                    f"{__name__:s}:  Cannot set {self.wait_time!s} as wait time for coordinate {self.name!s}: float needed"
                )
            return True

        def create_dataset(self, hdf_file):
            if (
                self.hdf_dataset is None or self.hdf_dataset.hf != hdf_file.hf
            ):  # If dataset not yet created or belongs to old hdf file
                ...
                logging.info(
                    __name__
                    + f": Dataset for coordinate '{self.name}' was already created."
                )
    ...
    class Data:
        def __init__(
            ...
            **kwargs
            ...
            if type(name) is not str:
                raise TypeError(
                    f"{__name__:s}: Cannot set {name!s} as name for Data: string needed"
                )
            if type(coords) not in (list, tuple):
                raise TypeError(
                    f"{__name__:s}: Cannot set {coords!s} as coordinates for data {name!s}: list or tuple of Coordinates needed"
                )
            for c in coords:
                if not isinstance(c, MeasureBase.Coordinate):
                    raise TypeError(
                        f"{__name__:s}: Cannot set {c!s} as coordinate for data {name!s}: Please use the qkit.measure.MeasureBase.Coordinate type"
                    )
            self.name = name
            ...
            self.kwargs = kwargs
            self.kwargs.update(
                {"save_timestamp": save_timestamp, "average_over": average_over}
            )
            self.hdf_dataset = None
        ...
        def validate_parameters(self):
            if type(self.name) is not str:
                raise TypeError(
                    f"{__name__:s}: Cannot set {self.name!s} as name for Data: string needed"
                )
            if type(self.coordinates) not in (list, tuple):
                raise TypeError(
                    f"{__name__:s}: Cannot set {self.coordinates!s} as coordinates for data {self.name!s}: list or tuple of Coordinates needed"
                )
            for c in self.coordinates:
                if not isinstance(c, MeasureBase.Coordinate):
                    raise TypeError(
                        f"{__name__:s}: Cannot set {c!s} as coordinate for data {self.name!s}: Please use the qkit.measure.MeasureBase.Coordinate type"
                    )
            self.dim = len(self.coordinates)
            if type(self.unit) is not str:
                raise TypeError(
                    f"{__name__:s}: Cannot set {self.unit!s} as unit for data {self.name!s}: string needed"
                )
            return True
        ...
        def create_dataset(self, hdf_file):
            if (
                ...
                if self.dim == 1:
                    self.hdf_dataset = hdf_file.add_value_vector(
                        self.name, x=c[0], unit=self.unit, **self.kwargs
                    )
                elif self.dim == 2:
                    self.hdf_dataset = hdf_file.add_value_matrix(
                        self.name, x=c[0], y=c[1], unit=self.unit, **self.kwargs
                    )
                elif self.dim == 3:
                    self.hdf_dataset = hdf_file.add_value_box(
                        self.name, x=c[0], y=c[1], z=c[2], unit=self.unit, **self.kwargs
                    )
            else:
                logging.info(
                    __name__
    ...
    def set_x_parameters(self, vec, coordname, set_obj, unit, dt=None):
        ...
        # vec
        try:
            self._x_parameter = self.Coordinate(
                coordname, unit, np.array(vec, dtype=float), set_obj, dt
            )
    ...
    def set_y_parameters(self, vec, coordname, set_obj, unit, dt=None):
        ...
        # vec
        try:
            self._y_parameter = self.Coordinate(
                coordname, unit, np.array(vec, dtype=float), set_obj, dt
            )
    ...
    def _create_file_name(self, data):
        ...
        for d in data:
            for c in d.coordinates:
                coordinates.add(
                    c.name.rstrip("_0123456789")
                )  # removes indices like frequency_0 from the file name
            self._dim = max(
                self._dim, len(d.coordinates)
            )  # if you have several 2D scans, the dimension is still 2D
        if not self.measurement_name:
            self.measurement_name = ", ".join(coordinates)
        self._file_name = f"{self._dim}D_" + self.measurement_name.replace(
            " ", ""
        ).replace(",", "_")
    ...
    def _prepare_measurement_file(self, data, coords=()):
        ...
        for c in coords:
            if not isinstance(c, self.Coordinate):
                raise TypeError(
                    f"{__name__:s}:  {c!s} is no valid coordinate object"
                )
        ...
        for d in data:
            ...
            for c in d.coordinates:
                self._coordinates[c.name] = c.create_dataset(
                    self._data_file
                )  # it doesn't matter if you call create_dataset multiple times.
        for c in coords:
            self._coordinates[c.name] = c.create_dataset(
                self._data_file
            )  # the additional coordinates
        ...
        if self._dim > 1:
            ...
            for [func, name, unit, log_dtype] in self.log_functions:
                self._log_datasets.append(
                    [
                        self._data_file.add_value_vector(
                            name,
                            x=data[0].coordinates[0].create_dataset(self._data_file),
                            unit=unit,
                            dtype=log_dtype,
                        ),
                        func,
    ...
    def _end_measurement(self):
        ...
        threading.Thread(
            target=qviewkit.save_plots, args=[self._data_file.get_filepath()]
        ).start()
