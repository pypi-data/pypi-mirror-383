...
class UUID_base:
    ...
    def get_time(self, uuid):
        ...
        while uuid != "":
            ...
            if f == -1:
                raise ValueError(
                    f"fid.get_time: Can not decode this: {uuid[::-1]}<--"
                )
            output += f * multiplier
            multiplier *= la
            uuid = uuid[1:]
...
class file_system_service(UUID_base):
    ...
    def _store_cache_files(self):
        ...
        """
        write_protocol = (
            2  # For Python 2 compatibility 0=text, 1...x binary, -1 highest binary.
        )
    ...
    def update_file_db(self):
        with self.lock:
            ...
            print(
                "Initialized the file info database (qkit.fid) in %.3f seconds."
    ...
    def _collect_info(self, uuid, path):
        ...
        if ord(uuid[0]) > ord("L"):
            ...
            try:
                if j_split[-3][0:3] != 201:  # not really a measurement file then
                    ...
                    dt = f"{j_split[-3][:4]}-{j_split[-3][4:6]}-{j_split[-3][6:]} {tm[:2]}:{tm[2:4]}:{tm[4:]}"
            except IndexError:
                dt = None
            user = None
            run = None
        ...
        if qkit.cfg.get("fid_scan_hdf", False):
            ...
            try:
                ...
                if "measurement" in h5f["/entry/data0"]:
                    try:
                        ...
                        h5_info_db.update(
                            {
                                arg: mmt[arg]
                                for arg in ["run_id", "user", "rating", "smt"]
                                if mmt.has_key(arg)
                            }
                        )
                ...
                logging.debug(
                    f"fid could not index file {path}, probably it is just new and empty. Original message: {e}"
                )
            except OSError as e:
                logging.error(f"fid {path}:{e}")
            finally:
                h5f.close()
    ...
    def add_h5_file(self, h5_filename):
        if qkit.cfg["fid_scan_datadir"]:
            threading.Timer(
                20, function=self._add, kwargs={"h5_filename": h5_filename}
            ).start()
    ...
    def _add(self, h5_filename):
        ...
        if h5_filename[-3:] != ".h5":
            logging.error(
                f"Tried to add '{h5_filename:s}' to the qkit.fid database: Not a .h5 filename."
            )
        with self.lock:
            if os.path.isfile(h5_filename):
                ...
                logging.error(
                    f"Tried to add '{h5_filename:s}' to the qkit.fid database: File does not exist."
                )
            ...
            if os.path.isfile(h5_filename[:-2] + "measurement"):
                logging.debug(
                    "Store_db: Adding manually measurement: " + basename + "measurement"
                )
    ...
    def wait(self):
        with self.lock:
            pass
        qkit.flow.sleep(0.1)  # to prevent timing issues
        if self.lock.locked():
            self.wait()
        return True
