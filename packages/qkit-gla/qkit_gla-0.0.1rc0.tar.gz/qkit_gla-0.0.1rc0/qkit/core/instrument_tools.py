...
import os
import sys
from importlib import reload  # Python 3 way to reload modules
...
class Insttools:
    ...
    def create(self, name, instype, **kwargs):
        ...
        try:
            ...
            for param_name in ins.get_parameter_names():
                if (
                    ins.get_parameter_options(param_name)["flags"] & ins.FLAG_GET
                ):  # do not query non-get or softget parameters
                    ins.get(
                        param_name
                    )  # Get all device parameters. This ensures that all get functions are working.
        ...
        fname = os.path.join(
            qkit.cfg["datadir"], "instrument.txt"
        )  # save to datadir, because this is synced to backup server
        open(fname, "a").close()  # create file if not existing
        with open(fname, "r+") as f:
            if descr.strip() not in [r.strip() for r in f.readlines()]:
                f.write(descr)
        return self.get(name)
    ...
    def reload(self, ins):
        ...
        logging.info("reloading %r, type: %r, kwargs: %r", insname, instype, kwargs)
