...
class Resonator:
    ...
    def set_x_coord(self, x_co):
        ...
        try:
            ...
            logging.warning(
                "Unable to open any x_coordinate. Please set manually using 'set_x_coord()'."
            )
    ...
    def set_y_coord(self, y_co):
        ...
        try:
            ...
            logging.warning(
                "Unable to open any y_coordinate. Please set manually using 'set_y_coord()'."
            )
    ...
    def _set_data_range(self, data):
        ...
        if data.ndim == 1:
            return data[
                (self._frequency >= self._f_min) & (self._frequency <= self._f_max)
            ]
        if data.ndim == 2:
            ret_array = np.empty(
                shape=(data.shape[0], self._fit_frequency.shape[0]), dtype=np.float64
            )
    ...
    def _get_datasets(self):
        ...
        try:
            ...
            try:
                self._x_co = self._hf.get_dataset(
                    self.ds_url_power
                )  # hardcode a std url
            except:
                logging.warning(
        ...
        try:
            ...
            try:
                self._y_co = self._hf.get_dataset(
                    self.ds_url_freq
                )  # hardcode a std url
            except:
                logging.warning(
    ...
    def _prepare_f_range(self, f_min, f_max):
        ...
        self._frequency_co = self._hf.add_coordinate(
            "frequency", folder="analysis", unit="Hz"
        )
    ...
    def fit_circle(
        self, reflection=False, notch=False, fit_all=False, f_min=None, f_max=None
    ):
    ...
    def _do_fit_circle(self):
        ...
        for i, z_data_raw in enumerate(self._z_data_raw):
            ...
            try:
                ...
                self._circ_amp_gen.append(
                    np.absolute(self._circle_port.z_data_sim), reset=(i == 0)
                )
                self._circ_pha_gen.append(
                    np.angle(self._circle_port.z_data_sim), reset=(i == 0)
                )
                self._circ_real_gen.append(
                    np.real(self._circle_port.z_data_sim), reset=(i == 0)
                )
                self._circ_imag_gen.append(
                    np.imag(self._circle_port.z_data_sim), reset=(i == 0)
                )
                print(i, i == 0)

                for key in iter(self._results):
                    self._results[str(key)].append(
                        float(self._circle_port.fitresults[str(key)])
                    )
            trace += 1
    ...
    def _prepare_circle(self):
        ...
        if self._ds_type == ds_types["vector"]:  # data from measure_1d
            self._data_real_gen = self._hf.add_value_vector(
                "data_real_gen", self._frequency_co, folder="analysis", unit=""
            )
            self._data_imag_gen = self._hf.add_value_vector(
                "data_imag_gen", self._frequency_co, folder="analysis", unit=""
            )

            self._circ_amp_gen = self._hf.add_value_vector(
                "circ_amp_gen", self._frequency_co, folder="analysis", unit="arb. unit"
            )
            self._circ_pha_gen = self._hf.add_value_vector(
                "circ_pha_gen", self._frequency_co, folder="analysis", unit="rad"
            )
            self._circ_real_gen = self._hf.add_value_vector(
                "circ_real_gen", self._frequency_co, folder="analysis", unit=""
            )
            self._circ_imag_gen = self._hf.add_value_vector(
                "circ_imag_gen", self._frequency_co, folder="analysis", unit=""
            )

            for key in self._result_keys:
                self._results[key] = self._hf.add_coordinate(
                    "circ_" + key, folder="analysis", unit=""
                )

        if self._ds_type == ds_types["matrix"]:  # data from measure_2d
            self._data_real_gen = self._hf.add_value_matrix(
                "data_real_gen",
                self._x_co,
                self._frequency_co,
                folder="analysis",
            ...
            self._data_imag_gen = self._hf.add_value_matrix(
                "data_imag_gen",
                self._x_co,
                self._frequency_co,
                folder="analysis",
            ...
            self._circ_amp_gen = self._hf.add_value_matrix(
                "circ_amp_gen",
                self._x_co,
                self._frequency_co,
                folder="analysis",
            ...
            self._circ_pha_gen = self._hf.add_value_matrix(
                "circ_pha_gen",
                self._x_co,
                self._frequency_co,
                folder="analysis",
            ...
            self._circ_real_gen = self._hf.add_value_matrix(
                "circ_real_gen",
                self._x_co,
                self._frequency_co,
                folder="analysis",
            ...
            self._circ_imag_gen = self._hf.add_value_matrix(
                "circ_imag_gen",
                self._x_co,
                self._frequency_co,
                folder="analysis",
            ...
            for key in self._result_keys:
                self._results[key] = self._hf.add_value_vector(
                    "circ_" + key, folder="analysis", x=self._x_co, unit=""
                )
        ...
        circ_view_iq = self._hf.add_view(
            "circ_IQ",
            x=self._data_real_gen,
            y=self._data_imag_gen,
    ...
    def _get_data_circle(self):
        ...
        if not self._fit_all:
            self._z_data_raw = np.empty(
                (1, self._fit_frequency.shape[0]), dtype=np.complex64
            )

            if self._fit_amplitude.ndim == 1:
                self._z_data_raw[0] = np.array(
                    self._fit_amplitude * np.exp(1j * self._fit_phase),
                    dtype=np.complex64,
                )
            else:
                self._z_data_raw[0] = np.array(
                    self._fit_amplitude[-1] * np.exp(1j * self._fit_phase[-1]),
                    dtype=np.complex64,
                )
        ...
        if self._fit_all:
            ...
            for i, a in enumerate(self._fit_amplitude):
                self._z_data_raw[i] = self._fit_amplitude[i] * np.exp(
                    1j * self._fit_phase[i]
                )
    ...
    def fit_lorentzian(
        self, fit_all=False, f_min=None, f_max=None, pre_filter_data=None
    ):
        """
        ...
        for amplitudes in self._fit_amplitude:
            ...
            s_offs = np.mean(
                np.array(
                    [
                        amplitudes_sq[: int(np.size(amplitudes_sq) * 0.1)],
                        amplitudes_sq[
                            int(
                                np.size(amplitudes_sq)
                                - int(np.size(amplitudes_sq) * 0.1)
                            ) :
                        ],
                    ]
                )
            ...
            if np.abs(np.max(amplitudes_sq) - np.mean(amplitudes_sq)) > np.abs(
                np.min(amplitudes_sq) - np.mean(amplitudes_sq)
            ):
                """peak is expected"""
            ...
            mid = (
                s_offs + 0.5 * s_a
            )  # estimated mid region between base line and peak/dip
            m = []  # mid points
            for i in range(len(amplitudes_sq) - 1):
                if np.sign(amplitudes_sq[i] - mid) != np.sign(
                    amplitudes_sq[i + 1] - mid
                ):  # mid level crossing
                    m.append(i)
            if len(m) > 1:
                ...
                s_k = 0.15 * (
                    self._fit_frequency[-1] - self._fit_frequency[0]
                )  # try 15% of window
            p0 = [s_f0, s_k, s_a, s_offs]
            try:
                fit = leastsq(residuals, p0, args=(self._fit_frequency, amplitudes_sq))
            except:
                self._lrnz_amp_gen.append(
                    np.array([np.nan for f in self._fit_frequency])
                )
                ...
                self._lrnz_amp_gen.append(
                    np.sqrt(np.array(self._lorentzian_from_fit(popt)))
                )
    ...
    def _prepare_lorentzian(self):
        ...
        """
        if self._ds_type == ds_types["vector"]:  # data from measure_1d
            self._lrnz_amp_gen = self._hf.add_value_vector(
                "lrnz_amp_gen",
                folder="analysis",
                x=self._frequency_co,
                unit="arb. unit",
            )
            self._lrnz_f0 = self._hf.add_coordinate(
                "lrnz_f0", folder="analysis", unit="Hz"
            )
            self._lrnz_k = self._hf.add_coordinate(
                "lrnz_k", folder="analysis", unit="Hz"
            )
            self._lrnz_a = self._hf.add_coordinate("lrnz_a", folder="analysis", unit="")
            self._lrnz_offs = self._hf.add_coordinate(
                "lrnz_offs", folder="analysis", unit=""
            )
            self._lrnz_Ql = self._hf.add_coordinate(
                "lrnz_ql", folder="analysis", unit=""
            )

            self._lrnz_chi2_fit = self._hf.add_coordinate(
                "lrnz_chi2", folder="analysis", unit=""
            )

        if self._ds_type == ds_types["matrix"]:  # data from measure_2d
            self._lrnz_amp_gen = self._hf.add_value_matrix(
                "lrnz_amp_gen",
                folder="analysis",
                x=self._x_co,
                y=self._frequency_co,
                unit="arb. unit",
            )
            self._lrnz_f0 = self._hf.add_value_vector(
                "lrnz_f0", folder="analysis", x=self._x_co, unit="Hz"
            )
            self._lrnz_k = self._hf.add_value_vector(
                "lrnz_k", folder="analysis", x=self._x_co, unit="Hz"
            )
            self._lrnz_a = self._hf.add_value_vector(
                "lrnz_a", folder="analysis", x=self._x_co, unit=""
            )
            self._lrnz_offs = self._hf.add_value_vector(
                "lrnz_offs", folder="analysis", x=self._x_co, unit=""
            )
            self._lrnz_Ql = self._hf.add_value_vector(
                "lrnz_ql", folder="analysis", x=self._x_co, unit=""
            )

            self._lrnz_chi2_fit = self._hf.add_value_vector(
                "lrnz_chi2", folder="analysis", x=self._x_co, unit=""
            )
    ...
    def _lorentzian_from_fit(self, fit):
        return (
            fit[2] / (1 + (4 * ((self._fit_frequency - fit[0]) / fit[1]) ** 2)) + fit[3]
        )

    def _lorentzian_fit_chi2(self, fit, amplitudes_sq):
        chi2 = np.sum((self._lorentzian_from_fit(fit) - amplitudes_sq) ** 2) / (
            len(amplitudes_sq) - len(fit)
        )
        return chi2
    ...
    def fit_skewed_lorentzian(
        ...
        def residuals2(p, x, y):
            ...
            err = y - (
                A1
                + A2 * (x - fr)
                + (A3 + A4 * (x - fr)) / (1.0 + 4.0 * Qr**2 * ((x - fr) / fr) ** 2)
            )
            return err
        ...
        for amplitudes in self._fit_amplitude:
            ...
            try:
                p_final = leastsq(
                    residuals, p0, args=(self._fit_frequency, amplitudes_sq)
                )
                A2a, A4a, Qra = p_final[0]

                p0 = [A1a, A2a, A3a, A4a, fra, Qra]
                p_final = leastsq(
                    residuals2, p0, args=(self._fit_frequency, amplitudes_sq)
                )
                popt = p_final[0]
            except:
                self._skwd_amp_gen.append(
                    np.array([np.nan for f in self._fit_frequency])
                )
    ...
    def _prepare_skewed_lorentzian(self):
        ...
        """
        if self._ds_type == ds_types["vector"]:  # data from measure_1d
            self._skwd_amp_gen = self._hf.add_value_vector(
                "sklr_amp_gen",
                folder="analysis",
                x=self._frequency_co,
                unit="arb. unit",
            )
            self._skwd_f0 = self._hf.add_coordinate(
                "sklr_f0", folder="analysis", unit="Hz"
            )
            self._skwd_a1 = self._hf.add_coordinate(
                "sklr_a1", folder="analysis", unit="Hz"
            )
            self._skwd_a2 = self._hf.add_coordinate(
                "sklr_a2", folder="analysis", unit=""
            )
            self._skwd_a3 = self._hf.add_coordinate(
                "sklr_a3", folder="analysis", unit=""
            )
            self._skwd_a4 = self._hf.add_coordinate(
                "sklr_a4", folder="analysis", unit=""
            )
            self._skwd_Qr = self._hf.add_coordinate(
                "sklr_qr", folder="analysis", unit=""
            )
            self._skwd_Qi = self._hf.add_coordinate(
                "sklr_qi", folder="analysis", unit=""
            )

            self._skwd_chi2_fit = self._hf.add_coordinate(
                "sklr_chi2", folder="analysis", unit=""
            )

        if self._ds_type == ds_types["matrix"]:  # data from measure_2d
            self._skwd_amp_gen = self._hf.add_value_matrix(
                "sklr_amp_gen",
                folder="analysis",
                x=self._x_co,
                y=self._frequency_co,
            ...
            self._skwd_f0 = self._hf.add_value_vector(
                "sklr_f0", folder="analysis", x=self._x_co, unit="Hz"
            )
            self._skwd_a1 = self._hf.add_value_vector(
                "sklr_a1", folder="analysis", x=self._x_co, unit="Hz"
            )
            self._skwd_a2 = self._hf.add_value_vector(
                "sklr_a2", folder="analysis", x=self._x_co, unit=""
            )
            self._skwd_a3 = self._hf.add_value_vector(
                "sklr_a3", folder="analysis", x=self._x_co, unit=""
            )
            self._skwd_a4 = self._hf.add_value_vector(
                "sklr_a4", folder="analysis", x=self._x_co, unit=""
            )
            self._skwd_Qr = self._hf.add_value_vector(
                "sklr_qr", folder="analysis", x=self._x_co, unit=""
            )
            self._skwd_Qi = self._hf.add_value_vector(
                "sklr_qi", folder="analysis", x=self._x_co, unit=""
            )

            self._skwd_chi2_fit = self._hf.add_value_vector(
                "sklr_chi2", folder="analysis", x=self._x_co, unit=""
            )
    ...
    def _skewed_fit_chi2(self, fit, amplitudes_sq):
        chi2 = np.sum((self._skewed_from_fit(fit) - amplitudes_sq) ** 2) / (
            len(amplitudes_sq) - len(fit)
        )
        return chi2
    ...
    def _skewed_from_fit(self, p):
        ...
        return (
            A1
            + A2 * (self._fit_frequency - fr)
            + (A3 + A4 * (self._fit_frequency - fr))
            / (1.0 + 4.0 * Qr**2 * ((self._fit_frequency - fr) / fr) ** 2)
        )
    ...
    def _skewed_estimate_Qi(self, p):
        ...
        def skewed_from_fit(p, f):
            ...
            return (
                A1
                + A2 * (f - fr)
                + (A3 + A4 * (f - fr)) / (1.0 + 4.0 * Qr**2 * ((f - fr) / fr) ** 2)
            )
    ...
    def _prepare_fano(self):
        ...
        if self._ds_type == ds_types["vector"]:  # data from measure_1d
            self._fano_amp_gen = self._hf.add_value_vector(
                "fano_amp_gen",
                folder="analysis",
                x=self._frequency_co,
                unit="arb. unit",
            )
            self._fano_q_fit = self._hf.add_coordinate(
                "fano_q", folder="analysis", unit=""
            )
            self._fano_bw_fit = self._hf.add_coordinate(
                "fano_bw", folder="analysis", unit="Hz"
            )
            self._fano_fr_fit = self._hf.add_coordinate(
                "fano_fr", folder="analysis", unit="Hz"
            )
            self._fano_a_fit = self._hf.add_coordinate(
                "fano_a", folder="analysis", unit=""
            )

            self._fano_chi2_fit = self._hf.add_coordinate(
                "fano_chi2", folder="analysis", unit=""
            )
            self._fano_Ql_fit = self._hf.add_coordinate(
                "fano_Ql", folder="analysis", unit=""
            )
            self._fano_Q0_fit = self._hf.add_coordinate(
                "fano_Q0", folder="analysis", unit=""
            )

        if self._ds_type == ds_types["matrix"]:  # data from measure_2d
            self._fano_amp_gen = self._hf.add_value_matrix(
                "fano_amp_gen",
                folder="analysis",
                x=self._x_co,
                y=self._frequency_co,
            ...
            self._fano_q_fit = self._hf.add_value_vector(
                "fano_q", folder="analysis", x=self._x_co, unit=""
            )
            self._fano_bw_fit = self._hf.add_value_vector(
                "fano_bw", folder="analysis", x=self._x_co, unit="Hz"
            )
            self._fano_fr_fit = self._hf.add_value_vector(
                "fano_fr", folder="analysis", x=self._x_co, unit="Hz"
            )
            self._fano_a_fit = self._hf.add_value_vector(
                "fano_a", folder="analysis", x=self._x_co, unit=""
            )

            self._fano_chi2_fit = self._hf.add_value_vector(
                "fano_chi2", folder="analysis", x=self._x_co, unit=""
            )
            self._fano_Ql_fit = self._hf.add_value_vector(
                "fano_Ql", folder="analysis", x=self._x_co, unit=""
            )
            self._fano_Q0_fit = self._hf.add_value_vector(
                "fano_Q0", folder="analysis", x=self._x_co, unit=""
            )
    ...
    def fit_fano(self, fit_all=False, f_min=None, f_max=None, pre_filter_data=None):
        ...
        for amplitudes in self._fit_amplitude:
            ...
            try:
                ...
                q0 = self._fano_fit_q0(
                    np.sqrt(np.absolute(amplitudes_gen)), float(fit[2])
                )
    ...
    def _do_fit_fano(self, amplitudes_sq):
        ...
        p_fit = leastsq(
            fano_residuals, p0, args=(self._fit_frequency, np.array(amplitudes_sq))
        )
        # print(("q:%g bw:%g fr:%g a:%g")% (p_fit[0][0],p_fit[0][1],p_fit[0][2],p_fit[0][3]))
        return p_fit[0]

    def _fano_reflection_from_fit(self, fit):
        return self._fano_reflection(
            self._fit_frequency, fit[0], fit[1], fit[2], fit[3]
        )

    def _fano_fit_chi2(self, fit, amplitudes_sq):
        chi2 = np.sum((self._fano_reflection_from_fit(fit) - amplitudes_sq) ** 2) / (
            len(amplitudes_sq) - len(fit)
        )
        return chi2
    ...
    def _fano_fit_q0(self, amp_gen, fr):
        ...
        for i in range(len(amp_gen) - 1):
            if np.sign(amp_gen[i] - amp_3dB_lin) != np.sign(
                amp_gen[i + 1] - amp_3dB_lin
            ):  # crossing@amp_3dB
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="(optional) skewed lorentzian fit",
    )
    parser.add_argument(
        "-all",
        "--fit-all",
        default=False,
        action="store_true",
        help="(optional) fit all entries in dataset",
    )
    parser.add_argument(
        "-fr",
        "--frequency-range",
        type=str,
        help="(optional) frequency range for fitting, comma separated",
    )
    parser.add_argument(
        "-fg",
        "--filter-gaussian",
        default=False,
        action="store_true",
        help="(optional) (pre-) filter data: gaussian",
    )
    parser.add_argument(
        "-fm",
        "--filter-median",
        default=False,
        action="store_true",
        help="(optional) (pre-) filter data: median",
    )
    parser.add_argument(
        "-fp",
        "--filter-params",
        type=str,
        help="(optional) (pre-) filter data: parameter",
    )
    parser.add_argument(
        "-d",
        "--debug-output",
        default=False,
        action="store_true",
        help="(optional) debug: more verbose",
    )
    parser.add_argument(
        "-t", "--type", type=str, help="resonator type: (r)eflection or (n)otch"
    )
