...
class RS_FSUP(Instrument):
    ...
    def __init__(self, name, address, channel_index=1):
        ...
        self.add_parameter(
            "freqspan", type=float, flags=Instrument.FLAG_GETSET, tags=["sweep"]
        )

        self.add_parameter(
            "powerunit", type=str, flags=Instrument.FLAG_GETSET, tags=["sweep"]
        )

        self.add_parameter(
            "startfreq", type=float, flags=Instrument.FLAG_GETSET, tags=["sweep"]
        )

        self.add_parameter(
            "stopfreq", type=float, flags=Instrument.FLAG_GETSET, tags=["sweep"]
        )

        self.add_parameter(
            "sweeptime", type=float, flags=Instrument.FLAG_GETSET, tags=["sweep"]
        )

        self.add_parameter(
            "resolutionBW", type=float, flags=Instrument.FLAG_GETSET, tags=["sweep"]
        )

        self.add_parameter(
            "videoBW", type=float, flags=Instrument.FLAG_GETSET, tags=["sweep"]
        )

        self.add_parameter(
            "sweeptime_averages", type=float, flags=Instrument.FLAG_GET, tags=["sweep"]
        )

        self.add_parameter(
            "sweeptime_auto", type=float, flags=Instrument.FLAG_GETSET, tags=["sweep"]
        )

        self.add_parameter(
            "freqpoints", type=numpy.ndarray, flags=Instrument.FLAG_GET, tags=["sweep"]
        )
    ...
    def do_get_freqpoints(self):
        ...
        x_values = numpy.linspace(
            self.do_get_startfreq(), self.do_get_stopfreq(), self.do_get_nop()
        )
        return x_values
    ...
    def get_tracedata(self, tracenumber=1):
        amp = numpy.array(
            self._visainstrument.query_ascii_values("trac:data? trace%i" % tracenumber)
        )
        return amp
