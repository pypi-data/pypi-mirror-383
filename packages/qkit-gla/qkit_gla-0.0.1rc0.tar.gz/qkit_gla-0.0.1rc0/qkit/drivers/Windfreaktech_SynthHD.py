...
class Windfreaktech_SynthHD(Instrument):
    ...
    def reload_calibration(self):
        ...
        try:
            data = np.loadtxt(
                qkit.cfg.get("instruments_dir") + "/Windfreaktech_SynthHD.cal"
            )
            f = data[1:, 0]
            p = data[0, 1:]
            values = data[1:, 1:]
            ...
            raise OSError("Calibration file for WFT MW Source not found")
    ...
    def do_get_frequency(self, channel):
        self._frequency[channel] = (
            float(self._visainstrument.ask("C%if?" % (channel - 1))) * 1e6
        )
    ...
    def do_get_power(self, channel):
        ...
        if self._power[channel] == None:
            print(
                "Power was not set since reload of driver. You have to do this to get a proper value."
            )
    ...
    def do_set_power(self, power, channel):
        ...
        if np.isnan(float(self._interp_amplitude((self._frequency[channel], power)))):
            ...
            self.do_set_power_level(
                int(self._interp_amplitude((self._frequency[channel], power))), channel
            )
