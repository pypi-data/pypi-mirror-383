...
class virtual_MultiplexingReadout(Instrument):
    def __init__(self, name, sample):
        ...
        self.add_parameter(
            "tone_freq", type=list, flags=Instrument.FLAG_GETSET, units="Hz"
        )
        self.add_parameter(
            "tone_relamp", type=tuple, flags=Instrument.FLAG_GETSET, units=""
        )
        self.add_parameter(
            "tone_pha", type=tuple, flags=Instrument.FLAG_GETSET, units="rad"
        )
        self.add_parameter(
            "tone_amp", type=float, flags=Instrument.FLAG_GETSET, units="V"
        )
        self.add_parameter(
            "adc_clock", type=float, flags=Instrument.FLAG_GET, units="Hz"
        )
        self.add_parameter(
            "dac_clock", type=float, flags=Instrument.FLAG_SET, units="Hz"
        )
        self.add_parameter(
            "dac_duration", type=float, flags=Instrument.FLAG_GETSET, units="s"
        )
        ...
        try:
            ...
            self._awg = None
            logging.warning(
                "You haven't provided a readout_awg. I can't set the readout pulse for you."
        ...
        if self._awg:
            ...
            try:
                ...
                logging.error(
                    "Cannot resolve awg name. Provide name attribute in awg instrument driver."
                )
        ...
        self.cut_off_freq_ratio = (
            0.8  # ratio of the IQ frequency up to that is transmitted
        )
    ...
    def do_set_dac_channel_I(self, channel):
        ...
        if (self._dac_channel_I == -1) and (self._dac_channel_Q == -1):
            logging.warning(
                __name__
                + "no output signal is produced if both dac_channel_I and dac_channel_Q are none"
            )
    ...
    def do_set_adc_channel_I(self, channel):
        ...
        if (self._adc_channel_I == -1) and (self._adc_channel_Q == -1):
            logging.warning(
                __name__
                + "no signal is acquired if both dac_channel_I and dac_channel_Q are none"
            )
    ...
    def do_set_adc_channel_Q(self, channel):
        ...
        if (self._adc_channel_I == -1) and (self._adc_channel_Q == -1):
            logging.warning(
                __name__
                + "no signal is acquired if -1dac_channel_I and dac_channel_Q are none"
            )
        ...
        if ddc is None:
            if len(Is.shape) == 2:
                ...
                for idx in range(Is.shape[1]):
                    sig_amp[idx, :], sig_pha[idx, :] = self.IQ_decode(
                        Is[:, idx], Qs[:, idx]
                    )
            else:
            ...
            if len(Is.shape) == 2:
                ...
                for idx in range(Is.shape[1]):
                    (
                        sig_amp[idx, :, :],
                        sig_pha[idx, :, :],
                    ) = self.digital_down_conversion(Is[:, idx], Qs[:, idx])
            else:
    ...
    def IQ_fft(self, I, Q, samplerate=None):
        ...
        sig_x = self._LO + np.fft.fftshift(np.fft.fftfreq(sig_t.size, 1.0 / samplerate))
    ...
    def fourieranalysis(self, signal_t, freqs, samplerate):
        ...
        w = np.array(
            [
                np.exp(-2 * np.pi * 1j * f * np.arange(len(signal_t)) / samplerate)
                for f in freqs
            ]
        )
    ...
    def digital_down_conversion(self, I, Q, freqs=None):
        ...
        for i, f in enumerate(freqs):
            ...
            b, a = signal.butter(
                self.lowpass_order, cut_off_freq, "low"
            )  # design the filter
        ...
        return (
            sig_amp.T,
            sig_pha.T,
        )  # transform because readout expects the data this way
    ...
    def update(self):
        ...
        if IFMax > self._awg.get_clock() / 2:
            logging.warning(
                __name__
                + " : maximum IF frequency of %fMHz is above the limit of the DAC."
                % (IFMax / 1e6)
            )
        if IFMax > self._mspec.get_clock() / 2:
            logging.warning(
                __name__
                + " : maximum IF frequency of %fMHz is above the limit of the ADC."
                % (IFMax / 1e6)
            )

        # build I and Q waveforms and send them to the DAC
        if self._tone_relamp is None:
            amplitudes = 1.0 / ntones * np.ones(ntones)
        elif len(self._tone_relamp) == 1:
            amplitudes = 1.0 / ntones * self._tone_relamp * np.ones(ntones)
        elif len(self._tone_relamp) == ntones:
            amplitudes = 1.0 / ntones * self._tone_relamp
        else:
            print(
                "tone_relamp shape does not fit number of multiplexed frequencies. Setting tone_relamp to 1"
            )
            amplitudes = 1.0 / ntones * np.ones(ntones)
        ...
        I, Q, m1 = self.IQ_encode(
            self._dac_duration,
            IFtones,
            [amplitudes, amplitudes],
            [phases, phases + self.QtoI_relpha],
            self._dac_clock,
            self._dac_attack,
            self._dac_decay,
        )
        self._update_dac(
            [self._dac_channel_I, self._dac_channel_Q],
            [I, Q],
            marker1=[m1, m1],
            marker2=[m1, m1],
        )
    ...
    def _update_dac(
        self, channels, samples, marker1=None, marker2=None, drive=None, path=None
    ):
        """
        ...
        if self._awg is None:
            logging.warning(
                "You haven't provided a readout awg so I can't load a pulse in. "
        ...
        if "GHzDAC" == self._awg.get_type():  # Martinis board
            ...
            for idx in range(0, len(channels)):
                if channels[idx] == -1:
                    continue  # only apply I or Q if the user wishes that
                channel = 1 + channels[idx]

                fn = drive + path + "\\ch%d" % channel
                self._awg.send_waveform(
                    samples[idx, :],
                    marker1[idx, :],
                    marker2[idx, :],
                    fn,
            ...
            try:
                ...
                logging.warning("Clock and amplitude settings not written.")
            if self.sample:
                lawg.update_sequence(
                    [1],
                    lambda t, sample: [samples[channels[0]], samples[channels[1]]],
                    self.sample,
                    awg=self._awg,
                    marker=[[marker1, marker2], [marker1, marker2]],
                    show_progress_bar=False,
                )
            else:
                logging.error(
                    "Please provide sample object in instantiating readout when using Tektronix AWG for readout."
                )
    ...
    def IQ_encode(
        ...
        if amplitudes is None:
            ...
            if amplitudes.ndim == 1:
                amplitudes = np.array([amplitudes, amplitudes])
        if phases is None:
            phases = np.zeros((2, frequencies.size))
        else:
            phases = np.array(phases)
            if phases.ndim == 1:
                phases = np.array([phases, phases])

        # generate multitone waveform
        I = np.zeros(int(np.ceil((nSamples + 1) / 16.0) * 16))
        Q = np.zeros(int(np.ceil((nSamples + 1) / 16.0) * 16))
        for idx in range(0, frequencies.size):
            I[1 : nSamples + 1] += amplitudes[0, idx] * np.sin(
                omegas[idx] * indices + phases[0, idx]
            )
            Q[1 : nSamples + 1] += amplitudes[1, idx] * np.cos(
                omegas[idx] * indices + phases[1, idx]
            )
        ...
        if self.dac_delay != 0:
            if self.dac_delay <= -1:
                dac_delay = (
                    self.sample.exc_T - self.sample.overlap + self.sample.readout_delay
                )
            else:
