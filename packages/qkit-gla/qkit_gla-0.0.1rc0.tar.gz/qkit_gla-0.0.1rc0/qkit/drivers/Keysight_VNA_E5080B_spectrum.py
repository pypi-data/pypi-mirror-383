...
class Keysight_VNA_E5080B_spectrum(Keysight_VNA_E5080B.Keysight_VNA_E5080B):
    def __init__(self, name, address, channel_index=1):
        ...
        self.add_parameter("measurement_class", type=str, flags=Instrument.FLAG_GET)

        self.add_parameter(
            "signal_format", type=int, flags=Instrument.FLAG_GETSET, minval=0, maxval=1
        )
        self.signal_formats = {0: "MLIN", 1: "MLOG"}
        self.inv_signal_formats = {
            val.upper(): key for key, val in self.signal_formats.items()
        }

        self.add_parameter(
            "signal_unit", type=int, flags=Instrument.FLAG_GETSET, minval=0, maxval=3
        )
    ...
    def do_set_signal_format(self, signal_format):
        ...
        """
        self.write(f"CALC:MEAS:FORM {self.signal_formats[signal_format]:s}")
    ...
    def do_set_signal_unit(self, unit, signal_format=None):
        ...
        self.write(
            "CALC:MEAS:FORM:UNIT {:s}, {:s}".format(
                self.signal_formats[signal_format],
                {0: self.lin_units, 1: self.log_units}[signal_format][unit],
            )
        )
    ...
    def do_get_signal_unit(self):
        ...
        signal_format = self.get_signal_format()
        ans = self.query(
            f"CALC:MEAS:FORM:UNIT? {self.signal_formats[signal_format]:s}"
        )
    ...
    def get_tracedata(self, trace=1):
        if self.get_measurement_class() == "Spectrum Analyzer":
            ...
            data = self._visainstrument.query_binary_values(
                "CALC%i:MEAS%i:DATA:SDAT?" % (self._ci, trace)
            )
            return np.array(data)[::2]
        else:
            raise ValueError(
                "Keysight_VNA_E5080B_spectrum: Set the measurement class to the spectrum analyzer mode."
            )
