...
try:
    ...
    logging.warning(
        __name__ + ": unable to load instrument module. entering standalone mode."
    )
    ...
    class Instrument:
        ...
        def add_parameter(
            ...
            if flags & self.FLAG_GET:
                setattr(self, "get_%s" % name, getattr(self, "do_get_%s" % name))
            if flags & self.FLAG_SET:
                setattr(self, "set_%s" % name, getattr(self, "do_set_%s" % name))
...
class EthernetServerWin(Instrument):
    ...
    class raw_socket:
        ...
        def __init__(self, family, type, proto):
            ...
            if (family != socket.PF_PACKET) or (type != socket.SOCK_RAW):
                raise Exception(
                    "raw_socket supports only family=PF_PACKET, type=SOCK_RAW."
                )
        ...
        def bind(self, tuple):
            ...
            if not bool(self._fp):
                raise Exception(
                    "unable to open interface %s for packet capture." % self._interface
                )
        ...
        def recvfrom(self, bufsize):
            ...
            status = capy.pcap_next_ex(
                self._fp, ctypes.byref(header), ctypes.byref(pkt_data)
            )
            if status == 0:
                # timeout
                return False
    ...
    class udp_receiver(threading.Thread):
        ...
        def run(self):
            ...
            while True:
                ...
                (raw_packet, sender) = self.node._udp_socket.recvfrom(
                    self.node.RECV_SIZE
                )
                ...
                if not self.node._ip_filter(sender):
                    logging.debug(
                        self.name + " : udp packet from %s discarded." % sender[0]
                    )
                    continue
    ...
    class raw_receiver(threading.Thread):
        ...
        def run(self):
            ...
            while True:
                ...
                (raw_packet, sender) = self.node._raw_socket.recvfrom(
                    self.node.RECV_SIZE
                )
                packet = dpkt.ethernet.Ethernet(raw_packet)
                ...
                for client_list in [
                    self.node._clients.get(packet.src, None),
                    self.node._clients.get(self.node.MAC_BROADCAST, None),
                ]:
                    if client_list == None:
                        continue
    ...
    def _init_udp(self):
        ...
        logging.debug(
            __name__
            + " : Listening for packets on %s:%d." % (self._ip_host, self._ip_port)
        )
    ...
    def _init_raw(self):
        ...
        if self._ethertype == self.ETH_P_ALL:
            logging.info(
                __name__
                + " : EtherType set to ETH_P_ALL, this may generate high CPU load"
            )
        self._raw_socket = self.raw_socket(
            socket.PF_PACKET, socket.SOCK_RAW, self._ethertype
        )
