...
class Lmx2595(Instrument):
    ...
    def _set_parameter(self, name, value, mode="full"):
        ...
        if (
            mode == "full"
        ):  # So we do not care what is now written in the register and can overwrite it
            self.c.write_data(
                [self.params[name][0], value[0], value[1]]
            )  # Write the bytes
        if (
            mode == "part"
        ):  # Here we need to overwrite only sertain bits of the register using the function, written at script on rpi
            reg = self.c.read_data(self.params[name][0])  # Read the register value
            if (
                self.params[name][1] < 8
            ):  # That means that the value should be changed is the part of first byte
                wr_reg = reg[2]
                ...
                self.c.write_data(
                    [self.params[name][0], reg[1], wr_reg]
                ...
                for i in range(index, index + leng):
                    wr_reg = set_bit(wr_reg, i, get_bit(value, i - index))
                self.c.write_data(
                    [self.params[name][0], wr_reg, reg[2]]
                )  # Write the changed data
    ...
    def get_value(self, name, mode="full"):
        ...
        """
        if mode == "full":
            return self.c.read_data(
                self.params[name][0]
            )  # Just read the whole register and convert data into numbers
        if mode == "part":
            reg = self.c.read_data(self.params[name][0])
            if (
                self.params[name][1] < 8
            ):  # The value we are interested in is in the first byte
                rd_reg = reg[2]
                index = self.params[name][1]
                leng = self.params[name][2]
                res = []
                for i in range(index, index + leng):
                    res.append(
                        get_bit(rd_reg, i) * 2 ** (i - index)
                    )  # Read only part of a certain byte
                return sum(res)
            else:
                rd_reg = reg[1]
                index = self.params[name][1] - 8
    ...
    def do_get_frequency(self):
        ...
        num = from_bytes(
            self.get_value("Num_b0", mode="full")
            + self.get_value("Num_b1", mode="full")
        )
