...
class Keysight_B2900(Instrument):
    ...
    def __init__(self, name, address, reset=False):
        ...
        self._sweep_mode = (
            2 - self._channels
        )  # 1 (IV-mode) for single channel SMU and 0 (VV-mode) for two channel SMU
        ...
        self.measurement_ccr = [
            ("Ch1 Limit Test Summary", "Channel 1 failed one or more limit tests."),
            ("Ch1 Reading Available", "Reading of channel 1 was taken normally."),
            (
                "Ch1 Reading Overflow",
                "Reading of channel 1 exceeds the selected measurement range.",
            ),
            ...
            (
                "Ch2 Reading Overflow",
                "Reading of channel 2 exceeds the selected measurement range.",
            ),
        ...
        self.questionable_ccr = [
            ("Voltage Summary", "Over voltage in channel 1, and/or 2."),
            ...
            (
                "Ch1 Transition Event Lost",
                "Lost arm or trigger transition event on channel 1",
            ),
            (
                "Ch1 Acquire Event Lost",
                "Lost arm or trigger acquire event on channel 1",
            ),
            (
                "Ch2 Transition Event Lost",
                "Lost arm or trigger transition event on channel 2",
            ),
            (
                "Ch2 Acquire Event Lost",
                "Lost arm or trigger acquire event on channel 2",
            ),
            ("Not used", "0 is returned."),
        ]
        # error messages
        self.err_msg = {
            -100: """Command error: Generic syntax error that cannot be determined more specifically.""",
        ...
        if reset:
            self.reset()
            [
                self.set_sense_mode(channel=channel, mode=0)
                for channel in np.arange(self._channels) + 1
            ]
        else:
            self.get_all()
    ...
    def set_gpio_mode(self, val, channel=-1):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Set the GPIO mode of pin {channel!s} to {val!s}"
            )
            if channel == -1:
                for j in range(14):
                    self._write(f"sour:dig:ext{j + 1:d}:func {val:s}")
            else:
                self._write(f"sour:dig:ext{channel:d}:func {val:s}")
            self._write("SOUR:DIG:DATA 16383")
            self.ccr = bin(16383)
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot set the GPIO mode of pin {channel!s} to {val!s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot set the GPIO mode of pin {channel!s} to {val!s}\n{e!s}"
            )
    ...
    def get_gpio_mode(self, channel):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Get the GPIO mode of pin {channel!s}"
            )
            return self._ask(f"SOUR:DIG:EXT{channel:d}:FUNC")
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot get the GPIO mode of pin {channel!s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot get the GPIO mode of pin {channel!s}\n{e!s}"
            )
    ...
    def set_digio(self, status, channel):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Set the digital output of pin {channel!s} to {status!s}"
            )
            self.ccr = (
                self.ccr[:-channel] + str(int(not (status))) + self.ccr[-channel + 1 :]
            )[:16]
            self._write(f"sour:dig:data {int(self.ccr, 2):d}")
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot set the digital output of pin {channel!s} to {status!s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot set the digital output of pin {channel!s} to {status!s}\n{e!s}"
            )
    ...
    def get_digio(self, channel):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Get the digital output of pin {channel!s}"
            )
            return not (int(bin(int(self._ask("sour:dig:data?")))[-channel:][0]))
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot get the digital output of pin {channel!s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot get the digital output of pin {channel!s}\n{e!s}"
            )
    ...
    def set_measurement_mode(self, mode, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Set measurement mode{self._log_chans[self._channels][channel]:s} to {mode:d}"
            )
            self._write(
                f":sens{self._cmd_chans[self._channels][channel]:s}:rem {mode:d}"
            )
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot set measurement mode{self._log_chans[self._channels][channel]:s} to {mode!s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot set measurement mode{self._log_chans[self._channels][channel]:s} to {mode!s}\n{e!s}"
            )
    ...
    def set_sync(self, status):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Set interchannel synchronization to {status!r}"
            )
            if status:
                self._write(":syst:gro:def (@1,2)")
            else:
                self._write(":syst:gro:res")
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot set interchannel synchronization to {status!s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot set interchannel synchronization to {status!s}\n{e!s}"
            )
    ...
    def get_measurement_mode(self, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Get measurement mode{self._log_chans[self._channels][channel]:s}"
            )
            return int(
                self._ask(
                    f":sens{self._cmd_chans[self._channels][channel]:s}:rem"
                )
            )
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot get measurement mode{self._log_chans[self._channels][channel]:s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot get measurement mode{self._log_chans[self._channels][channel]:s}\n{e!s}"
            )
    ...
    def set_bias_mode(self, mode, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Set bias mode{self._log_chans[self._channels][channel]:s} to {mode:d}"
            )
            self._write(
                f":sour{self._cmd_chans[self._channels][channel]:s}:func:mode {self._IV_modes[mode]:s}"
            )
            self.set_bias_range(val=-1)  # set bias range to "auto"
            self._write(
                f''':sens{self._cmd_chans[self._channels][channel]:s}:func:on "{self._IV_modes[mode]:s}"'''
            )  # enables same sense function to measure bias value
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot set bias mode{self._log_chans[self._channels][channel]:s} to {mode!s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot set bias mode{self._log_chans[self._channels][channel]:s} to {mode!s}\n{e!s}"
            )
    ...
    def get_bias_mode(self, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Get bias mode{self._log_chans[self._channels][channel]:s}"
            )
            return int(
                list(self._IV_modes.keys())[
                    list(self._IV_modes.values()).index(
                        self._ask(
                            f":sour{self._cmd_chans[self._channels][channel]:s}:func:mode"
                        ).lower()
                    )
                ]
            )
            ...
            logging.error(
                f"{__name__!s}: Cannot get bias mode{self._log_chans[self._channels][channel]:s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot get bias mode{self._log_chans[self._channels][channel]:s}\n{e!s}"
            )
    ...
    def set_sense_mode(self, mode, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Set sense mode{self._log_chans[self._channels][channel]:s} to {mode:d}"
            )
            self._write(
                f":sens{self._cmd_chans[self._channels][channel]:s}:func:on:all"
            )
            self._write(
                ''':sens{:s}:func:on "{:s}"'''.format(
                    self._cmd_chans[self._channels][channel],
                    self._ask(":sour:func:mode").lower(),
                )
            )  # turn on bias mode in order to measure bias values
            self._write(
                f''':sens{self._cmd_chans[self._channels][channel]:s}:func:on "{self._IV_modes[mode]:s}"'''
            )  # turn on sense mode
            ...
            logging.error(
                f"{__name__!s}: Cannot set sense mode{self._log_chans[self._channels][channel]:s} to {mode!s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot set sense mode{self._log_chans[self._channels][channel]:s} to {mode!s}\n{e!s}"
            )
    ...
    def get_sense_modes(self, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Get sense mode{self._log_chans[self._channels][channel]:s}"
            )
            # return str(self._ask(':sens:func:on?').replace('''"''', '')).lower().split(',')
            return [
                key
                for key, val in self._IV_modes.items()
                if val
                in str(
                    self._ask(
                        f":sens{self._cmd_chans[self._channels][channel]:s}:func:on?"
                    ).replace('''"''', "")
                )
                .lower()
                .split(",")
            ...
            logging.error(
                f"{__name__!s}: Cannot get sense mode{self._log_chans[self._channels][channel]:s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot get sense mode{self._log_chans[self._channels][channel]:s}\n{e!s}"
            )
    ...
    def set_bias_range(self, val, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Set bias range{self._log_chans[self._channels][channel]:s} to {val:g}"
            )
            if val == -1:
                self._write(
                    f":sour{self._cmd_chans[self._channels][channel]:s}:{self._IV_modes[self.get_bias_mode()]:s}:rang:auto 1"
                )
            else:
                self._write(
                    f":sour{self._cmd_chans[self._channels][channel]:s}:{self._IV_modes[self.get_bias_mode()]:s}:rang {val:g}"
                )
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot set bias range{self._log_chans[self._channels][channel]:s} to {val!s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot set bias range{self._log_chans[self._channels][channel]:s} to {val!s}\n{e!s}"
            )
    ...
    def get_bias_range(self, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Get bias range{self._log_chans[self._channels][channel]:s}"
            )
            return float(
                self._ask(
                    f":sour{self._cmd_chans[self._channels][channel]:s}:{self._IV_modes[self.get_bias_mode()]:s}:rang"
                )
            )
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot get bias range{self._log_chans[self._channels][channel]:s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot get bias range{self._log_chans[self._channels][channel]:s}\n{e!s}"
            )
    ...
    def set_sense_range(self, val, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Set sense range{self._log_chans[self._channels][channel]:s} to {val:g}"
            )
            if val == -1:
                self._write(
                    f":sens{self._cmd_chans[self._channels][channel]:s}:{self._IV_modes[self._sense_mode[channel]]:s}:rang:auto 1"
                )
            else:
                self._write(
                    f":sens{self._cmd_chans[self._channels][channel]:s}:{self._IV_modes[self._sense_mode[channel]]:s}:rang:upp {val:g}"
                )
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot set sense range{self._log_chans[self._channels][channel]:s} to {val!s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot set sense range{self._log_chans[self._channels][channel]:s} to {val!s}\n{e!s}"
            )
    ...
    def get_sense_range(self, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Get sense range{self._log_chans[self._channels][channel]:s}"
            )
            return float(
                self._ask(
                    f":sens{self._cmd_chans[self._channels][channel]:s}:{self._IV_modes[self._sense_mode[channel]]:s}:rang:upp?"
                )
            )
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot get sense range{self._log_chans[self._channels][channel]:s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot get sense range{self._log_chans[self._channels][channel]:s}\n{e!s}"
            )
    ...
    def set_sense_limit(self, val, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Set sense compliance limit{self._log_chans[self._channels][channel]:s} to {val:g}"
            )
            self._write(
                f":sens{self._cmd_chans[self._channels][channel]:s}:{self._IV_modes[self._sense_mode[channel]]:s}:prot {val:g}"
            )
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot set sense compliance limit{self._log_chans[self._channels][channel]:s} to {val!s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot set sense compliance limit{self._log_chans[self._channels][channel]:s} to {val!s}\n{e!s}"
            )
    ...
    def get_sense_limit(self, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Get sense compliance limit{self._log_chans[self._channels][channel]:s}"
            )
            return float(
                self._ask(
                    f":sens{self._cmd_chans[self._channels][channel]:s}:{self._IV_modes[self._sense_mode[channel]]:s}:prot?"
                )
            )
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot get sense compliance limit{self._log_chans[self._channels][channel]:s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot get sense compliance limit{self._log_chans[self._channels][channel]:s}\n{e!s}"
            )
    ...
    def set_bias_trigger(self, mode, channel=1, **val):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Set bias trigger{self._log_chans[self._channels][channel]:s} to {mode:s}"
            )
            self._write(f":trig:tran:sour: {mode:s}")
            if "time" in val:
                logging.debug(
                    "{!s}: Set bias trigger timer{:s} to {:s}s".format(
                        __name__,
                        self._log_chans[self._channels][channel],
                        val.get("time", 50e-3),
                    )
                )
            ...
            logging.error(
                f"{__name__!s}: Cannot set bias trigger{self._log_chans[self._channels][channel]:s} to {mode!s}{val!s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot set bias trigger{self._log_chans[self._channels][channel]:s} to {mode!s}{val!s}\n{e!s}"
            )
    ...
    def get_bias_trigger(self, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Get bias trigger{self._log_chans[self._channels][channel]:s}"
            )
            mode = str(self._ask(":trig:tran:sour").lower())
            if mode == "tim":
                logging.debug(
                    f"{__name__!s}: Get bias trigger timer{self._log_chans[self._channels][channel]:s}"
                )
                val = float(self._ask(f":trig:tran:{mode:s}"))
                return mode, val
            else:
                return mode
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot get bias trigger{self._log_chans[self._channels][channel]:s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot get bias trigger{self._log_chans[self._channels][channel]:s}\n{e!s}"
            )
    ...
    def set_sense_trigger(self, mode, channel=1, **val):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Set sense trigger{self._log_chans[self._channels][channel]:s} to {mode:s}"
            )
            self._write(f":trig:acq:sour: {mode:s}")
            if "time" in val:
                logging.debug(
                    "{!s}: Set sense trigger timer{:s} to {:s}s".format(
                        __name__,
                        self._log_chans[self._channels][channel],
                        val.get("time", 50e-3),
                    )
            ...
            logging.error(
                f"{__name__!s}: Cannot set sense trigger{self._log_chans[self._channels][channel]:s} to {mode!s}{val!s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot set sense trigger{self._log_chans[self._channels][channel]:s} to {mode!s}{val!s}\n{e!s}"
            )
    ...
    def get_sense_trigger(self, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Get sense trigger{self._log_chans[self._channels][channel]:s}"
            )
            mode = str(self._ask(":trig:acq:sour").lower())
            if mode == "tim":
                logging.debug(
                    f"{__name__!s}: Get sense trigger timer{self._log_chans[self._channels][channel]:s}"
                )
                val = float(self._ask(f":trig:tran:{mode:s}"))
                return mode, val
            else:
            ...
            logging.error(
                f"{__name__!s}: Cannot get sense trigger{self._log_chans[self._channels][channel]:s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot get sense trigger{self._log_chans[self._channels][channel]:s}\n{e!s}"
            )
    ...
    def set_bias_delay(self, offset, status=True, channel=1, **kwargs):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Set bias delay status{self._log_chans[self._channels][channel]:s} to {status:g}"
            )
            self._write(
                f":sour{self._cmd_chans[self._channels][channel]:s}:wait:stat {int(status):d}"
            )
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot set bias delay status{self._log_chans[self._channels][channel]:s} to {status!s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot set bias delay status{self._log_chans[self._channels][channel]:s} to {status!s}\n{e!s}"
            )
        try:
            logging.debug(
                f"{__name__!s}: Set bias delay{self._log_chans[self._channels][channel]:s} to {offset:g}"
            )
            self._write(
                f":sour{self._cmd_chans[self._channels][channel]:s}:wait:offs {offset:g}"
            )
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot set bias delay{self._log_chans[self._channels][channel]:s} to {offset!s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot set bias delay{self._log_chans[self._channels][channel]:s} to {offset!s}\n{e!s}"
            )
        if "auto" in kwargs:
            ...
            try:
                logging.debug(
                    f"{__name__!s}: Set auto bias delay{self._log_chans[self._channels][channel]:s} to {auto!r}"
                )
                self._write(
                    f":sour{self._cmd_chans[self._channels][channel]:s}:wait:auto {int(auto):d}"
                )
            except Exception as e:
                logging.error(
                    f"{__name__!s}: Cannot set auto bias delay{self._log_chans[self._channels][channel]:s} to {auto!s}"
                )
                raise type(e)(
                    f"{__name__!s}: Cannot set auto bias delay{self._log_chans[self._channels][channel]:s} to {auto!s}\n{e!s}"
                )
        if "gain" in kwargs:
            ...
            try:
                logging.debug(
                    f"{__name__!s}: Set gain bias delay{self._log_chans[self._channels][channel]:s} to {gain!r}"
                )
                self._write(
                    f":sour{self._cmd_chans[self._channels][channel]:s}:wait:gain {int(gain):d}"
                )
            except Exception as e:
                logging.error(
                    f"{__name__!s}: Cannot set gain bias delay{self._log_chans[self._channels][channel]:s} to {gain!s}"
                )
                raise type(e)(
                    f"{__name__!s}: Cannot set gain bias delay{self._log_chans[self._channels][channel]:s} to {gain!s}\n{e!s}"
                )
    ...
    def get_bias_delay(self, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Get bias delay{self._log_chans[self._channels][channel]:s}"
            )
            return {
                "status": bool(
                    int(
                        self._ask(
                            f":sour{self._cmd_chans[self._channels][channel]:s}:wait:stat?"
                        )
                    )
                ),
                "offset": float(
                    self._ask(
                        f":sour{self._cmd_chans[self._channels][channel]:s}:wait:offs?"
                    )
                ),
                "auto": bool(
                    int(
                        self._ask(
                            f":sour{self._cmd_chans[self._channels][channel]:s}:wait:auto?"
                        )
                    )
                ),
                "gain": float(
                    self._ask(
                        f":sour{self._cmd_chans[self._channels][channel]:s}:wait:gain?"
                    )
                ),
            }
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot get bias delay{self._log_chans[self._channels][channel]:s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot get bias delay{self._log_chans[self._channels][channel]:s}\n{e!s}"
            )
    ...
    def set_sense_delay(self, offset, status=True, channel=1, **kwargs):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Set sense delay status{self._log_chans[self._channels][channel]:s} to {status:g}"
            )
            self._write(
                f":sens{self._cmd_chans[self._channels][channel]:s}:wait:stat {int(status):d}"
            )
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot set sense delay status{self._log_chans[self._channels][channel]:s} to {status!s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot set sense delay status{self._log_chans[self._channels][channel]:s} to {status!s}\n{e!s}"
            )
        try:
            logging.debug(
                f"{__name__!s}: Set sense delay{self._log_chans[self._channels][channel]:s} to {offset:g}"
            )
            self._write(
                f":sens{self._cmd_chans[self._channels][channel]:s}:wait:offs {offset:g}"
            )
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot set sense delay{self._log_chans[self._channels][channel]:s} to {offset!s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot set sense delay{self._log_chans[self._channels][channel]:s} to {offset!s}\n{e!s}"
            )
        if "auto" in kwargs:
            ...
            try:
                logging.debug(
                    f"{__name__!s}: Set auto sense delay{self._log_chans[self._channels][channel]:s} to {auto!r}"
                )
                self._write(
                    f":sens{self._cmd_chans[self._channels][channel]:s}:wait:auto {int(auto):d}"
                )
            except Exception as e:
                logging.error(
                    f"{__name__!s}: Cannot set auto sense delay{self._log_chans[self._channels][channel]:s} to {auto!s}"
                )
                raise type(e)(
                    f"{__name__!s}: Cannot set auto sense delay{self._log_chans[self._channels][channel]:s} to {auto!s}\n{e!s}"
                )
        if "gain" in kwargs:
            ...
            try:
                logging.debug(
                    f"{__name__!s}: Set gain sense delay{self._log_chans[self._channels][channel]:s} to {gain!r}"
                )
                self._write(
                    f":sens{self._cmd_chans[self._channels][channel]:s}:wait:gain {int(gain):d}"
                )
            except Exception as e:
                logging.error(
                    f"{__name__!s}: Cannot set gain sense delay{self._log_chans[self._channels][channel]:s} to {gain!s}"
                )
                raise type(e)(
                    f"{__name__!s}: Cannot set gain sense delay{self._log_chans[self._channels][channel]:s} to {gain!s}\n{e!s}"
                )
    ...
    def get_sense_delay(self, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Get sense delay{self._log_chans[self._channels][channel]:s}"
            )
            return {
                "status": bool(
                    int(
                        self._ask(
                            f":sens{self._cmd_chans[self._channels][channel]:s}:wait:stat?"
                        )
                    )
                ),
                "offset": float(
                    self._ask(
                        f":sens{self._cmd_chans[self._channels][channel]:s}:wait:offs?"
                    )
                ),
                "auto": bool(
                    int(
                        self._ask(
                            f":sens{self._cmd_chans[self._channels][channel]:s}:wait:auto?"
                        )
                    )
                ),
                "gain": float(
                    self._ask(
                        f":sens{self._cmd_chans[self._channels][channel]:s}:wait:gain?"
                    )
                ),
            }
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot get sense delay{self._log_chans[self._channels][channel]:s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot get sense delay{self._log_chans[self._channels][channel]:s}\n{e!s}"
            )
    ...
    def set_sense_nplc(self, val, channel=1):
        ...
        try:
            logging.debug(
                "{!s}: Set sense nplc{:s} to {!s}".format(
                    __name__,
                    self._log_chans[self._channels][channel],
            ...
            self._write(
                ":sens{:s}:{:s}:nplc{!s}".format(
                    self._cmd_chans[self._channels][channel],
                    self._IV_modes[self._sense_mode[channel]],
                    *[":auto 1" if val == -1 else f" {val!s}"]
                )
            )
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot set sense nplc{self._log_chans[self._channels][channel]:s} to {val!s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot set sense nplc{self._log_chans[self._channels][channel]:s} to {val!s}\n{e!s}"
            )
    ...
    def get_sense_nplc(self, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Get sense nplc{self._log_chans[self._channels][channel]:s}"
            )
            return float(
                self._ask(
                    f":sens{self._cmd_chans[self._channels][channel]:s}:{self._IV_modes[self._sense_mode[channel]]:s}:nplc"
                )
            )
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot get sense nplc{self._log_chans[self._channels][channel]:s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot get sense nplc{self._log_chans[self._channels][channel]:s}\n{e!s}"
            )
    ...
    def set_status(self, status, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Set output status{self._log_chans[self._channels][channel]:s} to {status!r}"
            )
            self._write(
                f":outp{self._cmd_chans[self._channels][channel]:s}:stat {status:d}"
            )
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot set output status{self._log_chans[self._channels][channel]:s} to {status!s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot set output status{self._log_chans[self._channels][channel]:s} to {status!s}\n{e!s}"
            )
    ...
    def get_status(self, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Get output status{self._log_chans[self._channels][channel]:s}"
            )
            return bool(
                int(
                    self._ask(
                        f":outp{self._cmd_chans[self._channels][channel]:s}:stat"
                    )
                )
            )
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot get output status{self._log_chans[self._channels][channel]:s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot get output status{self._log_chans[self._channels][channel]:s}\n{e!s}"
            )
    ...
    def set_bias_value(self, val, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Set bias value{self._log_chans[self._channels][channel]:s} to {val:g}"
            )
            self._write(
                f":sour{self._cmd_chans[self._channels][channel]:s}:{self._IV_modes[self.get_bias_mode(channel=channel)]:s}:lev {val:g}"
            )  # necessary to cast as scientific float! (otherwise only >= 1e-6 possible)
            self._write(f":disp:view sing{channel:d}")
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot set bias value{self._log_chans[self._channels][channel]:s} to {val!s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot set bias value{self._log_chans[self._channels][channel]:s} to {val!s}\n{e!s}"
            )
    ...
    def get_bias_value(self, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Get bias value{self._log_chans[self._channels][channel]:s}"
            )
            ...
            if self.get_status(channel):
                return float(
                    self._ask(
                        f":meas:{self._IV_modes[self.get_bias_mode(channel=channel)]:s}?"
                    )
                    .replace("+9.910000E+37", "nan")
                    .replace("9.900000E+37", "inf")
                )
            else:
                return float(
                    self._ask(
                        f":sour{self._cmd_chans[self._channels][channel]:s}:{self._IV_modes[self.get_bias_mode(channel=channel)]:s}:lev"
                    )
                )
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot get bias value{self._log_chans[self._channels][channel]:s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot get bias value{self._log_chans[self._channels][channel]:s}\n{e!s}"
            )
    ...
    def get_sense_value(self, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Get sense value{self._log_chans[self._channels][channel]:s}"
            )
            self._write(f":disp:view sing{channel:d}")
            return float(
                self._ask(
                    f":meas:{self._IV_modes[self._sense_mode[channel]]:s}?"
                )
                .replace("+9.910000E+37", "nan")
                .replace("9.900000E+37", "inf")
            )
            ...
            logging.error(
                f"{__name__!s}: Cannot get sense value{self._log_chans[self._channels][channel]:s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot get sense value{self._log_chans[self._channels][channel]:s}\n{e!s}"
            )
    ...
    def get_sense_values(self, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Get sense values of all active sense modes{self._log_chans[self._channels][channel]:s}"
            )
            self._write(f":disp:view sing{channel:d}")
            ...
            logging.error(
                f"{__name__!s}: Cannot get sense values of all active sense modes{self._log_chans[self._channels][channel]:s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot get sense values of all active sense modes{self._log_chans[self._channels][channel]:s}\n{e!s}"
            )
    ...
    def set_voltage(self, val, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__:s}: Set voltage value{self._log_chans[self._channels][channel]:s} to {val:g}"
            )
            self._write(
                f":sour{self._cmd_chans[self._channels][channel]:s}:volt {val:g}"
            )
            self._write(f":disp:view sing{channel:d}")
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot set voltage value{self._log_chans[self._channels][channel]:s} to {val!s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot set voltage value{self._log_chans[self._channels][channel]:s} to {val!s}\n{e!s}"
            )
    ...
    def get_voltage(self, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__:s}: Get voltage value{self._log_chans[self._channels][channel]:s}"
            )
            self._write(f":disp:view sing{channel:d}")
            ...
            logging.error(
                f"{__name__!s}: Cannot get voltage value{self._log_chans[self._channels][channel]:s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot get voltage value{self._log_chans[self._channels][channel]:s}\n{e!s}"
            )
    ...
    def set_current(self, val, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__:s}: Set current value{self._log_chans[self._channels][channel]:s} to {val:g}"
            )
            self._write(
                f":sour{self._cmd_chans[self._channels][channel]:s}:curr {val:g}"
            )
            self._write(f":disp:view sing{channel:d}")
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot set current value{self._log_chans[self._channels][channel]:s} to {val!s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot set current value{self._log_chans[self._channels][channel]:s} to {val!s}\n{e!s}"
            )
    ...
    def get_current(self, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__:s}: Get current value{self._log_chans[self._channels][channel]:s}"
            )
            self._write(f":disp:view sing{channel:d}")
            ...
            logging.error(
                f"{__name__!s}: Cannot get current value{self._log_chans[self._channels][channel]:s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot get current value{self._log_chans[self._channels][channel]:s}\n{e!s}"
            )
    ...
    def get_resistance(self, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__:s}: Get resistance value{self._log_chans[self._channels][channel]:s}"
            )
            self._write(f":disp:view sing{channel:d}")
            ...
            logging.error(
                f"{__name__!s}: Cannot get resistance value{self._log_chans[self._channels][channel]:s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot get resistance value{self._log_chans[self._channels][channel]:s}\n{e!s}"
            )
    ...
    def get_IV(self, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__:s}: Get current and voltage value{self._log_chans[self._channels][channel]:s}"
            )
            self._write(f":disp:view sing{channel:d}")
            return np.fromstring(
                self._ask(":meas?")
            ...
            logging.error(
                f"{__name__!s}: Cannot get current and voltage value{self._log_chans[self._channels][channel]:s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot get current and voltage value{self._log_chans[self._channels][channel]:s}\n{e!s}"
            )
    ...
    def ramp_voltage(self, stop, step, step_time=0.1, channel=1):
        ...
        if self.get_bias_mode(channel=channel):  # 1 (voltage bias)
            ...
            logging.error(
                __name__
                + f": Cannot ramp voltage{self._log_chans[self._channels][channel]:s} in the current bias"
            )
            raise ValueError(
                __name__
                + f": Cannot ramp voltage{self._log_chans[self._channels][channel]:s} in the current bias"
    ...
    def ramp_current(self, stop, step, step_time=0.1, channel=1):
        ...
        if not self.get_bias_mode(channel=channel):  # 0 (current bias)
            ...
            logging.error(
                __name__
                + f": Cannot ramp current{self._log_chans[self._channels][channel]:s} in the voltage bias"
            )
            raise ValueError(
                __name__
                + f": Cannot ramp current{self._log_chans[self._channels][channel]:s} in the voltage bias"
    ...
    def _set_sweep_start(self, val, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Set sweep start{self._log_chans[self._channels][channel]:s} to {val:g}"
            )
            self._write(
                f":sour{self._cmd_chans[self._channels][channel]:s}:{self._IV_modes[self.get_bias_mode(channel)]:s}:star {val:g}"
            )
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot set sweep start{self._log_chans[self._channels][channel]:s} to {val!s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot set sweep start{self._log_chans[self._channels][channel]:s} to {val!s}\n{e!s}"
            )
    ...
    def _get_sweep_start(self, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Get sweep start{self._log_chans[self._channels][channel]:s}"
            )
            return float(
                self._ask(
                    f":sour{self._cmd_chans[self._channels][channel]:s}:{self._IV_modes[self.get_bias_mode(channel)]:s}:star"
                )
            )
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot get sweep start{self._log_chans[self._channels][channel]:s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot get sweep start{self._log_chans[self._channels][channel]:s}\n{e!s}"
            )
    ...
    def _set_sweep_stop(self, val, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Set sweep stop{self._log_chans[self._channels][channel]:s} to {val:g}"
            )
            self._write(
                f":sour{self._cmd_chans[self._channels][channel]:s}:{self._IV_modes[self.get_bias_mode(channel)]:s}:stop {val:g}"
            )
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot set sweep stop{self._log_chans[self._channels][channel]:s} to {val!s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot set sweep stop{self._log_chans[self._channels][channel]:s} to {val!s}\n{e!s}"
            )
    ...
    def _get_sweep_stop(self, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Get sweep stop{self._log_chans[self._channels][channel]:s}"
            )
            return float(
                self._ask(
                    f":sour{self._cmd_chans[self._channels][channel]:s}:{self._IV_modes[self.get_bias_mode(channel)]:s}:stop"
                )
            )
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot get sweep stop{self._log_chans[self._channels][channel]:s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot get sweep stop{self._log_chans[self._channels][channel]:s}\n{e!s}"
            )
    ...
    def _set_sweep_step(self, val, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Set sweep step{self._log_chans[self._channels][channel]:s} to {val:g}"
            )
            self._write(
                f":sour{self._cmd_chans[self._channels][channel]:s}:{self._IV_modes[self.get_bias_mode(channel)]:s}:step {val:g}"
            )
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot set sweep step{self._log_chans[self._channels][channel]:s} to {val!s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot set sweep step{self._log_chans[self._channels][channel]:s} to {val!s}\n{e!s}"
            )
    ...
    def _get_sweep_step(self, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Get sweep step{self._log_chans[self._channels][channel]:s}"
            )
            return float(
                self._ask(
                    f":sour{self._cmd_chans[self._channels][channel]:s}:{self._IV_modes[self.get_bias_mode(channel)]:s}:step"
                )
            )
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot get sweep step{self._log_chans[self._channels][channel]:s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot get sweep step{self._log_chans[self._channels][channel]:s}\n{e!s}"
            )
    ...
    def _get_sweep_nop(self, channel=1):
        ...
        try:
            logging.debug(
                f"{__name__!s}: Get sweep nop{self._log_chans[self._channels][channel]:s}"
            )
            return int(
                self._ask(
                    f":sour{self._cmd_chans[self._channels][channel]:s}:{self._IV_modes[self.get_bias_mode(channel)]:s}:poin"
                )
            )
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot get sweep nop{self._log_chans[self._channels][channel]:s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot get sweep nop{self._log_chans[self._channels][channel]:s}\n{e!s}"
            )
    ...
    def set_sweep_parameters(self, sweep):
        ...
        if not self._sweep_mode:  # 0 (VV-mode)
            ...
            try:
                # bias channel
                self._write(
                    f":sour{self._cmd_chans[self._channels][channel_bias]:s}:{self._IV_modes[self.get_bias_mode(channel_bias)]:s}:mode swe"
                )
                self._write(
                    f":sour{self._cmd_chans[self._channels][channel_bias]:s}:swe:rang best"
                )
                self._write(
                    f":sour{self._cmd_chans[self._channels][channel_bias]:s}:swe:spac lin"
                )
                self._write(
                    f":sour{self._cmd_chans[self._channels][channel_bias]:s}:swe:sta sing"
                )
                self._write(
                    f":sour{self._cmd_chans[self._channels][channel_bias]:s}:swe:dir up"
                )
                ...
                self._set_sweep_step(
                    val=float(sweep[2] * np.sign(float(sweep[1]) - float(sweep[0]))),
                    channel=channel_bias,
                )
                self.set_bias_value(
                    val=self._get_sweep_start(channel=channel_bias),
                    channel=channel_bias,
                )
                self._write(f":trig{channel_bias:d}:acq:sour aint")
                self._write(f":trig{channel_bias:d}:tran:sour aint")
                self._write(
                    f":trig{channel_bias:d}:all:count {self._get_sweep_nop(channel=channel_bias):d}"
                )
                # sense channel
                self._write(
                    f":sour{self._cmd_chans[self._channels][channel_sense]:s}:{self._IV_modes[self.get_bias_mode(channel_sense)]:s}:mode swe"
                )
                self._write(
                    f":sour{self._cmd_chans[self._channels][channel_sense]:s}:swe:rang best"
                )
                self._write(
                    f":sour{self._cmd_chans[self._channels][channel_sense]:s}:swe:spac lin"
                )
                self._write(
                    f":sour{self._cmd_chans[self._channels][channel_sense]:s}:swe:sta sing"
                )
                self._write(
                    f":sour{self._cmd_chans[self._channels][channel_sense]:s}:swe:dir up"
                )
                self._set_sweep_start(val=0, channel=channel_sense)
                self._set_sweep_stop(val=0, channel=channel_sense)
                self._write(
                    f"sour{channel_sense:d}:curr:poin {self._get_sweep_nop():d}"
                )
                self._write(f":trig{channel_sense:d}:acq:sour aint")
                self._write(f":trig{channel_sense:d}:tran:sour aint")
                self._write(
                    f":trig{channel_sense:d}:all:count {self._get_sweep_nop(channel=channel_sense):d}"
                )
                # general
                self.set_sync(True)
                ...
                logging.error(
                    f"{__name__!s}: Cannot set sweep parameters of channels {self._sweep_channels!s} to {sweep!s}"
                )
                raise type(e)(
                    f"{__name__!s}: Cannot set sweep parameters of channels {self._sweep_channels!s} to {sweep!s}\n{e!s}"
                )
            ...
            try:
                logging.debug(
                    f"{__name__!s}: Set sweep parameters of channel {self._sweep_channels!s} to {sweep!s}"
                )
                self._write(
                    f":sour{self._cmd_chans[self._channels][channel_bias]:s}:{self._IV_modes[self.get_bias_mode(channel_bias)]:s}:mode swe"
                )
                ...
                self._write(
                    f":trig:all:count {self._get_sweep_nop(channel=channel_bias):d}"
                )
                self.set_display("grap")
                ### TODO: auto scale display
            except Exception as e:
                logging.error(
                    f"{__name__!s}: Cannot set sweep parameters of channel {self._sweep_channels!s} to {sweep!s}"
                )
                raise type(e)(
                    f"{__name__!s}: Cannot set sweep parameters of channel {self._sweep_channels!s} to {sweep!s}\n{e!s}"
                )
        ...
        try:
            if not self._sweep_mode:  # 0 (VV-mode)
                ...
                sense_values = np.fromstring(
                    self._ask(f":fetc:arr:volt? (@{channel_sense:d})")
                    .replace("+9.910000E+37", "nan")
                    .replace("9.900000E+37", "inf"),
                    sep=",",
                    dtype=float,
                ...
                channel_bias, channel_sense = self._sweep_channels * 2
                logging.debug(
                    f"{__name__!s}: Take sweep data of channel {self._sweep_channels!s}"
                )
                self._visainstrument.write(":init")
                self._wait_for_transition_idle(channel=channel_bias)
                I_values = np.fromstring(
                    self._ask(":fetc:arr:curr?")
                    .replace("+9.910000E+37", "nan")
                    .replace("9.900000E+37", "inf"),
                    sep=",",
                    dtype=float,
                )
                V_values = np.fromstring(
                    self._ask(":fetc:arr:volt?")
                    .replace("+9.910000E+37", "nan")
                    .replace("9.900000E+37", "inf"),
                    sep=",",
                    dtype=float,
                ...
                return (I_values, V_values)[
                    :: int(np.sign(0.5 - self.get_sweep_bias()))
                ]
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot take sweep data of channel {self._sweep_channels!s}"
            )
            raise type(e)(
                f"{__name__!s}: Cannot take sweep data of channel {self._sweep_channels!s}\n{e!s}"
            )
    ...
    def get_measurement_ccr(self):
        ...
        try:
            logging.debug(f"{__name__!s}: Get measurement ccr")
            ...
            logging.error(f"{__name__!s}: Cannot get measurement ccr")
            raise type(e)(f"{__name__!s}: Cannot get measurement ccr\n{e!s}")
    ...
    def print_measurement_ccr(self):
        ...
        msg = [
            (f"\n\t{sb[0]:36s}:{ccr[i]!r}\t({sb[1]:s})")
            for i, sb in enumerate(self.measurement_ccr)
            if sb != ("", "")
        ]
    ...
    def get_operation_ccr(self):
        ...
        # Corresponding Command: :STATus:<MEASurement|OPERation|QUEStionable>:CONDition?
        try:
            logging.debug(f"{__name__!s}: Get operation ccr")
            ...
            logging.error(f"{__name__!s}: Cannot get operation ccr")
            raise type(e)(f"{__name__!s}: Cannot get operation ccr\n{e!s}")
    ...
    def print_operation_ccr(self):
        ...
        msg = [
            (f"\n\t{sb[0]:36s}:{ccr[i]!r}\t({sb[1]:s})")
            for i, sb in enumerate(self.operation_ccr)
            if sb != ("", "")
        ]
    ...
    def get_questionable_ccr(self):
        ...
        # Corresponding Command: :STATus:<MEASurement|OPERation|QUEStionable>:CONDition?
        try:
            logging.debug(f"{__name__!s}: Get questionable ccr")
            ...
            logging.error(f"{__name__!s}: Cannot get questionable ccr")
            raise type(e)(f"{__name__!s}: Cannot get questionable ccr\n{e!s}")
    ...
    def print_questionable_ccr(self):
        ...
        msg = [
            (f"\n\t{sb[0]:36s}:{ccr[i]!r}\t({sb[1]:s})")
            for i, sb in enumerate(self.questionable_ccr)
            if sb != ("", "")
        ]
    ...
    def get_transition_idle(self, channel=1):
        ...
        try:
            logging.debug(
                f"""{__name__!s}: Get operation ccr condition "Transition idle"{self._log_chans[self._channels][channel]:s}"""
            )
            self._visainstrument.write(":stat:oper:enab 1")
            return bool(
                (
                    int(self._visainstrument.query(":stat:oper:cond?"))
                    >> (1 + 6 * (channel - 1))
                )
                % 2
            )
            ...
            logging.error(
                f'{__name__!s}: Cannot get operation ccr condition "Transition idle"{self._log_chans[self._channels][channel]:s}'
            )
            raise type(e)(
                f'{__name__!s}: Cannot get operation ccr condition "Transition idle"{self._log_chans[self._channels][channel]:s}\n{e!s}'
            )
    ...
    def set_display(self, mode):
        ...
        if mode.lower() in (
            ...
            logging.debug(f"{__name__!s}: Set display to {mode:s}")
            self._write(f":disp:view {mode:s}")
        else:
            logging.error(f"{__name__:s}: Cannot set display to {mode!s}")
            raise ValueError(f"{__name__:s}: Cannot set display to {mode!s}")
    ...
    def set_defaults(self, sweep_mode=None):
        ...
        self._write(
            ":sens:func:on:all"
        )  # enable all sense functions (voltage, current, resistance)
        ...
        new = f"sweep mode: {self._sweep_mode:d}:\n" + "".join(
            [
                ...
                + "".join(
                    [
                        "\t\t{:s}: {!s}\n".format(
                            func.__name__.replace("set_", ""), param
                        )
                        for func, param in self._defaults[self._sweep_mode][i].items()
                    ]
                )
    ...
    def get_all(self, channel=None):
        ...
        for chan in np.arange(self._channels) + 1 if channel == None else [channel]:
            print(f"channel: {chan:d}")
            print(
                f"\tmeasurement mode   = {self._measurement_modes[self.get_measurement_mode(channel=chan)]:s}"
            )
            print(
                f"\tbias mode          = {self._IV_modes[self.get_bias_mode(channel=chan)]:s}"
            )
            print(
                f"\tsense mode         = {self._IV_modes[self.get_sense_mode(channel=chan)]:s}"
            )
            print(
                f"\tbias range         = {self.get_bias_range():1.0e}{self._IV_units[self.get_bias_mode(channel=chan)]:s}"
            )
            print(
                f"\tsense range        = {self.get_sense_range():1.0e}{self._IV_units[self.get_sense_mode(channel=chan)]:s}"
            )
            print(
                f"\tbias delay         = {self.get_bias_delay(channel=chan)!s}"
            )
            print(
                f"\tsense delay        = {self.get_sense_delay(channel=chan)!s}"
            )
            print(
                f"\tsense nplc         = {self.get_sense_nplc(channel=chan):f}"
            )
            print(f"\tstatus             = {self.get_status(channel=chan)!r}")
            if self.get_status(channel=chan):
                print(
                    f"\tbias value         = {self.get_bias_value(channel=chan):g}{self._IV_units[self.get_bias_mode(channel=chan)]:s}"
                )
                print(
                    f"\tsense value        = {self.get_sense_value(channel=chan):g}{self._IV_units[self.get_sense_mode(channel=chan)]:s}"
                )
            print(
                f"\tsweep start        = {self._get_sweep_start(channel=chan):g}{self._IV_units[self.get_bias_mode(channel=chan)]:s}"
            )
            print(
                f"\tsweep stop         = {self._get_sweep_stop(channel=chan):g}{self._IV_units[self.get_bias_mode(channel=chan)]:s}"
            )
            print(
                f"\tsweep step         = {self._get_sweep_step(channel=chan):g}{self._IV_units[self.get_bias_mode(channel=chan)]:s}"
            )
            print(
                f"\tsweep nop          = {self._get_sweep_nop(channel=self._sweep_channels[0]):d}"
            )
    ...
    def get_parameters(self):
        ...
        """
        parlist = {
            "measurement_mode": {"channels": range(1, self._channels + 1)},
