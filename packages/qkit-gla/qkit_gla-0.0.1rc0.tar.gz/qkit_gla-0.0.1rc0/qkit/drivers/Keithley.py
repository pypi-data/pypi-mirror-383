...
class Keithley(Instrument):
    ...
    def __init__(self, name, address, reset=False):
        ...
        if LooseVersion(visa.__version__) < LooseVersion("1.5.0"):  # pyvisa 1.4
            ...
            self._read_stb = lambda: visa.visalib.read_stb(
                self._visainstrument.session
            )[0]
        # internal variables
        ...
        self._defaults = {
            0: [
                {
                    self.set_measurement_mode: 0,  # bias channel
    ...
    def _ask(self, cmd):
        ...
        """
        ans = self._visainstrument.query(f"print({cmd:s})").strip()
        while not bool(int(self._visainstrument.query("*OPC?"))):
            time.sleep(1e-6)
        self._raise_error()
        return ans
    ...
    def set_digio(self, line, status):
        ...
        try:
            logging.debug(
                "{!s}: Set digital I/O of line {:d} to {:d}".format(
                    __name__, line, status
                )
            )
            self._write(f"digio.writebit({line:d}, {status:d})")
        except Exception as e:
            logging.error(
                "{!s}: Cannot set digital I/O of line {!s} to {!s}".format(
                    __name__, line, status
                )
            )
            raise type(e)(
                "{!s}: Cannot set digital I/O of line {!s} to {!s}\n{!s}".format(
                    __name__, line, status, e
                )
            )
        return
    ...
    def get_digio(self, line):
        ...
        try:
            logging.debug(f"{__name__:s}: Get digital I/O of line {line:d}")
            return bool(int(float(self._ask(f"digio.readbit({line:d})"))))
        except Exception as e:
            logging.error(
                f"{__name__!s}: Cannot get digital I/O of line {line!s}"
            )
            raise type(e)(
                "{!s}: Cannot get digital I/O of line {!s}\n{!s}".format(
                    __name__, line, e
                )
            )
    ...
    def set_measurement_mode(self, mode, channel=1):
        ...
        try:
            logging.debug(
                "{!s}: Set measurement mode of channel {:s} to {:d}".format(
                    __name__, chr(64 + channel), mode
                )
            )
            self._write(f"smu{chr(96 + channel):s}.sense = {mode:d}")
        except Exception as e:
            logging.error(
                "{!s}: Cannot set measurement mode of channel {!s} to {!s}".format(
                    __name__, chr(64 + channel), mode
                )
            )
            raise type(e)(
                "{!s}: Cannot set measurement mode of channel {!s} to {!s}\n{!s}".format(
                    __name__, chr(64 + channel), mode, e
                )
            )
        return
    ...
    def get_measurement_mode(self, channel=1):
        ...
        try:
            logging.debug(
                "{!s}: Get measurement mode of channel {:s}".format(
                    __name__, chr(64 + channel)
                )
            )
            return int(float(self._ask(f"smu{chr(96 + channel):s}.sense")))
        except Exception as e:
            logging.error(
                "{!s}: Cannot get measurement mode of channel {!s}".format(
                    __name__, chr(64 + channel)
                )
            )
            raise type(e)(
                "{!s}: Cannot get measurement mode of channel {!s}\n{!s}".format(
                    __name__, chr(64 + channel), e
                )
            )
    ...
    def set_bias_mode(self, mode, channel=1):
        ...
        try:
            logging.debug(
                "{:s}: Set bias mode of channel {:s} to {:d}".format(
                    __name__, chr(64 + channel), mode
                )
            )
            self._write(f"smu{chr(96 + channel):s}.source.func = {mode:d}")
        except Exception as e:
            logging.error(
                "{!s}: Cannot set bias mode of channel {!s} to {!s}".format(
                    __name__, chr(64 + channel), mode
                )
            )
            raise type(e)(
                "{!s}: Cannot set bias mode of channel {!s} to {!s}\n{!s}".format(
                    __name__, chr(64 + channel), mode, e
                )
            )
        return
    ...
    def get_bias_mode(self, channel=1):
        ...
        try:
            logging.debug(
                "{:s}: Get bias mode of channel {:s}".format(
                    __name__, chr(64 + channel)
                )
            )
            return int(
                float(self._ask(f"smu{chr(96 + channel):s}.source.func"))
            )
        except Exception as e:
            logging.error(
                "{!s}: Cannot get bias mode of channel {!s}".format(
                    __name__, chr(64 + channel)
                )
            )
            raise type(e)(
                "{!s}: Cannot get bias mode of channel {!s}\n{!s}".format(
                    __name__, chr(64 + channel), e
                )
            )
    ...
    def set_sense_mode(self, mode, channel=1):
        ...
        try:
            logging.debug(
                "{:s}: Set sense mode of channel {:s} to {:d}".format(
                    __name__, chr(64 + channel), mode
                )
            )
            self._write(
                f"display.smu{chr(96 + channel):s}.measure.func = {mode:d}"
            )
        except Exception as e:
            logging.error(
                "{!s}: Cannot set sense mode of channel {!s} to {!s}".format(
                    __name__, chr(64 + channel), mode
                )
            )
            raise type(e)(
                "{!s}: Cannot set sense mode of channel {!s} to {!s}\n{!s}".format(
                    __name__, chr(64 + channel), mode, e
                )
            )
        return
    ...
    def get_sense_mode(self, channel=1):
        ...
        try:
            logging.debug(
                "{:s}: Get sense mode of channel {:s}".format(
                    __name__, chr(64 + channel)
                )
            )
            return int(
                float(
                    self._ask(f"display.smu{chr(96 + channel):s}.measure.func")
                )
            )
        except Exception as e:
            logging.error(
                "{!s}: Cannot get sense mode of channel {!s}".format(
                    __name__, chr(64 + channel)
                )
            )
            raise type(e)(
                "{!s}: Cannot get sense mode of channel {!s}\n{!s}".format(
                    __name__, chr(64 + channel), e
                )
            )
    ...
    def set_bias_range(self, val, channel=1):
        ...
        try:
            logging.debug(
                "{:s}: Set bias range of channel {:s} to {:f}".format(
                    __name__, chr(64 + channel), val
                )
            )
            if val == -1:
                self._write(
                    "smu{:s}.source.autorange{:s} = 1".format(
                        chr(96 + channel),
                        self._smu_command[self.get_bias_mode(channel=channel)],
                    )
                )
            else:
                self._write(
                    "smu{:s}.source.autorange{:s} = 0".format(
                        chr(96 + channel),
                        self._smu_command[self.get_bias_mode(channel=channel)],
                    )
                )
                self._write(
                    "smu{:s}.source.range{:s} = {:f}".format(
                        chr(96 + channel),
                        self._smu_command[self.get_bias_mode(channel=channel)],
                        val,
                    )
            ...
            logging.error(
                "{!s}: Cannot set bias range of channel {!s} to {!s}".format(
                    __name__, chr(64 + channel), val
                )
            )
            raise type(e)(
                "{!s}: Cannot set bias range of channel {!s} to {!s}\n{!s}".format(
                    __name__, chr(64 + channel), val, e
                )
            )
        return
    ...
    def get_bias_range(self, channel=1):
        ...
        try:
            logging.debug(
                "{:s}: Get bias range of channel {:s}".format(
                    __name__, chr(64 + channel)
                )
            )
            return float(
                self._ask(
                    "smu{:s}.source.range{:s}".format(
                        chr(96 + channel),
                        self._smu_command[self.get_bias_mode(channel=channel)],
                    )
                )
            ...
            logging.error(
                "{!s}: Cannot get bias range of channel {!s}".format(
                    __name__, chr(64 + channel)
                )
            )
            raise type(e)(
                "{!s}: Cannot get bias range of channel {!s}\n{!s}".format(
                    __name__, chr(64 + channel), e
                )
            )
    ...
    def set_bias_limit(self, val, channel=1):
        ...
        try:
            logging.debug(
                "{:s}: Set bias limit of channel {:s} to {:f}".format(
                    __name__, chr(64 + channel), val
                )
            )
            self._write(
                "smu{:s}.source.limit{:s} = {:f}".format(
                    chr(96 + channel),
                    self._smu_command[self.get_bias_mode(channel=channel)],
                    val,
                )
            ...
            logging.error(
                "{!s}: Cannot set bias limit of channel {!s} to {!s}".format(
                    __name__, chr(64 + channel), val
                )
            )
            raise type(e)(
                "{!s}: Cannot set bias limit of channel {!s} to {!s}\n{!s}".format(
                    __name__, chr(64 + channel), val, e
                )
            )
        return
    ...
    def get_bias_limit(self, channel=1):
        ...
        try:
            logging.debug(
                "{:s}: Get bias limit of channel {:s}".format(
                    __name__, chr(64 + channel)
                )
            )
            return float(
                self._ask(
                    "smu{:s}.source.limit{:s}".format(
                        chr(96 + channel),
                        self._smu_command[self.get_bias_mode(channel=channel)],
                    )
                )
            ...
            logging.error(
                "{!s}: Cannot get bias limit of channel {!s}".format(
                    __name__, chr(64 + channel)
                )
            )
            raise type(e)(
                "{!s}: Cannot get bias limit of channel {!s}\n{!s}".format(
                    __name__, chr(64 + channel), e
                )
            )
    ...
    def set_sense_range(self, val, channel=1):
        ...
        try:
            logging.debug(
                "{:s}: Set sense range of channel {:s} to {:f}".format(
                    __name__, chr(64 + channel), val
                )
            )
            if val == -1:
                self._write(
                    "smu{:s}.measure.autorange{:s} = 1".format(
                        chr(96 + channel),
                        self._smu_command[self.get_sense_mode(channel=channel)],
                    )
                )
            else:
                self._write(
                    "smu{:s}.measure.autorange{:s} = 0".format(
                        chr(96 + channel),
                        self._smu_command[self.get_sense_mode(channel=channel)],
                    )
                )
                self._write(
                    "smu{:s}.measure.range{:s} = {:f}".format(
                        chr(96 + channel),
                        self._smu_command[self.get_sense_mode(channel=channel)],
                        val,
                    )
            ...
            logging.error(
                "{!s}: Cannot set sense range of channel {!s} to {!s}".format(
                    __name__, chr(64 + channel), val
                )
            )
            raise type(e)(
                "{!s}: Cannot set sense range of channel {!s} to {!s}\n{!s}".format(
                    __name__, chr(64 + channel), val, e
                )
            )
        return
    ...
    def get_sense_range(self, channel=1):
        ...
        try:
            logging.debug(
                "{:s}: Get sense range of channel {:s}".format(
                    __name__, chr(64 + channel)
                )
            )
            return float(
                self._ask(
                    "smu{:s}.measure.range{:s}".format(
                        chr(96 + channel),
                        self._smu_command[self.get_sense_mode(channel=channel)],
                    )
                )
            ...
            logging.error(
                "{!s}: Cannot get sense range of channel {!s}".format(
                    __name__, chr(64 + channel)
                )
            )
            raise type(e)(
                "{!s}: Cannot get sense range of channel {!s}\n{!s}".format(
                    __name__, chr(64 + channel), e
                )
            )
    ...
    def set_bias_delay(self, val, channel=1):
        ...
        try:
            logging.debug(
                "{:s}: Set bias delay of channel {:s} to {:f}".format(
                    __name__, chr(64 + channel), val
                )
            )
            self._write(f"smu{chr(96 + channel):s}.source.delay = {val:f}")
        except Exception as e:
            logging.error(
                "{!s}: Cannot set bias delay of channel {!s} to {!s}".format(
                    __name__, chr(64 + channel), val
                )
            )
            raise type(e)(
                "{!s}: Cannot set bias delay of channel {!s} to {!s}\n{!s}".format(
                    __name__, chr(64 + channel), val, e
                )
            )
        return
    ...
    def get_bias_delay(self, channel=1):
        ...
        try:
            logging.debug(
                "{:s}: Get bias delay of channel {:s}".format(
                    __name__, chr(64 + channel)
                )
            )
            return float(self._ask(f"smu{chr(96 + channel):s}.source.delay"))
        except Exception as e:
            logging.error(
                "{!s}: Cannot get bias delay of channel {!s}".format(
                    __name__, chr(64 + channel)
                )
            )
            raise type(e)(
                "{!s}: Cannot get bias delay of channel {!s}\n{!s}".format(
                    __name__, chr(64 + channel), e
                )
            )
    ...
    def set_sense_delay(self, val, channel=1, **kwargs):
        ...
        try:
            logging.debug(
                "{:s}: Set sense delay of channel {:s} to {:f}".format(
                    __name__, chr(64 + channel), val
                )
            )
            self._write(f"smu{chr(96 + channel):s}.measure.delay = {val:f}")
            if val == -1:
                self._write(
                    "smu{:s}.measure.delayfactor = {:f}".format(
                        chr(96 + channel), kwargs.get("factor", 1)
                    )
                )
        except Exception as e:
            logging.error(
                "{!s}: Cannot set sense delay of channel {!s} to {!s}".format(
                    __name__, chr(64 + channel), val
                )
            )
            raise type(e)(
                "{!s}: Cannot set sense delay of channel {!s} to {!s}\n{!s}".format(
                    __name__, chr(64 + channel), val, e
                )
            )
        return
    ...
    def get_sense_delay(self, channel=1):
        ...
        try:
            logging.debug(
                "{:s}: Get sense delay of channel {:s}".format(
                    __name__, chr(64 + channel)
                )
            )
            return float(self._ask(f"smu{chr(96 + channel):s}.measure.delay"))
        except Exception as e:
            logging.error(
                "{!s}: Cannot get sense delay of channel {!s}".format(
                    __name__, chr(64 + channel)
                )
            )
            raise type(e)(
                "{!s}: Cannot get sense delay of channel {!s}\n{!s}".format(
                    __name__, chr(64 + channel), e
                )
            )
    ...
    def set_sense_average(self, val, channel=1, **kwargs):
        ...
        try:
            ...
            logging.debug(
                "{:s}: Set sense average of channel {:s} to {:d} and mode {:s}".format(
                    __name__, chr(64 + channel), val, self._avg_types[mode]
                )
            )
            status = not (
                0.5 * (1 - np.sign(val - 1))
            ...
            self._write(
                f"smu{chr(96 + channel):s}.measure.filter.enable = {status:d}"
            )
            if status:
                self._write(
                    f"smu{chr(96 + channel):s}.measure.filter.count = {val:d}"
                )
                self._write(
                    f"smu{chr(96 + channel):s}.measure.filter.type = {mode:d}"
                )
        except Exception as e:
            logging.error(
                "{!s}: Cannot set sense average of channel {!s} to {!s}".format(
                    __name__, chr(64 + channel), val
                )
            )
            raise type(e)(
                "{!s}: Cannot set sense average of channel {!s} to {!s}\n{!s}".format(
                    __name__, chr(64 + channel), val, e
                )
            )
        return
    ...
    def get_sense_average(self, channel=1):
        ...
        try:
            logging.debug(
                "{:s}: Get sense average of channel {:s}".format(
                    __name__, chr(64 + channel)
                )
            )
            status = bool(
                int(
                    float(
                        self._ask(
                            f"smu{chr(96 + channel):s}.measure.filter.enable"
                        )
                    )
                )
            )
            val = int(
                float(
                    self._ask(f"smu{chr(96 + channel):s}.measure.filter.count")
                )
            )
            mode = int(
                float(
                    self._ask(f"smu{chr(96 + channel):s}.measure.filter.type")
                )
            )
            return status, val, mode
        except Exception as e:
            logging.error(
                "{!s}: Cannot get sense average of channel {!s}".format(
                    __name__, chr(64 + channel)
                )
            )
            raise type(e)(
                "{!s}: Cannot get sense average of channel {!s}\n{!s}".format(
                    __name__, chr(64 + channel), e
                )
            )
    ...
    def set_plc(self, val):
        ...
        try:
            logging.debug(f"{__name__:s}: Set PLC to {str(val):s}")
            cmd = {-1: "autolinefreq = true", 50: "linefreq = 50", 60: "linefreq = 60"}
            self._write(f"localnode.{cmd[int(val)]:s}")
        except Exception as e:
            logging.error(f"{__name__!s}: Cannot set PLC to {val!s}")
            raise type(e)(f"{__name__!s}: Cannot set PLC to {val!s}\n{e!s}")
        return
    ...
    def set_sense_nplc(self, val, channel=1):
        ...
        try:
            logging.debug(
                "{:s}: Set sense nplc of channel {:s} to {:f} PLC".format(
                    __name__, chr(64 + channel), val
                )
            )
            self._write(f"smu{chr(96 + channel):s}.measure.nplc = {val:f}")
        except Exception as e:
            logging.error(
                "{!s}: Cannot set sense nplc of channel {!s} to {!s}".format(
                    __name__, chr(64 + channel), val
                )
            )
            raise type(e)(
                "{!s}: Cannot set sense nplc of channel {!s} to {!s}\n{!s}".format(
                    __name__, chr(64 + channel), val, e
                )
            )
        return
    ...
    def get_sense_nplc(self, channel=1):
        ...
        try:
            logging.debug(
                "{:s}: Get sense nplc of channel {:s}".format(
                    __name__, chr(64 + channel)
                )
            )
            return float(self._ask(f"smu{chr(96 + channel):s}.measure.nplc"))
        except Exception as e:
            logging.error(
                "{!s}: Cannot get sense nplc of channel {!s}".format(
                    __name__, chr(64 + channel)
                )
            )
            raise type(e)(
                "{!s}: Cannot get sense nplc of channel {!s}\n{!s}".format(
                    __name__, chr(64 + channel), e
                )
            )
    ...
    def set_sense_autozero(self, val, channel=1):
        ...
        try:
            logging.debug(
                "{:s}: Set sense autozero of channel {:s} to {:d}".format(
                    __name__, chr(64 + channel), val
                )
            )
            self._write(
                f"smu{chr(96 + channel):s}.measure.autozero = {val:d}"
            )
        except Exception as e:
            logging.error(
                "{!s}: Cannot set sense autozero of channel {!s} to {!s}".format(
                    __name__, chr(64 + channel), val
                )
            )
            raise type(e)(
                "{!s}: Cannot set sense autozero of channel {!s} to {!s}\n{!s}".format(
                    __name__, chr(64 + channel), val, e
                )
            )
        return
    ...
    def get_sense_autozero(self, channel=1):
        ...
        try:
            logging.debug(
                "{:s}: Get sense autozero of channel {:s}".format(
                    __name__, chr(64 + channel)
                )
            )
            val = int(
                float(self._ask(f"smu{chr(96 + channel):s}.measure.autozero"))
            )
            return 3 * int(bool(val)) - val  # swap 1 and 2
        except Exception as e:
            logging.error(
                "{!s}: Cannot get sense autozero of channel {!s}".format(
                    __name__, chr(64 + channel)
                )
            )
            raise type(e)(
                "{!s}: Cannot get sense autozero of channel {!s}\n{!s}".format(
                    __name__, chr(64 + channel), e
                )
            )
    ...
    def set_status(self, status, channel=1):
        ...
        try:
            logging.debug(
                "{:s}: Set output status of channel {:s} to {:d}".format(
                    __name__, chr(64 + channel), status
                )
            )
            self._write(
                f"smu{chr(96 + channel):s}.source.output = {status:d}"
            )
        except Exception as e:
            logging.error(
            ...
            raise type(e)(
                "{!s}: Cannot set output status of channel {!s} to {!s}\n{!s}".format(
                    __name__, chr(64 + channel), status, e
                )
            )
        return
    ...
    def get_status(self, channel=1):
        ...
        try:
            logging.debug(
                "{:s}: Get output status of channel {:s}".format(
                    __name__, chr(64 + channel)
                )
            )
            return int(
                float(self._ask(f"smu{chr(96 + channel):s}.source.output"))
            )
        except Exception as e:
            logging.error(
                "{!s}: Cannot get output status of channel {!s}".format(
                    __name__, chr(64 + channel)
                )
            )
            raise type(e)(
                "{!s}: Cannot get output status of channel {!s}\n{!s}".format(
                    __name__, chr(64 + channel), e
                )
            )
    ...
    def set_bias_value(self, val, channel=1):
        ...
        try:
            logging.debug(
                "{:s}: Set bias value of channel {:s} to {:f}".format(
                    __name__, chr(64 + channel), val
                )
            )
            self._write(
                "smu{:s}.source.level{:s} = {:f}".format(
                    chr(96 + channel),
                    self._smu_command[self.get_bias_mode(channel=channel)],
                    val,
                )
            ...
            logging.error(
                "{!s}: Cannot set bias value of channel {!s} to {!s}".format(
                    __name__, chr(64 + channel), val
                )
            )
            raise type(e)(
                "{!s}: Cannot set bias value of channel {!s} to {!s}\n{!s}".format(
                    __name__, chr(64 + channel), val, e
                )
            )
        return
    ...
    def get_bias_value(self, channel=1):
        ...
        try:
            logging.debug(
                "{:s}: Get bias value of channel {:s}".format(
                    __name__, chr(64 + channel)
                )
            )
            return float(
                self._ask(
                    "smu{:s}.source.level{:s}".format(
                        chr(96 + channel),
                        self._smu_command[self.get_bias_mode(channel=channel)],
                    )
                )
            ...
            logging.error(
                "{!s}: Cannot get bias value of channel {!s}".format(
                    __name__, chr(64 + channel)
                )
            )
            raise type(e)(
                "{!s}: Cannot get bias value of channel {!s}\n{!s}".format(
                    __name__, chr(64 + channel), e
                )
            )
        return
    ...
    def get_sense_value(self, channel=1, readingbuffer=None):
        ...
        try:
            logging.debug(
                "{:s}: Get sense value of channel {:s}".format(
                    __name__, chr(64 + channel)
                )
            )
            return float(
                self._ask(
                    "smu{:s}.measure.{:s}({:s})".format(
                        chr(96 + channel),
                        self._smu_command[self.get_sense_mode(channel=channel)],
                        readingbuffer if readingbuffer is not None else "",
                    )
                )
            )
        except Exception as e:
            logging.error(
                "{!s}: Cannot get sense value of channel {!s}".format(
                    __name__, chr(64 + channel)
                )
            )
            raise type(e)(
                "{!s}: Cannot get sense value of channel {!s}\n{!s}".format(
                    __name__, chr(64 + channel), e
                )
            )
    ...
    def set_voltage(self, val, channel=1):
        ...
        try:
            logging.debug(
                "{:s}: Set voltage value of channel {:s} to {:f}".format(
                    __name__, chr(64 + channel), val
                )
            )
            self._write(f"smu{chr(96 + channel):s}.source.levelv = {val:f}")
        except Exception as e:
            logging.error(
                "{!s}: Cannot set voltage value of channel {!s} to {!s}".format(
                    __name__, chr(64 + channel), val
                )
            )
            raise type(e)(
                "{!s}: Cannot set voltage value of channel {!s} to {!s}\n{!s}".format(
                    __name__, chr(64 + channel), val, e
                )
            )
        return
    ...
    def get_voltage(self, channel=1, readingbuffer=None):
        ...
        try:
            logging.debug(
                "{:s}: Get voltage value of channel {:s}".format(
                    __name__, chr(64 + channel)
                )
            )
            return float(
                self._ask(
                    "smu{:s}.measure.v({:s})".format(
                        chr(96 + channel),
                        readingbuffer if readingbuffer is not None else "",
                    )
                )
            ...
            logging.error(
                "{!s}: Cannot get voltage value of channel {!s}".format(
                    __name__, chr(64 + channel)
                )
            )
            raise type(e)(
                "{!s}: Cannot get voltage value of channel {!s}\n{!s}".format(
                    __name__, chr(64 + channel), e
                )
            )
    ...
    def set_current(self, val, channel=1):
        ...
        try:
            logging.debug(
                "{:s}: Set current value of channel {:s} to {:f}".format(
                    __name__, chr(64 + channel), val
                )
            )
            self._write(f"smu{chr(96 + channel):s}.source.leveli = {val:f}")
        except Exception as e:
            logging.error(
                "{!s}: Cannot set current value of channel {!s} to {!s}".format(
                    __name__, chr(64 + channel), val
                )
            )
            raise type(e)(
                "{!s}: Cannot set current value of channel {!s} to {!s}\n{!s}".format(
                    __name__, chr(64 + channel), val, e
                )
            )
        return
    ...
    def get_current(self, channel=1, readingbuffer=None):
        ...
        try:
            logging.debug(
                "{:s}: Get current value of channel {:s}".format(
                    __name__, chr(64 + channel)
                )
            )
            return float(
                self._ask(
                    "smu{:s}.measure.i({:s})".format(
                        chr(96 + channel),
                        readingbuffer if readingbuffer is not None else "",
                    )
                )
            ...
            logging.error(
                "{!s}: Cannot get current value of channel {!s}".format(
                    __name__, chr(64 + channel)
                )
            )
            raise type(e)(
                "{!s}: Cannot get current value of channel {!s}\n{!s}".format(
                    __name__, chr(64 + channel), e
                )
            )
        return
    ...
    def get_IV(self, channel=1, *readingbuffer):
        ...
        try:
            logging.debug(
                "{:s}: Get current and voltage sense value of channel {:s}".format(
                    __name__, chr(64 + channel)
                )
            )
            return np.array(
                [
                    float(val)
                    for val in self._ask(
                        "smu{:s}.measure.iv{:s}".format(
                            chr(96 + channel), readingbuffer
                        ).replace(",)", ")")
                    ).split("\t")
                ]
            )
            ...
            logging.error(
                "{!s}: Cannot get current and voltage sense value of channel {!s}".format(
                    __name__, chr(64 + channel)
                )
            )
            raise type(e)(
                "{!s}: Cannot get current and voltage sense value of channel {!s}\n{!s}".format(
                    __name__, chr(64 + channel), e
                )
            )
    ...
    def ramp_voltage(self, stop, step, step_time=0.1, channel=1):
        ...
        if self.get_bias_mode(channel=channel):  # 1 (voltage bias)
            ...
            logging.error(
                __name__
                + ": Cannot set voltage value of channel {!s}: in the current bias".format(
                    channel
                )
            )
            return
    ...
    def ramp_current(self, stop, step, step_time=0.1, channel=1):
        ...
        if not self.get_bias_mode(channel=channel):  # 0 (current bias)
            ...
            logging.error(
                __name__
                + ": Cannot ramp current value of channel {!s}: in the voltage bias".format(
                    channel
                )
            )
            ...
            if len(channels) == int(not mode) + 1:
                ...
                logging.debug(
                    "{!s}: Set sweep channels to {!s}".format(
                        __name__, default_channels[mode]
                    )
                )
    ...
    def create_readingbuffer(self, name, size, timestamps=None, channel=1):
        ...
        try:
            logging.debug(
                "{:s}: create reading buffer {:s} for channel {:s} with size {:d}".format(
                    __name__, name, chr(64 + channel), size
                )
            )
            self._write(
                f"{name:s} = smu{chr(96 + channel):s}.makebuffer({size:d})"
            )
        except Exception as e:
            logging.error(
                "{:s}: Cannot create reading buffer {:s} for channel {:s} with size {:d}".format(
                    __name__, name, chr(64 + channel), size
                )
            )
            raise type(e)(
        ...
        if timestamps is not None:
            try:
                logging.debug(
                    "{:s}: Set timestamps of reading buffer {:s} to {!r}".format(
                        __name__, name, timestamps
                    )
                )
                self._write(f"{name:s}.collecttimestamps = {timestamps!d}")
            except Exception as e:
                logging.error(
                    "{:s}: Cannot set timestamps of reading buffer {:s} to {!r}".format(
                        __name__, name, timestamps
                    )
                )
                raise type(e)(
    ...
    def set_sweep_parameters(self, sweep):
        ...
        try:
            ...
            if not self._sweep_mode:  # 0 (VV-mode)
                ...
                readingbuffer_bias = (
                    f"smu{chr(96 + channel_bias):s}.nvbuffer1"
                    if self._readingbuffer_bias is None
                    else self._readingbuffer_bias
                )
                readingbuffer_sense = (
                    f"smu{chr(96 + channel_sense):s}.nvbuffer1"
                    if self._readingbuffer_sense is None
                    else self._readingbuffer_sense
                )
                self._step_signed = np.sign(self._stop - self._start) * np.abs(
                    self._step
                )
                self.set_bias_value(val=self._start, channel=channel_bias)
                cmd = f"{readingbuffer_bias:s}.clear()"
                cmd += f"{readingbuffer_bias:s}.appendmode = 1"
                cmd += f"{readingbuffer_sense:s}.clear()"
                cmd += f"{readingbuffer_sense:s}.appendmode = 1"
            elif self._sweep_mode in [1, 2]:  # 1 (IV-mode) | 2 (VI-mode)
                (channel,) = self._sweep_channels
                readingbuffer_bias = (
                    f"smu{chr(96 + channel):s}.nvbuffer1"
                    if self._readingbuffer_bias is None
                    else self._readingbuffer_bias
                )
                readingbuffer_sense = (
                    f"smu{chr(96 + channel):s}.nvbuffer2"
                    if self._readingbuffer_sense is None
                    else self._readingbuffer_sense
                )
                self._nop = int(np.abs((self._stop - self._start) / self._step) + 1)
                cmd = "smu{:s}.trigger.source.linear{:s}({:f}, {:f}, {:d})".format(
                    chr(96 + channel),
                    self._smu_command[self.get_bias_mode(channel=channel)],
                    self._start,
                    self._stop,
                ...
                cmd += f"smu{chr(96 + channel):s}.trigger.source.action = 1"
                cmd += f"smu{chr(96 + channel):s}.trigger.measure.action = 1"
                cmd += f"{readingbuffer_bias:s}.clear()"
                cmd += f"{readingbuffer_sense:s}.clear()"
                cmd += "smu{:s}.trigger.measure.iv({:s}, {:s})".format(
                    chr(96 + channel),
                    *[readingbuffer_bias, readingbuffer_sense][
                        :: int(2 * (1.5 - self._sweep_mode))
                    ]
                )
                cmd += "smu{:s}.trigger.count = {:d}".format(
                    chr(96 + channel), self._nop
                )
                cmd += f"smu{chr(96 + channel):s}.trigger.arm.count = 1"
            else:
                logging.error(
                    "{:s}: Cannot set sweep parameters in sweep mode {!s}".format(
                        __name__, self._sweep_mode
                    )
                )
            ...
            logging.error(
                "{!s}: Cannot set sweep parameters of channel {:s} to {!s}".format(
                    __name__, self._sweep_channels, sweep
                )
            )
            raise type(e)(
                "{!s}: Cannot set sweep parameters of channel {:s} to {!s}\n{!s}".format(
                    __name__, self._sweep_channels, sweep, e
                )
            )
        return
    ...
    def get_tracedata(self):
        ...
        try:
            if not self._sweep_mode:  # 0 (VV-mode)
                ...
                cmd = "for i = {:f}, {:f}, {:f} do".format(
                    self._start, self._stop + self._step_signed / 2.0, self._step_signed
                )
                cmd += f"\tsmu{chr(96 + channel_bias):s}.source.levelv = i"
                cmd += "\tsmu{:s}.measure.v({:s})".format(
                    chr(96 + channel_bias), readingbuffer_bias
                )
                cmd += "\tsmu{:s}.measure.v({:s})".format(
                    chr(96 + channel_sense), readingbuffer_sense
                )
                cmd += "end\n"
                self._visainstrument.write(cmd)
            elif self._sweep_mode in [1, 2]:  # 1 (IV-mode) | 2 (VI-mode)
                ...
                self._visainstrument.write(
                    f"smu{chr(96 + channel):s}.trigger.initiate()"
                )
            ...
            self._visainstrument.write(
                "printbuffer(1, {:s}.n, {:s}, {:s})".format(
                    readingbuffer_bias, readingbuffer_bias, readingbuffer_sense
                )
            )
            self._wait_for_stb()
            data = np.fromstring(
                string=self._visainstrument.read(), dtype=float, count=-1, sep=","
            )
            return data[0::2], data[1::2]
        except Exception as e:
            logging.error(
                "{!s}: Cannot take sweep data of channel {:s}".format(
                    __name__, self._sweep_channels
                )
            )
            raise type(e)(
                "{!s}: Cannot take sweep data of channel {:s}\n{!s}".format(
                    __name__, self._sweep_channels, e
                )
            )
    ...
    def _prepare_stb(self, stb):
        ...
        cmd = """status.reset()
                 ...
                 status.request_enable = {:s}""".format(
            stb
        )
    ...
    def get_all(self, channel=1):
        ...
        print(
            "measurement mode   = {:s}".format(
                self._measurement_modes[self.get_measurement_mode(channel=channel)]
            )
        )
        print(f"bias mode          = {self.get_bias_mode(channel=channel):d}")
        print(f"sense mode         = {self.get_sense_mode(channel=channel):d}")
        print(
            "bias range         = {:1.0e}{:s}".format(
                self.get_bias_range(channel=channel),
                self._IV_units[self.get_bias_mode(channel=channel)],
            )
        )
        print(
            "sense range        = {:1.0e}{:s}".format(
                self.get_sense_range(channel=channel),
                self._IV_units[self.get_sense_mode(channel=channel)],
            )
        )
        print(
        ...
        print(
            "sense delay        = {:1.3e}s".format(
                self.get_sense_delay(channel=channel)
            )
        )
        print(
            "sense average      = {:f}".format(
                self.get_sense_average(channel=channel)[1]
            )
        )
        print(
            "sense average type = {:s}".format(
                self._avg_types[self.get_sense_average(channel=channel)[2]]
            )
        )
        print(f"plc                = {self.get_plc():f}Hz")
        print(f"sense nplc         = {self.get_sense_nplc(channel=channel):f}")
        print(
            f"sense autozero     = {self.get_sense_autozero(channel=channel):f}"
        )
        print(f"status             = {self.get_status(channel=channel)!r}")
        print(
            "bias value         = {:f}{:s}".format(
                self.get_bias_value(channel=channel),
                self._IV_units[self.get_bias_mode(channel=channel)],
            )
        )
        print(
            "sense value        = {:f}{:s}".format(
                self.get_sense_value(channel=channel),
                self._IV_units[self.get_sense_mode(channel=channel)],
            )
        )
    ...
    def reset(self, channel=None):
        ...
        if channel is None:
            try:
                logging.debug(
                    f"{__name__!s}: Reset instrument to factory settings"
                )
                self._write("reset()")
            ...
            try:
                logging.debug(
                    "{!s}: Reset channel {:s} to factory settings".format(
                        __name__, chr(64 + channel)
                    )
                )
                self._write(f"smu{chr(96 + channel):s}.reset()")
            except Exception as e:
                logging.error(
                    "{!s}: Cannot reset channel {:s} to factory settings".format(
                        __name__, chr(64 + channel)
                    )
                )
                raise type(e)(
                    "{!s}: Cannot reset channel {:s} to factory settings\n{!s}".format(
                        __name__, chr(64 + channel), e
                    )
                )
        return
    ...
    def abort(self, channel=1):
        ...
        try:
            logging.debug(
                "{:s}: Abort running command of channel {:s}".format(
                    __name__, chr(64 + channel)
                )
            )
            return self._write(f"smu{chr(96 + channel):s}.abort()")
        except Exception as e:
            logging.error(
                "{!s}: Cannot abort running command of channel {:s}".format(
                    __name__, chr(64 + channel)
                )
            )
            raise type(e)(
                "{!s}: Cannot abort running command of channel {:s}\n{!s}".format(
                    __name__, chr(64 + channel), e
                )
            )
    ...
    def get_error(self):
        ...
        try:
            ...
            err = [
                self._visainstrument.query("print(errorqueue.next())").split("\t")
                for i in range(
                    int(float(self._visainstrument.query("print(errorqueue.count)")))
                )
            ]
    ...
    def clear_error(self):
        ...
        try:
            logging.debug(f"{__name__:s}: Clear error")
            self._write("errorqueue.clear()")
        except AttributeError as e:
            logging.error(f"{__name__!s}: Cannot clear errors")
            raise type(e)(f"{__name__!s}: Cannot clear errors\n{e!s}")
        return
