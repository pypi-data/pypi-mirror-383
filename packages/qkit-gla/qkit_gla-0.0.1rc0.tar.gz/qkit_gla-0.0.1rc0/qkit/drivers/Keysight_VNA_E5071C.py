...
class Keysight_VNA_E5071C(Instrument):
    ...
    def __init__(self, name, address, channel_index=1):
        ...
        self.add_parameter(
            "edel", type=float, minval=-10, maxval=10, units="s"
        )  # JB 2021
        ...
        self.add_parameter(
            "edel_status", type=bool
        )  # legacy name for parameter. This corresponds to the VNA's port extension values.
        self.add_parameter(
            "sweep_mode", type=str
        )  # JDB This parameter switches on/off hold. The hold function below does the same job, this is just for code compatibility to the agilent and anritsu drivers.
    ...
    def get_freqpoints(self, query=False):
        if query:
            ...
            self._freqpoints = self.ask_for_values(
                ":SENS%i:FREQ:DATA?" % (self._ci), format=visa.double
            )
        elif self.get_cw():
    ...
    def do_set_active_trace(self, trace):
        ...
        self.write(f"CALC{self._ci}:PAR{trace}:SEL")
    ...
    def do_get_meas_parameter(self):
        ...
        """
        return self.ask(f"CALC{self._ci}:PAR:DEF?")
    ...
    def do_set_meas_parameter(self, value):
        ...
        Output:
           None
        """
        self.write(f"CALC{self._ci}:PAR:DEF {value}")
        return
    ...
    def pre_measurement(self):
        ...
        self.write(
            ":TRIG:AVER ON"
        )  # Tell the instrument to do the specific number of averages on every trigger.
