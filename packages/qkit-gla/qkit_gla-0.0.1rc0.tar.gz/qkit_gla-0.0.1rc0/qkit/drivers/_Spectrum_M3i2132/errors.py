class errors:
    ...
    ERR_KERNELVERSION = (
        0x0006  # 6 The kernel version and the dll version are mismatching
    )
    ...
    ERR_BOARDINUSE = (
        0x000B  # 11 Card mit given name is already in use by another application
    )
    # ERR_EXPHW64BITADDR = 0x000C
    # ERR_FWVERSION = 0x000D
    ERR_LASTERR = 0x0010  # 16 Old Error waiting to be read
    # ERR_BOARDINUSE = 0x0011
    ...
    ERR_CLOCKNOTLOCKED = 0x010C  # 268 Synchronisation to external clock failed
    ...
    ERR_POSTEXCDSEGMENT = (
        0x0142  # 322 posttrigger exceeds segment size in multiple recording mode
    )
    ...
    ERR_ANDMASKEDGE = (
        0x0147  # 327 AND mask together with edge trigger mode is not allowed
    )
    ERR_ORMASKLEVEL = (
        0x0148  # 328 OR mask together with level trigger mode is not allowed
    )
    ...
    ERR_INTERNAL_ERROR = (
        0xFFFF  # 65535 Internal hardware error detected, please check for update
    )
