...
class Tunnel_DAC(Instrument):
    def __init__(self, name, serial=None, channel="A0", numdacs=3, delay=1e-3):
        ...
        if len(devs) == 0:
            ...
            logging.error(
                __name__ + ": more than one converter found and no serial number given."
            )
            logging.info(
                __name__
                + ": available devices are: %s." % str([dev[2] for dev in devs])
            )
        ...
        if (self._bit < 0) or (self._bit > 7):
            logging.error(
                __name__ + ": subchannel must be between 0 and 7, not %d." % self._bit
            )
    ...
    def _encode(self, data, channel=0, bits_per_item=16, big_endian=True):
        ...
        for item in data:
            for bit in (
                range(bits_per_item - 1, -1, -1)
                if big_endian
                else range(0, bits_per_item)
            ):
    ...
    def commit(self):
        ...
        voltages = [
            max(
                ...
                min(
                    2 ** (self._resolution - 1) - 1,
                    int(2 ** (self._resolution - 1) * x),
                ),
            )
            for x in voltages
        ]
    ...
    def set_voltages(self, valuedict):
        ...
        for channel, value in valuedict.iteritems():
            ...
            if (value < self._minval) or (value >= self._maxval):
                ...
                value = max(
                    self._minval, min(self._maxval, value)
                )  # does not handle maxval correctly
            self._voltages[channel - 1] = value
        self.commit()
