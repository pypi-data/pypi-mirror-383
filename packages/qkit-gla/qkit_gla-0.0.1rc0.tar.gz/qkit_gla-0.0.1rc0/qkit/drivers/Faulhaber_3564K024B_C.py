...
class Faulhaber_3564K024B_C(Instrument):
    ...
    def _write(self, cmd):
        ...
        """
        with self.lock:
            return self.ins.write(f"{cmd:s}\r".encode())
    ...
    def _query(self, cmd, size=100):
        ...
        """
        with self.lock:
            self.ins.write(f"{cmd:s}\r".encode())
    ...
    def set_status(self, status):
        ...
        try:
            if self.mode == "SOR0":
                self.set_speed(
                    speed={True: self.speed, False: 0}[status], direction=self.direction
                )
    ...
    def move_to(self, pos, t=1e-2):
        ...
        if self.mode == "SOR1":
            ...
            def _move_to():
                ...
                while {1: self.get_position() > pos, -1: self.get_position() < pos}[
                    rel
            ...
            raise NotImplementedError(
                """'move_to' not yet implemented for 'SOR0' mode."""
            )
    ...
    def set_bounds(self, bounds=(0, 1510000)):
        ...
        None
        """
        self._write(f"LL{bounds[0]:+d}")
        self._write(f"LL{bounds[1]:+d}")
        self._write("APL1")
    ...
    def run(self, bounds=None, speed=None, t=1e-2):
        ...
        if self.mode == "SOR1":
            ...
            def run_endless():
                while (
                    True
                ):  # ugly bugfix, but I don't know, how to solve the problem of a restart (MMW)
                    while self.running:
                        self.set_status(True)
