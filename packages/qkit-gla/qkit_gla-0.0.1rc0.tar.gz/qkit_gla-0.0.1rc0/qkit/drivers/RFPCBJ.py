...
class RFPCBJ(Instrument):
    ...
    def set_shiftreg_switch(self, boardnr, channel, boolean):
        self._rpc.writeval(
            "SHIFTREG" + str(boardnr), "switches", str(channel) + "," + str(boolean)
        )

    def set_shiftreg_attenuation(self, boardnr, channel, value):
        self._rpc.writeval(
            "SHIFTREG" + str(boardnr),
            "attenuation",
            str(channel) + "," + str(int(2 * value)),
        )

    def shiftreg_flush(self, boardnr, channel, boolean):
        self._rpc.writeval(
            "SHIFTREG" + str(boardnr), "read_flush", str(channel) + "," + str(boolean)
        )
