import numpy as np
from zhinst.toolkit import Alignment, SequenceType, TriggerMode
...
class ZHInst_Virtual_VNA(Instrument, AbstractVNA):
    ...
    def configure_frequency_range(
        self, lower_freq: float, upper_freq: float, steps: int
    ):
        assert (
            0 < lower_freq < upper_freq < MAXIMUM_FREQUENCY
        ), "Frequency bounds invalid! (Too large? Order?)"
    ...
    def start_measurement(self):
        ...
        uhfqa_frequencies = self.get_freqpoints() - (
            self.get_freqpoints()[0] - MINIMUM_UHFQA_FREQUENCY
        )
