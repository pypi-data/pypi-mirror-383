...
class Yokogawa_GS200(Instrument):
    ...
    def __init__(self, name, address, reset=False):
        ...
        self.add_parameter(
            "source_mode", flags=Instrument.FLAG_GETSET, type=str, units=""
        )

        self.add_parameter(
            "source_range", flags=Instrument.FLAG_GETSET, units="", type=str
        )

        self.add_parameter("output", flags=Instrument.FLAG_GETSET, units="", type=str)

        self.add_parameter("level", flags=Instrument.FLAG_GETSET, type=float, units="")
    ...
    def do_get_output(self):
        ...
        if int(ans):
            ...
            )  # warning if output is on! No fast current changes if coil is attached
            return "on"
        else:
            return "off"
    ...
    def ramp_current(self, target, step, wait=0.1, showvalue=False):
        ...
        if showvalue == True:
            print(f"{round(start * 1e3, 3):g}mA")
        ...
        for i in a:
            if showvalue == True:
                print(f"{round(i * 1e3, 3):g}mA", end=" ")
            self.set_level(i)
            time.sleep(wait)
    ...
    def _get_func_par(self, func, par):
        ...
        logging.debug(__name__ + f" : ask instrument for {string} (result {ans})")
        return ans.lower()
