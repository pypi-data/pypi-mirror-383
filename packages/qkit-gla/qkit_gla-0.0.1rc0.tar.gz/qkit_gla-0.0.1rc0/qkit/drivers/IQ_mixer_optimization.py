...
class IQ_mixer_optimization:
    ...
    def awg_run(self, off=False, set_par=True):
        ...
        if self.awg_channel == 1 or self.awg_channel == 2:
            ...
            raise ValueError(
                "Given awg channel is wrong! Please choose from {1,3} options."
            )
        ...
        for ll, phase in enumerate(rel_pha):
            if self.pulse_type == "rect":
                for ii in range(int(self.pulse_length * self.sample.clock)):
                    ...
                    waf_Q[ii] = np.cos(
                        2 * np.pi * self.IF * ii / self.sample.clock
                        + phase * np.pi / 180
                    )
                ...
                for ii in range(int(self.pulse_length * self.sample.clock)):
                    waf_I[ii] = np.exp(
                        -((ii / self.sample.clock - t0) ** 2) / 2 / sigma**2
                    ) * np.sin(2 * np.pi * self.IF * ii / self.sample.clock + 0)
                    waf_Q[ii] = np.exp(
                        -((ii / self.sample.clock - t0) ** 2) / 2 / sigma**2
                    ) * np.cos(
                        2 * np.pi * self.IF * ii / self.sample.clock
            ...
            self.awg.wfm_send2(
                waf_I, waf_Q, channel=self.awg_channel, seg=seg_off + ll + 1
            )
            time.sleep(0.1)
            print(
                "\rLoading the waveforms to AWG: "
                + str(ll + 1)
                + "/"
                + str(len(rel_pha)),
                end="",
    ...
    def opt_sideband(self, max_iter=10):
        ...
        while count <= max_iter:
            ...
            for ind, pha in enumerate(pha_array):
                self.awg.write(
                    f":INST{self.awg_channel};:TRAC:SEL{np.where(self.pha_array == pha)[0][0] + 1}"
                )
                self.spec.sweep()
                level_array_pha.append(np.max(self.spec.get_trace()))
                print(
                    f"\r{count + 1}: Optimizing relative phase parameter... "
                    + str(ind + 1)
                    + "/"
                    + str(len(pha_array)),
                    end="",
            ...
            if self.opt_rel_pha - round(pha_span / 4, 0) < 0:
                pha_array = np.concatenate(
                    (
                        np.arange(
                            0, self.opt_rel_pha + round(pha_span / 4, 0), pha_step
                        ),
                        np.arange(
                            360 + self.opt_rel_pha - round(pha_span / 4, 0),
                            360,
                            pha_step,
                        ),
                    )
                ...
                pha_array = np.concatenate(
                    (
                        np.arange(
                            self.opt_rel_pha - round(pha_span / 4, 0), 360, pha_step
                        ),
                        np.arange(
                            0, self.opt_rel_pha + round(pha_span / 4, 0) - 360, pha_step
                        ),
                    )
                )
            else:
                pha_array = np.arange(
                    self.opt_rel_pha - round(pha_span / 4, 0),
                    self.opt_rel_pha + round(pha_span / 4, 0),
                    pha_step,
                )
            ...
            if abs(level - noise_level) < 2 * noise_std:
                break

            # amplitude optimization iteration
            level_array_amp = []
            self.awg.write(
                f":INST{self.awg_channel};:TRAC:SEL{np.where(self.pha_array == self.opt_rel_pha)[0][0] + 1}"
            )
            for i, amp in enumerate(amp_array):
                ...
                print(
                    f"\r{count + 1}: Optimizing relative amplitude parameter... "
                    + str(i + 1)
                    + "/"
                    + str(len(amp_array)),
                    end="",
            ...
            logging.info(
                __name__
                + f": \nOptimal values after {count + 1:d} iteration:\nRelative phase: {self.opt_rel_pha:f}; Relative amplitude: {self.opt_rel_amp:f}; Sideband level: {level:f}."
            )
            ...
            if self.awg_channel == 1 or self.awg_channel == 2:
                self.awg.set_ch2_amplitude(opt_rel_amp)
            elif self.awg_channel == 3 or self.awg_channel == 4:
                self.awg.set_ch4_amplitude(opt_rel_amp)
            else:
                raise ValueError("Unknown channel number!")
    ...
    def opt_LO_leakage(self, max_iter=10):
        ...
        self.awg.write(f":INST{self.awg_channel};:TRAC:SEL{361 + 1}")
        ...
        while count <= max_iter:
            ...
            for i, I_off in enumerate(I_off_array):
                if self.awg_channel == 1 or self.awg_channel == 2:
                    self.awg.set_ch1_offset(I_off)
                elif self.awg_channel == 3 or self.awg_channel == 4:
                    self.awg.set_ch3_offset(I_off)
                else:
                    raise ValueError("Unknown channel number!")
                self.spec.sweep()
                level_array_I_off.append(np.max(self.spec.get_trace()))
                print(
                    "\rOptimizing I offset parameter... "
                    + str(i + 1)
                    + "/"
                    + str(len(I_off_array)),
                    end="",
            ...
            if (
                ...
                self.opt_I_offset + round((I_off_array[-1] - I_off_array[0]) / 4, 3) > 1
            ):
                I_off_array = np.arange(round(I_off_array[-1] / 2, 3), 1, off_step)
            else:
                I_off_array = np.arange(
                    self.opt_I_offset
                    - round((I_off_array[-1] - I_off_array[0]) / 4, 3),
                    self.opt_I_offset
                    + round((I_off_array[-1] - I_off_array[0]) / 4, 3),
                    off_step,
                )

            if abs(level - noise_level) < 3 * noise_std:
                break
            ...
            for i, Q_off in enumerate(Q_off_array):
                if self.awg_channel == 1 or self.awg_channel == 2:
                    self.awg.set_ch2_offset(Q_off)
                elif self.awg_channel == 3 or self.awg_channel == 4:
                    self.awg.set_ch4_offset(Q_off)
                else:
                    raise ValueError("Unknown channel number!")
                self.spec.sweep()
                level_array_Q_off.append(np.max(self.spec.get_trace()))
                print(
                    "\rOptimizing Q offset parameter... "
                    + str(i + 1)
                    + "/"
                    + str(len(Q_off_array)),
                    end="",
            ...
            if (
                ...
                self.opt_Q_offset + round((Q_off_array[-1] - Q_off_array[0]) / 4, 3) > 1
            ):
                Q_off_array = np.arange(round(Q_off_array[-1] / 2, 3), 1, off_step)
            else:
                Q_off_array = np.arange(
                    self.opt_Q_offset
                    - round((Q_off_array[-1] - Q_off_array[0]) / 4, 3),
                    self.opt_Q_offset
                    + round((Q_off_array[-1] - Q_off_array[0]) / 4, 3),
                    off_step,
                )
            ...
            logging.info(
                __name__
                + f": \nOptimal values after {count + 1:d} iteration:\nI offset: {self.opt_I_offset:f}; Q offset: {self.opt_Q_offset:f}; LO level: {level:f}."
            )
    ...
    def set_opt_parameters(self, reset=False):
        ...
        self.awg.write(f":INST{self.awg_channel};:TRAC:SEL{segment}")

        logging.info(
            __name__
            + f": Parameters defined for pulse @segment{segment}. Optimal values? {not reset}"
        )
        return True
