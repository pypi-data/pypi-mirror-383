...
class Keysight_VNA_P9375A(Instrument):
    ...
    def __init__(self, name, address, channel_index=1):
        ...
        self.add_parameter(
            "edel",
            type=float,  # legacy name for parameter. This corresponds to the VNA's port extension values.
            flags=Instrument.FLAG_GETSET,
            minval=-10,
            maxval=10,
            units="s",
        ...
        )  # the channel option for qtlab's Instument class is used here to easily address the two VNA ports

        self.add_parameter(
            "edel_status",
            type=bool,  # legacy name for parameter. This corresponds to the VNA's port extension values.
            flags=Instrument.FLAG_GETSET,
        )

        self.add_parameter(
            "sweep_mode",
            type=str,  # JDB This parameter switches on/off hold. The hold function below does the same job, this is just for code compatibility to the agilent and anritsu drivers.
            flags=Instrument.FLAG_GETSET,
            tags=["sweep"],
        )

        self.add_parameter(
            "sweep_type", type=str, flags=Instrument.FLAG_GETSET, tags=["sweep"]
        )

        self.add_parameter("active_trace", type=int, flags=Instrument.FLAG_GETSET)
    ...
    def get_tracedata(self, format="AmpPha", single=False, averages=1.0):
        """
        ...
        if single == True:  # added MW July.
            ...
            self._visainstrument.write(
                "TRIG:SOUR INT"
            )  # added MW July 2013. start single sweep.
            self._visainstrument.write(
                "INIT%i:CONT ON" % (self._ci)
            )  # added MW July 2013. start single sweep.
            self.hold(True)
        ...
        data = self._visainstrument.ask_for_values(
            "CALC%i:MEAS:DATA:SDAT?" % (self._ci), fmt=1
        )
        ...
        if format == "RealImag":
            ...
            if self._zerospan:
                ...
                if self.get_cw():
                    ...
                    dataamp, datapha = numpy.array([numpy.mean(dataamp)]), numpy.array(
                        [numpy.mean(datapha)]
                    )
                return dataamp, datapha
        else:
    ...
    def get_freqpoints(self, query=False):
        if query == True:
            self._freqpoints = self._visainstrument.ask_for_values(
                "FORM:DATA REAL; FORM:BORD SWAPPED; :SENS%i:FREQ:DATA?" % (self._ci),
                format=visa.double,
            )
    ...
    def do_set_power(self, pow):
        ...
        if self.get_cw():
            ...
            self._visainstrument.write(
                "SOUR%i:POW1:LEV:IMM:AMPL %.1f" % (self._ci, pow)
            )
    ...
    def do_get_power(self):
        ...
        if self.get_cw():
            ...
            return float(
                self._visainstrument.ask("SOUR%i:POW1:LEV:IMM:AMPL?" % (self._ci))
            )
    ...
    def do_get_span(self):
        ...
        """
        # logging.debug(__name__ + ' : getting center frequency')
        span = self._visainstrument.ask(
            "SENS%i:FREQ:SPAN?" % (self._ci)
        )  # float( self.ask('SENS1:FREQ:SPAN?'))
        return span
    ...
    def do_set_edel(self, val, channel):  # MP 04/2017
        ...
        """
        logging.debug(
            __name__ + f" : setting port {channel} extension to {val} sec"
        )
    ...
    def do_get_edel(self, channel):  # MP 04/2017
        ...
        self._edel = float(
            self._visainstrument.ask("SENS1:CORR:EXT:PORT%i:TIME?" % channel)
        )
        return self._edel
    ...
    def do_get_cw(self):
        ...
        """
        sweep_type = str(
            self._visainstrument.ask(":SENS%i:SWE:TYPE?" % (self._ci))
        ).rstrip()
        if sweep_type == "POW":
            ret = True
        else:
    ...
    def do_set_trigger_source(self, source):
        ...
        if source.upper() in ["INT", "MAN", "EXT", "BUS"]:
            ...
            raise ValueError(
                "set_trigger_source(): must be INTernal | MANual | EXTernal | REMote"
            )
    ...
    def do_set_active_trace(self, trace):
        ...
        self._visainstrument.write(f"CALC{self._ci}:PAR{trace}:SEL")
