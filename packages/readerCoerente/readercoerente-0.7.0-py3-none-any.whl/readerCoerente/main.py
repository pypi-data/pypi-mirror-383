...
def get_subset_details(
    files, first_time: np.datetime64 = None, last_time: np.datetime64 = None
    ...
    for file in files:
        ...
        freq_sampling = freq_sampling = round(
            1 / tdms_file.groups()[0].channels()[0].properties["wf_increment"], 5
        )  # round(tdms_file.properties["SampleFrequency"], 5)
    ...
    if first_time:
        ...
        first_samples_to_keep = int(
            (first_time - start_times[first_file_to_load])
            / np.timedelta64(1, "s")
            * freq_sampling
        )
    else:
    ...
    samples_to_keep = int(
        (last_time - first_time) / np.timedelta64(1, "s") * freq_sampling
    )
...
class multipleFiles(File):
    def __init__(
        ...
        if start_time_utc or stop_time_utc:
            (files, first_samples_to_keep, samples_to_keep) = get_subset_details(
                files, np.datetime64(start_time_utc), np.datetime64(stop_time_utc)
            )
        ...
        if start_time_utc or stop_time_utc:
            for ch in self.channels_name + self.phasename:
                data = getattr(self, ch.lower())
                setattr(
                    self,
                    ch.lower(),
                    data[
                        first_samples_to_keep : first_samples_to_keep + samples_to_keep
                    ],
                )
