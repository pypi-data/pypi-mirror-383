...
from nemo_library_etl.adapter.migman.config_models import (
    DuplicatesInforCOM,
    PipelineMigMan,
)
...
class MigManTransform:
    ...
    def transform(self) -> None:
        ...
        with ETLDuckDBHandler(
            ...
            cfg=self.cfg,
            logger=self.logger,
            database=self.cfg.extract.source.duckdb_path,
        ) as self.db:
    ...
    def duplicates(self) -> None:
        ...
        self.db.con.create_function("normalize_text", normalize_text)
        self.db.con.create_function("text_similarity", text_similarity)

        for duplicate_name, model in self.cfg.transform.duplicates.items():
    ...
    def _perform_duplicate_check(
        self, duplicate_name: str, model: DuplicatesInforCOM
    ) -> None:
        """
        ...
        table = MigManTransformStep.JOINS.value + "_" + duplicate_name
