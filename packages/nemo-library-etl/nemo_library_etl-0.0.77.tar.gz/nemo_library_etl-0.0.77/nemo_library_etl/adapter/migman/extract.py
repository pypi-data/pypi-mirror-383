    ...
    def extract(self) -> None:
        ...
        if self.cfg.extract.inforcom and self.cfg.extract.inforcom.active:
            ...
            if not self.cfg.extract.inforcom.tables:
                self.logger.warning(
                    "No tables configured for extraction in MigMan INFORCOM section"
                )
            else:
                for table_name, table_cfg in self.cfg.extract.inforcom.tables.items():
                    if not table_cfg.active:
                        ...
                        continue
                    self.logger.info(
                        f"Extracting table {table_name} from MigMan Infor.COM"
                    )
                    odbc.generic_odbc_extract(
                        ...
                        entity=table_name,
                        chunksize=(
                            self.cfg.extract.inforcom.chunk_size
                            if table_cfg.big_data
                            else None
                        ),
                        gzip_enabled=table_cfg.big_data,
                    )
                    ...
                    if self.cfg.extract.load_to_nemo:
                        ...
                        self.nl.ReUploadDataFrame(
                            df=df,
                            projectname=f"{self.cfg.extract.nemo_project_prefix}{table_name}",
