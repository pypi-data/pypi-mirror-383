...
class Subscription(BaseModel):
    ...
    filter: StrictStr = Field(description="The filter to apply to the subscription. The message has attributes due to the target of the subscription as follows. import   - datasetId, datasetName, importId dbt      - dbtProjectId, dbtProjectName, dbtRunId pipeline - pipelineId, pipelineName, pipelineRunId See https://cloud.google.com/pubsub/docs/subscription-message-filter for the filter syntax.")
    token_audience: StrictStr = Field(description="Audience used in the generated Open ID Connect token for authenticated push. If not specified, it will be set to the push-endpoint. See https://cloud.google.com/sdk/gcloud/reference/pubsub/subscriptions/create#--push-auth-token-audience", alias="tokenAudience")
    ack_deadline: Annotated[int, Field(le=600, strict=True, ge=10)] = Field(description="The acknowledge deadline of the subscription in seconds.", alias="ackDeadline")
