...
def make(model_src, documentation_src, formatters: list[Formatter], cache_dir) -> None:
    ...
    if False:
        # TODO: remove
        from dexpi.specificator.utils.uml_diagrams import make_diagram
       # from specificator.uml.uml_diagrams import make_uml_diagram as make_diagram
        texes = [r'\documentclass{article}',
                 r'\usepackage{tikz}\usetikzlibrary{arrows,arrows.meta}',
                 r'\begin{document}']

        for element in [
              #  model_by_name['Core'].Note,
              #  model_by_name['Core'].QualifiedValue,
                model_set.search_unique('ProcessStep'),
                model_set.search_unique('Distilling'),
                model_set.search_unique('ConceptualModel', type=metamodel.Class),
              #  model_by_name['Core'].PhysicalQuantities.LengthUnit
           ]:
            
            
            out = out_dir / 'latex' / (element.name + '.tex')
            tex = make_diagram([element], 'tex', out=out, usages_by_class=None)
            texes.append(tex)
            
            
            
        texes.append(r'\end{document}')
        
        
        
        
        pathlib.Path(r'E:\s\test.tex').write_text('\n\n'.join(texes), encoding='utf-8')
