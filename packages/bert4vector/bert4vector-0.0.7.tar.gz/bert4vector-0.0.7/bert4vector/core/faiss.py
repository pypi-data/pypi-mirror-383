...
class FaissSimilarity(BertSimilarity):
    ...
    >>> model = FaissSimilarity('E:/pretrain_ckpt/simbert/sushen@simbert-chinese-tiny')
    ...
    def _add_embedding(self, corpus:List[str], name:str='default', 
        ...
        '''
        super()._add_embedding(corpus, name=name, **kwargs)
    ...
    def search(self, queries: Union[str, List[str]], topk:int=10, name:str='default', return_dict:bool=True, **kwargs) -> dict:
        ...
        ```python
        >>> from bert4vector import FaissSimilarity
        >>> model = FaissSimilarity('/data/pretrain_ckpt/simbert/sushen@simbert-chinese-tiny')
        >>> model.add_corpus(['你好', '我选你'], gpu_index=True)
        >>> model.add_corpus(['天气不错', '人很好看'], gpu_index=True)
