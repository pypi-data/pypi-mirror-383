...
CONFIG = get_config()
LLM = ChatGoogleGenerativeAI(model=CONFIG.genai_model, google_api_key=CONFIG.google_api_key).bind_tools(TOOLS)
...
def has_tools(state: AgentState):
    ...
    return "True" if hasattr(result, 'tool_calls') and len(result.tool_calls) == 1 and len(result.tool_calls[0]["args"]["options"]) > 0 else "False"
...
def create_and_run_agent(input: str):
    ...
    graph.add_edge("invalid_call", "cache")
    graph.add_edge("tool_node", "cache")
    graph.add_edge("call_node", "cache")
    graph.add_edge("cache", END)
