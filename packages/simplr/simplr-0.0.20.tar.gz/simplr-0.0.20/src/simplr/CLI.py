...
class CLI:
    ...
    def create_config():
        ...
        max_search_depth = CLI.ask("Search depth for file structure? (1-2 Recommended for low token limits)", style=CLI.ASK_DEFAULT_STYLE, prefixed=False, validate=lambda text: text.isdigit())
        genai_model = CLI.select("Genai model to use?", ["gemini-2.5.flash", "gemini-2.5-pro"], style=CLI.SELECT_DEFAULT_STYLE, prefixed=False)
