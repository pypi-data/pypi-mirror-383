    ...
    def process_sheet(self, sheet, selection_rule_1, selection_rule_2, comparison_rule):
        ...
        for idx, row in enumerate(all_values[1:], start=2):
            ...
            if self.output_batch_size > 0 and len(result1_list) >= self.output_batch_size:
                self.batch_update(sheet, col_index, result1_list, result2_list, status_list, idx - len(result1_list) + 1)
                result1_list, result2_list, status_list = [], [], []  # Reset after batch update

        # If there are remaining results to be updated
        if result1_list:
            self.batch_update(sheet, col_index, result1_list, result2_list, status_list, num_rows - len(result1_list) + 1)
