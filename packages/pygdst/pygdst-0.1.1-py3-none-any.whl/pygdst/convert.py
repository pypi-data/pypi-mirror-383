    ...
    for i, sn_i in enumerate(sns):
        ...
        for j, hour in enumerate(hours):
            ...
            if not os.path.exists(file_ij):
                print(f'{file_ij} not exists, filled with zeros')
                health_i[j*60:(j+1)*60]=0
                continue
            
            H, data_ij, hs = read_bin(file_ij, dt=dt*1000,IS_Z_CHN=False, DB=DB)
            # print(file_ij,data_ij.shape,N_CHN)

            f_inf_dict = fheader_decode(H)
            if 1/f_inf_dict['sampling_rate']!=dt and data_ij.shape[0]!=N_CHN:
                print(f'dt {1/f_inf_dict['sampling_rate']} != targeted dt={dt} &{data_ij.shape[0]=}!={N_CHN=}')
                print(f'{file_ij=} is skipped')
                inf_list = bheader2list(hs[0,:,:], start=0, interval=int(60/dt/DATA_SIZE))
                header_list+=inf_list
                continue
