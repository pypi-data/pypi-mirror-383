    ...
    for env in env_config["environments"]:
        ...
        for robot_entry in robots:
            for robot_name, robot_config in robot_entry.items():
                ...
                assert (max_episode_steps is not None), f"max_episode_steps must be specified in the environment config for {env_name} "
...
def register_envs_v1(registry_name: str):
    ...
    for env in env_config["environments"]:
        ...
        for robot_entry in env["robot_configs"]:
            ...
            for robot_config_name, robot_config in robot_entry["robots"].items():
                assert "control_freq" in robot_config, f"control_freq must be specified for robot {robot_config_name} in environment {env_name}"
