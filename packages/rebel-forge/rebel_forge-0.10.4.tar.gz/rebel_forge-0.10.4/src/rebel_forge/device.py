...
def _bootstrap_server_credentials(
    base: str, token: str, handshake: Mapping[str, Any], project_id: str
) -> bool:
...
def _server_provision(
    *,
    ...
    if not token or not base or not user_id:
        return None
    project_id = (
        _clean(os.environ.get("project_id")) or _clean(os.environ.get("NEBIUS_PROJECT_ID"))
    )
    if not project_id:
        handshake = _fetch_handshake_bundle(base, token, user_id)
        if not handshake:
            return None
        project_id = _clean(handshake.get("projectId"))
        if project_id:
    ...
    try:
        ...
        if error.code == 404:
            handshake = _fetch_handshake_bundle(base, token, user_id)
            if handshake and _bootstrap_server_credentials(base, token, handshake, project_id):
