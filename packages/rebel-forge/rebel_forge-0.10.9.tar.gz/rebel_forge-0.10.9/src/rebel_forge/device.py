...
def _fetch_default_project_metadata(
    ...
    url = f"{base.rstrip('/')}/api/nebius/project"
    headers = {
        "Accept": "application/json",
    }
    if token:
    ...
    try:
        ...
        raise RemoteError(
            f"Failed to resolve default Nebius project ({error.code}): {detail or error.reason}"
        ) from error
...
def _resolve_project_id(session_payload: dict[str, Any]) -> tuple[str | None, dict[str, Any]]:
    ...
    base = _clean(session_payload.get("login_url")) or os.environ.get("REBEL_FORGE_PORTAL_URL")
    token = _clean(session_payload.get("token"))
    user_id = _clean(session_payload.get("userId") or session_payload.get("user_id"))
    if not base or not (token or user_id):
...
def _server_provision(
    *,
    ...
    try:
        ...
        if error.code == 404 and allow_legacy:
        ...
        if error.code == 404:
            raise RemoteError(
                "Nebius credentials are not configured on the Rebel backend for this project."
            ) from error
        ...
        raise RemoteError(
            "Unable to reach the Rebel provisioning service. Check your network connection and portal availability."
        ) from error
