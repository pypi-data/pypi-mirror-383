...
from .device import prime_provisioning_token
...
def _handle_logout(argv: list[str]) -> None:
    parser = argparse.ArgumentParser(
        prog="rebel-forge logout",
        description="Clear the rebel-forge CLI session and cached Nebius credentials.",
    )
    parser.add_argument(
        "--quiet",
        action="store_true",
    ...
    args = parser.parse_args(argv)
    session_payload = get_cached_session()
    remote_cleared = False
    if session_payload:
        remote_cleared = _perform_remote_logout(session_payload, quiet=args.quiet)

    removed = clear_cli_session()
    if args.quiet:
        return
    if remote_cleared and removed:
        print("Rebel Forge account signed out and local session cleared.")
    elif remote_cleared:
        print("Rebel Forge account signed out.")
    elif removed:
        print("Rebel Forge CLI session cleared. Re-run `rebel-forge` to sign in again.")
    else:
        print("No Rebel Forge session artifacts were found.")


def _perform_remote_logout(session_payload: Mapping[str, Any], *, quiet: bool) -> bool:
    ...
    login_url = _read("login_url", "loginUrl") or resolve_login_url()
    device_id = _read("device_id", "deviceId")
    user_id = _read("userId", "user_id")
    token = _read("token")

    if not (device_id and user_id and token):
        return False

    endpoint = f"{login_url.rstrip('/')}/api/cli-auth/logout"
    payload = json.dumps(
        {
            "userId": user_id,
            "deviceId": device_id,
    ...
    try:
        with urlrequest.urlopen(request_obj, timeout=6) as response:
            return 200 <= response.status < 300
    except urlerror.URLError as error:
        if not quiet:
            print(f"[rebel-forge] Warning: Unable to reach logout endpoint ({error}).")
    except Exception as error:  # pragma: no cover - defensive
        if not quiet:
            print(f"[rebel-forge] Warning: Logout request failed ({error}).")
    return False
...
def _wait_for_interactive_handshake(session: HandshakeSession) -> None:
    ...
    if record:
        ...
        prime_provisioning_token(force_refresh=True)
