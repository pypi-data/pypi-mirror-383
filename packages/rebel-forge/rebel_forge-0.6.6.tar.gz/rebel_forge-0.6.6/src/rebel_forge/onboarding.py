...
def ensure_onboarding(*, workspace: Path) -> None:
    """Show the welcome banner and guide the user through Rebel account login."""
...
def _load_handshake_payload(handshake_path: Path, token: str) -> Optional[HandshakeRecord]:
    ...
    return HandshakeRecord(token=token, user_id=user_raw.strip(), unlocked_at=unlocked_at)
