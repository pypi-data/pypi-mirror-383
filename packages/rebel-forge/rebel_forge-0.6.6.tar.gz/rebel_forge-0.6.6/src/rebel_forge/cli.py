...
def _load_cli_environment() -> None:
    ...
    for candidate in candidates:
        ...
        try:
            ...
            print(f"Warning: unable to load environment values from {env_file}: {exc}")
        finally:
            return
...
def _apply_env_file(env_file: Path) -> None:
    ...
    for line in content:
        ...
        if any(key.startswith(prefix) for prefix in _ENV_KEY_PREFIXES) or key in _ENV_KEY_EXACT:
