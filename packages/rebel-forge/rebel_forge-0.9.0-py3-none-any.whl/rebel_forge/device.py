    ...
    *,
    ...
    for key in ("session_id", "sessionId", "sessionId".lower()):
        ...
        if isinstance(value, str) and value.strip():
            ...
            break
    if isinstance(session.get("device_id"), str) and session["device_id"].strip():
    ...
    try:
        ...
        if error.code in {401, 403}:
            raise RemoteError(
                "Rebel Forge portal rejected the provisioning request. Re-run `rebel-forge` to refresh your session."
            ) from error
        if error.code == 404:
            return None
        if error.code >= 500:
    ...
    if "error" in data:
        ...
        if error_code == "token_mismatch":
            raise RemoteError("Provisioning rejected because the CLI session token does not match the signed-in account.")
        if error_code in {"token_required", "user_required"}:
            raise RemoteError("Provisioning rejected due to missing authentication headers.")
        return None
...
def device(
    ...
    *,
    ...
    The helper first attempts to delegate provisioning to the Rebel Forge service
    (``/api/forge/device``) so provider credentials remain on the server. When the
    service is unavailable it falls back to provisioning locally using the Nebius
    CLI credentials resolved from the portal bundle or ``NEBIUS_*`` environment
    variables. An optional ``cluster`` hint is forwarded to the service for audit
    logging and subnet resolution.
