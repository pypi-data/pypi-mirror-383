...
class Page(
    ...
    def as_page(self, ar, display_mode="detail", hlevel=1, home=None):
        ...
        if display_mode in ("detail", "story"):
            ...
            title = format_html("<p class=\"display-4\">{}</p>", self.title)
        else:
...
def make_demo_pages(pages_desc, root_ref, group=None):
    ...
    if dd.plugins.publisher.with_trees:
        ...
        tree = dict(publisher_tree=get_or_create(Tree, ref=root_ref, group=group))
    else:
        tree = dict()
        ...
        with translation.override(lng.django_code):
            ...
            def make_pages(pages, parent=None, root_ref=None):
                for page in pages:
                    ...
                    kwargs = dict(title=title, language=lng.django_code, **tree)
                    if body is None:
