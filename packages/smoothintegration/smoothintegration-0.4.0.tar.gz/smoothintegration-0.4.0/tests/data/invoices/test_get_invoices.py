...
class TestInvoicesGet:
    def test_can_retrieve_invoices(self, mocked_responses, test_client):
        mocked_responses.add(
            responses.GET,
            "https://api.smooth-integration.com/v1/data/invoices",
            json=SUCCESS_RESPONSE_BODY,
            status=200,
        )

        response = test_client.data.invoices.get()
    ...
    def test_can_use_all_parameters(self, mocked_responses, test_client):
        mocked_responses.add(
            responses.GET,
            url="https://api.smooth-integration.com/v1/data/invoices"
            + "?limit=678"
            + "&include_raw=true"
            + "&where="
            "external_id%3D5678"
