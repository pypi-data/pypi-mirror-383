...
class TestBalanceSheetGet:
    def test_can_retrieve_balance_sheet(self, mocked_responses, test_client):
        mocked_responses.add(
            responses.GET,
            "https://api.smooth-integration.com/v1/data/balance_sheet",
            json=SUCCESS_RESPONSE_BODY,
            status=200,
        )

        response = test_client.data.balance_sheet.get()
    ...
    def test_can_use_all_parameters(self, mocked_responses, test_client):
        mocked_responses.add(
            responses.GET,
            url="https://api.smooth-integration.com/v1/data/balance_sheet"
            + "?limit=678"
            + "&include_raw=true"
            + "&where="
