...
class BenchmarkRunner:
    """Run benchmarks using Python API (apple to apple comparison)."""
    ...
    def __init__(self, label="aiofiles", use_aiofilesx=False):
        ...
        # Import the right package through Python API
        if use_aiofilesx:
            # Test if aiofiles-x is installed
            try:
                import aiofiles

                # Check if it's our C++ version by looking for the module
                try:
            ...
            # Make sure we're using the pure Python aiofiles
            try:
                import aiofiles
    ...
    async def bench_sequential_reads(self, file_sizes, iterations=10):
        """Benchmark sequential reads with different file sizes."""
        print(f"\n{'=' * 60}")
        ...
        for size_name, size_bytes in file_sizes.items():
            ...
            try:
                for i in range(iterations):
                    ...
                    # Use Python API for both implementations
                    async with self.aiofiles.open(temp_file, "r") as f:
                        await f.read()  # We just need to read, don't store
    ...
    async def bench_sequential_writes(self, file_sizes, iterations=10):
        """Benchmark sequential writes with different file sizes."""
        print(f"\n{'=' * 60}")
        ...
        for num_files in num_files_list:
            ...
            for iteration in range(iterations):
                ...
                try:
                    ...
                    # Concurrent reads using Python API
                    start = time.perf_counter()
...
async def main():
    ...
    # Detect which implementation is currently installed
    try:
        import _aiofiles_core  # noqa: F401
