...
# Import aiofiles - will use aiofiles-x if installed
try:
    import aiofiles
except ImportError:
...
async def test_flush():
    ...
    try:
        async with aiofiles.open(temp_file, mode="w") as f:
            ...
            await f.flush()
            # If flush fails, this would raise an exception
...
async def test_context_manager_closes_file():
    ...
    try:
        ...
        # File should be closed here - we can verify by opening again
        async with aiofiles.open(temp_file, mode="r") as f:
            content = await f.read()
...
async def test_concurrent_operations():
    ...
    try:
        ...
        # Read from all files concurrently
