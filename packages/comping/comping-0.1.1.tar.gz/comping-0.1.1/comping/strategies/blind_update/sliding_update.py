"""
Sliding Update Strategy - Uses only recent data for retraining
"""
import numpy as np
import pandas as pd
...
from ...utils.adapter import SLIDING_ADAPTER


class SlidingUpdate:
    """
    Sliding update strategy that uses only recent data for retraining.
    
    This strategy:
    1. Predicts in batches for efficiency
    2. Detects drift using time/index-based triggers
    3. Retrains using only recent window_size samples
    
    Examples:
        >>> from comping.strategies.blind_update import SlidingUpdate
        >>> from sklearn.linear_model import LogisticRegression
        >>> 
        >>> strategy = SlidingUpdate(
    ...
    def __init__(
        ...
        """
        Initialize sliding update strategy.
        
        Args:
            model: Sklearn-compatible model
            period: Retraining period (for index_type='int')
            window_size: Number of recent samples to use for training
        ...
        self.adapter = SLIDING_ADAPTER(window_size)
    ...
    def fit_predict(
        ...
        """
        Fit model and predict with sliding window retraining.
        ...
        if self.verbose:
            print("="*80)
            print("Sliding Update Strategy")
            print(f"Period: {self.period}, Window size: {self.window_size}, Batch size: {self.batch_size}")
            print("="*80)
        ...
        # Initialize detector and adapter
        detector = BLIND_DETECTOR(self.index_type, self.day_of_month, self.period)
        adapter = SLIDING_ADAPTER(self.window_size)
        
        # Initial training
        scaler, X_tr_scaled = scale_fit(self.scaler, X_tr)
        ...
        while current_idx < len(X):
            ...
            X_batch_scaled = scale_transform(scaler, X_batch)
            ...
            # Detect drift
            drift_result = detector.detect(indices=indices, batch_size=self.batch_size)
            drift_flag = drift_result['drift_flag']
            
            # Get sliding window period
            retrain_period = adapter.adapt(indices=indices)['retrain_period']
            
            # Retrain if drift detected
            if drift_flag:
                drift_indices.append(batch_end_idx)
                
                # Sliding: use only recent window_size samples
                from_idx, to_idx = retrain_period
                tr_start_idx = from_idx
                tr_end_idx = to_idx
                
                X_train = X.iloc[tr_start_idx:tr_end_idx+1, :]
                y_train = y[tr_start_idx:tr_end_idx+1]
                
                scaler, X_train_scaled = scale_fit(scaler, X_train)
                self.model.fit(X_train_scaled, y_train)
                
                if self.verbose:
                    print(f"\nIteration {iteration}: Drift detected at {batch_end_idx}")
                    print(f"  Retrained on [{tr_start_idx}:{tr_end_idx}] (window size: {self.window_size})")
    ...
    def __repr__(self):
        return (f"SlidingUpdate(period={self.period}, "
