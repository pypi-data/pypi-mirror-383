...
class ResearchAgent:
    ...
    async def research(
        ...
        # Stage 3: Web Research (if enabled)
    ...
    async def _stage1_mcp_introspection(
        ...
        try:
            ...
            raise
        # Note: Connection cleanup is handled at the end of the full research phase
    ...
    async def _stage2_user_resources(
        ...
        for url in user_resources.documentation_urls:
            try:
                ...
                # Only log as debug, not warning - this is expected for invalid URLs
    ...
    async def _stage3_web_research(
        ...
        """Stage 3: Autonomous web research using Claude"""
        ...
        self.logger.info("🌐 Using Claude's real-time web search (Brave Search API)...")

        # Use Claude to research and synthesize findings
        web_findings = await self._claude_web_research(query, server_name, context)
        context.web_findings = web_findings

        self.logger.info(
            f"✅ Web research complete: {len(web_findings.sources_found)} sources analyzed"
        )

        # Log detailed findings
        if web_findings.key_insights:
            self.logger.info(f"\n💡 Key Insights ({len(web_findings.key_insights)}):")
            for i, insight in enumerate(web_findings.key_insights, 1):
                self.logger.info(f"   {i}. {insight}")

        if web_findings.usage_patterns:
            self.logger.info(
                f"\n📋 Usage Patterns ({len(web_findings.usage_patterns)}):"
            )
            for i, pattern in enumerate(web_findings.usage_patterns, 1):
                self.logger.info(f"   {i}. {pattern}")

        if web_findings.best_practices:
            self.logger.info(
                f"\n✨ Best Practices ({len(web_findings.best_practices)}):"
            )
            for i, practice in enumerate(web_findings.best_practices, 1):
                self.logger.info(f"   {i}. {practice}")

        if web_findings.limitations:
            self.logger.info(
                f"\n⚠️  Known Limitations ({len(web_findings.limitations)}):"
            )
            for i, limitation in enumerate(web_findings.limitations, 1):
                self.logger.info(f"   {i}. {limitation}")

        if web_findings.sources_found:
            self.logger.info("\n📚 Sources Analyzed:")
            for source in web_findings.sources_found:
    ...
    async def _fetch_url_content(self, url: str) -> str:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            # Truncate if too long to avoid excessive API costs
...
Return only valid JSON, no other text."""
        ...
        try:
            ...
            if not response or not response.content:
                # No content in response - fall back immediately
            ...
            try:
                ...
                # Merge citation sources with result sources (deduplicate)
                all_sources = list(
                ...
                # Still return sources from citations even if JSON parsing fails
            ...
            # Log as debug, not warning - web search failures are expected
            self.logger.debug(f"Web search failed: {e}")
            # Silently fall back to knowledge-based approach
            return await self._knowledge_based_fallback(server_name, context)
...
Return only valid JSON, no other text."""
    ...
    async def cleanup(self):
        """Clean up resources"""
        try:
            # Cleanup MCP connections using the correct method
            await self.mcp_client.disconnect_all()
        except Exception as e:
            # Silently ignore cleanup errors - connection is already closed
