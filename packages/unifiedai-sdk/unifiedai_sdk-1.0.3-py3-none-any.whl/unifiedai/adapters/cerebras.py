...
class CerebrasAdapter(BaseAdapter):
    def __init__(self, max_concurrent: int = 10, *, credentials: dict[str, str] | None = None) -> None:
    ...
    async def invoke(self, request: ChatRequest) -> UnifiedChatResponse:
        ...
        unified.provider_metadata = ProviderMetadata(provider=self.provider_name, raw=raw)
        return unified
    ...
    async def invoke_streaming(self, request: ChatRequest) -> AsyncIterator[StreamChunk]:
        ...
        if callable(getattr(client.chat.completions, "create", None)):
            try:
                ...
                for ev in chunks:
                    delta_text = (
                        (((ev.get("choices") or [{}])[0]).get("delta") or {}).get("content")
                        or ""
                    )
                    if delta_text:
                        yield StreamChunk(
