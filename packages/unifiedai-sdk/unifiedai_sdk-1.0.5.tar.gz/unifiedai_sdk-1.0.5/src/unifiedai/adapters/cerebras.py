...
class CerebrasAdapter(BaseAdapter):
    ...
    async def invoke(self, request: ChatRequest) -> UnifiedChatResponse:
        ...
        # Compute richer metrics (prefer provider-reported timing from time_info)
        ...
        round_trip_time_s = total_time if (total_time and total_time > 0) else (duration_ms / 1000.0)
        inference_time_s = (
            completion_time if (completion_time and completion_time > 0) else round_trip_time_s
        )
        # If provider exposes prompt+queue timings, approximate TTFB from them
        if (prompt_time or 0) + (queue_time or 0) > 0:
            ttfb_ms = ((prompt_time or 0.0) + (queue_time or 0.0)) * 1000.0

        completion_tokens = unified.usage.completion_tokens or 0
        total_tokens = unified.usage.total_tokens or 0
        output_tokens_per_sec = (
            (completion_tokens / inference_time_s) if inference_time_s and inference_time_s > 0 else None
        )
        total_tokens_per_sec = (
            (total_tokens / round_trip_time_s) if round_trip_time_s and round_trip_time_s > 0 else None
        )
