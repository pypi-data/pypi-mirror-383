...
class TestApplyMerchantEdit:
    ...
    def test_handles_empty_dataframe(self):
        ...
        df = pl.DataFrame({
            "id": [],
            "merchant": []
        }, schema={"id": pl.Utf8, "merchant": pl.Utf8})
...
class TestApplyEditToDataframe:
    ...
    def test_dispatches_category_edit(self):
        ...
        categories = {
            "cat2": {"name": "Groceries", "group": "Food"}
        }
...
class TestApplyEditsToDataframe:
    ...
    def test_applies_multiple_edits_same_transaction(self):
        ...
        def mock_apply_groups(df): return df.with_columns(pl.lit("Food").alias("group"))
...
class TestDataFramePurity:
    ...
    def test_does_not_mutate_original_dataframe(self):
        ...
        updated = CommitOrchestrator.apply_merchant_edit(
            original_df, "txn1", "Target"
        )
...
class TestRealWorldScenarios:
    ...
    def test_recategorize_with_group_update(self):
        ...
        categories = {
            "cat_groceries": {"name": "Groceries", "group": "Food & Dining"}
        }

        edit = TransactionEdit(
            "txn1", "category", "cat1", "cat_groceries", datetime.now()
        )
        ...
        updated = CommitOrchestrator.apply_edit_to_dataframe(
            df, edit, categories, apply_food_group
        )
