...
def _get_dendrogram(
    df: pd.DataFrame, rows_to_ignore=None
...
def _heatmap(
    ...
    for i, (ax, ax_dend, linkage_col, df, df_iso, name) in enumerate(
        zip(axs_heatmap, axs_dend, linkages_col, dfs, dfs_iso, names, strict=True)
    ):
        # df = df.copy()
        # columns = []
        # for j in df.columns:
...
def _barplot_with_whisker_strip(
    ...
    for i, (ax, df, name) in enumerate(zip(axs, dfs, names)):
        ...
        group2num_samples = group.size()
        groups_w_multi_samples = group2num_samples[group2num_samples > 1].index.tolist()
        ...
        std = group[column_of_interest].std()
        dfs_metrics.append(
            pd.concat(
                [value, std],
                axis=1,
                keys=[f"{column_of_interest}_mean", f"{column_of_interest}_std"],
            ).assign(**{"group": name})
        )
        values.extend(value)
    ...
    return pd.concat(dfs_metrics, axis=0)
...
def get_abundance_plot(
    ...
    *,
    ...
    cmap: str = "rocket_r",
    ret_data: bool = False,
) -> list[Figure] | tuple[list[Figure], pd.DataFrame]:
    ...
    for level, rel_ab_thres in zip(tax_levels, rel_ab_thresholds):
                ...
                if sample_hierarchical_clustering:
                    ...
                    for df in dfs:
                        if plot_type == "heatmap_log10":
                            mask = df == -np.inf
                            min_val = df[~mask].min().min()
                            df[mask] = min_val
                        else:
                            mask = df.isna()
                            df[mask] = 0
                        df, linkage_row = _get_dendrogram(df.transpose())
                        df = df.transpose()
                        # Convert back to -inf or NaN
                        if plot_type == "heatmap_log10":
                            df[mask] = -np.inf
                        else:
                            df[mask] = np.nan
                        dfs_temp.append(df)
                        linkages_col.append(linkage_row)
                    dfs = dfs_temp
                else:
                    linkages_col = [None] * len(dfs)
        ...
        dfs_data.append(
            pd.concat(
                [
                    df.transpose().assign(**{sample_group_key: name})
    ...
    *,
    ...
    for level in tax_levels:
        ...
        for column_of_interest in metrics:
            ...
            if len(groups) == 1:
                axs = [axs]
            df_metric = _barplot_with_whisker_strip(
                groups,
                names=names,
                title=title,
        ...
        df_metrics = pd.concat(df_metric_list, axis=1)
        df_metrics = pd.concat(
            [
                df_metrics["group"],
                df_metrics.loc[:, df_metrics.columns != "group"],
            ],
            axis=1,
        )
        df_metrics_list[level] = df_metrics
                    ...
                    if sample_hierarchical_clustering:
                        ...
                        for df in dfs:
                            if plot_type == "heatmap_log10":
                                ...
                                df = df.fillna(0)
                            df, linkage_row = _get_dendrogram(
                                df.transpose(), rows_to_ignore=["unknown", "others"]
                            )
                            dfs_temp = []
                            i = 0
                            for df_temp in dfs:
                                dfs_temp.append(df.iloc[:, i : i + df_temp.shape[1]])
                                i += df_temp.shape[1]
                            dfs = [df_temp.transpose() for df_temp in dfs_temp]
                    df = pd.concat(dfs, axis=1)
                    if feature_hierarchical_clustering:
                        df, linkage_row = _get_dendrogram(
                            df, rows_to_ignore=["unknown", "others"]
