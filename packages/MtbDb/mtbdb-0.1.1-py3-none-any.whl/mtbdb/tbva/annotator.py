...
class VariantAnnotator:
    def __init__(self, SNPdf):
        # Ensure POS column is integer type
        self.SNPdf = SNPdf.assign(POS=SNPdf['POS'].astype(int))
        self.genes = genes
    ...
    def SnpEffectAnnotation(self):
        # print(f"Start SnpEffectAnnotation - POS dtype: {self.SNPdf['POS'].dtype}") # Check dtype before loop
        AnnotatedSNP = []
        rows = self.SNPdf.itertuples(index=False)
        current_snp = next(rows, None)

        while current_snp is not None:
            # print(f"Loop - current_snp.POS type: {type(current_snp.POS)}, value: {current_snp.POS}") # Check type inside loop
            next_snp = next(rows, None)
            if next_snp:
                # print(f"Loop - next_snp.POS type: {type(next_snp.POS)}, value: {next_snp.POS}") # Check type inside loop
