"""Turkish sample records for cadelphi."""
...
def load_turkish_samples(session: Session) -> None:
    """Seed the database with Turkish demonstration data if missing."""
    if session.scalar(select(Topic.id).where(Topic.title == "Sürdürülebilir Şehir Planlaması")):
        return
    ...
    votes = [
        ...
        Vote(
            ...
            score=3,
            comment="Su yönetimi açısından mantıklı.",
        ),
        Vote(
            argument_id=arguments[3].id,
            participant_id=participants[0].id,
            score=5,
            comment="Bağımsız denetim güven sağlar.",
        ),
        Vote(
            argument_id=arguments[4].id,
            participant_id=participants[3].id,
            score=4,
            comment="Veri koruması için gerekli.",
        ),
        Vote(
            argument_id=arguments[4].id,
            participant_id=participants[1].id,
            score=1,
            comment="Tam şeffaflık rekabeti zedeleyebilir.",
        ),
        Vote(
            argument_id=arguments[5].id,
            participant_id=participants[0].id,
            score=5,
            comment="Kararların gerekçesi kamuya açık olmalı.",
        ),
        Vote(
            argument_id=arguments[5].id,
            participant_id=participants[2].id,
            score=4,
            comment="Güven tesis etmek için önemli.",
        ),
    ]
