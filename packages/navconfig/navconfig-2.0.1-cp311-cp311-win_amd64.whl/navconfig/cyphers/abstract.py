...
class AbstractCypher:
    ...
    async def open_file(self, path: PurePath):
        content = None
        if not path.exists():
            raise FileNotFoundError(f"Cypher: File {path} does not exist")
        try:
            async with aiofiles.open(path) as f:
                content = await f.read()
        except IOError as ex:
            raise Exception(f"NavConfig: Error loading Environment File {path}") from ex
        return content
    ...
    async def read_file(self, filename) -> str:
        ...
        try:
            ...
            raise RuntimeError(f"NavConfig: Error reading {fpath}: {ex}") from ex
