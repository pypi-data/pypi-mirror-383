...
class TestIntegration:
    ...
    def test_package_importable(self):
        ...
        import databricks_claude
        assert databricks_claude.__version__ == "1.0.0"
        
        from databricks_claude.core import DatabricksClaudeCore
        from databricks_claude.cli import cli
        from databricks_claude.exceptions import DatabricksClaudeError

    def test_cli_help(self):
        """Test that CLI help works."""
        result = subprocess.run([
            'python', '-m', 'databricks_claude.cli', '--help'
        ], capture_output=True, text=True)
        
        assert result.returncode == 0
        assert "Databricks Claude CLI Integration" in result.stdout

    def test_cli_version(self):
        """Test that CLI version works.""" 
        result = subprocess.run([
            'python', '-m', 'databricks_claude.cli', '--version'
        ], capture_output=True, text=True)
        
        assert result.returncode == 0
        assert "databricks-claude 1.0.0" in result.stdout

    @pytest.mark.skipif(
        not Path("/usr/bin/databricks").exists() and not Path("/opt/homebrew/bin/databricks").exists(),
    ...
    def test_status_command_real_databricks_cli(self):
        ...
        result = subprocess.run([
            'python', '-m', 'databricks_claude.cli', 'status'
        ], capture_output=True, text=True)
    ...
    def test_configuration_file_handling(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                assert core.config_dir.name == '.databricks-claude'
    ...
    def test_claude_config_update(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                assert config['env']['ANTHROPIC_BASE_URL'].endswith('/serving-endpoints/anthropic')
