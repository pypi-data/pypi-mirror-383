"""Command-line interface for Databricks Claude CLI."""
...
def cli(ctx: click.Context, debug: bool, host: Optional[str], version: bool) -> None:
    ...
    if version:
        click.echo(f"databricks-claude {__version__}")
        return
        ...
        try:
            ...
            if not core.is_claude_cli_installed():
                if not core.setup_claude_cli():
                    ...
                    click.echo("   Please install manually from: https://claude.ai/download", err=True)
                    sys.exit(1)
...
def login(core: DatabricksClaudeCore) -> None:
    ...
    try:
        ...
        click.echo("\n🎉 Setup complete!")
        click.echo("\nYou can now use:")
        click.echo("  databricks-claude 'your prompt'")
        click.echo("  databricks-claude --print 'quick question'")
        click.echo("  databricks-claude status")
...
def logout(core: DatabricksClaudeCore) -> None:
    ...
    try:
        ...
        click.echo("  databricks-claude login")
    except DatabricksClaudeError as e:
        click.echo(f"❌ Error: {e}", err=True)
        sys.exit(1)
...
def status(core: DatabricksClaudeCore) -> None:
    ...
    try:
        ...
        if is_auth:
            ...
            click.echo("   Run: databricks-claude login")
            sys.exit(1)
...
def config(core: DatabricksClaudeCore, host: Optional[str], reset: bool) -> None:
    ...
    try:
        ...
        if host:
            ...
            click.echo("   You may need to re-authenticate: databricks-claude login")
            return
        ...
        click.echo("\n💡 Available options:")
        click.echo("   databricks-claude config --host <url>    # Update workspace URL")
        click.echo("   databricks-claude config --reset         # Reset all settings")
