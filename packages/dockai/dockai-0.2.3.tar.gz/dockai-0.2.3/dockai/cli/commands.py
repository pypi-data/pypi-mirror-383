...
@click.option("--lang", default="en", help="Çıktı dili (en|tr)")
def analyze(container_name, mode, lang):
    logs = get_logs(container_name)
    print(f"🪶 Log alındı: {len(logs)} karakter.")
    print(f"🔍 {mode} modunda analiz ediliyor...")

    if mode == "cloud":
        response = analyze_with_openai(logs, lang=lang)
    else:
        response = analyze_with_ollama(logs, lang=lang)
