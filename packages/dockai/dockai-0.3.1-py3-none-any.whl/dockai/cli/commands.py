...
def analyze(mode, since, tail, perf, instant_perf, container_name):
    ...
    # Mode resolution: config > CLI > default('cloud')
    effective_mode = cfg_get("DOCKAI_MODE", None) or (mode if mode else "cloud")
    ...
    if not logs or not logs.strip():
        ...
        if meta.get("perf"):
            ...
            try:
                response = analyze_with_ollama("", container_name=container_name, meta=meta)
                click.echo("\n🤖 AI Analizi:\n")
                click.echo(response)
            except Exception as e:
