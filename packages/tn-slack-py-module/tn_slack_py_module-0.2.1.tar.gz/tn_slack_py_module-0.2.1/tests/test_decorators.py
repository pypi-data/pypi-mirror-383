...
from tnslack.decorators import block_set, processor, route
...
class TestRouteDecorator:
    """Test cases for route decorator."""

    def test_route_decorator_basic_registration(self):
        """Test route decorator registers function with app."""
        mock_app = Mock()
        mock_app.register_route = Mock()

        @route(mock_app, "block_actions")
        def test_handler(payload, context):
            return "handled"

        mock_app.register_route.assert_called_once_with(
            "block_actions", test_handler, "test_handler"
        )

    def test_route_decorator_with_custom_name(self):
        """Test route decorator with custom route name."""
        mock_app = Mock()
        mock_app.register_route = Mock()

        @route(mock_app, "view_submission", name="custom_handler")
        def test_handler(payload, context):
            return "handled"

        mock_app.register_route.assert_called_once_with(
            "view_submission", test_handler, "custom_handler"
        )

    def test_route_decorator_with_required_context(self):
        """Test route decorator with required context validation."""
        mock_app = Mock()
        mock_app.register_route = Mock()

        @route(mock_app, "block_actions", required_context=["access_token", "user_id"])
        def test_handler(payload, context):
            return f"User {context['user_id']} handled {payload['type']}"

        # Check registration occurred
        mock_app.register_route.assert_called_once()
        registered_route_type, registered_handler, registered_name = mock_app.register_route.call_args[0]
        
        assert registered_route_type == "block_actions"
        assert registered_name == "test_handler"

        # Test successful execution with required context
        payload = {"type": "button_click"}
        context = {"access_token": "xoxb-token", "user_id": "U123"}
        result = registered_handler(payload, context)
        assert result == "User U123 handled button_click"

    def test_route_decorator_missing_required_context(self):
        """Test route decorator validation with missing required context."""
        mock_app = Mock()
        mock_app.register_route = Mock()

        @route(mock_app, "view_submission", required_context=["access_token"])
        def test_handler(payload, context):
            return "processed"

        # Get the registered handler
        registered_handler = mock_app.register_route.call_args[0][1]
        
        payload = {"type": "test"}
        
        # Missing required context should raise ValueError
        with pytest.raises(ValueError, match="context missing: access_token, in test_handler"):
            registered_handler(payload, {})

    def test_route_decorator_with_args_kwargs(self):
        """Test route decorator with additional args and kwargs."""
        mock_app = Mock()
        mock_app.register_route = Mock()

        @route(mock_app, "block_suggestion", required_context=["user_id"])
        def test_handler(payload, context, extra_arg, extra_kwarg=None):
            return {
                "payload_type": payload["type"],
                "user": context["user_id"],
                "extra_arg": extra_arg,
        ...
        context = {"user_id": "U123"}
        result = registered_handler(payload, context, "arg_value", extra_kwarg="kwarg_value")
        
        assert result["payload_type"] == "suggestion"
        assert result["user"] == "U123"
        assert result["extra_arg"] == "arg_value"
        assert result["extra_kwarg"] == "kwarg_value"

    def test_route_decorator_none_values_in_context(self):
        """Test route decorator with None values in required context."""
        mock_app = Mock()
        mock_app.register_route = Mock()

        @route(mock_app, "view_closed", required_context=["required_key"])
        def test_handler(payload, context):
            return "processed"
        ...
        # None values should trigger missing context error
        with pytest.raises(ValueError, match="context missing: required_key, in test_handler"):
            registered_handler(payload, {"required_key": None})