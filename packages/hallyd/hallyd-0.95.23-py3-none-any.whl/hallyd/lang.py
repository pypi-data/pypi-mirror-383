...
def lock(lock_path: "_fs.TInputPath", *, is_reentrant: bool = True, peek_interval: float = 0.25) -> "Lock":
    ...
    with _locks_lock:
        ...
        if not result:
            result = lock_ = _Lock(lock_path, is_reentrant, peek_interval)
            _locks[str(lock_path)] = weakref.ref(lock_)
            weakref.finalize(lock_, lambda: weak_ref_check(str(lock_path)))
        return result
...
class _Lock(Lock):
    ...
    def acquire(self, blocking=True, timeout=-1):
        ...
        try:
            self.__lock_fd = open(self.__lock_path, "w")
            while True:
                try:
