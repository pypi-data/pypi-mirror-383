...
class AutomationToolkit:
    ...
    def ocr_find_text(self, target_text: str,
        ...
        try:
            ...
            #     region_screenshot_rgb = cv2.cvtColor(ocr_screenshot, cv2.COLOR_BGR2RGB)
            region_screenshot_rgb = ocr_screenshot
            ...
            for result in ocr_results:
                if isinstance(result, dict) and 'text' in result:
                    ...
                    if target_text in text:
                        # 计算文字在整个屏幕中的位置
                        # 修复：传递全屏截图形状而不是区域截图形状
                        screen_position = self._calculate_screen_position(
                            position, region_offset, full_screenshot_shape  # 使用全屏形状
                        )
                        ...
                        if debug:
                            self._save_ocr_debug_image(
                                ocr_screenshot, result, screen_position,
        ...
        try:
            ...
            if position_length < 4:
                ...
                return {
                    'center': (center_x_screen, center_y_screen),
                    'bounding_box': None,
                    'message': f'使用区域中心点（位置信息不足: {position_length}）'
                }
            ...
            if points.size == 0 or points.ndim != 2 or points.shape[1] != 2:
                ...
                return {
                    'center': (center_x_screen, center_y_screen),
                    'bounding_box': None,
                    'message': f'使用区域中心点（无效的点阵形状: {points.shape}）'
                }
