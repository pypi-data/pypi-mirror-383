...
class AutomationToolkit:
    ...
    def __init__(self, device: str, img_path: str, task_id: str = None,
                 debug_img: str = None,  max_retries: int = 10,
                  accidental_processing: list = None,
        ...
        # 初始化OCR
        self.ocr_engine = self._init_ocr_engine(ocr_model_path)
        # 设备连接
    ...
    def _init_ocr_engine(self, ocr_model_path: str = None):
        """初始化OCR引擎"""
        if not CNOCR_AVAILABLE:
            logger.warning("CnOCR不可用，文字识别功能将被禁用")
            return None

        try:
            if ocr_model_path and os.path.exists(ocr_model_path):
                logger.info(f"使用自定义OCR模型: {ocr_model_path}")
                # 如果提供了模型路径，使用自定义配置
                ocr = CnOcr(
                    det_model_name=ocr_model_path
                )
            else:
                logger.info("使用默认OCR模型")
                # 使用默认模型
                ocr = CnOcr()
    ...
    def ocr_find_text(self, target_text: str,
        ...
        try:
            ...
            # 使用OCR识别文字
            ocr_results = self.ocr_engine.ocr(region_screenshot)
            ...
            for result in ocr_results:
                if isinstance(result, dict) and 'text' in result:
                    ...
                    if target_text in text:
                        # 计算文字在整个屏幕中的位置
                        screen_position = self._calculate_screen_position(
                            position, region_offset, region_screenshot.shape
                        )
                        ...
                        result_info = {
                            ...
                            'position': position,  # 区域内的相对位置
                            'screen_position': screen_position,  # 屏幕绝对位置
                            'center_point': screen_position['center'],
                        ...
                        logger.info(
                            ...
                            f"中心位置: {screen_position['center']}"
                        )

                        if debug:
                            self._save_ocr_debug_image(
                                region_screenshot, result, screen_position,
    ...
    def _calculate_screen_position(self, position: List,
                                   region_offset: Tuple[int, int],
                                   region_shape: Tuple) -> Dict[str, Any]:
        """
        计算文字在屏幕中的绝对位置

        Args:
            position: OCR返回的位置信息
            region_offset: 区域偏移量 (x, y)
            region_shape: 区域截图形状 (height, width, channels)
        ...
        try:
            if not position or len(position) < 4:
                ...
                center_x_relative = region_width // 2
                center_y_relative = region_height // 2

                center_x_screen = center_x_relative + region_offset[0]
                center_y_screen = center_y_relative + region_offset[1]

                return {
                    'center': (center_x_screen, center_y_screen),
                    'bounding_box': None,
                    'message': '使用区域中心点（无位置信息）'
                }
            ...
            # 计算区域内的中心点
            center_x_relative = int(np.mean(points[:, 0]))
            center_y_relative = int(np.mean(points[:, 1]))
            ...
            # 计算边界框
            top_left_relative = (int(np.min(points[:, 0])), int(np.min(points[:, 1])))
            bottom_right_relative = (int(np.max(points[:, 0])), int(np.max(points[:, 1])))

            top_left_screen = (top_left_relative[0] + region_offset[0],
                               top_left_relative[1] + region_offset[1])
            bottom_right_screen = (bottom_right_relative[0] + region_offset[0],
            ...
            return {
                ...
                'bounding_box': {
                    ...
                    'top_left_relative': top_left_relative,
                    'bottom_right_relative': bottom_right_relative
                },
                'width': bottom_right_relative[0] - top_left_relative[0],
                'height': bottom_right_relative[1] - top_left_relative[1]
            }

        except Exception as e:
            logger.error(f"计算屏幕位置失败: {e}")
            # 返回区域中心作为备选
            region_height, region_width = region_shape[:2]
            center_x_screen = region_width // 2 + region_offset[0]
            center_y_screen = region_height // 2 + region_offset[1]

            return {
                'center': (center_x_screen, center_y_screen),
                'bounding_box': None,
                'message': f'使用区域中心点（计算失败: {e}）'
            }
    ...
    def ocr_click_text(self, target_text: str,
        ...
        Returns:
            bool: 是否点击成功
        """
        result = self.ocr_find_text(target_text, region, min_confidence)
    ...
    def click_coordinate(self, x: int, y: int, time_sleep: float = 2) -> None:
        """点击坐标"""
        self.d.click(x, y)
