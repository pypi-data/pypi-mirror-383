...
# Constants
API_BASE = "http://order_backend:3300/api/v1"  # Docker service name
USER_AGENT = "ordercloud-mcp/1.0"

# Logger setup
logger = logging.getLogger("ordercloud_mcp")
...
async def make_request(
    ...
    logger.info(f"Making {method} request to URL: {url} with params: {params} and body: {body}")
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            logger.error(f"Request failed: {error_msg} (URL: {url})")
            return {"success": False, "error": error_msg, "url": url}
        except Exception as e:
            logger.error(f"Request exception: {str(e)} (URL: {url})")
...
def format_order(order: dict) -> str:
    ...
    created_at = created_on.replace('T', ' ').replace('Z', '').split('.')[0] if created_on != 'N/A' else 'N/A'
