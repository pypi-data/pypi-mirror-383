...
def mpDataGraphs(Q, conf, WaitTime=500.0, name='effective Voltage', XYmode=False, cmdQ=None):
    ...
    def yieldEvt_fromQ():
        ...
        interval = WaitTime / 1000.0  # in ms
        cnt = 0
        lagging = False
    ...
    def cmdSave():
        cmdPause()
        try:
            filename = asksaveasfilename(initialdir='.', initialfile='DGraphs.png', title='select file name')
    ...
    tw = max(WaitTime - 50.0, 0.5)  # smaller than WaitTime to allow for processing
    DGAnim = anim.FuncAnimation(
        figDG,
        DG,
        yieldEvt_fromQ,
        interval=tw,
