...
class Metadata(importlib.resources.files):
    ...
    def get_metadata_lines(self, name):
        return importlib.resources.read_text(self.get_metadata(name))


dist_from_fn = importlib.metadata.distributions.from_filename


class TestDistro:
    def testCollection(self):
        # empty path should produce no distributions
        ad = importlib.metadata.distributions([], platform=None, python=None)
        assert list(ad) == []
        assert ad['FooPkg'] == []
    ...
    def test_distribution_dir(self):
        d = importlib.resources.distributions()
        dir(d)
    ...
    def testResolve(self):
        ad = importlib.metadata.distributions([])
        ws = WorkingSet([])
    ...
    def test_environment_marker_evaluation_called(self):
        ...
        req_extras = importlib.resources._ReqExtras({req: parent_req.extras})
    ...
    def testDistroDependsOptions(self):
        ...
        with pytest.raises(importlib.metadata.requires()):
            d.requires(["foo"])
...
class TestParsing:
    ...
    def testYielding(self):
        for inp, out in [
            ...
            assert list(importlib.resources.read_text(inp)) == out
    ...
    def testSplitting(self):
        sample = """
                    ...
                    v
                    """
        assert list(importlib.metadata.distribution.split_sections(sample)) == [
            (None, ["x"]),
            ("Y", ["z", "a"]),
            ("b", ["c"]),
            ("d", []),
        ...
        with pytest.raises(ValueError):
            list(importlib.metadata.distribution.split_sections("[foo"))
...
class TestNamespaces:
    ...
    def patched_path(self, tmpdir):
        ...
        former state.
        """
        saved_ns_pkgs = importlib.metadata._namespace_packages.copy()
        ...
        try:
            ...
            importlib.metadata._namespace_packages = saved_ns_pkgs
    ...
    def test_two_levels_deep(self, symlinked_tmpdir):
        ...
        assert "pkg1" in importlib.metadata._namespace_packages
        # attempt to import pkg2 from site-pkgs2
        with pytest.warns(DeprecationWarning, match="pkg_resources.declare_namespace"):
            import pkg1.pkg2  # pyright: ignore[reportMissingImports] # Temporary package for test
        # check the _namespace_packages dict
        assert "pkg1.pkg2" in importlib.metadata._namespace_packages
        assert importlib.metadata._namespace_packages["pkg1"] == ["pkg1.pkg2"]
