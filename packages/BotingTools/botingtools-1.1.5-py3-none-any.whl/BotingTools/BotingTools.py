...
class BotTools:
    ...
    def text_to_video_30s(text, t, img_folder, clip1_path, img, font_path):
        ...
        BotTools.txt_to_img(text, 1, img_folder, font_path)
    ...
    def create_info_video(driver, video_path, img_folder, clip1_path, img, clip2_path, font_path, out_folder, out_video_path):    
        ...
        a = BotTools.text_to_video_2s(titleQuest2, t, clip2_path, font_path)
        clip1 = a[0]
        t = a[1]
        
        # ADD QUESTION TEXT TO VIDEO #
        a = BotTools.text_to_video_30s(question, t, img_folder, clip1_path, img, font_path)
        clip2 = a[0]
        t = a[1]
        ...
        for index, an in zip(range(5), top_answer['items']):
            ...
            a = BotTools.text_to_video_2s(titleAnwser, t, clip2_path, font_path)
            clips[i] = a[0]
            t = a[1]
            i += 1
            
            # ADD ANSWER TEXT TO VIDEO #
            a = BotTools.text_to_video_30s(AnwserText, t, img_folder, clip1_path, img, font_path)
            clips[i] = a[0]
            t = a[1]
            i += 1
