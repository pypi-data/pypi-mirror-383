...
from typing import Annotated, Generic, Literal, Self, TypeGuard, Union, overload
from maleo.types.dict import (
    StringToAnyDict,
    OptionalStringToAnyDict,
    OptionalStringToStringDict,
)
from maleo.types.integer import OptionalInteger, OptionalIntegerT
...
class BaseMeasurement(
    GenericMeasurement[
        MeasurementType, OptionalAggregateMeasurementType, OptionalInteger
    ]
):
    ...
    window: Annotated[
        OptionalInteger, Field(None, description="Measurement window", ge=1)
    ] = None
    ...
    def from_message(cls, message: Message) -> "BaseMeasurement":
        message_obj: StringToAnyDict = json.loads(message.data.decode())
        ...
        aggregate_type: OptionalStringToStringDict = message_obj["aggregate_type"]
        ...
        # Parse window
        window: OptionalStringToAnyDict = message_obj["window"]
        if window is None:
            window_value = None
        else:
            if not (
...
class Factory:
    @classmethod
    def to_message(cls, measurement: AnyMeasurement) -> StringToAnyDict:
