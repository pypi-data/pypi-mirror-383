...
export default function Dashboard() {
  ...
  const fetchRollbackOptions = async () => {
    try {
      const response = await apiClient.get('/rollback/available');
  ...
  const executeHardening = async () => {
    ...
    try {
      const response = await apiClient.post(`/run?level=${selectedLevel}&dry_run=${dryRun}`);
  ...
  const rollbackRule = async (ruleId: string) => {
    try {
      await apiClient.post(`/rollback/${ruleId}`);
      fetchRollbackOptions();
                    ...
                    <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div>
                                ...
                                <select value={selectedLevel} onChange={(e) => setSelectedLevel(e.target.value as 'basic' | 'moderate' | 'strict')} className="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
        ...
        case 'reports':
            return (
                <div className="space-y-8">
                    <div className="bg-white border border-gray-200 rounded-lg shadow-sm">
                        <div className="p-6 border-b">
                            <h3 className="text-lg font-semibold text-gray-900">Stored Reports</h3>
                            <p className="text-sm text-gray-500 mt-1">Reports generated from CLI and stored in MongoDB with device names.</p>
                        </div>
                        {reports.length > 0 ? (
                            <div className="overflow-x-auto">
                                <table className="min-w-full divide-y divide-gray-200">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <div className="flex items-center">
                                                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                                                            report.report_data.summary.score >= 80 
                                                                ? 'bg-green-100 text-green-800' 
                                                                : report.report_data.summary.score >= 60 
                                                                ? 'bg-yellow-100 text-yellow-800' 
                                                                : 'bg-red-100 text-red-800'
                                                        }`}>
                                                            {report.report_data.summary.score.toFixed(1)}%
                                                        </span>
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                    <div className="flex space-x-2">
                                                        <span className="text-green-600">{report.report_data.summary.passed} passed</span>
                                                        <span className="text-red-600">{report.report_data.summary.failed} failed</span>
                                                        <span className="text-yellow-600">{report.report_data.summary.errors} errors</span>
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            ...
                            <div className="text-center py-12">
                                ...
                                <p className="mt-1 text-sm text-gray-500">Generate reports using the CLI to see them here.</p>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
                            ...
                            <button onClick={() => window.open('/report/pdf', '_blank')} className="w-full flex items-center justify-center space-x-2 bg-red-600 hover:bg-red-700 text-white font-semibold py-2.5 px-4 rounded-md transition-colors">
                        ...
                        <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
                            ...
                            <button onClick={() => window.open('/report', '_blank')} className="w-full flex items-center justify-center space-x-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2.5 px-4 rounded-md transition-colors">
                                <Download className="h-5 w-5" />
                ...
                <div className="flex items-center space-x-4">
                  ...
                  {user && (
                    <div className="flex items-center space-x-3">
                      <div className="flex items-center text-sm text-gray-600 bg-blue-50 px-3 py-1.5 rounded-md">
                        <User className="h-4 w-4 mr-2" />
                        <span className="font-medium">{user.email}</span>
                      </div>
                      <button
                        onClick={logout}
                        className="flex items-center text-sm text-gray-600 hover:text-red-600 bg-gray-100 hover:bg-red-50 px-3 py-1.5 rounded-md transition-colors"
                      >
