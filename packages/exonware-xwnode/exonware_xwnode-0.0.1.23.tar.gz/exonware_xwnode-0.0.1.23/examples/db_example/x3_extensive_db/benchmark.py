#!/usr/bin/env python3
"""
#exonware/xwnode/examples/db_example/db_exhaustive_mix/benchmark.py

Exhaustive Strategy Benchmark - All Combinations
...
from x0_common import (
    ...
    generate_all_combinations, StrategyCombo
)
...
def generate_models():
    ...
    for combo in combinations:
        ...
        MODELS.append(model)
        TEST_CONFIG_MODELS.append(model)
    
    return len(MODELS)

# Auto-generate models at import time
total_combinations = generate_models()
print(f"Auto-generated {total_combinations} strategy combinations for exhaustive testing")
...
class ExhaustiveBenchmark:
    ...
    def __init__(self, total_entities: int = 1000):
        ...
        # Lighter for exhaustive tests (10% of full scale)
        ...
        # Operations (lighter for exhaustive tests)
    ...
    def run_all(self):
        """Run benchmarks on all strategy combinations"""
        print(f"\n{'='*80}")
        print(f"EXHAUSTIVE BENCHMARK - {self.total_entities:,} ENTITIES")
        print(f"{'='*80}")
        print(f"\nConfiguration:")
        ...
        print(f"  Total Combinations: {len(self.databases)}")
        print(f"\nTesting ALL NodeMode × EdgeMode combinations...")
        print(f"This will take a while - testing {len(self.databases)} configurations!\n")
        ...
        # Show top 10 winners for this test
        ...
        if successful_results:
            ...
            print(f"\n{'='*80}")
            print(f"TOP 10 WINNERS - {self.total_entities:,} ENTITIES")
            print(f"{'='*80}")
...
def run_all_tests():
    ...
    print(f"\n{'='*80}")
    print(f"EXHAUSTIVE BENCHMARK - AUTO RUN")
    print(f"{'='*80}")
    print(f"\nTests to run: {', '.join([f'{t:,}' for t in TESTS])} entities")
    print(f"Total combinations per test: {len(MODELS)}")
    ...
    for i, entities in enumerate(TESTS, 1):
        ...
        benchmark = ExhaustiveBenchmark(total_entities=entities)
        benchmark.run_all()
        
        # Store results with entity count as key
        all_results[str(entities)] = benchmark.results
        
        # Show top 10 for this test in the auto-run output
        ...
        if successful_results:
            ...
            print(f"\n{'='*80}")
            print(f"TOP 10 WINNERS - TEST {i}/{len(TESTS)} ({entities:,} entities)")
            print(f"{'='*80}")
            for rank, (name, data) in enumerate(sorted_results[:10], 1):
                time_ms = data.get('total_time_ms', 0)
                memory = data.get('peak_memory_mb', 0)
    ...
    try:
        with open(csv_file, 'w', newline='', encoding='utf-8') as f:
            ...
            # Write header
            writer.writerow(['# Exhaustive Strategy Benchmark Results - Combined'])
            writer.writerow([f'# All Tests: {", ".join([f"{k} entities" for k in all_results.keys()])}'])
            writer.writerow([f'# Total combinations per test: {len(MODELS)}'])
            writer.writerow([])
            ...
            for entities_str, results in all_results.items():
                ...
                # Detailed results header (same as classic benchmark)
                writer.writerow([
                    'Rank', 'Configuration', 'Node Mode', 'Edge Mode', 
                ...
                for rank, (name, data) in enumerate(sorted_results, 1):
                    ...
                    # Extract detailed metrics (same as classic)
                ...
                if sorted_results:
                    ...
                    writer.writerow(['Total Combinations Tested', len(results)])
                    ...
                    writer.writerow(['Memory Champion', min(successful_results.items(), key=lambda x: x[1].get('peak_memory_mb', float('inf')))[0]])
                writer.writerow([])
    ...
    print(f"\n{'='*80}")
    print(f"ALL EXHAUSTIVE BENCHMARKS COMPLETE")
    print(f"{'='*80}")
    print(f"\nFiles generated:")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Exhaustive Strategy Benchmark - All Combinations')
    ...
    if args.entities is None:
        ...
        benchmark = ExhaustiveBenchmark(total_entities=args.entities)
        benchmark.run_all()
        ...
        if successful_results:
            ...
            print(f"\n{'='*80}")
            print(f"TOP 10 WINNERS - {args.entities:,} ENTITIES")
            print(f"{'='*80}")
            for rank, (name, data) in enumerate(sorted_results[:10], 1):
                time_ms = data.get('total_time_ms', 0)
                memory = data.get('peak_memory_mb', 0)
