...
class BatchTaskRunner:
    ...
    def _get_access_token(self) -> str:
        """Get Azure access token for Batch API"""
        current_time = datetime.now(timezone.utc)

        # Refresh token if it's expired or about to expire (5 minutes buffer)
        if (
            self._access_token is None
            or self._token_expires_on is None
            or current_time.timestamp() >= (self._token_expires_on - 300)
        ):
            self.logger.info("Refreshing Azure access token...")
            token = self.credential.get_token("https://batch.core.windows.net/")
            self._access_token = token.token
            self._token_expires_on = token.expires_on
            self.logger.info("Azure access token refreshed successfully")
