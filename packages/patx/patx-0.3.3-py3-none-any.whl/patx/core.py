...
def generate_bspline_pattern(control_points: List[float], width: int) -> np.ndarray:
    degree = 3
    knots = np.concatenate([np.zeros(degree + 1), np.linspace(0, 1, len(control_points) - degree + 1)[1:-1], np.ones(degree + 1)])
    return BSpline(knots, np.array(control_points), degree)(np.linspace(0, 1, width))
...
def evaluate_model_performance(model, X_combined, y_train, metric, cached_indices):
    ...
    model = model.clone()
    model.fit(X_combined[train_idx], y_train[train_idx], X_combined[val_idx], y_train[val_idx])
    y_val = y_train[val_idx]
    if metric == 'accuracy': return accuracy_score(y_val, model.predict(X_combined[val_idx]))
    if metric == 'rmse': return np.sqrt(mean_squared_error(y_val, model.predict(X_combined[val_idx])))
    y_pred = model.predict_proba(X_combined[val_idx])
...
def feature_extraction(input_series_train: Union[np.ndarray, List], 
    ...
    def objective(trial):
        ...
        width, start = trial.suggest_int('pattern_width', 1, n_time_points - 1), trial.suggest_int('pattern_start', 0, n_time_points - 1)
        ...
        X = pd.concat([pd.DataFrame(f) for f in model_features_list + [feat]], axis=1).values if model_features_list else feat.reshape(-1, 1)
        return evaluate_model_performance(model, X, y_train, metric, cached_indices)
    
    extracted_patterns = []
    while True:
        study = optuna.create_study(direction='minimize' if metric == 'rmse' else 'maximize', sampler=optuna.samplers.TPESampler(multivariate=True, seed=42))
    ...
    model_features = pd.concat([pd.DataFrame(f) for f in model_features_list], axis=1).values if model_features_list else np.empty((len(y_train), 0))
    ...
    if input_series_test is not None:
        ...
        test_features = pd.concat([pd.DataFrame(f) for f in all_test_feats], axis=1).values if all_test_feats else np.empty((len(input_series_test), 0))
