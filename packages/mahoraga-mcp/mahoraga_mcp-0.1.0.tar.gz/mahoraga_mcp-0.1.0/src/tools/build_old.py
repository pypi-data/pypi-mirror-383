"""
Build tool - Setup dependencies for Mahoraga MCP.
...
def check_mahoraga() -> Tuple[bool, str]:
    """Check if Mahoraga package is available."""
    try:
        import mahoraga
        return True, "✓ Mahoraga package ready"
    except ImportError:
        return False, "✗ Mahoraga package not installed"
...
def install_mahoraga() -> Tuple[bool, str]:
    ...
    try:
        ...
        if os.path.exists(mahoraga_path):
            ...
            return True, "✓ Mahoraga installed successfully"
        else:
            return False, f"✗ Mahoraga directory not found at: {mahoraga_path}"
    except subprocess.CalledProcessError as e:
...
async def build() -> Dict[str, Any]:
    """
    Setup and verify all dependencies required for Mahoraga.
