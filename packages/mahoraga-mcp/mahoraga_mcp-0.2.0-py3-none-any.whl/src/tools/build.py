...
class DependencyChecker:
    ...
    def check_mahoraga(self) -> Tuple[bool, str, str]:
        """Check if Mahoraga package is installed"""
        try:
            import mahoraga
            version = getattr(mahoraga, "__version__", "unknown")
            location = Path(mahoraga.__file__).parent
            return True, f"✓ Mahoraga v{version} installed at {location}", None
        except ImportError:
            return False, "✗ Mahoraga package not installed", None
    ...
    def install_mahoraga(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            if mahoraga_src.exists() and (mahoraga_src / "pyproject.toml").exists():
                ...
                print(f"  Installing Mahoraga from {mahoraga_src}...")
                subprocess.run(
                    [sys.executable, "-m", "pip", "install", "-e", str(mahoraga_src)],
                    check=True,
                    capture_output=True
                )
                return True, f"✓ Mahoraga installed from local source: {mahoraga_src}"
            else:
                # Try to install from PyPI (if published)
                print("  Installing Mahoraga from PyPI...")
                subprocess.run(
                    [sys.executable, "-m", "pip", "install", "mahoraga"],
                    check=True,
                    capture_output=True
                )
                return True, "✓ Mahoraga installed from PyPI"
    ...
    def check_mahoraga_version(self) -> Tuple[bool, str, str]:
        """Check if Mahoraga version is compatible"""
        try:
            import mahoraga
            ...
            if int(major) == 0 and int(minor) >= 3:
                return True, f"✓ Mahoraga v{version} (compatible)", None
            elif int(major) > 0:
                return True, f"✓ Mahoraga v{version} (compatible)", None
            else:
                return False, f"✗ Mahoraga v{version} (requires >= 0.3.0)", version
        except Exception as e:
    ...
    def install_python_dependencies(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            # Install from Mahoraga's pyproject.toml if available
            project_root = Path(__file__).parent.parent.parent.parent
            mahoraga_src = project_root / "mahoraga"
...
async def build() -> Dict[str, Any]:
    ...
    # 4. Check Mahoraga Package
    print("4️⃣  Checking Mahoraga package...")
    ...
    if not mhg_ok:
        # Try to install
        print("   Attempting to install Mahoraga...")
        ...
        if not install_ok:
            ...
            fixes_applied.append("Installed Mahoraga")
            mahoraga_just_installed = True  # Track that we just installed Mahoraga
    print()

    # 5. Check Mahoraga Version
    print("5️⃣  Checking Mahoraga version...")
    ...
    # Note: Only check if Mahoraga wasn't just installed, because 'pip install -e'
    ...
    if mahoraga_just_installed:
        ...
        print(f"   ⏭️  Skipped (dependencies installed with Mahoraga in step 4)")
        details["python_dependencies"] = "⏭️  Skipped (installed with Mahoraga)"
    else:
        print("6️⃣  Checking Python dependencies...")
    ...
    if all_ok:
        status = "success"
        message = "✅ All dependencies ready! You can now use Mahoraga."
        if fixes_applied:
