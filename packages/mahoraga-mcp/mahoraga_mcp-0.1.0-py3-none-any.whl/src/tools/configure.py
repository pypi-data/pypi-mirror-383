...
Allows users to set and update Mahoraga agent execution parameters.
"""
...
async def configure(
    mahoraga_api_key: Optional[str] = None,
    ...
    Args:
        mahoraga_api_key: Mahoraga API key for authentication and access
    ...
    updates = {}
    if mahoraga_api_key is not None:
        updates["api_key"] = mahoraga_api_key
    ...
    # Validate updates (map api_key back to mahoraga_api_key for validation)
    validation_updates = updates.copy()
    if "api_key" in validation_updates:
        validation_updates["mahoraga_api_key"] = validation_updates.pop("api_key")
    ...
    if "api_key" in updated_keys:
        updated_keys[updated_keys.index("api_key")] = "mahoraga_api_key"
