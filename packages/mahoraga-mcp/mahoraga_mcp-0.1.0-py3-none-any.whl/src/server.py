...
"""
Mahoraga MCP Server
A Model Context Protocol server for mobile automation testing with Mahoraga.
"""
...
from src.tools.build import build
from src.tools.connect import connect
from src.tools.configure import configure
from src.tools.execute import execute
from src.tools.runsuite import runsuite
from src.tools.usage import usage
from src.state import get_state
from src.usage_tracker import get_tracker

# Setup logging
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger("mahoraga-mcp")

# Create MCP server instance
app = Server("mahoraga-mcp")


@app.list_tools()
async def list_tools() -> list[Tool]:
    """List all available Mahoraga tools with dynamic state information."""
    ...
    connect_desc = ("Connect to an Android device or emulator. "
                   ...
                   "Verifies connectivity and checks/installs Mahoraga Portal accessibility service.")
    ...
    configure_desc = ("Configure Mahoraga agent execution parameters. "
    ...
    # USAGE tool description with current stats
    usage_desc = ("View usage statistics and costs for OpenRouter API usage. "
                 "Shows token counts, image counts, and costs per API key with execution history. "
                 "Can view stats for a specific API key or all keys combined.")

    # Get current usage summary
    usage_summary = tracker.get_usage_summary()
    if usage_summary.get('total_api_keys', 0) > 0:
        usage_desc += f"\n\n📊 CURRENT USAGE STATS:\n"
        usage_desc += f"  • Total API Keys: {usage_summary['total_api_keys']}\n"
        usage_desc += f"  • Total Executions: {usage_summary['grand_total_executions']}\n"
        usage_desc += f"  • Total Cost: ${usage_summary['grand_total_cost']:.4f}\n"

        # Show per-key breakdown
        for key_info in usage_summary['api_keys']:
            usage_desc += f"\n  Key: {key_info['api_key_masked']}\n"
            usage_desc += f"    - Executions: {key_info['total_executions']}\n"
            usage_desc += f"    - Cost: ${key_info['total_cost_usd']:.4f}\n"
            usage_desc += f"    - Last used: {key_info['last_used']}"
    else:
        usage_desc += "\n\n📊 CURRENT USAGE STATS: No usage data yet"

    return [
        Tool(
            name="build",
            description="Setup and verify all dependencies required for Mahoraga mobile automation. "
                       "Checks Python version, ADB installation, Mahoraga package, and Portal APK. "
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    "mahoraga_api_key": {
                        "type": "string",
                        "description": "Mahoraga API key for authentication and access"
                    },
                    "model": {
                        "type": "string",
    ...
    try:
        if name == "build":
            ...
            result = await configure(
                mahoraga_api_key=arguments.get("mahoraga_api_key"),
...
async def async_main():
    ...
    async with stdio_server() as (read_stream, write_stream):
        logger.info("🚀 Mahoraga MCP Server started")
        await app.run(
            read_stream,
            write_stream,
...
def main():
    """Entry point for the mahoraga-mcp command."""
