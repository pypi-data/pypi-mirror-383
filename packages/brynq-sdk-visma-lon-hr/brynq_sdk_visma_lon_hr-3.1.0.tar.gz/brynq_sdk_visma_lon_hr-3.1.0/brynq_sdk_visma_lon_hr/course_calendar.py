...
import pandas as pd
from .schemas.course_calendar import CourseCalendarGet
...
class CourseCalendar:
    ...
    def get(self,
        ...
        return self.visma.get(
            ...
            skiptoken=skiptoken,
            max_pages=max_pages,
            schema=CourseCalendarGet
        )

    def get_by_id(self, course_calendar_id: str) -> tuple[pd.DataFrame, pd.DataFrame]:
        """
        Get a specific course calendar record by its ID.

        Args:
            course_calendar_id (str): The unique identifier of the course calendar record.

        Returns:
            tuple[pd.DataFrame, pd.DataFrame]: A tuple containing (valid_data, invalid_data).
        """
        filter_expression = f"CourseCalendarRID eq '{course_calendar_id}'"
