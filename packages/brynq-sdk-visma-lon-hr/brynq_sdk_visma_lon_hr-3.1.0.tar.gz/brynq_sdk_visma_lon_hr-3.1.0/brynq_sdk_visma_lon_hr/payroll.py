...
import pandas as pd
from .schemas.payroll import PayrollGet
...
class Payroll:
    ...
    def get_by_id(self, payroll_id: str) -> tuple[pd.DataFrame, pd.DataFrame]:
        """
        Get a specific payroll record by its ID.

        Args:
            payroll_id (str): The unique identifier of the payroll record.

        Returns:
            tuple[pd.DataFrame, pd.DataFrame]: A tuple containing (valid_data, invalid_data).
        """
        filter_expression = f"PayrollRID eq '{payroll_id}'"
