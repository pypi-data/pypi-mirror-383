...
import pandas as pd
from .schemas.customer import CustomerGet
...
class Customer:
    ...
    def get_by_id(self, customer_id: str) -> tuple[pd.DataFrame, pd.DataFrame]:
        """
        Get a specific customer record by its ID.

        Args:
            customer_id (str): The unique identifier of the customer record.

        Returns:
            tuple[pd.DataFrame, pd.DataFrame]: A tuple containing (valid_data, invalid_data).
        """
        filter_expression = f"CustomerRID eq '{customer_id}'"
