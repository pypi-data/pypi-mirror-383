...
class PhoneNumberGet(BrynQPanderaDataFrameModel):
    """
    Schema for phone number data from Visma Lon HR.
    Represents phone number information from the VISMALØN table TELEFON.
    """

    rid: Series[pd.StringDtype] = pa.Field(coerce=True, description="Unique key for use of OData", alias="PhoneNumberRID")
    phone: Series[pd.StringDtype] = pa.Field(coerce=True, description="Phone Number", alias="Phone")
    extension: Series[pd.StringDtype] = pa.Field(coerce=True, description="extension of phone", alias="Extension")
    ...
    comment: Series[pd.StringDtype] = pa.Field(coerce=True, description="Text", alias="Comment", nullable=True)
