...
import pandas as pd
from .schemas.organizational_unit import OrganizationalUnitGet
...
class OrganizationalUnit:
    ...
    def get(self,
        ...
        return self.visma.get(
            ...
            skiptoken=skiptoken,
            max_pages=max_pages,
            schema=OrganizationalUnitGet
        )

    def get_by_id(self, organizational_unit_id: str) -> tuple[pd.DataFrame, pd.DataFrame]:
        """
        Get a specific organizational unit record by its ID.

        Args:
            organizational_unit_id (str): The unique identifier of the organizational unit record.

        Returns:
            tuple[pd.DataFrame, pd.DataFrame]: A tuple containing (valid_data, invalid_data).
        """
        filter_expression = f"OrganizationalUnitRID eq '{organizational_unit_id}'"
