...
import pandas as pd
from .schemas.processed_absence import ProcessedAbsenceGet
...
class ProcessedAbsence:
    ...
    def get(self,
        ...
        return self.visma.get(
            ...
            skiptoken=skiptoken,
            max_pages=max_pages,
            schema=ProcessedAbsenceGet
        )

    def get_by_id(self, processed_absence_id: str) -> tuple[pd.DataFrame, pd.DataFrame]:
        """
        Get a specific processed absence record by its ID.

        Args:
            processed_absence_id (str): The unique identifier of the processed absence record.

        Returns:
            tuple[pd.DataFrame, pd.DataFrame]: A tuple containing (valid_data, invalid_data).
        """
        filter_expression = f"ProcessedAbsenceRID eq '{processed_absence_id}'"
