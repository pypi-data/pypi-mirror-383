...
class Employment:
    ...
    def get(self,
        ...
        return self.visma.get(
            ...
            skiptoken=skiptoken,
            max_pages=max_pages,
            schema=EmploymentGet
        )

    def get_by_id(self, employment_id: str) -> tuple[pd.DataFrame, pd.DataFrame]:
        """
        Get a specific employment record by its ID.

        Args:
            employment_id (str): The unique identifier of the employment record.

        Returns:
            tuple[pd.DataFrame, pd.DataFrame]: A tuple containing (valid_data, invalid_data).
        """
        filter_expression = f"EmploymentRID eq '{employment_id}'"
