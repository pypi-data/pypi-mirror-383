...
import pandas as pd
from .schemas.work_calendar import WorkCalendarGet

class WorkCalendar:
    """
    Handles work calendar operations for Visma Lon HR API.
    WorkCalendar holds information about work calendar.
    """

    def __init__(self, visma_instance):
        """
        Initialize the WorkCalendar class.
    ...
    def get(self,
        ...
        return self.visma.get(
            ...
            skiptoken=skiptoken,
            max_pages=max_pages,
            schema=WorkCalendarGet
        )

    def get_by_id(self, work_calendar_id: str) -> tuple[pd.DataFrame, pd.DataFrame]:
        """
        Get a specific work calendar record by its ID.

        Args:
            work_calendar_id (str): The unique identifier of the work calendar record.

        Returns:
            tuple[pd.DataFrame, pd.DataFrame]: A tuple containing (valid_data, invalid_data).
        """
        filter_expression = f"VismaLoenWorkCalendarRID eq '{work_calendar_id}'"
