...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, 
                             ...
                             QCheckBox, QGridLayout, QMessageBox, QSplitter, QTabWidget)
from PyQt5.QtCore import Qt, QObject, pyqtSignal

class SignalEmitter(QObject):
    update_response = pyqtSignal(str)
    update_headers = pyqtSignal(str)
...
class APITesterApp(QMainWindow):
    def __init__(self):
        ...
        self.headers_text_response = QTextEdit()
        self.headers_text_response.setReadOnly(True)  # Headers are read-only
        self.headers_text_response.setFixedHeight(100)
        self.response_tabs.addTab(self.response_text, "Body")
        self.response_tabs.addTab(self.headers_text_response, "Headers")
        content_layout.addWidget(self.response_tabs)
        ...
        self.emitter.update_headers.connect(self.update_headers_text)
    ...
    def update_response_text(self, text):
        self.logger.debug(f"Updating response body text area with: {text[:100]}...")
        self.response_text.clear()
        self.response_text.setText(text)

    def update_headers_text(self, headers):
        self.logger.debug(f"Updating response headers text area with: {headers[:100]}...")
        self.headers_text_response.clear()
        self.headers_text_response.setText(headers)
    ...
    def test_cookie_request(self):
        ...
        if not cookies:
            ...
            self.emitter.update_headers.emit("")
            self.reset_ui()
            return
        ...
        self.logger.info(f"Testing cookie with GET request to {url}, cookies: {str(cookies)}, proxies: {self.get_proxies(url)}")

        try:
            response = self.session.get(url, headers={}, verify=self.ssl_verify.isChecked(), proxies=self.get_proxies(url))
            response.raise_for_status()
            response_time = (datetime.now() - datetime.now()).total_seconds()
            response_text = f"Cookie Test Status: {response.status_code}\nResponse Time: {response_time:.2f} seconds\nBody:\n{response.text}"
            headers_text = json.dumps(dict(response.headers), indent=2)
            self.logger.info(f"Cookie test successful: Status {response.status_code}, headers: {headers_text}")
            self.emitter.update_response.emit(response_text)
            self.emitter.update_headers.emit(headers_text)
