...
class CookingConversion(BASE,Template):
    ...
    def __str__(self):
        msg=[]
        f=f"Cooking Conversion(ccid={self.ccid},{Fore.orange_red_1}Mass_with_unit='{self.Mass_with_unit}' of ResourceName='{Fore.light_yellow}{self.ResourceName}' {Fore.medium_violet_red}~={Fore.magenta} Converts_To_Volume_With_Unit='{Fore.light_green}{self.Converts_To_Volume_With_Unit}'){Style.reset}"
        msg.append(f)
        return '\n'.join(msg)
