...
class ProbabilisticTFA:
    ...
    def predict(self, X, standardize = True, compute_variance = False):
        ...
        if standardize:
            h = X.shape[0]
            scale_factor = np.std(X, axis = 0, where = np.logical_not(X_missing_index)) if h > 1 else 1.0
            X = (X - np.mean(X, axis = 0, where = np.logical_not(X_missing_index))) / scale_factor
